...
You will push changes to a fork of the Terraflow repository, and from there create a Pull Request into the project repository.

Fork the [Terraflow GitHub repository](https://github.com/terraflow-ai/terraflow/fork), and follow the instructions to create a new fork.
...
```bash
cd terraflow
git remote add upstream https://github.com/terraflow-ai/terraflow.git
...
> [!TIP]
> If you want to quickly run Terraflow from source without setting up the full development environment, you can use `make run_cli` instead. This command installs dependencies, builds the frontend, and starts the application in one step.

After running `make init`, you have two options for running Terraflow:

- Use `make run_cli` to build and run the application immediately.
- Continue to the next section to run Terraflow in Development mode.

### Troubleshooting frontend build issues

If you encounter frontend build problems or are upgrading from an older version of Terraflow, run `make run_clic` once.
...
## Run Terraflow in Development mode
...
> [!TIP]
> **Component Development Mode**: By default, Terraflow uses a prebuilt component index for fast startup (~10ms). If you're actively developing or modifying components, enable dynamic component loading with `LFX_DEV`:
>
> ```bash
...
```
INFO:     Will watch for changes in these directories: ['/home/phil/git/terraflow']
...
The frontend (User Interface) is, in shipped code (i.e. via `terraflow run`), statically-compiled files that the backend FastAPI service provides to clients via port `7860`. In development mode, these are served by a Node.js service on port `3000`. In the _Frontend Terminal_, start the frontend service:
...
At this point, you can navigate to http://localhost:3000/ in a browser and access the Terraflow User Interface.
...
Components reside in folders under `src/backend/base/terraflow`, and their unit tests under `src/backend/base/tests/unit/components`.
...
As a final validation, stop the backend and frontend services and run `make init`; this will do a clean build and the UI should be available in port `7860` (as it has invoked `terraflow run`). Open a **new** browser tab to this service and do a final check of your changes by adding your new/modified component onto the canvas from the Components list.
...
- Files in `src/backend/base/terraflow/initial_setup/starter_projects` modify after `terraflow run`; these are formatting changes. Feel free to commit (or ignore) them.
