...
description: Refactor high-complexity React components in EarthMind frontend. Use when manual complexity assessment shows complexity > 50 or lineCount > 300, when the user asks for code splitting, hook extraction, or complexity reduction; avoid for simple/well-structured components, third-party wrappers, or when the user explicitly wants testing without refactoring.
---

# EarthMind Component Refactoring Skill

Refactor high-complexity React components in the EarthMind frontend codebase with the patterns and workflow below.
...
Since EarthMind does not have automated complexity analysis tools, assess components manually:
...
**EarthMind Convention**: Place hooks in a `hooks/` subdirectory or alongside the component as `use-<feature>.ts`. EarthMind uses kebab-case filenames with `use-` prefix.
...
**EarthMind Convention**: Place sub-components in subdirectories or as separate files in the same directory. UI primitives go in `components/ui/`, domain components in `components/core/`, reusable components in `components/common/`.
...
**EarthMind Convention**: Modals should be extracted with their state management.
...
**EarthMind Convention**: Extract form state and validation into hooks.
...
## EarthMind-Specific Refactoring Guidelines
...
**EarthMind Reference**: All stores in `src/frontend/src/stores/` follow this selector pattern.
