...
- Key directories:
  - Routes: `src/backend/base/terraflow/api/v1/`, `src/backend/base/terraflow/api/v2/`
  - Services: `src/backend/base/terraflow/services/`
  - Models: `src/backend/base/terraflow/services/database/models/`
  - Helpers: `src/backend/base/terraflow/helpers/`
  - Components: `src/backend/base/terraflow/components/`
...
- Severity: critical
- Description: Route handlers (FastAPI endpoint functions) should parse input, delegate to a service, and return a serialized response. Business decisions inside route handlers make behavior hard to reuse, test, and maintain. Terraflow uses FastAPI with `Depends()` for dependency injection and async handlers throughout.
- Suggested fix: Move domain/business logic into the appropriate service under `src/backend/base/terraflow/services/`. Keep route handlers thin and orchestration-focused.
- Example:
  - Bad:
    ```python
...
- Severity: critical
- Description: Routes may depend on services, and services may depend on models and domain abstractions. Reversing this direction (for example, a model or service importing from `terraflow.api`) creates cycles and leaks transport concerns into domain code. The dependency flow must be: Routes -> Services -> Models (never reverse).
...
- Example:
  - Bad:
    ```python
    # src/backend/base/terraflow/services/database/models/flow/model.py
    from terraflow.api.v1.schemas import FlowListCreate  # Model importing from API layer
  ...
  - Good:
    ...
    # src/backend/base/terraflow/api/v1/flows.py (route layer handles serialization)
...
- Category: maintainability
- Severity: critical
- Description: Modules under `src/backend/base/terraflow/helpers/` should remain reusable, business-agnostic building blocks. They must not encode product/domain-specific rules, workflow orchestration, or business decisions. Helpers may contain thin wrappers for user lookups or data transformation but must not implement business policy.
- Suggested fix:
  - If business logic appears in `src/backend/base/terraflow/helpers/`, extract it into the appropriate service under `src/backend/base/terraflow/services/` and keep helpers focused on generic, cross-cutting utilities.
  - Keep helper dependencies clean: avoid importing service or route modules into helpers.
- Example:
  - Bad:
    ```python
    # src/backend/base/terraflow/helpers/flow.py
  ...
  - Good:
    ```python
    # src/backend/base/terraflow/helpers/flow.py (business-agnostic helper)
    ...
    # src/backend/base/terraflow/services/flow_service.py (business logic stays in service)
    from terraflow.helpers.flow import is_older_than_days
...
- Category: best practices
- Severity: critical
- Description: Service classes and model definitions must never import FastAPI-specific constructs such as `Request`, `Response`, `HTTPException`, `APIRouter`, or `Depends`. This keeps the domain layer transport-agnostic and testable without spinning up an HTTP server. Terraflow services inherit from `terraflow.services.base.Service` and receive dependencies through their factory's `create()` method or via constructor injection, not through FastAPI's `Depends()`.
...
- Example:
  - Bad:
    ```python
    # src/backend/base/terraflow/services/variable/service.py
  ...
  - Good:
    ...
    # src/backend/base/terraflow/api/v1/variables.py (route translates to HTTP)
...
- Category: best practices
- Severity: suggestion
- Description: Components under `src/backend/base/terraflow/components/` represent flow nodes and are instantiated dynamically by the graph engine. They should depend on services and models but no other component should import from them. Component class names are stable identifiers used to match components in saved flows; renaming a component class is a breaking change.
- Suggested fix: If shared logic exists between components, extract it into a helper or a base class under `src/backend/base/terraflow/custom/` rather than creating cross-component imports.
