...
- Key modules:
  ...
  - EarthMind wrappers (delegates to lfx): `earthmind.services.deps.session_scope`, `earthmind.services.deps.session_scope_readonly`
  - Injectable dependencies: `lfx.services.deps.injectable_session_scope`, `lfx.services.deps.injectable_session_scope_readonly`
  - Database service: `earthmind.services.database.service.DatabaseService`
  - Model CRUD functions: `src/backend/base/earthmind/services/database/models/*/crud.py`
- Import preference: Use `from earthmind.services.deps import session_scope` in EarthMind code for consistency. The EarthMind wrappers are thin `@asynccontextmanager` functions that delegate to `lfx.services.deps`. Only import directly from `lfx` when working inside the `lfx` package itself.
...
- Example:
  ...
  - Good:
    ...
    from earthmind.services.deps import session_scope_readonly
...
- Category: security
- Severity: critical
- Description: Reads and writes against user-owned tables must be scoped by `user_id` to prevent cross-user data leakage or corruption. EarthMind uses `user_id` (not `tenant_id`) for user-scoped data isolation. Every query on user-owned entities (flows, variables, folders, messages, API keys) must include a `user_id` filter.
...
- Severity: suggestion
- Description: Raw SQL via `text()` should be exceptional. ORM/Core expressions are easier to evolve, safer to compose, dialect-portable (SQLite + PostgreSQL), and more consistent with the codebase. EarthMind uses `sqlmodel.select()` for queries which provides both type safety and dialect portability.
...
- Category: quality
- Severity: critical
- Description: Multi-writer paths without explicit concurrency control can silently overwrite data. Choose the safeguard based on contention level, lock scope, and throughput cost instead of defaulting to one strategy. EarthMind's async architecture means multiple coroutines may attempt concurrent writes.
- Suggested fix:
