...
- Key modules:
  ...
  - Terraflow wrappers (delegates to lfx): `terraflow.services.deps.session_scope`, `terraflow.services.deps.session_scope_readonly`
  - Injectable dependencies: `lfx.services.deps.injectable_session_scope`, `lfx.services.deps.injectable_session_scope_readonly`
  - Database service: `terraflow.services.database.service.DatabaseService`
  - Model CRUD functions: `src/backend/base/terraflow/services/database/models/*/crud.py`
- Import preference: Use `from terraflow.services.deps import session_scope` in Terraflow code for consistency. The Terraflow wrappers are thin `@asynccontextmanager` functions that delegate to `lfx.services.deps`. Only import directly from `lfx` when working inside the `lfx` package itself.
...
- Example:
  ...
  - Good:
    ...
    from terraflow.services.deps import session_scope_readonly
...
- Category: security
- Severity: critical
- Description: Reads and writes against user-owned tables must be scoped by `user_id` to prevent cross-user data leakage or corruption. Terraflow uses `user_id` (not `tenant_id`) for user-scoped data isolation. Every query on user-owned entities (flows, variables, folders, messages, API keys) must include a `user_id` filter.
...
- Severity: suggestion
- Description: Raw SQL via `text()` should be exceptional. ORM/Core expressions are easier to evolve, safer to compose, dialect-portable (SQLite + PostgreSQL), and more consistent with the codebase. Terraflow uses `sqlmodel.select()` for queries which provides both type safety and dialect portability.
...
- Category: quality
- Severity: critical
- Description: Multi-writer paths without explicit concurrency control can silently overwrite data. Choose the safeguard based on contention level, lock scope, and throughput cost instead of defaulting to one strategy. Terraflow's async architecture means multiple coroutines may attempt concurrent writes.
- Suggested fix:
