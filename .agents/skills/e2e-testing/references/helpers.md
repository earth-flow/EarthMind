...
Why: Terraflow supports 3 event delivery modes. A bug that only appears in polling mode would be missed if tests only run in streaming mode. This helper ensures all modes are covered without writing 3x the tests.
...
- The helper encapsulates Terraflow-specific knowledge (e.g., global variable badge behavior)
