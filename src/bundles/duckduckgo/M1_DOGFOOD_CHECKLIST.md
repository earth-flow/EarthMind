...
| Date | _YYYY-MM-DD_ |
| Pre-migration Terraflow version | `1.9.x` (last release before the pilot landed) |
| Post-migration Terraflow version | this branch / `1.10.x` |
| Result | _pass / fail_ |
| Notes | _free text_ |
...
1. **Pre-migration save.**
   - [ ] `pip install terraflow==<pre-migration version>` in a clean venv
   - [ ] `terraflow run` and open the UI
...
3. **Upgrade.**
   - [ ] In a separate clean venv: `pip install terraflow==<this branch>`
...
4. **Post-migration load.**
   - [ ] `terraflow run`, open the saved flow JSON from step 1
