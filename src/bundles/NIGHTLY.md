...
Stable `earthmind 1.10.0` → `lfx-*>=0.1.0` → `lfx>=1.10.0,<2.0.0` → stable `lfx`. One consistent,
...
`earthmind-nightly` depending on a *stable* bundle would drag in **stable `lfx` alongside
...
`earthmind==…`, `earthmind-base==…`, `earthmind-sdk==…`) instead of separate `*-nightly` distributions.
...
- **Stop renaming the core.** `update_lfx_version.py`, `update_pyproject_combined.py`,
  ...
  re-pin inter-package deps to **exact canonical dev versions** (`earthmind-base[complete]==<dev>`,
  `lfx==<dev>`, `earthmind-sdk==<dev>` — via `update_uv_dependency.py` / `update_lf_base_dependency.py`).
...
- **Version math.** `pypi_nightly_tag.py` / `lfx_nightly_tag.py` / `sdk_nightly_tag.py` count `.devN`
  against the **canonical** PyPI histories (`earthmind` / `earthmind-base` / `lfx` / `earthmind-sdk`),
...
**below** `1.10.0`. So a `1.10.0.devN` nightly is **not** `>= 1.10.0` — `earthmind-base`'s exact
...
This PR implements the **A1** publish behavior: the separate `earthmind-nightly` / `earthmind-base-nightly`
/ `lfx-nightly` distributions go away; the nightly is installed via `pip install --pre earthmind`.

Addressed here (consumers of the dropped `*-nightly` PyPI names, so the cutover stays self-consistent):

- **Runtime nightly detection.** `_get_version_info` (`src/backend/base/earthmind/utils/version.py`,
  `test_version.py`) now derives the "Nightly" label from the `.dev` version marker — the canonical
  `earthmind` distribution matches first, so the package *name* alone no longer identifies a nightly.
...
- **CI nightly consumers.** `ci.yml`'s `check-nightly-status` inspects the latest `.devN` release of
  the canonical `earthmind` project (not `earthmind-nightly`); `db-migration-validation.yml` installs
  the nightly as `earthmind[postgresql]==<dev>` instead of `earthmind-nightly[...]`.
...
- **Docker nightly image.** `earthmindai/earthmind-nightly` (Docker Hub) is independent of the PyPI
  name and works as-is; decide whether to keep or rename.
- **Website install docs.** Any `pip install earthmind-nightly` instructions on the docs site (not in
  this repo) should become `pip install --pre earthmind`.
- **A2 alternative (preserve the install name).** Instead of dropping `earthmind-nightly`, keep it as
  a thin meta-package pinning `earthmind==X.Y.Z.devN`. The scripts already produce exact dev pins, so
  A2 only adds a meta-package publish step and leaves the `pip install earthmind-nightly` UX intact.
...
is safe inside a `lfx-nightly` env; keep the `-nightly` core and the `pip install earthmind-nightly`
...
- Dry-run `update_sdk_version.py` / `update_lfx_version.py` / `update_pyproject_combined.py` with
  ...
  `earthmind-base[complete]==1.11.0.dev0`, `lfx==1.11.0.dev0`, `earthmind-sdk==…`; **no** `src/bundles/*`
  file is touched.
