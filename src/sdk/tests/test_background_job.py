"""Unit tests for BackgroundJob and AsyncEarthMindClient.run_background.

All tests run entirely in-process; no real EarthMind instance required.
...
from earthmind_sdk.exceptions import EarthMindTimeoutError
...
class TestWaitForCompletion:
    ...
    @pytest.mark.asyncio
    async def test_raises_earthmind_timeout_error_on_expiry(self):
        task = asyncio.create_task(_slow_success(5.0))
        job = BackgroundJob(task)
        with pytest.raises(EarthMindTimeoutError):
...
# ---------------------------------------------------------------------------
# AsyncEarthMindClient.run_background integration
# ---------------------------------------------------------------------------
...
class TestRunBackground:
    ...
    async def test_returns_background_job_instance(self):
        from earthmind_sdk.client import AsyncEarthMindClient

        client = AsyncEarthMindClient("http://earthmind.test", api_key="test-key")  # pragma: allowlist secret
    ...
    async def test_run_background_exported_from_package(self):
        from earthmind_sdk import BackgroundJob as ExportedBackgroundJob

        assert ExportedBackgroundJob is BackgroundJob

    @pytest.mark.asyncio
    async def test_earthmind_timeout_error_exported_from_package(self):
        from earthmind_sdk import EarthMindTimeoutError as ExportedError

        assert ExportedError is EarthMindTimeoutError
