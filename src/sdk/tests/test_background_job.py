"""Unit tests for BackgroundJob and AsyncTerraflowClient.run_background.

All tests run entirely in-process; no real Terraflow instance required.
...
from terraflow_sdk.exceptions import TerraflowTimeoutError
...
class TestWaitForCompletion:
    ...
    @pytest.mark.asyncio
    async def test_raises_terraflow_timeout_error_on_expiry(self):
        task = asyncio.create_task(_slow_success(5.0))
        job = BackgroundJob(task)
        with pytest.raises(TerraflowTimeoutError):
...
# ---------------------------------------------------------------------------
# AsyncTerraflowClient.run_background integration
# ---------------------------------------------------------------------------
...
class TestRunBackground:
    ...
    async def test_returns_background_job_instance(self):
        from terraflow_sdk.client import AsyncTerraflowClient

        client = AsyncTerraflowClient("http://terraflow.test", api_key="test-key")  # pragma: allowlist secret
    ...
    async def test_run_background_exported_from_package(self):
        from terraflow_sdk import BackgroundJob as ExportedBackgroundJob

        assert ExportedBackgroundJob is BackgroundJob

    @pytest.mark.asyncio
    async def test_terraflow_timeout_error_exported_from_package(self):
        from terraflow_sdk import TerraflowTimeoutError as ExportedError

        assert ExportedError is TerraflowTimeoutError
