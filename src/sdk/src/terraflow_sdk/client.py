"""Sync HTTP client for the Terraflow REST API.
...
The async counterpart lives in :mod:`terraflow_sdk._async_client`.
"""
...
# Re-export async client so that existing ``from terraflow_sdk.client import ...``
...
class TerraflowClient(_ClientCommon):
    """Synchronous client for the Terraflow REST API.
...
#: Short alias for :class:`TerraflowClient`.
#:
#: Example::
#:
#:     from terraflow_sdk import Client
...
#:     flows  = client.list_flows()
#:     result = client.run_flow("my-endpoint", RunRequest(input_value="Hello"))
Client = TerraflowClient

__all__ = ["AsyncClient", "AsyncTerraflowClient", "Client", "TerraflowClient"]
