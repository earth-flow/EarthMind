"""Environment configuration for terraflow-sdk.

Loads named environment definitions from a TOML file so teams can switch
between Terraflow instances (dev / staging / production) without code changes.

Config file lookup order
------------------------
1. Path given explicitly to ``load_environments()`` or ``get_client()``.
2. The ``TERRAFLOW_ENVIRONMENTS_FILE`` environment variable.
3. ``terraflow-environments.toml`` in the current working directory.
4. ``~/.config/terraflow/environments.toml``
...
.. code-block:: toml
    ...
    api_key_env = "TERRAFLOW_STAGING_API_KEY"   # env-var that holds the key  # pragma: allowlist secret

    [environments.production]
    url = "https://terraflow.example.com"
    api_key_env = "TERRAFLOW_PROD_API_KEY"  # pragma: allowlist secret
...
from terraflow_sdk.exceptions import EnvironmentConfigError, EnvironmentNotFoundError
...
_ENV_VAR = "TERRAFLOW_ENVIRONMENTS_FILE"
_LOCAL_NAME = "terraflow-environments.toml"
_USER_PATH = Path.home() / ".config" / "terraflow" / "environments.toml"

_EXAMPLE_CONFIG = """\
# terraflow-environments.toml
#
# Define named Terraflow environments.  The api_key_env field is the *name*
# of an environment variable that holds the API key for that instance.
#
# [environments.staging]
# url = "https://staging.terraflow.example.com"
# api_key_env = "TERRAFLOW_STAGING_API_KEY"  # pragma: allowlist secret
#
# [environments.production]
# url = "https://terraflow.example.com"
# api_key_env = "TERRAFLOW_PROD_API_KEY"  # pragma: allowlist secret
...
def load_environments(
    ...
    ----------
    config_file:
        Explicit path to a ``terraflow-environments.toml`` file. If omitted,
    ...
    if file_path is None:
        raise EnvironmentConfigError(
            "No terraflow-environments.toml found. "
...
def get_client(
    ...
    *,
    ...
    Example::
        ...
        from terraflow_sdk import get_client
...
def get_async_client(
    ...
    *,
    ...
    Example::
        ...
        from terraflow_sdk import get_async_client
