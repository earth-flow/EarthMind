"""Async HTTP client for the Terraflow REST API.

Preferred usage via the short alias::

    from terraflow_sdk import AsyncClient

    async with AsyncClient("https://terraflow.example.com", api_key="...") as client:
...
from terraflow_sdk._client_common import _ClientCommon
from terraflow_sdk._http import (
    _DEFAULT_TIMEOUT,
    _build_headers,
    _connection_error,
    _logger,
...
class AsyncTerraflowClient(_ClientCommon):
    """Async client for the Terraflow REST API.
    ...
    async def run_background(
        ...
        *,
        ...
        Args:
            ...
            input_type: Terraflow input type (default ``"chat"``).
            output_type: Terraflow output type (default ``"chat"``).
            tweaks: Optional component tweaks dict.

        Returns:
            A :class:`BackgroundJob` wrapping the in-flight asyncio task.

        Adapted from ``BackgroundJob`` in terraflow-ai/sdk PR #1
    ...
    async def pull_project(
        ...
        *,
        ...
        .. note::
            ...
            :meth:`TerraflowClient.pull_project` for details.
...
#: Short alias for :class:`AsyncTerraflowClient`.
#:
#: Example::
#:
#:     from terraflow_sdk import AsyncClient
#:     async with AsyncClient("https://terraflow.example.com", api_key="...") as c:
#:         flows = await c.list_flows()
AsyncClient = AsyncTerraflowClient
