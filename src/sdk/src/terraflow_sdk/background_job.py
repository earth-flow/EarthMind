...
from terraflow-ai/sdk PR #1 (Janardan Singh Kavia, IBM Corp., Apache 2.0)
adapted for the Terraflow V1 ``/api/v1/run/{id}`` endpoint.
...
class BackgroundJob:
    """Non-blocking handle for an in-flight :meth:`AsyncTerraflowClient.run` call.

    Returned by :meth:`AsyncTerraflowClient.run_background`.  The underlying
        ...
        *,
        ...
        Args:
            timeout: Maximum seconds to wait.  ``None`` (default) means wait
                     indefinitely.  Raises :exc:`TerraflowTimeoutError` on
                     expiry.
        ...
        Raises:
            TerraflowTimeoutError: If *timeout* elapses before the run finishes.
        ...
        try:
            ...
            raise TerraflowTimeoutError(msg) from exc
