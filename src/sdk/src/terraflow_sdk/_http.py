...
_logger = logging.getLogger("terraflow_sdk.client")
...
def _raise_for_status_code(status: int, detail: str) -> None:
    ...
    if status in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
        raise TerraflowAuthError(status, detail)
    if status == HTTPStatus.NOT_FOUND:
        raise TerraflowNotFoundError(status, detail)
    if status == HTTPStatus.UNPROCESSABLE_ENTITY:
        raise TerraflowValidationError(status, detail)
    raise TerraflowHTTPError(status, detail)
...
def _connection_error(base_url: str, exc: Exception) -> TerraflowConnectionError:
    msg = f"Could not connect to Terraflow at {base_url}: {exc}"
    return TerraflowConnectionError(msg)
