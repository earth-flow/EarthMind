"""Environment configuration for earthmind-sdk.

Loads named environment definitions from a TOML file so teams can switch
between EarthMind instances (dev / staging / production) without code changes.

Config file lookup order
------------------------
1. Path given explicitly to ``load_environments()`` or ``get_client()``.
2. The ``EARTHMIND_ENVIRONMENTS_FILE`` environment variable.
3. ``earthmind-environments.toml`` in the current working directory.
4. ``~/.config/earthmind/environments.toml``
...
.. code-block:: toml
    ...
    api_key_env = "EARTHMIND_STAGING_API_KEY"   # env-var that holds the key  # pragma: allowlist secret

    [environments.production]
    url = "https://earthmind.example.com"
    api_key_env = "EARTHMIND_PROD_API_KEY"  # pragma: allowlist secret
...
from earthmind_sdk.exceptions import EnvironmentConfigError, EnvironmentNotFoundError
...
_ENV_VAR = "EARTHMIND_ENVIRONMENTS_FILE"
_LOCAL_NAME = "earthmind-environments.toml"
_USER_PATH = Path.home() / ".config" / "earthmind" / "environments.toml"

_EXAMPLE_CONFIG = """\
# earthmind-environments.toml
#
# Define named EarthMind environments.  The api_key_env field is the *name*
# of an environment variable that holds the API key for that instance.
#
# [environments.staging]
# url = "https://staging.earthmind.example.com"
# api_key_env = "EARTHMIND_STAGING_API_KEY"  # pragma: allowlist secret
#
# [environments.production]
# url = "https://earthmind.example.com"
# api_key_env = "EARTHMIND_PROD_API_KEY"  # pragma: allowlist secret
...
def load_environments(
    ...
    ----------
    config_file:
        Explicit path to a ``earthmind-environments.toml`` file. If omitted,
    ...
    if file_path is None:
        raise EnvironmentConfigError(
            "No earthmind-environments.toml found. "
...
def get_client(
    ...
    *,
    ...
    Example::
        ...
        from earthmind_sdk import get_client
...
def get_async_client(
    ...
    *,
    ...
    Example::
        ...
        from earthmind_sdk import get_async_client
