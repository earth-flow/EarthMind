"""Async HTTP client for the EarthMind REST API.

Preferred usage via the short alias::

    from earthmind_sdk import AsyncClient

    async with AsyncClient("https://earthmind.example.com", api_key="...") as client:
...
from earthmind_sdk._client_common import _ClientCommon
from earthmind_sdk._http import (
    _DEFAULT_TIMEOUT,
    _build_headers,
    _connection_error,
    _logger,
...
class AsyncEarthMindClient(_ClientCommon):
    """Async client for the EarthMind REST API.
    ...
    async def run_background(
        ...
        *,
        ...
        Args:
            ...
            input_type: EarthMind input type (default ``"chat"``).
            output_type: EarthMind output type (default ``"chat"``).
            tweaks: Optional component tweaks dict.

        Returns:
            A :class:`BackgroundJob` wrapping the in-flight asyncio task.

        Adapted from ``BackgroundJob`` in earthmind-ai/sdk PR #1
    ...
    async def pull_project(
        ...
        *,
        ...
        .. note::
            ...
            :meth:`EarthMindClient.pull_project` for details.
...
#: Short alias for :class:`AsyncEarthMindClient`.
#:
#: Example::
#:
#:     from earthmind_sdk import AsyncClient
#:     async with AsyncClient("https://earthmind.example.com", api_key="...") as c:
#:         flows = await c.list_flows()
AsyncClient = AsyncEarthMindClient
