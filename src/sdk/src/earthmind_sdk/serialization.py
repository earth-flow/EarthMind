"""Git-friendly flow serialization for earthmind-sdk.

``normalize_flow`` transforms a raw EarthMind flow dict into a stable,
...
def normalize_flow(
    ...
    *,
    ...
    ----------
    flow:
        Raw flow dict as returned by the EarthMind API or read from a ``.json``
        file.
    strip_volatile:
