...
from earthmind-ai/sdk PR #1 (Janardan Singh Kavia, IBM Corp., Apache 2.0)
adapted for the EarthMind V1 ``/api/v1/run/{id}`` endpoint.
...
class BackgroundJob:
    """Non-blocking handle for an in-flight :meth:`AsyncEarthMindClient.run` call.

    Returned by :meth:`AsyncEarthMindClient.run_background`.  The underlying
        ...
        *,
        ...
        Args:
            timeout: Maximum seconds to wait.  ``None`` (default) means wait
                     indefinitely.  Raises :exc:`EarthMindTimeoutError` on
                     expiry.
        ...
        Raises:
            EarthMindTimeoutError: If *timeout* elapses before the run finishes.
        ...
        try:
            ...
            raise EarthMindTimeoutError(msg) from exc
