"""Sync HTTP client for the EarthMind REST API.
...
The async counterpart lives in :mod:`earthmind_sdk._async_client`.
"""
...
# Re-export async client so that existing ``from earthmind_sdk.client import ...``
...
class EarthMindClient(_ClientCommon):
    """Synchronous client for the EarthMind REST API.
...
#: Short alias for :class:`EarthMindClient`.
#:
#: Example::
#:
#:     from earthmind_sdk import Client
...
#:     flows  = client.list_flows()
#:     result = client.run_flow("my-endpoint", RunRequest(input_value="Hello"))
Client = EarthMindClient

__all__ = ["AsyncClient", "AsyncEarthMindClient", "Client", "EarthMindClient"]
