...
# ---------------------------------------------------------------------------
#
# Layers above lfx (the terraflow component cache; eventually the events
# service) need to react to a successful reload.  ``lfx`` cannot import
# ``terraflow``, so we expose a tiny registration API: ``terraflow`` calls
...
def register_post_swap_hook(hook: Callable[[BundleRecord], None]) -> None:
    ...
    canonical use case is the terraflow component cache, which needs to
...
def _run_pipeline_body(
    *,
    ...
    # the default terraflow verbosity.
    logger.info(
