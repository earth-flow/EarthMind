...
class InitOptions:
    ...
    display_name: str
    """Human-readable display name shown in Terraflow's palette."""
...
Replace ``build`` with your real logic.  See the Terraflow Component docs
at https://docs.terraflow.org/components for input/output options.
"""
...
def _test_source(options: InitOptions) -> str:
    ...
    a constructed instance.  It does NOT depend on Terraflow's full graph
...
A Terraflow Extension generated by `lfx extension init`.
...
> **Prerequisite**: `terraflow` (which brings `lfx`) must be installed in
> your active environment before running any of the commands below or
> executing the generated `pytest` suite.  Install it with
> `pip install terraflow` or your package manager of choice.

```bash
# Validate the manifest + bundle without executing imports
lfx extension validate .

# Run the bundled smoke tests (requires terraflow / lfx)
pytest

# Launch a Terraflow dev server with this extension loaded
lfx extension dev .
```

After `lfx extension dev`, edit any file under `components/` and click
**Reload** in the Terraflow palette to pick up the change.
...
def _pyproject_payload(options: InitOptions) -> str:
    ...
    The ``[tool.terraflow.extension]`` entry mirrors the ``extension.json``
...
version = "0.1.0"
description = "{options.display_name} -- a Terraflow Extension."
