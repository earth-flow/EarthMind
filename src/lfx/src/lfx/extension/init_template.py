...
class InitOptions:
    ...
    display_name: str
    """Human-readable display name shown in EarthMind's palette."""
...
Replace ``build`` with your real logic.  See the EarthMind Component docs
at https://docs.earthmind.org/components for input/output options.
"""
...
def _test_source(options: InitOptions) -> str:
    ...
    a constructed instance.  It does NOT depend on EarthMind's full graph
...
A EarthMind Extension generated by `lfx extension init`.
...
> **Prerequisite**: `earthmind` (which brings `lfx`) must be installed in
> your active environment before running any of the commands below or
> executing the generated `pytest` suite.  Install it with
> `pip install earthmind` or your package manager of choice.

```bash
# Validate the manifest + bundle without executing imports
lfx extension validate .

# Run the bundled smoke tests (requires earthmind / lfx)
pytest

# Launch a EarthMind dev server with this extension loaded
lfx extension dev .
```

After `lfx extension dev`, edit any file under `components/` and click
**Reload** in the EarthMind palette to pick up the change.
...
def _pyproject_payload(options: InitOptions) -> str:
    ...
    The ``[tool.earthmind.extension]`` entry mirrors the ``extension.json``
...
version = "0.1.0"
description = "{options.display_name} -- a EarthMind Extension."
