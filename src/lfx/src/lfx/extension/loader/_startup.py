...
def load_seed_extensions(
    *,
    ...
    going through pip.  Default location is ``/opt/terraflow/bundles``;
    override via ``$TERRAFLOW_SEED_DIR``.  Each immediate subdirectory that
    ships a v0 manifest becomes one Extension at the @official slot.

    Args:
        seed_dir_env: Test seam.  ``None`` reads ``$TERRAFLOW_SEED_DIR``
            from the live environment; pass an explicit string to bypass
            ``os.environ``.
        default_seed_dir: Test seam.  ``None`` uses the discovery layer's
            default (``/opt/terraflow/bundles``); pass an explicit ``Path``
