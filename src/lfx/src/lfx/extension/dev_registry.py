...
state file, then hand off to ``terraflow run``.  At server startup, the
...
State file shape (kebab-case path under the terraflow user-cache dir)::
...
Scope notes:
    ...
    - This module only writes / reads / lists the state file; the actual
      registration is consumed by Terraflow's startup hook.  The runtime
    ...
    - Installed-package discovery keeps its own primitives and is
      ...
      Terraflow startup merges in the same step.
"""
...
def _default_state_dir() -> Path:
    ...
    Resolution order:
        1. ``TERRAFLOW_DEV_EXTENSIONS_DIR`` (test seam + override).
        2. ``TERRAFLOW_CONFIG_DIR/extensions`` if ``TERRAFLOW_CONFIG_DIR`` is set.
        3. ``platformdirs.user_cache_dir("terraflow", "terraflow")/extensions``.

    Created lazily on first write.
    """
    override = os.environ.get("TERRAFLOW_DEV_EXTENSIONS_DIR")
    if override:
    ...
    return Path(user_cache_dir("terraflow", "terraflow")) / "extensions"
...
def _read_state(state_path: Path) -> list[DevExtensionEntry]:
    ...
    * **File present but unreadable** (permission error etc.) -- log a
      ...
      extension on the next ``terraflow run`` with no diagnostic.  A
...
def _write_state(state_path: Path, entries: Iterable[DevExtensionEntry]) -> None:
    ...
    ``dev_extensions.json`` gets code loaded at the next ``terraflow run``;
...
def dev_extension_component_paths(*, state_dir: Path | None = None) -> tuple[list[Path], list[ExtensionError]]:
    ...
    paths, IDE integrations).  The Terraflow startup path itself uses
    ...
    for result in load_dev_extensions(state_dir=state_dir):
        ...
        # caller (Terraflow's lifespan hook) is the only thing that turns
