"""Offline, non-executing validator for a Terraflow Extension.
...
CLI.  It performs four passes:
    ...
    4. **(opt-in)** ``--execute-imports``: forks a subprocess with a temporary
       Terraflow state dir, a strict env allowlist, and no inherited server
...
def _discover_manifest_data(
    ...
    return (
        ...
        ExtensionError(
            ...
            message=(f"No extension.json or [tool.terraflow.extension] entry found in {root}."),
            location=str(root),
            hint=(
                "Create an extension.json at the extension root or add a "
                "[tool.terraflow.extension] section to pyproject.toml. See "
...
def _has_build_method(node: ast.ClassDef) -> bool:
    ...
    Terraflow Components reach the runtime in two shapes:
...
def _build_probe_env(tmp_path: Path) -> dict[str, str]:
    ...
    TERRAFLOW_CONFIG_DIR are pinned to the throwaway temp directory so a
    misbehaving bundle cannot read or pollute the developer's real
    Terraflow state.  Cloud credentials (AWS_*, OPENAI_API_KEY, GITHUB_TOKEN,
    ...
    env["TERRAFLOW_CONFIG_DIR"] = str(tmp_path / "config")
