"""FastMCP server exposing Terraflow operations as MCP tools.

Connects to a running Terraflow server via REST API. Flow data is never
...
_client_var: contextvars.ContextVar[TerraflowClient | None] = contextvars.ContextVar("_client", default=None)
_registry_var: contextvars.ContextVar[dict[str, dict] | None] = contextvars.ContextVar("_registry", default=None)
_shared_client: TerraflowClient | None = None
...
mcp = FastMCP(
    "terraflow-mcp",
    instructions=(
        "Terraflow MCP server -- build and run AI flows on a Terraflow instance.\n"
        "\n"
        "Typical workflow:\n"
        "  1. login (or set TERRAFLOW_API_KEY env var)\n"
...
def _get_client() -> TerraflowClient:
    ...
    if _shared_client is None:
        _shared_client = TerraflowClient()
    return _shared_client


def _set_client(client: TerraflowClient) -> None:
...
async def login(username: str, password: str, server_url: str | None = None) -> dict[str, str]:
    """Authenticate with a Terraflow server. Call this first.

    Credentials are stored and reused for all subsequent calls.

    Args:
        username: Terraflow username.
        password: Terraflow password.
        server_url: Server URL (defaults to TERRAFLOW_SERVER_URL env var or http://10.171.205.153:7860).
    """
    ...
    client = TerraflowClient(server_url=server_url)
    _set_client(client)
