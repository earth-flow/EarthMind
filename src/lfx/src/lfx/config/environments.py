"""lfx environment configuration — resolve Terraflow instance URL and API key.
...
4.  ``terraflow-environments.toml`` in the current working directory
    (backward-compatible with the terraflow-sdk TOML format).
...
.. code-block:: yaml
    ...
    environments:
      local:
        url: http://10.171.205.153:7860
        api_key_env: TERRAFLOW_LOCAL_API_KEY
...
TOML format is also accepted (``terraflow-environments.toml`` or any ``.toml``
...
class ConfigError(Exception):
    ...
    will simply be ``None`` on the returned :class:`TerraflowEnvironment`.
...
@dataclass
class TerraflowEnvironment:
    """A fully-resolved Terraflow target instance.

    Attributes:
        name:    Human-readable label (environment name or ``"__inline__"``).
        url:     Base URL of the Terraflow instance.
...
_TOML_FALLBACK = "terraflow-environments.toml"
_LFX_DIR = ".lfx"
...
def _find_config_file(override: Path | None) -> Path | None:
    ...
    # Backward-compat: terraflow-environments.toml in cwd
...
def _parse_env_block(name: str, block: Any, config_path: Path) -> TerraflowEnvironment:
    ...
    return TerraflowEnvironment(name=name, url=url, api_key=api_key)


def _load_config(path: Path) -> tuple[dict[str, TerraflowEnvironment], str | None]:
    ...
    envs: dict[str, TerraflowEnvironment] = {}
...
def resolve_environment(
    ...
    *,
...
) -> TerraflowEnvironment:
    """Resolve an environment name (or inline flags) to a :class:`TerraflowEnvironment`.
    ...
    3. **Env-var fallback** — if no config file exists and no *env* was
       requested, fall back to ``TERRAFLOW_URL`` / ``TERRAFLOW_API_KEY`` (or
    ...
    if target is not None:
        ...
        return TerraflowEnvironment(name=name, url=target, api_key=api_key)
    ...
    if config_path is None:
        ...
        lf_url = os.environ.get("TERRAFLOW_URL") or os.environ.get("LFX_URL")
        if lf_url and env is None:
            lf_key = api_key or os.environ.get("TERRAFLOW_API_KEY") or os.environ.get("LFX_API_KEY")
            return TerraflowEnvironment(name="__env__", url=lf_url, api_key=lf_key)
        ...
        msg = (
            ...
            "  • export TERRAFLOW_URL=<url>            (env-var fallback)\n"
    ...
    if api_key is not None:
        resolved = TerraflowEnvironment(name=resolved.name, url=resolved.url, api_key=api_key)
