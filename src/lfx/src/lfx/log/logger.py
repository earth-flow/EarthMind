"""Logging configuration for Terraflow using structlog."""
...
class SizedLogBuffer:
    ...
    def __init__(
        ...
        The buffer can be overwritten by an env variable TERRAFLOW_LOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("TERRAFLOW_LOG_RETRIEVER_BUFFER_SIZE", "0")
...
def _get_service_info() -> dict[str, str]:
    ...
    service = os.getenv("TERRAFLOW_SERVICE_NAME", "terraflow")
    version = os.getenv("TERRAFLOW_VERSION", "")
    environment = os.getenv("TERRAFLOW_ENVIRONMENT", "")
...
def _apply_logger_level_overrides() -> None:
    """Apply ``TERRAFLOW_LOG_LEVELS`` env var: ``name=LEVEL,name=LEVEL,...``.
    ...
    """
    raw = os.getenv("TERRAFLOW_LOG_LEVELS", "").strip()
    if not raw:
        return
    for pair in raw.split(","):
        ...
        if "=" not in entry:
            warnings.warn(
                f"TERRAFLOW_LOG_LEVELS: ignoring {entry!r} (expected 'name=LEVEL')",
                stacklevel=2,
            )
            continue
        ...
        if not name:
            warnings.warn(
                f"TERRAFLOW_LOG_LEVELS: ignoring {entry!r} (empty logger name)",
                stacklevel=2,
            )
            continue
        ...
        if numeric is None:
            warnings.warn(
                f"TERRAFLOW_LOG_LEVELS: ignoring {entry!r} (unknown level {level_str!r}, "
...
def _forward_loguru_message(message) -> None:
    """Forward Loguru messages through Terraflow's configured structlog pipeline."""
...
def setup_loguru_logger(log_level: str, *, enqueue: bool = False) -> None:
    """Route Loguru's default logger through Terraflow logging."""
...
def setup_log_file(log_file: Path, *, max_bytes: int, formatter: logging.Formatter | None = None) -> None:
    """Set up Terraflow's rotating file handler.
...
def configure(
    *,
    ...
    if log_level is None and os.getenv("TERRAFLOW_LOG_LEVEL", "").upper() in VALID_LOG_LEVELS:
        log_level = os.getenv("TERRAFLOW_LOG_LEVEL")
    if log_level is None or log_level.upper() not in LOG_LEVEL_MAP:
        log_level = "ERROR"

    if log_file is None:
        env_log_file = os.getenv("TERRAFLOW_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("TERRAFLOW_LOG_ENV", "")

    # Get log format from env if not provided
    if log_format is None:
        log_format = os.getenv("TERRAFLOW_LOG_FORMAT")
    ...
    # rebuilds from scratch. Env-only toggles (e.g. TERRAFLOW_PRETTY_LOGS) are not
    ...
    extra_redact = frozenset(
        k.strip().lower() for k in os.getenv("TERRAFLOW_LOG_REDACT_KEYS", "").split(",") if k.strip()
    )
    ...
    # Add callsite information only when TERRAFLOW_DEV is set
    if DEV:
        processors.append(
    ...
    # TERRAFLOW_LOG_TRACE_LOCALS=true when you need it for local debugging.
    show_locals = os.getenv("TERRAFLOW_LOG_TRACE_LOCALS", "false").lower() == "true"
    ...
    if log_env.lower() in ("container", "container_json"):
        ...
        log_stdout_pretty = os.getenv("TERRAFLOW_PRETTY_LOGS", "true").lower() == "true"
    ...
    if log_file:
        if not log_file.parent.exists():
            ...
            log_file = cache_dir / "terraflow.log"
    ...
    json_mode = log_env.lower() in ("container", "container_json") or (
        not log_env and os.getenv("TERRAFLOW_PRETTY_LOGS", "true").lower() != "true"
    )
