...
from lfx.io.schema import schema_to_terraflow_inputs
...
class MCPToolsComponent(ComponentWithCache):
    ...
    Behaviour notes:
    - Stale agent tools vs server were caused by Terraflow caching the Toolset output
    ...
    def _build_tool_output(self) -> Output:
        ...
        # toggle: Terraflow's Output.cache defaults to True and was memoizing the first to_toolkit()
    ...
    documentation: str = "https://docs.terraflow.org/mcp-tools"
    icon = "Mcp"
    name = "MCPTools"
    ...
    async def _validate_schema_inputs(self, tool_obj) -> list[InputTypes]:
        ...
        try:
            ...
            schema_inputs = schema_to_terraflow_inputs(input_schema)
    ...
    async def update_tool_list(self, mcp_server_value=None):
        ...
        async with self._update_tool_list_lock:
            ...
            try:
                ...
                # When running in LFX standalone mode the full Terraflow package and
                ...
                try:
                    from terraflow.api.v2.mcp import get_server
                    ...
                    # from an installed Terraflow — a real API break that should NOT be
                    # swallowed as "standalone mode". ModuleNotFoundError alone covers the
                    # "Terraflow absent" case.
                    #
                    # Even within ModuleNotFoundError, only treat this as LFX standalone mode
                    # when one of the target Terraflow modules is itself missing. Transitive
                    # ModuleNotFoundError (e.g. a dependency like sqlmodel failing to import
                    # inside terraflow.*) indicates a real bug in the full Terraflow stack and
                    ...
                    is_terraflow_standalone = missing_module == "terraflow" or missing_module.startswith("terraflow.")
                    if not is_terraflow_standalone:
                        raise
                    await logger.ainfo(
                        "Terraflow package not available; using MCP server config from flow value (LFX standalone mode)."
                    )
                else:
                ...
                msg = (
                    ...
                    "Raise ``TERRAFLOW_MCP_SERVER_TIMEOUT`` for the deployment if the MCP "
    ...
    def get_inputs_for_all_tools(self, tools: list) -> dict:
        ...
        for tool in tools:
            if not tool or not hasattr(tool, "name"):
                continue
            try:
                terraflow_inputs = schema_to_terraflow_inputs(tool.args_schema)
                inputs[tool.name] = terraflow_inputs
    ...
    async def build_output(self) -> DataFrame:
        ...
        try:
            ...
            if self.tool != "":
                # Set session context for persistent MCP sessions using Terraflow session ID
    ...
    def _get_session_context(self) -> str | None:
        """Get the Terraflow session ID for MCP session caching."""
