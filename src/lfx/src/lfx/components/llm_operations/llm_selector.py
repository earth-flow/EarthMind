...
class LLMSelectorComponent(Component):
    ...
    documentation: str = "https://docs.earthmind.org/llm-selector"
    icon = "git-branch"
    ...
    def _get_api_model_id_for_earthmind_model(self, earthmind_model_name: str) -> str | None:
        """Attempt to find the OpenRouter API ID for a given EarthMind model name."""
        if not earthmind_model_name:
            return None

        potential_names_to_check = [earthmind_model_name, self._simplify_model_name(earthmind_model_name)]

        if earthmind_model_name.startswith("models/"):
            name_without_prefix = earthmind_model_name[len("models/") :]
        ...
        elif earthmind_model_name.startswith("community_models/"):
            name_without_prefix = earthmind_model_name[len("community_models/") :]
            potential_names_to_check.append(name_without_prefix)
        ...
        self.log(
            f"Could not map EarthMind model name '{earthmind_model_name}' "
            f"(tried variants: {unique_names_to_check}) to an OpenRouter API ID."
        )
        return None

    def _get_model_specs_dict(self, earthmind_model_name: str) -> dict[str, Any]:
        """Get a dictionary of relevant model specifications for a given EarthMind model name."""
        if not self.use_openrouter_specs or not self._models_api_cache:
            return {
                "id": earthmind_model_name,
                "name": earthmind_model_name,
                "description": "Specifications not available.",
            }

        api_model_id = self._get_api_model_id_for_earthmind_model(earthmind_model_name)

        if not api_model_id or api_model_id not in self._models_api_cache:
            log_msg = (
                f"No cached API data found for EarthMind model '{earthmind_model_name}' "
...
If no model seems suitable, pick the first model in the list (index 0) as a fallback."""
    ...
    async def route_to_model(self) -> Message:
        ...
        try:
            self.log(f"Starting model routing with {len(self.models)} available EarthMind models.")
            ...
            for i, earthmind_model_instance in enumerate(self.models):
                earthmind_model_name = get_model_name(earthmind_model_instance)
                if not earthmind_model_name:
                    ...
                    spec_dict = self._get_model_specs_dict(earthmind_model_name)

                model_specs_for_judge.append({"index": i, "earthmind_name": earthmind_model_name, "specs": spec_dict})
                self.log(
                    f"Prepared specs for EarthMind model {i} ('{earthmind_model_name}'): {spec_dict.get('name', 'N/A')}"
                )
...
Return ONLY the index number:"""
            ...
            if self._selected_model_name:
                self._selected_api_model_id = (
                    self._get_api_model_id_for_earthmind_model(self._selected_model_name) or self._selected_model_name
                )
            else:
            ...
            specs_source = (
                ...
                else "Basic (EarthMind model names only)"
            )
            self._routing_decision = f"""Model Selection Decision:
- Selected Model Index: {selected_index}
- Selected EarthMind Model Name: {self._selected_model_name}
...
- Specifications Source: {specs_source}"""
            ...
            log_msg = (
                ...
                f"EarthMind Name: '{self._selected_model_name}', API ID: '{self._selected_api_model_id}'"
            )
            self.log(log_msg)
            ...
            if self.fallback_to_first and self.models:
                ...
                self._selected_model_name = get_model_name(chosen_model_instance)
                if self._selected_model_name:
                    mapped_id = self._get_api_model_id_for_earthmind_model(self._selected_model_name)
...
- Fallback Model EarthMind Name: {self._selected_model_name}
...
- Reason: Automatic fallback enabled"""
    ...
    def get_selected_model_info(self) -> list[Data]:
        ...
        if self._selected_model_name:
            ...
            if "earthmind_name" not in specs_dict:
                specs_dict["earthmind_model_name_used_for_lookup"] = self._selected_model_name
