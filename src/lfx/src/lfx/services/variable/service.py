...
class VariableService(Service):
    ...
    def _normalize_global_var_key(name: str) -> str:
        return f"x-terraflow-global-var-{name.lower().replace('_', '-')}"

    @staticmethod
    def _get_request_variables() -> dict[str, str]:
        """Request-scoped variables from serve ContextVar or TERRAFLOW_REQUEST_VARIABLES env."""
        active = get_active_request_variables()
        if active is not None:
            return active

        # TERRAFLOW_REQUEST_VARIABLES is a process-wide env var, so reading it is an
        ...
        raw = os.getenv("TERRAFLOW_REQUEST_VARIABLES")
        if not raw:
            return {}
        try:
            parsed = json.loads(raw)
        except json.JSONDecodeError:
            logger.debug("Invalid TERRAFLOW_REQUEST_VARIABLES JSON; skipping request-scoped lookup")
            return {}
        if not isinstance(parsed, dict):
            logger.debug("TERRAFLOW_REQUEST_VARIABLES must be a JSON object; skipping request-scoped lookup")
            return {}
    ...
    async def get_variable(self, name: str, **kwargs) -> str | None:  # noqa: ARG002
        ...
        Resolution order (first match wins):
          ...
          2. Request-scoped exact name (serve ContextVar or ``TERRAFLOW_REQUEST_VARIABLES``).
          3. Request-scoped ``x-terraflow-global-var-*`` alias.
          4. Environment variable (exact name).
          5. Environment variable (``x-terraflow-global-var-*`` alias).
        ...
        or the ``x-terraflow-global-var-*`` form — always beats a same-named variable left
