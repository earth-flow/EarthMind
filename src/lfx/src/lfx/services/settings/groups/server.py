...
class ServerSettings(BaseModel):
    ...
    """The host on which Terraflow will run."""
    port: int = 7860
    """The port on which Terraflow will run."""
    ...
    """The log level for Terraflow."""
    log_file: str | None = "logs/terraflow.log"
    """The path to log file for Terraflow."""
    ...
    """If set to True, Terraflow will open the browser on startup."""
    backend_only: bool = False
    """If set to True, Terraflow will not serve the frontend."""
    ...
    prefix (e.g. '/terraflow').  When set, the MCP SSE transport includes this
    prefix in the POST-back URL so clients can reach the correct endpoint.
    Can also be set via the TERRAFLOW_ROOT_PATH environment variable."""
    user_agent: str = "terraflow"
    ...
    def validate_runtime_port(cls, value):
        ...
        Kubernetes auto-creates env vars like TERRAFLOW_RUNTIME_PORT=tcp://<ip>:<port>
        for services, which collides with the TERRAFLOW_ env prefix. Extract the port
