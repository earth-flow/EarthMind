...
class PathSettings(BaseModel):
    """Filesystem paths EarthMind reads from and writes to."""

    config_dir: str | None = None
    """Base directory for EarthMind data (db, logs, caches)."""

    knowledge_bases_dir: str | None = "~/.earthmind/knowledge_bases"
    ...
    Comma-separated when set via env (``EARTHMIND_KB_ALLOWED_FOLDER_ROOTS``),
    ...
    @classmethod
    def set_earthmind_dir(cls, value: Any) -> str:
        if not value:
            from platformdirs import user_cache_dir

            app_name = "earthmind"
            app_author = "earthmind"
