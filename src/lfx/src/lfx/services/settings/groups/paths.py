...
class PathSettings(BaseModel):
    """Filesystem paths Terraflow reads from and writes to."""

    config_dir: str | None = None
    """Base directory for Terraflow data (db, logs, caches)."""

    knowledge_bases_dir: str | None = "~/.terraflow/knowledge_bases"
    ...
    Comma-separated when set via env (``TERRAFLOW_KB_ALLOWED_FOLDER_ROOTS``),
    ...
    @classmethod
    def set_terraflow_dir(cls, value: Any) -> str:
        if not value:
            from platformdirs import user_cache_dir

            app_name = "terraflow"
            app_author = "terraflow"
