...
class ComponentsSettings(BaseModel):
    ...
    permitted to execute, even when TERRAFLOW_ALLOW_CUSTOM_COMPONENTS is False.
    """
    ...
    """If set to True, Terraflow will only partially load components at startup and fully load them on demand.
    ...
    """If set to True, Terraflow will create starter projects. If False, skips all starter project setup.
    ...
    """If set to True, Terraflow will update starter projects."""
    ...
    explicit opt-in.  Set ``TERRAFLOW_ENABLE_EXTENSION_RELOAD=true`` in your
    ...
    def set_components_path(cls, value):
        ...
        If the `TERRAFLOW_COMPONENTS_PATH` environment variable is set and points to an existing path, it is
        appended to the provided list if not already present. If the input list is empty or missing, it is
        set to an empty list.
        """
        env_value = os.getenv("TERRAFLOW_COMPONENTS_PATH")
        if env_value:
            logger.debug("Adding TERRAFLOW_COMPONENTS_PATH to components_path")
            ...
            for raw_entry in env_value.split(os.pathsep):
                ...
                if not Path(entry).exists():
                    # Surface at warning so a typo in TERRAFLOW_COMPONENTS_PATH
    ...
    def _enforce_components_paths_override(self):
        ...
        matching built-in templates. By default ``TERRAFLOW_COMPONENTS_PATH`` and
        ``TERRAFLOW_COMPONENTS_INDEX_PATH`` still contribute to that trust set (an
        ...
        env_components_path = os.getenv("TERRAFLOW_COMPONENTS_PATH")
        if env_components_path:
            ...
            # would re-read TERRAFLOW_COMPONENTS_PATH and append the paths again.
            ...
            if stripped_any:
                logger.warning(
                    "Ignoring TERRAFLOW_COMPONENTS_PATH=%s: "
                    "TERRAFLOW_ALLOW_CUSTOM_COMPONENTS=False and "
                    "TERRAFLOW_ALLOW_COMPONENTS_PATHS_OVERRIDE=False.",
                    env_components_path,
                )
        ...
        env_components_index_path = os.getenv("TERRAFLOW_COMPONENTS_INDEX_PATH")
        if env_components_index_path and self.components_index_path == env_components_index_path:
            logger.warning(
                "Ignoring TERRAFLOW_COMPONENTS_INDEX_PATH=%s: "
                "TERRAFLOW_ALLOW_CUSTOM_COMPONENTS=False and "
                "TERRAFLOW_ALLOW_COMPONENTS_PATHS_OVERRIDE=False.",
