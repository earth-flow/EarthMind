...
Cache layout:
    ...
    * Disk snapshot at ``<terraflow-cache>/models_dev/snapshot.json`` for
      offline cold starts.
...
# Translation: models.dev provider slug -> Terraflow display name. Only
# providers Terraflow already knows how to instantiate (have an entry in
...
def _snapshot_dir() -> Path:
    ...
    """
    cache_root = Path(user_cache_dir("terraflow", "terraflow"))
...
def _translate_model_entry(
    ...
    *,
    ...
    """Convert one models.dev model entry to Terraflow's ``ModelMetadata`` shape.
