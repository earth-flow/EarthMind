"""Generate requirements.txt from a Terraflow flow JSON.
...
# Import names that are internal to the lfx/terraflow runtime and should
# never appear as separate requirements.
_INTERNAL_IMPORT_NAMES: frozenset[str] = frozenset({"lfx", "terraflow", "terraflow_base"})
        ...
        if source and isinstance(source, str):
            ...
            for imp in imports:
                ...
                if imp in STDLIB_MODULES:
                    continue
                # Skip lfx / terraflow internal imports - lfx provides these
...
def generate_requirements_from_flow(
    ...
    *,
    ...
    """Generate a requirements list from a Terraflow flow JSON.

    Args:
        flow: Parsed Terraflow flow JSON (dict).
...
def generate_requirements_txt(
    ...
    *,
    ...
    """Generate requirements.txt content from a Terraflow flow JSON.
    ...
    lines = [
        "# Auto-generated requirements for Terraflow flow",
...
def generate_requirements_from_file(
    ...
    *,
    ...
    Args:
        flow_path: Path to a Terraflow flow JSON file.
        lfx_package: Name of the LFX package to include.
        include_lfx: Whether to include the LFX package itself.
        pin_versions: If True, pin each package to the currently installed
            version.
