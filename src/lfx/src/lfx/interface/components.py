...
def _get_cache_path() -> Path:
    ...
    cache_dir = Path(user_cache_dir("lfx", "terraflow"))
...
def _save_generated_index(modules_dict: dict) -> None:
    ...
    try:
        ...
        terraflow_version = version("terraflow")

        # Build index structure
        index = {
            "version": terraflow_version,
            "metadata": {
...
async def _send_telemetry(
    ...
    try:
        ...
        from terraflow.services.telemetry.schema import ComponentIndexPayload
...
async def _load_components_dynamically(
    ...
    try:
        ...
        await logger.aerror(f"Failed to import terraflow.components package: {e}", exc_info=True)
        return modules_dict
...
async def import_terraflow_components(
    settings_service: Optional["SettingsService"] = None,
    telemetry_service: Any | None = None,
) -> dict[str, dict[str, Any]]:
    """Asynchronously discovers and loads all built-in Terraflow components.
...
def _components_path_extension_paths(settings_service: "SettingsService") -> list[Path]:
    ...
    Terraflow components path is excluded (it is not an inline-bundle root).
...
# distribution = explicit, packaged install) to least (TERRAFLOW_COMPONENTS_PATH
...
async def import_extension_components(
    ...
    Two sources feed this:
        ...
        - Subfolders of every ``TERRAFLOW_COMPONENTS_PATH`` entry (parsed
    ...
    # ``$TERRAFLOW_SEED_DIR`` (or the default ``/opt/terraflow/bundles``)
    ...
    # When neither $TERRAFLOW_SEED_DIR is set nor /opt/terraflow/bundles
    ...
    # approach that appended their bundle directories to TERRAFLOW_COMPONENTS_PATH,
    ...
    # dev (`lfx extension dev`) > inline (TERRAFLOW_COMPONENTS_PATH) -- and the
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Terraflow
    ...
    if component_cache.all_types_dict is None:
        ...
        terraflow_components = await import_terraflow_components(settings_service, telemetry_service)
        ...
        component_cache.all_types_dict = {
            **terraflow_components["components"],
            **custom_flat,
            **extension_components,
        }
