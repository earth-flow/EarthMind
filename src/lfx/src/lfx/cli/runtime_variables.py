...
_TERRAFLOW_REQUEST_VARIABLES_KEY = "TERRAFLOW_REQUEST_VARIABLES"
...
def build_request_variables_from_global_vars(global_vars: dict[str, str] | None) -> dict[str, str]:
    ...
    TRM sends:
    - ``TERRAFLOW_REQUEST_VARIABLES``: JSON blob of merged context + connection_details
    - raw credential keys (e.g. ``access_token``, ``wxo_github_access_token``)
    - ``x-terraflow-global-var-*`` aliases
    ...
    merged: dict[str, str] = {}
    raw_json = global_vars.get(_TERRAFLOW_REQUEST_VARIABLES_KEY)
    if raw_json:
        try:
            parsed = json.loads(raw_json)
        except json.JSONDecodeError as exc:
            logger.warning(
                f"Failed to parse {_TERRAFLOW_REQUEST_VARIABLES_KEY} JSON ({exc}); "
        ...
        if isinstance(parsed, dict):
            ...
            logger.warning(
                f"{_TERRAFLOW_REQUEST_VARIABLES_KEY} must be a JSON object, got {type(parsed).__name__}; ignoring blob."
            )

    for key, value in global_vars.items():
        if key == _TERRAFLOW_REQUEST_VARIABLES_KEY:
            continue
...
def apply_global_vars_to_graph(graph: Graph, global_vars: dict[str, str] | None) -> None:
    ...
    # Flatten the TERRAFLOW_REQUEST_VARIABLES blob here so the no-DB load_from_db path
