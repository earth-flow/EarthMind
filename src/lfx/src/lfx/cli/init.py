...
#
# Configure your Terraflow instances here.
...
#
# Quick start:
#   1. Open Terraflow → Settings → API Keys → Create a new key
#   2. export TERRAFLOW_LOCAL_API_KEY=<your key>
...
_TEST_FLOWS_PY = '''\
"""Integration tests for Terraflow flows.

Run against a local instance (started with ``lfx serve``):

    pytest tests/ --terraflow-url http://localhost:8000

Run against a named environment (staging, production, etc.):

    pytest tests/ --terraflow-env staging -m integration
...
def test_flow_output_quality(flow_runner):
    ...
    result = flow_runner("my-flow-endpoint", "What is Terraflow?")
...
_GITIGNORE = """\
# Terraflow credentials -- never commit API keys
# (terraflow-environments.toml may contain literal keys; .lfx/environments.yaml is safe to commit)
...
def init_command(
    ...
    *,
    ...
    # .gitignore — keep terraflow-environments.toml ignored for backward compat
    gitignore = target / ".gitignore"
    if gitignore.exists():
        existing_content = gitignore.read_text(encoding="utf-8")
        if "terraflow-environments.toml" not in existing_content:
    ...
    console.print("  2. [bold]export TERRAFLOW_LOCAL_API_KEY=<key>[/bold]   (Settings → API Keys)")
    if example:
        ...
        console.print("  5. [bold]lfx push --dir flows/ --env local[/bold]    (deploy to Terraflow)")
    else:
        console.print("  3. [bold]lfx create my-flow --template hello-world[/bold]")
        console.print("  4. [bold]lfx push --dir flows/ --env local[/bold]")
    console.print(f"  {'6' if example else '5'}. [bold]pytest tests/ --terraflow-env local[/bold]")
    console.print()
