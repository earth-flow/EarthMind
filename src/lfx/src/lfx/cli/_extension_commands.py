...
Commands shipped here:
    ...
    - ``dev``       -- register a local extension and launch Terraflow with
                       it loaded.
...
extension_app = typer.Typer(
    name="extension",
    help="Author and inspect Terraflow Extensions.",
...
def validate_command(
    ...
    *,
    ...
    subprocess with a temporary Terraflow state directory.
    """
...
def list_command(
    ...
    seed_dir: str | None = typer.Option(
        ...
        "--seed-dir",
        help=(
            "Override $TERRAFLOW_SEED_DIR for this invocation. "
    ...
    # and ``terraflow run`` without a separate "did my bundle install?"
    # debug cycle.
...
def reload_command(
    ...
    target: str | None = typer.Option(
        None,
        "--target",
        help=("Terraflow server URL (default: $TERRAFLOW_HOST / http://10.171.205.153:7860)."),
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="API key for the Terraflow server (default: $TERRAFLOW_API_KEY).",
    ),
    output_format: str = typer.Option(
        "text",
        "--format",
...
def schema_command(
    ...
    ``schemas.terraflow.org/extension/v1.json``; the release pipeline uploads
    this same artifact.
    """
...
def init_command(
    ...
    *,
    ...
    name: str | None = typer.Option(
        None,
        "--name",
        help=("Human-readable display name shown in Terraflow.  Defaults to the title-cased extension id."),
    ),
) -> None:
...
def _resolve_terraflow_executable() -> str | None:
    """Find the ``terraflow`` binary on PATH, if any.

    Returns ``None`` when terraflow isn't installed; the dev command then
    falls back to ``python -m terraflow.__main__`` so an author with the
    ``lfx`` package alone still gets a usable error message.
    """
    return shutil.which("terraflow")


@extension_app.command(
    name="dev",
    help="Register a local extension and launch Terraflow with it loaded.",
)
def dev_command(
    target: str = typer.Argument(
        ".",
        help=(
            ...
            "[tool.terraflow.extension] in pyproject.toml."
        ),
    ),
    *,
    ...
    skip_launch: bool = typer.Option(
        ...
        help=(
            ...
            "``terraflow run``.  Useful for tests and for embedding in "
            "external dev-server scripts."
        ),
    ),
    extra_args: list[str] | None = typer.Argument(
        None,
        help="Extra arguments forwarded to ``terraflow run`` (after a ``--`` separator).",
    ),
) -> None:
    """Register the extension and launch a Terraflow dev server.
    ...
    Flow:
        ...
        5. ``exec``-style hand-off to ``terraflow run`` (or
           ``python -m terraflow``) with the env var
           ``TERRAFLOW_LAZY_LOAD_COMPONENTS=false`` so dev-extensions are
           visible in the palette immediately.

    AC #4 ("boots Terraflow with the new Extension visible in the palette
    within 5s") is delivered jointly by this command and the startup
    hook in ``terraflow.main`` that consults the dev registry.
    """
    ...
    typer.echo("  - Click 'Reload' on the bundle header in the Terraflow palette.")

    if skip_launch:
        return

    extras = list(extra_args or [])
    cmd = _build_terraflow_run_argv(extras)
    typer.echo("")
    typer.echo("Launching: " + " ".join(shlex.quote(part) for part in cmd))

    env = _build_dev_launch_env(os.environ)

    # Replace the current process so Ctrl-C in the launched terraflow
    # propagates without an extra pty/job-control hop.  When ``terraflow``
...
def _build_dev_launch_env(base_env: os._Environ[str] | dict[str, str]) -> dict[str, str]:
    """Build the env dict handed to the launched ``terraflow run`` process.
    ...
    Flags set:
        - ``TERRAFLOW_LAZY_LOAD_COMPONENTS=false`` (always, overriding the
          author's shell): dev components must appear in the palette
          eagerly so the AC's 5-second budget holds.
        - ``TERRAFLOW_ENABLE_EXTENSION_RELOAD=true`` (setdefault): turns on
    ...
    """
    env = dict(base_env)
    env["TERRAFLOW_LAZY_LOAD_COMPONENTS"] = "false"
    env.setdefault("TERRAFLOW_ENABLE_EXTENSION_RELOAD", "true")
    return env


def _build_terraflow_run_argv(extra_args: list[str]) -> list[str]:
    """Build the argv list that exec's ``terraflow run``.

    Falls back to ``python -m terraflow`` when the ``terraflow`` binary is
    ...
    one).
    """
    binary = _resolve_terraflow_executable()
    if binary is not None:
        return [binary, "run", *extra_args]
    return [sys.executable, "-m", "terraflow", "run", *extra_args]
