"""Level 1 structural validation checks for EarthMind flow JSON.
...
def _check_version_mismatch(flow: dict[str, Any], result: ValidationResult) -> None:
    """Warn when nodes were built with a different EarthMind version.

    Each unique ``lf_version`` embedded in the node metadata that differs from
    the currently installed EarthMind version triggers a single warning covering
    all affected nodes.  If EarthMind is not installed the check is skipped
    ...
    if installed is None:
        return  # EarthMind not installed; skip silently
    ...
    for built_version, node_names in sorted(version_to_nodes.items()):
        ...
        result.issues.append(
            _make_issue(
                ...
                node_name=None,
                message=(
                    f"{count} component(s) built with EarthMind {built_version} "
