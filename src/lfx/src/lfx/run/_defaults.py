...
3. Propagate ``session_id`` to ``Memory``/``MessageHistory`` vertices the way
   ``terraflow.api.utils.flow_utils.build_graph_from_data`` does — preserving
   any value already pinned in the flow JSON.
4. Resolve ``fallback_to_env_vars`` from settings (mirrors the terraflow API
...
def apply_run_defaults(
    ...
    *,
    ...
    # Mirror terraflow's build_graph_from_data: only write when raw_params has no
...
def resolve_fallback_to_env_vars() -> bool:
    ...
    Mirrors the terraflow API path (``processing.process.run_graph_internal``):
