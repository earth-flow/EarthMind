...
HTTP 500), and ``TERRAFLOW_KB_ALLOWED_FOLDER_ROOTS`` bound to nothing because
...
def test_kb_allowed_folder_roots_binds_single_value_from_env(monkeypatch):
    """A single directory binds from the TERRAFLOW_-prefixed env var."""
    monkeypatch.setenv("TERRAFLOW_KB_ALLOWED_FOLDER_ROOTS", "/srv/docs")
    settings = Settings()
...
def test_kb_allowed_folder_roots_parses_comma_separated_env(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_KB_ALLOWED_FOLDER_ROOTS", "/srv/docs,/data/shared")
    settings = Settings()
