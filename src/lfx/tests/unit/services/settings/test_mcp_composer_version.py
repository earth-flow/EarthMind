...
def test_bare_version_gets_tilde_equals_prefix(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_MCP_COMPOSER_VERSION", "0.1.0.7")
    settings = Settings()
...
def test_empty_version_gets_default(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_MCP_COMPOSER_VERSION", "")
    settings = Settings()
...
def test_no_env_var_uses_default(monkeypatch):
    ...
    monkeypatch.delenv("TERRAFLOW_MCP_COMPOSER_VERSION", raising=False)
    settings = Settings()
    assert settings.mcp_composer_version == "==0.1.0.8.10"
...
def test_three_part_version_gets_prefix(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_MCP_COMPOSER_VERSION", "1.2.3")
    settings = Settings()
...
def test_two_part_version_gets_prefix(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_MCP_COMPOSER_VERSION", "1.2")
    settings = Settings()
...
def test_single_digit_version_gets_prefix(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_MCP_COMPOSER_VERSION", "1")
    settings = Settings()
