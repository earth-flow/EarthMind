...
def test_redis_queue_startup_grace_s_rejects_negative(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_STARTUP_GRACE_S", "-1")
...
def test_redis_queue_startup_grace_s_accepts_zero(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_STARTUP_GRACE_S", "0")
    settings = Settings()
...
def test_redis_queue_cancel_marker_ttl_rejects_zero(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_CANCEL_MARKER_TTL", "0")
    with pytest.raises(ValidationError):
        Settings()
...
def test_redis_queue_cancel_marker_ttl_rejects_negative(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_CANCEL_MARKER_TTL", "-1")
    with pytest.raises(ValidationError):
        Settings()
...
def test_redis_queue_polling_watchdog_interval_s_rejects_zero(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_POLLING_WATCHDOG_INTERVAL_S", "0")
    with pytest.raises(ValidationError):
        Settings()
...
def test_redis_queue_polling_stale_threshold_s_rejects_negative(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S", "-1")
    with pytest.raises(ValidationError):
        Settings()
...
def test_redis_queue_polling_stale_threshold_s_accepts_zero(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S", "0")
    settings = Settings()
