...
TERRAFLOW_RUNTIME_PORT=tcp://<ip>:<port> which collide with
pydantic-settings TERRAFLOW_ prefix. The validator should
...
def test_runtime_port_from_k8s_tcp_url(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.96.0.1:7865")
    settings = Settings()
...
def test_runtime_port_from_k8s_tcp_url_different_port(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.0.0.5:8080")
    settings = Settings()
...
def test_runtime_port_from_integer_string(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "7865")
    settings = Settings()
    assert settings.runtime_port == 7865
...
def test_runtime_port_default_is_none(monkeypatch):
    ...
    monkeypatch.delenv("TERRAFLOW_RUNTIME_PORT", raising=False)
    settings = Settings()
...
def test_runtime_port_garbage_value_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "not-a-port")
    settings = Settings()
    assert settings.runtime_port is None
...
def test_runtime_port_from_http_url(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "http://10.96.0.1:7865")
    settings = Settings()
    assert settings.runtime_port == 7865
...
def test_runtime_port_url_without_port_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.96.0.1")
    settings = Settings()
    assert settings.runtime_port is None
...
def test_runtime_port_url_with_out_of_range_port_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.0.0.1:99999")
    settings = Settings()
    assert settings.runtime_port is None
...
def test_runtime_port_url_with_non_numeric_port_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.0.0.1:abc")
    settings = Settings()
    assert settings.runtime_port is None
...
def test_runtime_port_url_with_negative_port_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("TERRAFLOW_RUNTIME_PORT", "tcp://10.0.0.1:-1")
    settings = Settings()
    assert settings.runtime_port is None
