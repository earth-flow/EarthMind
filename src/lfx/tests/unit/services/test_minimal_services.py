...
class TestVariableService:
    ...
    async def test_get_variable_absorbs_extra_kwargs(self, variables):
        """Extra kwargs are accepted for compatibility with terraflow call signature."""
    ...
    @pytest.mark.asyncio
    async def test_get_variable_from_terraflow_request_variables(self, variables):
        """Test request-scoped variables are read from TERRAFLOW_REQUEST_VARIABLES."""
        os.environ["TERRAFLOW_REQUEST_VARIABLES"] = '{"runtime_token":"abc123","normal_key":"value1"}'
        try:
            assert await variables.get_variable("runtime_token") == "abc123"
            assert await variables.get_variable("normal_key") == "value1"
        finally:
            del os.environ["TERRAFLOW_REQUEST_VARIABLES"]

    async def test_global_alias_from_request_scope(self, variables):
        """x-terraflow-global-var-* aliases resolve from request-scoped variables."""
        from lfx.services.variable.request_scope import activate_request_variables, reset_request_variables

        token = activate_request_variables({"x-terraflow-global-var-access-token": "alias-token"})
        try:
    ...
    async def test_request_scope_alias_overrides_env(self, variables):
        """A request-scoped x-terraflow-global-var-* alias wins over an env var of the same name.
    ...
    @pytest.mark.asyncio
    async def test_terraflow_request_variables_invalid_json_falls_back(self, variables):
        """Test invalid request variable JSON does not break env fallback."""
        os.environ["TERRAFLOW_REQUEST_VARIABLES"] = "{not-json"
    ...
    async def test_strict_no_bearer_token_from_request_access_token(self, variables):
        ...
        os.environ["TERRAFLOW_REQUEST_VARIABLES"] = '{"wxo_github_access_token":"request-token"}'
        try:
    ...
    async def test_strict_no_token_access_token_alias_resolution(self, variables):
        ...
        os.environ["TERRAFLOW_REQUEST_VARIABLES"] = '{"access_token":"request-access"}'
        try:
    ...
    async def test_strict_no_prefixed_token_alias_resolution(self, variables):
        ...
        os.environ["TERRAFLOW_REQUEST_VARIABLES"] = '{"my_app_access_token":"prefixed-token"}'
        try:
    ...
    async def test_global_var_alias_resolution(self, variables):
        """Test x-terraflow-global-var-* alias lookup."""
        os.environ["x-terraflow-global-var-access-token"] = "global-token"  # noqa: SIM112
        try:
            assert await variables.get_variable("access_token") == "global-token"
            assert await variables.get_variable("token") is None
        finally:
            del os.environ["x-terraflow-global-var-access-token"]  # noqa: SIM112
