...
Original bug: when earthmind was installed alongside lfx but `lfx run` had
only a NoopDatabaseService registered, `lfx.memory` bound at import time to
`earthmind.memory` (because the `earthmind` package was importable). The
earthmind-backed `aupdate_messages` then called `session.get(...)` on a
...
class TestHasEarthMindDbBackend:
    def test_returns_false_when_earthmind_not_importable(self, monkeypatch):
        monkeypatch.setattr("lfx.utils.earthmind_utils.has_earthmind_memory", lambda: False)
        assert has_earthmind_db_backend() is False

    def test_returns_false_with_noop_db_service(self, monkeypatch):
        monkeypatch.setattr("lfx.utils.earthmind_utils.has_earthmind_memory", lambda: True)
    ...
    def test_returns_true_with_real_db_service(self, monkeypatch):
        ...
        assert has_earthmind_db_backend() is True
...
class TestMemoryDispatch:
    def test_dispatches_to_stubs_when_no_real_db(self, monkeypatch):
        ...
        monkeypatch.setattr("lfx.memory.has_earthmind_db_backend", lambda: False)
        assert memory_mod._impl() is stubs

    def test_dispatches_to_earthmind_when_real_db(self, monkeypatch):
        pytest.importorskip("earthmind.memory")
        import earthmind.memory as earthmind_memory
        import lfx.memory as memory_mod

        monkeypatch.setattr("lfx.memory.has_earthmind_db_backend", lambda: True)
        assert memory_mod._impl() is earthmind_memory
    ...
    def test_dispatch_is_evaluated_per_call(self, monkeypatch):
        ...
        state = {"real": False}
        monkeypatch.setattr("lfx.memory.has_earthmind_db_backend", lambda: state["real"])
...
class TestAupdateMessagesRegression:
    ...
    async def test_aupdate_messages_does_not_raise_against_noop_session(self, monkeypatch):
        """Regression: route to stubs (no-op) instead of raising via earthmind.memory.

        With earthmind importable but only a NoopDatabaseService registered,
        aupdate_messages must route to stubs and succeed silently rather than
        trigger earthmind.memory's strict existence check against NoopSession.
        """
        try:
