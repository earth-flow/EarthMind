...
def test_importing_terraflow_module_in_lfx():
    code = dedent("""from terraflow.custom import   Component
...
def test_importing_terraflow_logging_in_lfx():
    """Test that terraflow.logging can be imported in lfx context without errors."""
    code = dedent("""
...
class TestGetModuleFallbacks:
    ...
    def test_terraflow_falls_back_to_lfx(self):
        result = _get_module_fallbacks("terraflow.custom")
        assert result == ["terraflow.custom", "lfx.custom"]

    def test_terraflow_deep_path(self):
        result = _get_module_fallbacks("terraflow.custom.validate")
        assert result == ["terraflow.custom.validate", "lfx.custom.validate"]
    ...
    def test_bare_terraflow_no_fallback(self):
        assert _get_module_fallbacks("terraflow") == ["terraflow"]
    ...
    def test_original_always_first(self):
        ...
        for name in ["terraflow.custom", "langchain.agents", "requests"]:
