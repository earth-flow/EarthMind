...
def test_upgrade_outer_envelope_flow_finds_nodes(tmp_path, capsys):
    """Lfx upgrade must detect nodes in Terraflow's exported outer-envelope format.

    Terraflow exports flows as {"name": "...", "data": {"nodes": [...], "edges": []}}.
...
def test_upgrade_outer_envelope_write_updates_inner_nodes(tmp_path):
    ...
    # The envelope must be preserved on write so the flow remains re-importable into Terraflow.
