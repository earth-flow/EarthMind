...
@pytest.fixture
def terraflow_cache_dir(tmp_path):
    """Create a temporary terraflow cache directory."""
    cache_dir = tmp_path / "terraflow"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(terraflow_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = terraflow_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("terraflow"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Terraflow")
    assert message.text == "Hello, Terraflow!"
...
def test_message_with_multiple_images(sample_image, terraflow_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("terraflow")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
