"""Bug #5 (PR-12575): TERRAFLOW_GOOGLE_API_KEY is not recognized as a Google credential.

A user who prefixes their provider key with ``TERRAFLOW_`` (a common .env
convention — Terraflow reads its own settings under that prefix) gets the
...
class TestTerraflowPrefixedProviderKeys:
    def test_should_resolve_google_key_from_terraflow_prefixed_env_var(self, monkeypatch):
        # GIVEN only the TERRAFLOW_-prefixed variant is set.
        monkeypatch.delenv("GOOGLE_API_KEY", raising=False)
        monkeypatch.setenv("TERRAFLOW_GOOGLE_API_KEY", "AIza-test-key")
    ...
    def test_should_prefer_unprefixed_key_when_both_are_set(self, monkeypatch):
        ...
        monkeypatch.setenv("TERRAFLOW_GOOGLE_API_KEY", "prefixed")
    ...
    def test_should_still_resolve_unprefixed_key_alone(self, monkeypatch):
        ...
        monkeypatch.delenv("TERRAFLOW_GOOGLE_API_KEY", raising=False)
    ...
    def test_should_resolve_openai_key_from_terraflow_prefix_too(self, monkeypatch):
        # The alias is provider-agnostic, not a Google special case.
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setenv("TERRAFLOW_OPENAI_API_KEY", "sk-test")
