"""Tests for manifest-first precedence over ``terraflow.plugins``.
...
def test_finds_pyproject_only_distribution(tmp_path: Path) -> None:
    """A distribution shipping pyproject.toml with [tool.terraflow.extension] is found."""
...
def test_pyproject_without_extension_section_is_ignored(tmp_path: Path) -> None:
    """A pyproject.toml without [tool.terraflow.extension] is NOT treated as a manifest.
...
def test_malformed_pyproject_section_surfaces_manifest_invalid(tmp_path: Path) -> None:
    """A pyproject with [tool.terraflow.extension] missing required fields surfaces manifest-invalid.
...
def test_malformed_pyproject_section_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    its legacy ``terraflow.plugins`` component entry-points must be
...
def test_pyproject_only_extension_loads_at_official_slot(tmp_path: Path) -> None:
    ...
    Without this path the AC's "extension.json or [tool.terraflow.extension]"
...
def test_pyproject_only_distribution_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    A legacy ``terraflow.plugins`` component entry-point on a pyproject-form
...
def test_uses_real_distributions_by_default(tmp_path: Path) -> None:
    ...
    Terraflow itself does not ship an extension manifest today, but if a
...
def test_component_filter_keeps_components_on_non_manifest_distribution(tmp_path: Path) -> None:
    ...
    The legacy ``terraflow.plugins`` path is still the registration channel
