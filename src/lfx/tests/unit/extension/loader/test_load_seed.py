...
``$TERRAFLOW_SEED_DIR`` (or the default ``/opt/terraflow/bundles/``) that
...
def test_seed_default_absent_is_silent_no_op(tmp_path: Path) -> None:
    """Mode A: $TERRAFLOW_SEED_DIR unset and the default doesn't exist -> []."""
...
def test_pathsep_split_loads_multiple_seed_roots(tmp_path: Path) -> None:
    """``$TERRAFLOW_SEED_DIR`` is pathsep-separated for multiple roots."""
    root_a = tmp_path / "a"
    root_b = tmp_path / "b"
    root_a.mkdir()
    root_b.mkdir()
