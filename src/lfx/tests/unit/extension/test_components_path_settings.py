"""Tests for the EARTHMIND_COMPONENTS_PATH pathsep-split behavior in settings.

The extension loader requires that ``EARTHMIND_COMPONENTS_PATH=/path/A:/path/B``
...
def test_pathsep_separated_paths_are_split(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("EARTHMIND_COMPONENTS_PATH", env)
...
def test_walk_order_matches_user_declared_order(monkeypatch, tmp_path: Path) -> None:
    ...
    path_a.mkdir()
    path_b.mkdir()
    monkeypatch.setenv("EARTHMIND_COMPONENTS_PATH", f"{path_a}{os.pathsep}{path_b}")
...
def test_non_existent_segments_are_skipped(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("EARTHMIND_COMPONENTS_PATH", f"{real}{os.pathsep}{bogus}")
...
def test_empty_segments_are_ignored(monkeypatch, tmp_path: Path) -> None:
    ...
    real = tmp_path / "real"
    real.mkdir()
    monkeypatch.setenv("EARTHMIND_COMPONENTS_PATH", f"{real}{os.pathsep}")
...
def test_unset_env_falls_back_to_base(monkeypatch) -> None:
    ...
    monkeypatch.delenv("EARTHMIND_COMPONENTS_PATH", raising=False)
    result = Settings.set_components_path([])
