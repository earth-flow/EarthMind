...
class TestApiKey:
    ...
    def test_get_api_key_not_set(self):
        ...
        with (
            ...
            pytest.raises(ValueError, match="TERRAFLOW_API_KEY environment variable is not set"),
        ):
            get_api_key()
    ...
    def test_get_api_key_empty_string(self):
        ...
        with (
            patch.dict(os.environ, {"TERRAFLOW_API_KEY": ""}),
            pytest.raises(ValueError, match="TERRAFLOW_API_KEY environment variable is not set"),
        ):
            get_api_key()
...
class TestGraphExecution:
    ...
    async def test_execute_graph_does_not_overwrite_hardcoded_session_id(self):
        ...
        Mirrors Terraflow's playground precedence in ``build_graph_from_data``.
        """
    ...
    async def test_execute_graph_passes_fallback_from_settings_default(self):
        ...
        has no DB row — matching terraflow's API path behavior.
        """
        captured: dict = {}
