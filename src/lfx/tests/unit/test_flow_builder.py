...
class TestConnect:
    ...
    def test_add_connection_dedupes_against_ui_saved_edges(self):
        ...
        from older Terraflow versions use the `xy-edge__` id prefix
...
# pattern as real Terraflow components: tool_mode=True on an INPUT, plain
...
class TestConnectComponentAsTool:
    ...
    def test_synthesized_tool_output_matches_canonical_terraflow_shape(self):
        ...
        # Required keys present in every real Terraflow tool output entry.
        required_keys = {
            "allows_loop",
            "cache",
            "display_name",
...
class TestCustomStringifyExtended:
    ...
    def test_scaped_round_trips_handle_format(self):
        """A source handle dict should produce a string that matches Terraflow's format."""
        handle = {
            "dataType": "ChatInput",
            "id": "ChatInput-abc12",
            "name": "message",
