...
def test_schema_to_terraflow_inputs():
    ...
    # Convert schema to Terraflow inputs
    inputs = schema_to_terraflow_inputs(TestSchema)
...
def test_schema_to_terraflow_inputs_sets_input_types_for_connectable_handles():
    """Non-string schema fields must carry input_types so the frontend renders connection handles.

    Regression test for https://github.com/terraflow-ai/terraflow/issues/9424:
    MCP Tool components did not expose input ports for number, bool, or dict/JSON fields
    because schema_to_terraflow_inputs left input_types unset, and the frontend's
    computeDisplayHandle hides handles for TERRAFLOW_SUPPORTED_TYPES with no input_types.
    """
    ...
    inputs = {inp.name: inp for inp in schema_to_terraflow_inputs(ConnectableSchema)}
...
def test_schema_to_terraflow_inputs_sets_data_input_types_for_nested_dict():
    ...
    inputs = {inp.name: inp for inp in schema_to_terraflow_inputs(model)}

    assert isinstance(inputs["config"], NestedDictInput)
    assert inputs["config"].input_types == ["JSON"]


def test_schema_to_terraflow_inputs_preserves_optional_defaults_and_nullable_objects():
    schema = {
        "type": "object",
        "properties": {
            "task": {"type": "string"},
    ...
    inputs = {input_.name: input_ for input_ in schema_to_terraflow_inputs(model)}
...
def test_schema_to_terraflow_inputs_invalid_type():
    ...
    with pytest.raises(TypeError, match="Unsupported field type:"):
        schema_to_terraflow_inputs(InvalidSchema)
