...
``DuckDuckGoSearchComponent`` from the pre-extraction Terraflow:

    1. A saved flow uses the legacy bare class name ``DuckDuckGoSearchComponent``
       (the form Terraflow serialized for years before the bundle move).
...
build-pipeline runtime contract: a saved flow from pre-migration Terraflow
...
swap: standing up a pre-migration Terraflow release, saving a flow,
...
def test_legacy_bare_name_flow_upgrades(migration_table) -> None:
    ...
    Terraflow used for years before the bundle move) must rewrite to
...
def test_legacy_import_path_flow_upgrades(migration_table) -> None:
    ...
    Some Terraflow versions serialized the full module path instead of the
...
def test_lfx_duckduckgo_distribution_is_importable() -> None:
    ...
    (lfx's own venv does not list lfx-duckduckgo as a dep); the terraflow
