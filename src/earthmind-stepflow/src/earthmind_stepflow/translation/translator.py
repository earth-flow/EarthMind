"""Main EarthMind to Stepflow converter implementation."""
...
class WorkflowAnalysis:
    """Typed analysis results from analyzing a EarthMind workflow."""

    node_count: int  # Total number of nodes in the EarthMind workflow
...
class EarthMindConverter:
    """Convert EarthMind JSON workflows to Stepflow YAML workflows."""
    ...
    def convert_file(self, input_path: str | Path) -> str:
        """Convert a EarthMind JSON file to Stepflow YAML.

        Args:
            input_path: Path to the EarthMind JSON file
        ...
        workflow = self.convert(earthmind_data)
        return self.to_yaml(workflow)

    def convert(self, earthmind_data: dict[str, Any]) -> Flow:
        """Convert EarthMind data structure to Stepflow workflow.

        Args:
            earthmind_data: Parsed EarthMind JSON data
        ...
        try:
            ...
            if "data" not in earthmind_data:
                raise ConversionError("Invalid EarthMind JSON: missing 'data' key")

            data = earthmind_data["data"]
            nodes = data.get("nodes", [])
            edges = data.get("edges", [])

            if not nodes:
                raise ConversionError("No nodes found in EarthMind workflow")
            ...
            # Create FlowBuilder
            builder = FlowBuilder(name=self._generate_workflow_name(earthmind_data))
    ...
    def analyze(self, earthmind_data: dict[str, Any]) -> WorkflowAnalysis:
        """Analyze EarthMind workflow structure without conversion.
    ...
    def _generate_workflow_name(self, earthmind_data: dict[str, Any]) -> str:
        """Generate a workflow name from EarthMind data."""
        # Try to get name from various sources
        if "name" in earthmind_data:
            name = earthmind_data["name"]
            return str(name) if name is not None else "Converted EarthMind Workflow"
        ...
        return "Converted EarthMind Workflow"
    ...
    def _build_flow_output(
        ...
        Args:
            ...
            nodes: Original EarthMind nodes
