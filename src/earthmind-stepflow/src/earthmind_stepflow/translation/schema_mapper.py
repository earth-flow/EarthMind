"""Schema mapping between EarthMind and Stepflow formats."""

from typing import Any


class SchemaMapper:
    """Maps schemas between EarthMind and Stepflow formats."""

    def __init__(self):
        """Initialize schema mapper with type mappings."""
        self.earthmind_to_json_schema = {
            "str": "string",
            "int": "integer",
            "float": "number",
            "bool": "boolean",
        ...
        # component_output_heuristics: Maps EarthMind component types to their expected
    ...
    def extract_output_schema(self, node: dict[str, Any]) -> dict[str, Any]:
        """Extract output schema from a EarthMind node.

        Args:
            node: EarthMind node object
        ...
        if outputs:
            return self._convert_earthmind_outputs_to_schema(outputs)

        # Method 2: Check base classes
        base_classes = node_data.get("base_classes", [])
        if base_classes:
            return self._convert_earthmind_types_to_schema(base_classes)
        ...
        if component_type in self.component_output_heuristics:
            ...
            return self._convert_earthmind_types_to_schema(types)
    ...
    def extract_input_schema(self, node: dict[str, Any]) -> dict[str, Any]:
        """Extract input schema from a EarthMind node template.
        ...
        for field_name, field_config in template.items():
            ...
            json_type = self.earthmind_to_json_schema.get(field_type, "string")
    ...
    def _convert_earthmind_outputs_to_schema(self, outputs: list[dict[str, Any]]) -> dict[str, Any]:
        """Convert EarthMind outputs metadata to JSON schema."""
        if not outputs:
            return {"type": "object"}
        ...
        return self._convert_earthmind_types_to_schema(output_types)

    def _convert_earthmind_types_to_schema(self, earthmind_types: list[str]) -> dict[str, Any]:
        """Convert EarthMind types to JSON schema.

        Args:
            earthmind_types: List of EarthMind type names

        Returns:
            JSON schema object
        """
        if "Message" in earthmind_types:
            return {
                "type": "object",
                "properties": {
                    "text": {"type": "string"},
        ...
        elif "DataFrame" in earthmind_types:
            return {
                "type": "object",
                "properties": {
