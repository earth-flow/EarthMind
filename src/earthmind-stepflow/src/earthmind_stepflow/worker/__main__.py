"""Standalone Stepflow component server for EarthMind integration.

Run with: python -m earthmind_stepflow.worker
"""
...
from earthmind_stepflow.worker.component_tool import component_tool_executor
from earthmind_stepflow.worker.core_executor import CoreExecutor
from earthmind_stepflow.worker.custom_code_executor import CustomCodeExecutor
...
async def custom_code_component(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Execute a EarthMind custom code component."""
...
async def core_component(
    ...
    """Execute a known core EarthMind component by module path."""
...
async def component_tool_component(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Create tool wrappers from EarthMind components."""
    return await component_tool_executor(input_data, context)


def main():
    """Main entry point for the EarthMind component server.
    ...
    - STEPFLOW_QUEUE_NAME: Queue name for gRPC transport (default: earthmind)
    ...
    if os.environ.get("EARTHMIND_DATABASE_URL"):
    ...
    parser = argparse.ArgumentParser(description="EarthMind Stepflow Component Server")
    parser.add_argument(
        "--tasks-url",
        type=str,
    ...
    parser.add_argument(
        "--queue-name",
        type=str,
        default=os.environ.get("STEPFLOW_QUEUE_NAME", "earthmind"),
