"""Input handler that coerces EarthMind Message objects to plain strings."""
...
class StringCoercionInputHandler(InputHandler):
    """Coerce EarthMind Message objects to their ``.text`` for string fields.

    Matches template fields with ``type == "str"`` whose runtime value is a
    EarthMind ``Message`` instance. Extracts the ``text`` attribute so the
