"""Handlers for EarthMind core types (Message, Data, DataFrame).

Input: deserialize dicts with ``__earthmind_type__`` markers back to lfx objects.
Output: serialize Message/Data objects to dicts with ``__earthmind_type__`` markers.
"""
...
# back to the canonical EarthMind type names used in serialized payloads.
_CLASS_TO_EARTHMIND_TYPE: dict[str, str] = {
    "JSON": "Data",
    "Table": "DataFrame",
}


def _earthmind_type_name(value: Any) -> str:
    """Return the canonical EarthMind type name for a value."""
    cls_name = type(value).__name__
    return _CLASS_TO_EARTHMIND_TYPE.get(cls_name, cls_name)


def _is_earthmind_type_dict(value: Any) -> bool:
    """Check if a single value is a dict with a ``__earthmind_type__`` marker."""
    return isinstance(value, dict) and "__earthmind_type__" in value


def _has_earthmind_type_marker(value: Any) -> bool:
    """Check if a value (or list of values) contains __earthmind_type__ markers."""
    if _is_earthmind_type_dict(value):
        return True
    if isinstance(value, list):
        return any(_is_earthmind_type_dict(item) for item in value)
    return False


def _deserialize_single(obj: dict[str, Any]) -> Any:
    """Deserialize a single dict with __earthmind_type__ marker."""
    try:
    ...
    earthmind_type = obj.get("__earthmind_type__")
    if not earthmind_type:
        return obj

    obj_data = {k: v for k, v in obj.items() if k != "__earthmind_type__"}

    if earthmind_type == "Message":
        return Message(**obj_data)
    elif earthmind_type == "Data":
        return Data(**obj_data)
    elif earthmind_type == "DataFrame":
...
class EarthMindTypeInputHandler(InputHandler):
    """Deserialize dicts with ``__earthmind_type__`` markers to lfx objects.
    ...
    def matches(self, *, template_field: dict[str, Any], value: Any) -> bool:
        return _has_earthmind_type_marker(value)
    ...
    async def prepare(self, fields: dict[str, tuple[Any, dict[str, Any]]], context: Any) -> dict[str, Any]:
        ...
        for key, (value, _template_field) in fields.items():
            if _is_earthmind_type_dict(value):
                ...
                result[key] = [_deserialize_single(item) if _is_earthmind_type_dict(item) else item for item in value]

        return result


class EarthMindTypeOutputHandler(OutputHandler):
    """Serialize Message and Data objects with ``__earthmind_type__`` markers.

    Matches ``isinstance(value, (Message, Data))`` and produces dicts with
    ``model_dump(mode="json")`` plus a ``__earthmind_type__`` key.
    """
    ...
    async def process(self, value: Any) -> Any:
        ...
        serialized["__earthmind_type__"] = _earthmind_type_name(value)
        return serialized
