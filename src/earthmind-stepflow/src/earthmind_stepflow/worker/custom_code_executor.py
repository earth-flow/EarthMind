"""Custom code executor for EarthMind components.

Executes EarthMind components by compiling their code from blob storage.
...
class CustomCodeExecutor(BaseExecutor):
    """Executes EarthMind custom code components by compiling code from blobs.
    ...
    def _get_input_handlers(self) -> list[InputHandler]:
        ...
        conversion needed for EarthMind type transformations.
        """
        return [
            EarthMindTypeInputHandler(),
            BaseModelInputHandler(),
            ...
            if blob_id not in self.compiled_components:
                try:
                    ...
                    raise ExecutionError(
                        f"No component code found for blob {blob_id}. All components must have real EarthMind code."
                    ) from e
