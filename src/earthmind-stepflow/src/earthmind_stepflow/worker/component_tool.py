"""Component for creating tool wrappers from EarthMind components."""
...
async def component_tool_executor(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Create a tool wrapper from EarthMind component code and inputs.

    This component takes a EarthMind component's code/JSON and static inputs,
    ...
    try:
        ...
        for field_name, field_data in template.items():
            if isinstance(field_data, dict) and field_data.get("tool_mode", False):
                ...
                prop: dict[str, Any] = {
                    "type": _map_earthmind_type_to_json_schema(field_type),
...
def _map_earthmind_type_to_json_schema(earthmind_type: str) -> str:
    """Map EarthMind field types to JSON Schema types.

    Args:
        earthmind_type: EarthMind field type (e.g., "str", "int", "bool")
    ...
    return type_mapping.get(earthmind_type, "string")
