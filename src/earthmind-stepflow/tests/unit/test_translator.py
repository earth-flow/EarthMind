"""Unit tests for EarthMindConverter."""

from typing import Any

import pytest

from earthmind_stepflow.exceptions import ConversionError
...
def _custom_node(node_id: str, component_type: str, outputs: list[dict[str, str]]) -> dict[str, Any]:
    """Build a custom-code EarthMind node with the given outputs."""
    return {
        "id": node_id,
        "type": "genericNode",
        "data": {
...
def _edge(source: str, target: str, source_output: str, target_field: str) -> dict[str, Any]:
    """Build a EarthMind edge carrying explicit source-output and target-field handles."""
    return {
        "source": source,
        "target": target,
        "data": {
...
def _known_core_node(node_id: str, component_type: str, outputs: list[dict[str, str]]) -> dict[str, Any]:
    """Build a EarthMind node that routes through the core executor (no blob)."""
    return {
        "id": node_id,
        "type": "genericNode",
        "data": {
...
class TestEarthMindConverter:
    """Test EarthMindConverter functionality."""

    def test_init_default(self):
        """Test default initialization."""
        converter = EarthMindConverter()
        assert converter is not None

    def test_convert_simple_workflow(self, converter: EarthMindConverter, simple_earthmind_workflow: dict[str, Any]):
        """Test conversion of simple workflow."""
        workflow = converter.convert(simple_earthmind_workflow)

        assert workflow.name == "Converted EarthMind Workflow"
    ...
    def test_convert_empty_nodes(self, converter: EarthMindConverter):
    ...
    def test_convert_missing_data(self, converter: EarthMindConverter):
    ...
    def test_to_yaml(self, converter: EarthMindConverter, simple_earthmind_workflow: dict[str, Any]):
        """Test YAML generation."""
        workflow = converter.convert(simple_earthmind_workflow)
        yaml_output = converter.to_yaml(workflow)

        assert isinstance(yaml_output, str)
        assert "name: Converted EarthMind Workflow" in yaml_output
        # Should have output section since ChatInput/ChatOutput create passthrough
        assert "output:" in yaml_output

    def test_analyze_workflow(self, converter: EarthMindConverter, simple_earthmind_workflow: dict[str, Any]):
        """Test workflow analysis."""
        analysis = converter.analyze(simple_earthmind_workflow)
    ...
    def test_step_ordering_with_dependencies(self, converter: EarthMindConverter):
...
"""
    ...
    def test_complex_dependency_ordering(self, converter: EarthMindConverter):
...
"""
        ...
        assert "/earthmind/custom_code" in step_components, "Should route custom code to custom code executor"
    ...
    def test_build_output_mapping_is_edge_scoped(self, converter: EarthMindConverter):
    ...
    def test_build_source_outputs_collects_distinct_outputs(self, converter: EarthMindConverter):
    ...
    def test_multi_output_fanout_routes_each_branch_to_its_own_output(self, converter: EarthMindConverter):
        ...
        assert "earthmind_splitter" in step_ids
        assert "earthmind_splitter__output_b" in step_ids

        # Each materialized source step selects the output its branch needs.
        primary_blob = get_step_input_dict(find_step(workflow, "earthmind_splitter_blob"))
        secondary_blob = get_step_input_dict(find_step(workflow, "earthmind_splitter__output_b_blob"))
        ...
        consumer_a_inputs = get_step_input_dict(find_step(workflow, "earthmind_consumer-a"))["input"]
        consumer_b_inputs = get_step_input_dict(find_step(workflow, "earthmind_consumer-b"))["input"]
        a_ref = ref_step_id(consumer_a_inputs["input_0"])
        b_ref = ref_step_id(consumer_b_inputs["input_0"])

        assert a_ref == "earthmind_splitter"
        assert b_ref == "earthmind_splitter__output_b"
        assert a_ref != b_ref

    def test_single_output_fanout_reuses_one_source_step(self, converter: EarthMindConverter):
        ...
        source_executor_steps = [
            sid for sid in step_ids if sid == "earthmind_source" or sid.startswith("earthmind_source__")
        ]
        assert source_executor_steps == ["earthmind_source"]

        # Both consumers reference that single source step.
        for consumer in ("earthmind_consumer-a", "earthmind_consumer-b"):
            consumer_inputs = get_step_input_dict(find_step(workflow, consumer))["input"]
            assert ref_step_id(consumer_inputs["input_0"]) == "earthmind_source"

    def test_multi_output_fanout_core_executor_path(self, converter: EarthMindConverter):
        ...
        assert "earthmind_splitter-core" in step_ids
        assert "earthmind_splitter-core__output_b" in step_ids

        # Each core step carries the correct selected_output inline.
        primary = get_step_input_dict(find_step(workflow, "earthmind_splitter-core"))
        secondary = get_step_input_dict(find_step(workflow, "earthmind_splitter-core__output_b"))
        ...
        a_ref = ref_step_id(get_step_input_dict(find_step(workflow, "earthmind_consumer-a"))["input"]["input_0"])
        b_ref = ref_step_id(get_step_input_dict(find_step(workflow, "earthmind_consumer-b"))["input"]["input_0"])
        assert a_ref == "earthmind_splitter-core"
        assert b_ref == "earthmind_splitter-core__output_b"

    def test_chat_output_resolves_non_primary_fanout_output(self, converter: EarthMindConverter):
        ...
        consumer_inputs = get_step_input_dict(find_step(workflow, "earthmind_consumer"))["input"]
        assert ref_step_id(consumer_inputs["input_0"]) == "earthmind_splitter"

        # ...and the workflow output (driven by ChatOutput) takes output_b.
        assert ref_step_id(flow_output(workflow)) == "earthmind_splitter__output_b"
