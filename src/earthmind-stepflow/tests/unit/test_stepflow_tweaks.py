...
from earthmind_stepflow.translation.stepflow_tweaks import (
...
class TestStepflowTweaks:
    ...
    def test_convert_tweaks_to_overrides_basic(self):
        ...
        assert "earthmind_LanguageModelComponent-kBOja" in overrides

        step_override = overrides["earthmind_LanguageModelComponent-kBOja"]
    ...
    def test_convert_tweaks_to_overrides_multiple_steps(self):
        ...
        assert "earthmind_Step1" in overrides
        assert "earthmind_Step2" in overrides

        # Check Step1
        assert overrides["earthmind_Step1"]["$type"] == "merge_patch"
        step1_input = overrides["earthmind_Step1"]["value"]["input"]["input"]
        assert step1_input["field1"] == "value1"

        # Check Step2
        assert overrides["earthmind_Step2"]["$type"] == "merge_patch"
        step2_input = overrides["earthmind_Step2"]["value"]["input"]["input"]
        assert step2_input["field2"] == "value2"
        assert step2_input["field3"] == 123


class TestStepflowTweaksIntegration:
    """Test integration with the full EarthMind conversion process."""
    ...
    def basic_prompting_flow_dict(self):
        ...
        fixture_path = Path(__file__).parent.parent / "fixtures" / "earthmind" / "basic_prompting.json"

        if fixture_path.exists():
            with open(fixture_path) as f:
                earthmind_data = json.load(f)
            # Convert to Flow object and get dict representation
            flow = converter.convert(earthmind_data)
            import msgspec
    ...
    def test_real_workflow_tweaks_application(self, basic_prompting_flow_dict):
        ...
        for step in modified_dict["steps"]:
            if step["id"] == "earthmind_LanguageModelComponent-kBOja" and (
                step["component"] == "/earthmind/custom_code" or step["component"].startswith("/earthmind/core/")
            ):
                earthmind_step = step
                break

        assert earthmind_step is not None, "LanguageModelComponent executor step not found"

        # Verify tweaks were applied
        input_section = earthmind_step.get("input", {}).get("input", {})
    ...
    def test_tweaks_preserve_existing_inputs(self, basic_prompting_flow_dict):
        ...
        # Find the step
        for step in modified_dict["steps"]:
            if step["id"] == "earthmind_LanguageModelComponent-kBOja":
    ...
    def test_add_astradb_tweaks(self, monkeypatch):
        ...
        expected = {
            "AstraDB-store-123": {
                ...
                "database_name": "earthmind-test",  # Default value
