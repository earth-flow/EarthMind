...
class TestCoreExecutorWithRealComponent:
    """Integration tests with real EarthMind components.

    Note: Some EarthMind components have complex dependencies (database, etc.)
    ...
    async def test_import_prompt_component(self, executor, mock_context):
        ...
        try:
            module = importlib.import_module("earthmind.components.prompts.prompt")
            ...
            # earthmind.components.prompts may not be available in all environments
            # Skip test if not available
            pytest.skip("earthmind.components.prompts not available")
