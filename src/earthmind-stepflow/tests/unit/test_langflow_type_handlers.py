"""Unit tests for EarthMindType input/output handlers and DataFrame output handler.

Tests serialization (output) and deserialization (input) of EarthMind
...
from earthmind_stepflow.worker.handlers import (
    DataFrameOutputHandler,
    EarthMindTypeInputHandler,
    EarthMindTypeOutputHandler,
)

# ---------------------------------------------------------------------------
# EarthMindTypeOutputHandler — matches()
# ---------------------------------------------------------------------------


class TestEarthMindTypeOutputHandlerMatches:
    def setup_method(self):
        self.handler = EarthMindTypeOutputHandler()
...
# ---------------------------------------------------------------------------
# EarthMindTypeOutputHandler — process()
# ---------------------------------------------------------------------------


class TestEarthMindTypeOutputHandlerProcess:
    def setup_method(self):
        self.handler = EarthMindTypeOutputHandler()
    ...
    async def test_serialize_message(self):
        ...
        assert result["__earthmind_type__"] == "Message"
    ...
    async def test_serialize_data(self):
        ...
        assert result["__earthmind_type__"] == "Data"
...
class TestDataFrameOutputHandlerProcess:
    ...
    async def test_serialize_dataframe(self):
        ...
        assert result["__earthmind_type__"] == "DataFrame"
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — matches()
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerMatches:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()

    def test_matches_dict_with_marker(self):
        value = {"__earthmind_type__": "Message", "text": "hi"}
        assert self.handler.matches(template_field={}, value=value) is True

    def test_matches_list_with_marked_items(self):
        value = [
            {"__earthmind_type__": "Data", "text": "row1"},
            {"__earthmind_type__": "Data", "text": "row2"},
        ]
        assert self.handler.matches(template_field={}, value=value) is True
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — prepare() (Message)
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerMessage:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_message(self):
        serialized = {"__earthmind_type__": "Message", "text": "hello"}
    ...
    async def test_deserialize_message_empty_text(self):
        serialized = {"__earthmind_type__": "Message", "text": ""}
        fields = {"msg": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — prepare() (Data)
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerData:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_data(self):
        serialized = {"__earthmind_type__": "Data", "text": "some data"}
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — prepare() (DataFrame)
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerDataFrame:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — prepare() (list handling)
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerLists:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()
    ...
    async def test_deserialize_list_of_messages(self):
        value = [
            {"__earthmind_type__": "Message", "text": "msg1"},
            {"__earthmind_type__": "Message", "text": "msg2"},
        ]
        fields = {"msgs": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_list_of_data(self):
        value = [
            {"__earthmind_type__": "Data", "text": "d1"},
            {"__earthmind_type__": "Data", "text": "d2"},
        ]
        fields = {"items": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_multiple_fields(self):
        ...
        fields = {
            "msg": ({"__earthmind_type__": "Message", "text": "hello"}, {}),
            "data": ({"__earthmind_type__": "Data", "text": "world"}, {}),
        }
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# EarthMindTypeInputHandler — unknown / edge cases
# ---------------------------------------------------------------------------


class TestEarthMindTypeInputHandlerEdgeCases:
    def setup_method(self):
        self.handler = EarthMindTypeInputHandler()

    @pytest.mark.asyncio
    async def test_unknown_earthmind_type_returns_dict(self):
        """Unknown __earthmind_type__ values should return the raw dict."""
        serialized = {"__earthmind_type__": "UnknownType", "data": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)

        assert isinstance(result["field"], dict)
        assert result["field"]["__earthmind_type__"] == "UnknownType"

    @pytest.mark.asyncio
    async def test_empty_earthmind_type_returns_dict(self):
        """Empty __earthmind_type__ value should return the raw dict."""
        serialized = {"__earthmind_type__": "", "text": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
class TestEarthMindTypeRoundTrip:
    def setup_method(self):
        self.output_handler = EarthMindTypeOutputHandler()
        self.input_handler = EarthMindTypeInputHandler()
...
class TestDataFrameRoundTrip:
    ...
    deserialize with EarthMindTypeInputHandler."""
    ...
    async def test_dataframe_round_trip(self):
        ...
        serialized = await self.output_handler.process(original)
        assert serialized["__earthmind_type__"] == "DataFrame"
...
class TestPandasDataFrameRoundTrip:
    ...
    DataFrames instead of lfx DataFrames, depending on the EarthMind
