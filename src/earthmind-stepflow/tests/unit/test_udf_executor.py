"""Unit test for custom code execution in isolation (with real EarthMind code)."""
...
from earthmind_stepflow.exceptions import ExecutionError
from earthmind_stepflow.worker.custom_code_executor import (
    CustomCodeExecutor,
)


class TestCustomCodeExecutor:
    """Test CustomCodeExecutor functionality with real EarthMind component code."""
...
''',
    ...
    def chat_input_component_blob(self):
        """Real ChatInput component blob data from EarthMind."""
        return {
            "code": '''
from earthmind.base.io.chat import ChatComponent
from earthmind.inputs.inputs import BoolInput
from earthmind.io import DropdownInput, MessageTextInput, MultilineInput, Output
from earthmind.schema.message import Message
from earthmind.utils.constants import (
...
''',
    ...
    async def test_execute_simple_component(
        ...
        # EarthMind Message: result data is nested in ["result"] field
    ...
    async def test_execute_chat_input_component(
        ...
        # Should have EarthMind Message structure
        # Handle nested result structure from EarthMind Messages
        message_data = result_data["result"] if "result" in result_data else result_data
        assert "text" in message_data
        assert message_data["text"] == "What is Python?"
        assert message_data["sender"] == "User"
        assert message_data["sender_name"] == "Test User"
        assert message_data.get("__earthmind_type__") == "Message"
...
''',
    ...
    async def test_plain_pandas_dataframe_serializes(
        ...
        assert output["__earthmind_type__"] == "DataFrame"
        assert "json_data" in output


class TestCustomCodeExecutorIntegration:
    """Integration tests with mock EarthMind imports."""

    @pytest.fixture
    def executor_with_mocks(self):
        """Create CustomCodeExecutor with mocked EarthMind imports."""
        # We'll test this without real EarthMind imports to avoid dependency issues
        return CustomCodeExecutor()
...
class TestCustomCodeExecutorWithRealEarthMindComponents:
    """Test CustomCodeExecutor with real converted EarthMind workflow components."""
...
"""
        ...
        stepflow_workflow = converter.convert(earthmind_data)

        # Find steps that use /earthmind/custom_code
        custom_code_steps = [step for step in stepflow_workflow.steps if step.component == "/earthmind/custom_code"]
...
""",
    ...
    def basic_prompting_flow(self):
        ...
        flow_path = Path(__file__).parent.parent / "fixtures" / "earthmind" / "basic_prompting.json"
    ...
    async def test_prompt_component(self, executor: CustomCodeExecutor, mock_context, prompt_component_data):
        ...
        has_message_marker = "__earthmind_type__" in output or "__class_name__" in output
    ...
    async def test_language_model_message_to_string_conversion(
        ...
        This test verifies the fix for EarthMind 1.6.4+ lfx components that
    ...
    async def test_chat_input_message_to_string_conversion(
        ...
        This is a specific test for the EarthMind 1.6.4+ fix where Message objects
