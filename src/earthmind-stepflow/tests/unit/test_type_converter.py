...
from earthmind_stepflow.worker.base_executor import BaseExecutor
from earthmind_stepflow.worker.handlers import (
    BaseModelInputHandler,
    BaseModelOutputHandler,
)
from earthmind_stepflow.worker.handlers.base_model import (
    _is_secret_str_type,
)
...
class TestOutputTreeWalker:
    ...
    @pytest.mark.asyncio
    async def test_earthmind_types_still_work(self):
        """Test that existing EarthMind type serialization still works."""
        try:
            ...
            handlers = [EarthMindTypeOutputHandler(), BaseModelOutputHandler()]
            serialized = await self.executor._apply_output_handlers(message, handlers)

            # Should have EarthMind type metadata, not BaseModel metadata
            assert "__earthmind_type__" in serialized
            assert serialized["__earthmind_type__"] == "Message"

        except ImportError:
            # Skip if EarthMind not available
            pytest.skip("EarthMind not available for testing")
