...
Tests the full end-to-end path used by the ``stepflow-earthmind run`` CLI:
EarthMindConverter -> StepflowClient -> orchestrator -> earthmind worker.
...
def _find_poc_flow() -> Path:
    ...
    # here.parents[4] = earthmind repo root (e.g. /Users/.../earthmind)
    earthmind_repo = here.parents[4]
    candidate = earthmind_repo.parent / "stepflow" / "docs" / "static" / "files" / "2025-09-earthmind-poc-flow.json"
    if candidate.exists():
        return candidate
...
def test_poc_flow_translation(converter, poc_flow_data):
    ...
    for step in flow.steps:
        assert step.component.startswith(("/earthmind/", "/builtin/")), f"unexpected component prefix: {step.component}"
...
async def test_poc_flow_execution(runner, poc_flow_data):
    ...
    as ``stepflow-earthmind run --local``.
    """
