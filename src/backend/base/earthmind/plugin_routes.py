...
Plugins register via the ``earthmind.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing EarthMind routes.
"""
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing EarthMind routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``earthmind.plugins`` entry-point group.
    ...
    raw_eps = list(entry_points(group="earthmind.plugins"))
