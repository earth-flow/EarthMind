...
from earthmind.api import health_check_router, log_router
from earthmind.api.router import router
from earthmind.api.v1.mcp_projects import init_mcp_servers
...
from earthmind.middleware import ContentSizeLimitMiddleware
from earthmind.plugin_routes import load_plugin_routes
from earthmind.services.database.models.deployment.exceptions import DeploymentGuardError
from earthmind.services.database.service import UnsupportedPostgreSQLVersionError
from earthmind.services.deps import (
    get_queue_service,
    get_service,
...
from earthmind.services.utils import initialize_services, initialize_settings_service, teardown_services
from earthmind.utils.mcp_cleanup import cleanup_mcp_sessions
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Set EARTHMIND_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        from earthmind.preload import PreloadStep, get_owned_temp_dirs, is_step_complete

        configure()

        # Startup message
        if version:
            await logger.adebug(f"Starting EarthMind v{version}...")
        else:
            await logger.adebug("Starting EarthMind...")
        ...
        try:
            ...
            if get_settings_service().settings.sentry_dsn:
                try:
                    ...
                    await logger.awarning(
                        "EARTHMIND_SENTRY_DSN is set but sentry-sdk is not installed; "
                    ...
                    try:
                        ...
                        await logger.awarning(f"Failed to initialize Sentry SDK (check EARTHMIND_SENTRY_DSN): {e}")
            ...
            if is_step_complete(PreloadStep.STARTER_PROJECTS):
                ...
                if all_types_dict is None:
                    ...
                    lock_file = Path(tempfile.gettempdir()) / "earthmind_starter_projects.lock"
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_GLOBALS):
                    ...
                    from earthmind.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_MCP):
                    ...
                    from earthmind.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            # EARTHMIND_MODELS_DEV_REFRESH=false disables the live models.dev
            ...
            if os.getenv("EARTHMIND_MODELS_DEV_REFRESH", "true").lower() not in ("false", "0", "no"):
                models_dev_refresh_task = asyncio.create_task(refresh_models_dev_periodically())
            else:
                await logger.adebug("models.dev refresh disabled via EARTHMIND_MODELS_DEV_REFRESH")

            # v1 and project MCP server context managers
            from earthmind.api.v1.mcp import start_streamable_http_manager
            from earthmind.api.v1.mcp_projects import start_project_task_group
            ...
            if "earthmind migration --fix" not in str(exc):
            ...
            from earthmind.__main__ import get_number_of_workers
            from earthmind.cli.progress import create_earthmind_shutdown_progress

            log_level = os.getenv("EARTHMIND_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_earthmind_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from earthmind.api.v1.mcp import stop_streamable_http_manager
                    from earthmind.api.v1.mcp_projects import stop_project_task_group
                ...
                with shutdown_progress.step(2):
                    ...
                    try:
                        from earthmind.services.authorization.utils import drain_pending_audit_writes
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("EarthMind shutdown complete")
...
def create_app():
    ...
    async def set_locale(request: Request, call_next):
        ...
        if _supported_locales is None:
            from earthmind.utils.i18n import get_supported_locales
    ...
    if prome_port_str := os.environ.get("EARTHMIND_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from earthmind.api.v1 import mcp_router
    ...
    # Discover and register additional routers from plugins (earthmind.plugins entry-point)
    ...
    async def rate_limit_exception_handler(request: Request, _exc: RateLimitExceeded):
        ...
        from earthmind.services.rate_limit.service import get_limiter_key
    ...
    from earthmind.services.rate_limit import get_rate_limiter
...
def get_static_files_dir():
    """Get the static files directory relative to EarthMind's main.py file."""
