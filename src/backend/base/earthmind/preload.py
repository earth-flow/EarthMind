"""Gunicorn master preload hook for EarthMind.

When ``EARTHMIND_GUNICORN_PRELOAD=true`` (which flips gunicorn's
``preload_app`` option), this module runs the fork-safe parts of
EarthMind's startup in the master process so that worker processes
...
async def _run_master_preload() -> None:
    ...
    from earthmind.main import load_bundles_with_error_handling
    ...
    try:
        ...
        if settings_service.settings.agentic_experience:
            from earthmind.api.utils.mcp.agentic_mcp import (
...
def preload_master() -> None:
    """Run one-time EarthMind initialization in the gunicorn master before workers are forked.
