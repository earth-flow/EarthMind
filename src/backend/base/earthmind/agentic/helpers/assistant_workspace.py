"""Resolve the FileSystem sandbox root for the EarthMind Assistant.

The shipped EarthMindAssistant flow embeds a FileSystemTool whose `root_path`
...
that PR's contract:
    ...
    Env var:  EARTHMIND_FS_TOOL_BASE_DIR  (same name PR #13031 uses)
    Default:  ~/.earthmind/fs_tool/fs_sandbox  (same default PR #13031 uses)
...
Resolution order:
    ...
    2. EARTHMIND_FS_TOOL_BASE_DIR env var (after expanduser + strip), if set.
    3. ~/.earthmind/fs_tool/fs_sandbox.
...
DEFAULT_BASE_SUBPATH = Path(".earthmind") / "fs_tool" / "fs_sandbox"
