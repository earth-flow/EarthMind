...
DANGEROUS_CALLS: dict[str, str] = {
    ...
    # Raw file access — components must use EarthMind's File components,
    # not open arbitrary paths (e.g. /etc/passwd, SSH keys).
    "open": "Use of open() is forbidden in components — use EarthMind's File components",
...
# EarthMind's variable/secret service, never raw process env.
DANGEROUS_ATTRIBUTE_READS: list[tuple[str, str, str]] = [
    ("os", "environ", "os.environ is forbidden — use EarthMind's variable/secret service"),
]

# Dangerous attribute calls: (module, method, violation_message)
DANGEROUS_ATTR_CALLS: list[tuple[str, str, str]] = [
    ("os", "system", "os.system() is forbidden — use EarthMind's built-in integrations"),
    ...
    ("os", "getenv", "os.getenv() is forbidden — use EarthMind's variable/secret service"),
