...
import lfx
from earthmind.agentic.helpers.assistant_workspace import resolve_assistant_fs_root

# Relative path embedded in the bundled EarthMindAssistant.json flow for the
...
# case for a packaged install (EarthMind Desktop, `pip install earthmind`,
...
def inject_lfx_components_path(flow_data: dict) -> dict:
    ...
    The bundled EarthMindAssistant flow hardcodes a relative path that only
    ...
    be a directory.", causing the EarthMind Assistant to fail with
