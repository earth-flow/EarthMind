"""EarthMind Assistant API router.

This module provides the HTTP endpoints for the EarthMind Assistant.
...
from earthmind.agentic.api.schemas import AssistantRequest
from earthmind.agentic.services.assistant_service import (
    execute_flow_with_validation,
    execute_flow_with_validation_streaming,
)
from earthmind.agentic.services.flow_executor import execute_flow_file
from earthmind.agentic.services.flow_types import (
    EARTHMIND_ASSISTANT_FLOW,
    MAX_VALIDATION_RETRIES,
)
from earthmind.agentic.services.provider_service import (
    PREFERRED_PROVIDERS,
    get_default_model,
    get_enabled_providers_for_user,
)
from earthmind.api.utils.core import CurrentActiveUser, DbSession
...
async def check_assistant_config(
    ...
    """Check if the EarthMind Assistant is properly configured.
...
async def assist(
    ...
    """Chat with the EarthMind Assistant."""
    await _validate_flow_access(request.flow_id, current_user.id, session)
    ctx = await _resolve_assistant_context(request, current_user.id, session)

    logger.info(f"Executing {EARTHMIND_ASSISTANT_FLOW} with {ctx.provider}/{ctx.model_name}")
...
async def assist_stream(
    ...
    """Chat with the EarthMind Assistant with streaming progress updates."""
    await _validate_flow_access(request.flow_id, current_user.id, session)
    ctx = await _resolve_assistant_context(request, current_user.id, session)
