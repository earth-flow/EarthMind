...
class EarthMindUvicornWorker(UvicornWorker):
...
class EarthMindApplication(BaseApplication):
    def __init__(self, app_factory: Callable[[], Any], options=None) -> None:
        self.options = options or {}

        self.options["worker_class"] = "earthmind.server.EarthMindUvicornWorker"
    ...
    def pre_fork(cls, server, _worker):
        ...
        debug_mode = os.environ.get("EARTHMIND_DEBUG_FORK_GHOSTS", "").lower() in ("1", "true", "yes")
    ...
    def load(self):
        if self.application is None:
            ...
            if self.cfg.preload_app:
                from earthmind.preload import preload_master
