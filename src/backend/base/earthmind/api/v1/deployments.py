...
from earthmind.api.utils import CurrentActiveUser, DbSession, DbSessionReadOnly
from earthmind.api.v1.mappers.deployments import get_deployment_mapper
from earthmind.api.v1.mappers.deployments.helpers import (
...
from earthmind.services.adapters.deployment.context import deployment_provider_scope
from earthmind.services.authorization import DeploymentAction, ensure_deployment_permission, filter_visible_resources
from earthmind.services.authorization.fetch import deny_to_404
...
from earthmind.services.database.models.deployment_provider_account.crud import (
...
from earthmind.services.deps import get_authorization_service, get_telemetry_service
...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="EarthMind DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="EarthMind DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="EarthMind DB deployment UUID (`deployment.id`)."),
]
...
DeploymentIdQuery = Annotated[
    UUID,
    Query(description="EarthMind DB deployment UUID (`deployment.id`)."),
]
IncludeProviderDeleteQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "When false, only the local EarthMind DB row is removed; "
...
async def _delete_local_deployment_row_with_commit_retry(
    *,
    ...
    stale EarthMind row that still blocks later reads or provider-account
    deletion.
    """
    try:
...
async def create_deployment(
    ...
    if existing_resource_key is not None:
        ...
        if existing_deployment is not None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"The agent '{existing_resource_key}' is already managed by EarthMind. "
...
async def list_deployments(
    ...
    *,
    load_from_provider: Annotated[
        bool,
        Query(
            description=("When true, list deployments directly from the provider (bypassing EarthMind deployment rows).")
        ),
    ] = False,
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional EarthMind flow version ids (pass as repeated query params, "
...
async def update_snapshot(
    ...
    *,
    ...
    ``provider_snapshot_id``.  Only EarthMind-tracked snapshots (those with
    a ``flow_version_deployment_attachment`` row) can be updated.
    """
    from earthmind.services.database.models.deployment.crud import get_deployment as get_deployment_row
    from earthmind.services.database.models.flow_version.crud import get_flow_version_entry
