...
from earthmind.api.utils import CurrentActiveUser, DbSession
from earthmind.api.v1.chat import build_flow_and_stream
from earthmind.memory import aadd_messagetables
from earthmind.schema.properties import Properties
from earthmind.services.auth.utils import get_current_user_for_websocket
...
from earthmind.services.deps import get_variable_service, session_scope
from earthmind.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
        ...
        await websocket.send_json(
            {
                "type": "error",
                "code": "earthmind_auth",
                "message": "You must pass a valid EarthMind token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "earthmind.voice_mode.config":
                            await logger.ainfo(f"earthmind.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "earthmind.elevenlabs.config":
                            await logger.ainfo(f"earthmind.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "earthmind.elevenlabs.config":
                            await logger.ainfo(f"earthmind.elevenlabs.config {event}")
