...
from earthmind.schema.schema import InputType, OutputType, OutputValue
from earthmind.serialization.serialization import get_max_items_length, get_max_text_length, serialize
from earthmind.services.database.models.api_key.model import ApiKeyRead
from earthmind.services.database.models.base import orjson_dumps
from earthmind.services.database.models.flow.model import FlowCreate, FlowRead
...
class SimplifiedAPIRequest(BaseModel):
    ...
    user_id: str | None = Field(
        ...
        description=(
            ...
            "API key owner remains the effective EarthMind user."
        ),
    )
...
class BaseConfigResponse(BaseModel):
    ...
    # but ``lfx extension dev`` and ``--env-file EARTHMIND_ENABLE_EXTENSION_RELOAD=true``
    # opt in after the build is frozen, so the UI consults this field too.
    enable_extension_reload: bool
    # Mirrors ``EARTHMIND_AUTHZ_ENABLED``. EE/custom frontends gate the Access
