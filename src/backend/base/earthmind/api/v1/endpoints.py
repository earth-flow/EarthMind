...
from earthmind.api.utils import CurrentActiveUser, DbSession, extract_global_variables_from_headers, parse_value
from earthmind.api.v1.files import get_flow
from earthmind.api.v1.global_variable_defaults import apply_global_variable_defaults
from earthmind.api.v1.schemas import (
    ConfigResponse,
...
from earthmind.events.event_manager import create_stream_tokens_event_manager
from earthmind.exceptions.api import APIException, InvalidChatInputError
from earthmind.exceptions.serialization import SerializationError
from earthmind.helpers.flow import get_flow_by_id_or_endpoint_name
from earthmind.interface.initialize.loading import update_params_with_load_from_db_fields
from earthmind.processing.process import process_tweaks, run_graph_internal
from earthmind.schema.graph import Tweaks
from earthmind.services.auth.utils import (
    api_key_security,
    get_current_active_user,
    get_current_user_for_sse,
    get_optional_user,
)
from earthmind.services.authorization import FlowAction, ensure_flow_permission
from earthmind.services.cache.utils import save_uploaded_file
from earthmind.services.database.models.flow.model import Flow, FlowRead
from earthmind.services.database.models.flow.utils import get_all_webhook_components_in_flow
from earthmind.services.database.models.jobs.model import JobType
...
from earthmind.services.event_manager import create_webhook_event_manager, webhook_event_manager
from earthmind.services.telemetry.schema import RunPayload
from earthmind.utils.compression import compress_response
...
async def get_all(request: Request):
    ...
    """
    from earthmind.interface.components import get_and_cache_all_types_dict
    from earthmind.utils.i18n import build_component_display_names, translate_component_dict
...
async def simple_run_flow(
    ...
    *,
    ...
    try:
        ...
        # See: https://github.com/earthmind-ai/earthmind/issues/11781
...
async def _run_flow_internal(
    *,
    ...
    # Extract request-level variables from headers with prefix X-EARTHMIND-GLOBAL-VAR-*
...
async def simplified_run_flow(
    *,
    ...
    Notes:
        ...
        - Extracts global variables from HTTP headers with prefix X-EARTHMIND-GLOBAL-VAR-*
...
# get endpoint to return version of earthmind
@router.get("/version")
...
async def custom_component(
    ...
    locale = getattr(request.state, "locale", "en")
    if locale != "en":
        from earthmind.utils.i18n import translate_component_node
