...
from earthmind.api.v1.schemas.deployments import ValidatedUrl
from earthmind.services.database.models.deployment_provider_account.utils import validate_provider_url

if TYPE_CHECKING:
    from earthmind.api.v1.schemas.deployments import DeploymentCreateRequest
...
class WatsonxApiDeploymentCreatePayload(BaseModel):
    ...
    display_name: NormalizedStr | None = Field(
        ...
        description=(
            ...
            "Required unless tracking an existing wxO agent in EarthMind (via ``existing_agent_id`` field)."
        ),
    )
    llm: NormalizedStr | None = Field(
        default=None,
    ...
    existing_agent_id: NormalizedStr | None = Field(
        default=None,
        description=(
            "Provider-owned agent id to track in EarthMind instead of creating a new wxO agent. "
    ...
    def validate_create_operation_requirements(self) -> WatsonxApiDeploymentCreatePayload:
        if "existing_agent_id" in self.model_fields_set:
            ...
            if len(self.model_fields_set) > 1:
                msg = (
                    "existing_agent_id is only for tracking existing wxO agents in EarthMind and cannot include "
    ...
    def validate_with_outer_fields(self, outer_payload: DeploymentCreateRequest) -> None:
        if self.existing_agent_id is None or "description" not in outer_payload.model_fields_set:
            return
        msg = "When existing_agent_id is provided, EarthMind uses the description already set for the agent in wxO."
        raise ValueError(msg)


class WatsonxApiCreatedTool(BaseModel):
    """API response shape for a tool created from a EarthMind flow version."""
...
class _WatsonxApiAgentExecutionResultBase(BaseModel):
    ...
    ``provider_data``.  The enclosing response only carries EarthMind-owned
    fields (``deployment_id``).  ``deployment_id`` (EarthMind DB UUID) is
