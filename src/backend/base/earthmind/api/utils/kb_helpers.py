...
from earthmind.api.utils import CurrentActiveUser
from earthmind.api.utils.kb_processing import process_knowledge_document
...
class KBIngestionHelper:
    ...
    async def perform_ingestion(
        ...
        CANCELLED.
        """
        # Lazy import: the service reaches into earthmind DB plumbing we
        ...
        from earthmind.api.utils.knowledge_base_service import (
            get_embedding_model,
        ...
        try:
            embeddings = await KBIngestionHelper.build_embeddings(
                embedding_provider,
                embedding_model,
                current_user,
                api_key=(kb_record.backend_config or {}).get("embedding_api_key") if kb_record else None,
                api_base=(kb_record.backend_config or {}).get("embedding_api_base") if kb_record else None,
                model_selection=(kb_record.model_selection or {}) if kb_record else None,
            )
            backend_type_value = (
            ...
            backend = create_backend(
                ...
                # pull their connection URI / tokens from EarthMind's
            ...
            async for item in source.list_items():
                ...
                processed = await process_knowledge_document(
                    file_name=content_obj.file_name,
                    raw_bytes=content_obj.raw_bytes,
                    chunk_size=chunk_size,
                    chunk_overlap=chunk_overlap,
                    separator=separator,
                    parser_strategy=parser_strategy,
                    chunk_strategy=chunk_strategy,
                    mineru_overrides=(kb_record.backend_config or {}) if kb_record else None,
                )
                ...
                docs = [
                    Document(
                        ...
                        metadata={
                            ...
                            METADATA_KEY_CHUNK_INDEX: chunk.index,
                            METADATA_KEY_TOTAL_CHUNKS: len(processed.chunks),
    ...
    async def build_embeddings(
        ...
        *,
        ...
        if model_selection:
            selected_option.update(model_selection)
