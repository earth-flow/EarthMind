...
from earthmind.services.database.models.knowledge_base import KnowledgeBaseRecord, KnowledgeBaseStatus
...
def _normalize_provider_for_model(selection: dict[str, Any]) -> dict[str, Any]:
    """Backfill provider names for legacy KB rows / metadata.

    Older KBs could persist OpenAI-compatible models under provider=OpenAI even
    when the model now belongs to a first-class provider like SiliconFlow.
    Normalize here so every ingestion/retrieval path resolves credentials from
    the right provider variables without requiring the user to recreate the KB.
    """
    if not isinstance(selection, dict):
        return selection
    normalized = dict(selection)
    model_name = str(normalized.get("name") or "").strip()
    provider = str(normalized.get("provider") or "").strip()
    inferred = infer_embedding_provider(model_name) if model_name else ""
    if inferred and inferred != "OpenAI" and provider in {"", "Unknown", "OpenAI"}:
        normalized["provider"] = inferred
    return normalized
...
def get_embedding_provider(model_selection: Any) -> str:
    ...
    """
    selection = _normalize_provider_for_model(_coerce_model_dict(model_selection) or {}) or None
    if not selection:
        return "Unknown"
...
def _coerce_model_dict(model_selection: Any) -> dict[str, Any] | None:
    ...
    if isinstance(model_selection, list):
        return _normalize_provider_for_model(model_selection[0]) if model_selection else None
    if isinstance(model_selection, dict):
        return _normalize_provider_for_model(model_selection)
    return None
...
async def backfill_all_users_from_disk(*, kb_root: Path | None = None) -> int:
    ...
    """
    from earthmind.api.utils.kb_helpers import KBStorageHelper
...
async def update_stats(
    ...
    *,
    ...
    ``parser_strategy`` and ``chunk_strategy`` are accepted for
    call-site compatibility but are not persisted here — they live in
    the on-disk ``embedding_metadata.json`` (written by the caller)
    because the DB table has no matching columns.
    """
    _ = parser_strategy, chunk_strategy  # noqa: SLF001 — acknowledged, not persisted
    async with session_scope() as session:
...
def record_to_metadata_dict(record: KnowledgeBaseRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "parser_strategy": record.backend_config.get("parser_strategy"),
        "chunk_strategy": record.backend_config.get("chunk_strategy"),
...
async def backfill_from_disk(
    *,
    ...
    for kb_dir in kb_user_root.iterdir():
        ...
        try:
            from earthmind.api.utils.kb_helpers import KBAnalysisHelper
...
def _normalize_model_selection(raw) -> dict[str, Any]:
    ...
    if isinstance(raw, list):
        return _normalize_provider_for_model(raw[0]) if raw else {}
    if isinstance(raw, dict):
        return _normalize_provider_for_model(raw)
    return {}
