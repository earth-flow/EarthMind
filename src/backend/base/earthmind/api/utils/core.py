...
from earthmind.services.auth.utils import get_current_active_user, get_current_active_user_mcp
...
from earthmind.services.store.utils import get_lf_version_from_pypi
from earthmind.utils.constants import EARTHMIND_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from earthmind.services.store.schema import StoreComponentCreate
...
async def check_earthmind_version(component: StoreComponentCreate) -> None:
    ...
    earthmind_version = await get_lf_version_from_pypi()
    if earthmind_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of EarthMind")
    if earthmind_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of EarthMind ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({earthmind_version}) and try again."
        )
...
def extract_global_variables_from_headers(headers, *, include_auth_headers: bool = False) -> dict[str, str]:
    ...
    By default, only headers with the ``X-EARTHMIND-GLOBAL-VAR-*`` prefix are
    ...
    ``/workflow``, ``x-api-key`` is EarthMind's own authentication key — exposing
    ...
    Example:
        headers = {"X-EARTHMIND-GLOBAL-VAR-API-KEY": "secret", "x-api-key": "mykey"}
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(EARTHMIND_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(EARTHMIND_GLOBAL_VAR_HEADER_PREFIX) :].upper()
