"""Utilities for auto-configuring the EarthMind Agentic MCP server."""
...
from earthmind.api.v2.mcp import get_server_list, update_server
from earthmind.services.database.models.user.model import User
from earthmind.services.deps import get_service, get_variable_service
from earthmind.services.schema import ServiceType
from earthmind.services.variable.constants import CREDENTIAL_TYPE, GENERIC_TYPE


async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the EarthMind Agentic MCP server for all users.

    This function adds the earthmind-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring EarthMind Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "earthmind-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "earthmind.agentic.mcp"],
            "metadata": {
                "description": "EarthMind Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "earthmind_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the EarthMind Agentic MCP server from all users.

    This function removes the earthmind-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing EarthMind Agentic MCP server from all users...")
