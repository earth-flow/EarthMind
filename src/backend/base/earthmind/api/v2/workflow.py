...
from earthmind.api.utils import extract_global_variables_from_headers
from earthmind.api.v1.schemas import RunResponse
from earthmind.api.v2.converters import (
    create_error_response,
    parse_flat_inputs,
    run_response_to_workflow_response,
)
from earthmind.api.v2.workflow_reconstruction import reconstruct_workflow_response_from_job_id
from earthmind.exceptions.api import (
...
from earthmind.services.auth.utils import api_key_security
...
from earthmind.services.database.models.user.model import UserRead
from earthmind.services.deps import get_job_service, get_memory_base_service, get_task_service
...
def _resolve_request_variables(
    ...
    values can be transported safely. The legacy ``X-EARTHMIND-GLOBAL-VAR-*``
    ...
    if legacy_globals:
        logger.warning(
            "X-EARTHMIND-GLOBAL-VAR-* headers are deprecated on /api/v2/workflows; "
            "send request-level globals in the JSON body instead. Header-based "
            "globals will be removed in a future EarthMind release."
        )
...
async def execute_workflow(
    ...
    **background** and **stream** can't be true at the same time.
    ...
    Args:
        ...
        http_request: The HTTP request, used to honor deprecated ``X-EARTHMIND-GLOBAL-VAR-*`` headers
...
async def execute_sync_workflow(
    ...
    # Unicode strings transport safely. The legacy X-EARTHMIND-GLOBAL-VAR-*
