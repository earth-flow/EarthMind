...
async def test_openai_response_stream_error_handling(client):
    ...
    with (
        patch("earthmind.api.v1.openai_responses.get_flow_by_id_or_endpoint_name") as mock_get_flow,
        patch("earthmind.api.v1.openai_responses.run_flow_generator") as _,
        patch("earthmind.api.v1.openai_responses.consume_and_yield") as mock_consume,
    ):
        # Setup mock flow
        mock_flow = MagicMock()
