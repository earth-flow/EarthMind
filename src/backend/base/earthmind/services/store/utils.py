...
if TYPE_CHECKING:
    from earthmind.services.store.schema import ListComponentResponse
...
# Get the latest released version of earthmind (https://pypi.org/project/earthmind/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/earthmind/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.debug("Error getting the latest version of earthmind from PyPI", exc_info=True)
        return None
