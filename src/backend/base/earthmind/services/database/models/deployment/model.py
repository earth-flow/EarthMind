...
from earthmind.schema.serialize import UUIDstr
from earthmind.services.database.utils import validate_non_empty_string, validate_non_empty_string_preserve_value
...
class Deployment(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # ``deployment_type_enum`` constraint is defined by a EarthMind alembic
    ...
    #                           by the DB until a EarthMind migration adds it
    #                           to the enum type.  EarthMind explicitly opts
    ...
    #   2. Create a EarthMind alembic migration that runs:
