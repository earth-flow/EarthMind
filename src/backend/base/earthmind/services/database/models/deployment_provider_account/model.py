...
from earthmind.services.database.utils import (
...
class DeploymentProviderAccount(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # by a EarthMind alembic migration.  The enum is EarthMind-owned (see
    ...
    #   2. Register the corresponding adapter in LFX and mapper in EarthMind.
    #   3. Create a EarthMind alembic migration that runs:
    #        op.execute(
