...
# Minimum PostgreSQL major version required by EarthMind.
...
POSTGRESQL_VERSION_REQUIRED_MESSAGE = (
    f"EarthMind requires PostgreSQL {MIN_POSTGRESQL_MAJOR_VERSION} or higher when using PostgreSQL as the database. "
    "The current PostgreSQL version does not support the syntax used by EarthMind's schema. "
    f"Please upgrade your PostgreSQL instance to version {MIN_POSTGRESQL_MAJOR_VERSION} or higher. "
    "See: https://docs.earthmind.org/configuration-custom-database"
)
