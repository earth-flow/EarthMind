...
from earthmind.services.cache.factory import CacheServiceFactory
...
from earthmind.services.database.utils import initialize_database
...
async def setup_superuser(settings_service: SettingsService, session: AsyncSession) -> SetupSuperuserResult:
    if settings_service.auth_settings.AUTO_LOGIN:
        ...
        lock_file = Path(gettempdir()) / "earthmind_auto_login_superuser.lock"
        ...
        try:
            with lock:
                ...
                if super_user:  # Only initialize if user was created
                    ...
                    # Recover MCP server config files orphaned when DB was reset but EARTHMIND_CONFIG_DIR was kept.
...
async def migrate_orphaned_mcp_servers_config(
    ...
    via a ``File`` row. When EarthMind starts with a fresh database but the same
...
def initialize_session_service() -> None:
    ...
    from earthmind.services.cache import factory as cache_factory
    from earthmind.services.session import factory as session_service_factory
...
async def clean_authz_audit_log(settings_service: SettingsService, session: AsyncSession) -> int:
    ...
    from earthmind.services.database.models.auth import AuthzAuditLog
...
def register_all_service_factories() -> None:
    ...
    from earthmind.services.auth import factory as auth_factory
    from earthmind.services.auth.service import AuthService
    from earthmind.services.authorization import factory as authorization_factory
    from earthmind.services.authorization.service import EarthMindAuthorizationService
    from earthmind.services.cache import factory as cache_factory
    from earthmind.services.chat import factory as chat_factory
    from earthmind.services.database import factory as database_factory
    from earthmind.services.job_queue import factory as job_queue_factory
    from earthmind.services.session import factory as session_factory
    from earthmind.services.shared_component_cache import factory as shared_component_cache_factory
    from earthmind.services.state import factory as state_factory
    from earthmind.services.storage import factory as storage_factory
    from earthmind.services.store import factory as store_factory
    from earthmind.services.task import factory as task_factory
    from earthmind.services.telemetry import factory as telemetry_factory
    from earthmind.services.telemetry_writer import factory as telemetry_writer_factory
    from earthmind.services.tracing import factory as tracing_factory
    from earthmind.services.transaction import factory as transaction_factory
    from earthmind.services.variable import factory as variable_factory
    ...
    # Override LFX's no-op auth service with EarthMind's full JWT implementation
    ...
    # ``override=True`` so EarthMind always has a default. A registered
    # authorization plugin replaces it by listing its class in
    # ``EARTHMIND_CONFIG_DIR/lfx.toml`` (config files use ``override=True`` via
    ...
    service_manager.register_service_class(
        ServiceType.AUTHORIZATION_SERVICE, EarthMindAuthorizationService, override=True
    )
...
def register_builtin_adapters() -> None:
    ...
    try:
        import_module("earthmind.services.adapters.deployment.watsonx_orchestrate.register")
...
def register_builtin_deployment_mappers() -> None:
    ...
    try:
        import_module("earthmind.api.v1.mappers.deployments.watsonx_orchestrate")
    except ModuleNotFoundError as exc:
