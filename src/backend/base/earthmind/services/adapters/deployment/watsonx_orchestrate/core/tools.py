...
from ibm_watsonx_orchestrate_core.types.tools.earthmind_tool import create_earthmind_tool
...
from earthmind.services.adapters.deployment.watsonx_orchestrate.utils import (
    dedupe_list,
...
if TYPE_CHECKING:
    from ibm_watsonx_orchestrate_core.types.tools.earthmind_tool import EarthMindTool
    from lfx.services.adapters.deployment.schema import BaseFlowArtifact, SnapshotItems, SnapshotListResult

    from earthmind.services.adapters.deployment.watsonx_orchestrate.types import WxOClient
...
def ensure_earthmind_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Ensure ``binding.earthmind.connections`` exists in *tool_payload* and return the mutable dict.
    ...
    earthmind = _ensure_dict(binding, "earthmind")
    return _ensure_dict(earthmind, "connections")


def verify_earthmind_owned(tool: dict[str, Any], *, tool_id: str) -> None:
    """Raise ``InvalidContentError`` if the tool lacks ``binding.earthmind``.

    Call before any mutating operation on an existing tool to ensure
    EarthMind created it.  Tools created manually in the wxO console or
    by other integrations will not have this marker.
    """
    binding = tool.get("binding")
    if not isinstance(binding, dict) or "earthmind" not in binding:
        msg = f"Cannot modify tool '{tool_id}': it does not have a EarthMind binding and may not be managed by EarthMind."
        raise InvalidContentError(message=msg)


def extract_earthmind_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Extract ``binding.earthmind.connections`` from a provider tool payload.
    ...
    if not isinstance(binding, dict):
        return {}
    earthmind = binding.get("earthmind")
    if not isinstance(earthmind, dict):
        return {}
    connections = earthmind.get("connections")
...
def extract_earthmind_artifact_from_zip(artifact_zip_bytes: bytes, *, snapshot_id: str) -> dict[str, Any]:
    """Read and parse the EarthMind flow JSON from a wxO snapshot artifact zip."""
    try:
...
def build_earthmind_artifact_bytes(
    *,
    tool: EarthMindTool,
    ...
    logger.debug("build_earthmind_artifact_bytes: filename='%s', requirements=%s", filename, requirements)
    ...
    *,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running EarthMind version for snapshot creation."
            raise InvalidContentError(message=msg)
        flow_definition["last_tested_version"] = detected_version

    tool: EarthMindTool = create_earthmind_tool(
    ...
    (tool_payload.setdefault("binding", {}).setdefault("earthmind", {})["project_id"]) = project_id
    logger.debug(
        "create_wxo_flow_tool_payload",
        tool_name=tool_payload["name"],
        tool_display_name=tool_payload["display_name"],
        project_id=project_id,
        binding=tool_payload.get("binding", {}).get("earthmind"),
    )

    artifacts: bytes = build_earthmind_artifact_bytes(
        tool=tool,
...
async def process_raw_flows_with_app_id(
    ...
    """Create earthmind tools in wxO and connect them to the given app_id."""
...
async def verify_tools_by_ids(
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_earthmind_connections_binding(tool)
