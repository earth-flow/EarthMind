...
from earthmind.services.adapters.deployment.watsonx_orchestrate.core.tools import extract_earthmind_connections_binding
...
async def validate_connection(connections_client: ConnectionsClient, *, app_id: str) -> GetConnectionResponse:
    ...
    if config.security_scheme != ConnectionSecurityScheme.KEY_VALUE:
        msg = f"Connection '{app_id}' must use key-value credentials for EarthMind flows."
        raise InvalidContentError(message=msg)
