...
EARTHMIND_SPAN_NAMESPACE = UUID("a3e1c2d4-5b6f-7890-abcd-ef1234567890")
...
def resolve_span_uuids(
    ...
    for span_data in completed_spans:
        try:
            ...
            span_uuid = uuid5(EARTHMIND_SPAN_NAMESPACE, f"{trace_id}-{span_data['id']}")
        ...
        if span_data.get("parent_span_id"):
            ...
            if isinstance(parent_id, UUID):
                ...
                try:
                    ...
                    parent_uuid = uuid5(EARTHMIND_SPAN_NAMESPACE, f"{trace_id}-{parent_id}")
