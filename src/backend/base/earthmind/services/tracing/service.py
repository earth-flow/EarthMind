...
def _get_langsmith_tracer():
    from earthmind.services.tracing.langsmith import LangSmithTracer

    return LangSmithTracer


def _get_langwatch_tracer():
    from earthmind.services.tracing.langwatch import LangWatchTracer
...
def _get_opik_tracer():
    from earthmind.services.tracing.opik import OpikTracer

    return OpikTracer


def _get_traceloop_tracer():
    from earthmind.services.tracing.traceloop import TraceloopTracer
...
def _get_openlayer_tracer():
    from earthmind.services.tracing.openlayer import OpenlayerTracer
...
class TraceContext:
    def __init__(
        ...
        # ``user_id`` is the authenticated EarthMind user (e.g. API-key owner)
...
class TracingService(Service):
    ...
    def _initialize_langfuse_tracer(self, trace_context: TraceContext) -> None:
        ...
        # ``user_id`` carries the authenticated EarthMind user and drives
    ...
    async def start_tracers(
        ...
        as ``earthmind.tracing_user_id``).
        """
        if self.deactivated:
            return
        try:
            project_name = project_name or os.getenv("LANGCHAIN_PROJECT", "EarthMind")
    ...
    def project_name(self):
        if self.deactivated:
            return os.getenv("LANGCHAIN_PROJECT", "EarthMind")
        trace_context = trace_context_var.get()
        if trace_context is None:
