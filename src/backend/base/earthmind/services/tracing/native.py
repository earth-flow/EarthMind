...
execution traces directly in EarthMind's database, enabling the Trace View
...
from earthmind.serialization.serialization import serialize
from earthmind.services.database.models.traces.model import SpanStatus, SpanType
from earthmind.services.tracing.base import BaseTracer
from earthmind.services.tracing.span_sorting import (
    EARTHMIND_SPAN_NAMESPACE,
    resolve_span_uuids,
...
class NativeTracer(BaseTracer):
    """Tracer that stores execution traces in EarthMind's database.

    This tracer captures:
    - Component-level traces (via add_trace/end_trace)
    - LangChain-level traces (via get_langchain_callback)

    Enabled by default. Disable with EARTHMIND_NATIVE_TRACING=false if needed.
    """
    ...
    def _is_enabled() -> bool:
        ...
        return os.getenv("EARTHMIND_NATIVE_TRACING", "true").lower() not in ("false", "0", "no")
    ...
    async def _flush_to_database(self, error: Exception | None = None) -> None:
        ...
        try:
            ...
            from earthmind.services.database.models.traces.model import SpanTable, TraceTable

            try:
                flow_uuid = UUID(self.flow_id)
            except (ValueError, TypeError):
                # Deterministic fallback so malformed flow_ids don't silently discard trace data.
                flow_uuid = uuid5(EARTHMIND_SPAN_NAMESPACE, f"invalid-flow-id:{self.flow_id}")
                logger.error(
            ...
            from earthmind.services.tracing.formatting import safe_int_tokens
    ...
    def get_langchain_callback(self) -> BaseCallbackHandler | None:
        ...
        from earthmind.services.tracing.native_callback import NativeCallbackHandler
        from earthmind.services.tracing.service import component_context_var
        ...
        if component_context:
            ...
            parent_span_id = uuid5(EARTHMIND_SPAN_NAMESPACE, f"{self.trace_id}-{component_id}")
        elif self._current_component_id:
            # Fallback for edge cases where component context might not be set
            parent_span_id = uuid5(EARTHMIND_SPAN_NAMESPACE, f"{self.trace_id}-{self._current_component_id}")
    ...
    def end_langchain_span(
        ...
        # Default to chat since most LLM usage in EarthMind is conversational
    ...
    def _map_trace_type(trace_type: str) -> SpanType:
        """Normalise EarthMind's string trace types to the SpanType enum, defaulting to CHAIN for unknown values."""
