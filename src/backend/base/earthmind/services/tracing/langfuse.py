...
class _SharedClient:
    ...
    See https://github.com/earthmind-ai/earthmind/issues/9066.
    """
...
def _get_or_create_shared_client(config: dict) -> Langfuse:
    ...
    ``FastAPIInstrumentor`` in ``earthmind.main``) would emit every HTTP request
    as a span into Langfuse, polluting traces with unrelated routes like health
    checks and flow list calls. See
    https://github.com/earthmind-ai/earthmind/issues/13319.
    """
    from langfuse import Langfuse
...
def delete_feedback_score(*, message_id: UUID | str) -> None:
    ...
    with EarthMind's UI state. Callers must gate on tracing being enabled
...
class LangFuseTracer(BaseTracer):
    ...
    def __init__(
        ...
        # ``user_id`` remains the authenticated EarthMind user and drives
        ...
        # trace metadata as ``earthmind.tracing_user_id`` so consumers can still
    ...
    def _setup_langfuse(self, config: dict) -> bool:
        ...
        https://github.com/earthmind-ai/earthmind/issues/13317.
        """
        try:
            from langfuse import Langfuse
            from langfuse.types import TraceContext
            ...
            # ``trace.userId`` stays the authenticated EarthMind user so existing
            # Langfuse consumers keep getting the same identity. When a caller
            # provides an override via ``tracing_user_id``, stamp it under
            # ``earthmind.tracing_user_id`` so it is still recoverable from trace
            ...
            if self.tracing_user_id and self.tracing_user_id != self.user_id:
                trace_kwargs["metadata"] = {"earthmind.tracing_user_id": self.tracing_user_id}
    ...
    def add_trace(
        ...
        metadata_: dict = {"from_earthmind_component": True, "component_id": trace_id}
