...
class LangWatchTracer(BaseTracer):
    ...
    def setup_langwatch(self) -> bool:
        ...
        try:
            ...
            if self.tracer_provider is None:
                ...
                resource = Resource.create(attributes={"service.name": "earthmind"})
    ...
    def _warn_langwatch_unavailable(cls) -> None:
        ...
        the official EarthMind Docker images. Warn only once to avoid log spam, since a tracer
        ...
        if sys.version_info >= (3, 14):
            logger.warning(
                ...
                "it is excluded from Python 3.14+ environments, including the official EarthMind "
                "Docker images. To enable LangWatch tracing, run EarthMind on Python 3.10-3.13 "
                "(for example via 'pip install earthmind')."
            )
        else:
            logger.warning(
                "LANGWATCH_API_KEY is set but the 'langwatch' package is not installed, so "
                "LangWatch tracing is disabled. Install it with 'pip install langwatch' or "
                "'pip install \"earthmind-base[langwatch]\"'."
            )
