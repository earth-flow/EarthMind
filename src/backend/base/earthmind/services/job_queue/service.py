...
_STREAM_PREFIX = "earthmind:queue:"
_OWNER_PREFIX = "earthmind:owner:"
...
_CANCEL_CHANNEL_PREFIX = "earthmind:cancel:"
...
_ACTIVITY_PREFIX = "earthmind:activity:"
...
class RedisQueueWrapper:
    ...
    * End-of-stream →  ``XADD key * event_id __sentinel__ data __sentinel__ ts <float>``
    ...
    # value can be driven from settings (EARTHMIND_REDIS_QUEUE_STARTUP_GRACE_S).
...
class RedisJobQueueService(JobQueueService):
    ...
    Configuration
    -------------
    Set ``EARTHMIND_JOB_QUEUE_TYPE=redis`` and, optionally:

    * ``EARTHMIND_REDIS_QUEUE_DB`` (default ``1``, separate from cache DB ``0``)
    * ``EARTHMIND_REDIS_QUEUE_URL`` (full URL, overrides host/port/db)
    * ``EARTHMIND_REDIS_QUEUE_HOST`` / ``EARTHMIND_REDIS_QUEUE_PORT``
    ...
    ``earthmind:cancel:*``. Any worker can publish a cancel signal via
    ...
    completion. See ``earthmind.api.build.create_flow_response``.
    """
    ...
    async def _check_connection(self) -> None:
        ...
        try:
            ...
            await logger.aerror(
                ...
                "Set EARTHMIND_JOB_QUEUE_TYPE=asyncio or start Redis before running EarthMind."
            )
    ...
    _CANCEL_MARKER_PREFIX = "earthmind:cancel-marker:"
    ...
    def get_queue_data(self, job_id: str) -> tuple[asyncio.Queue, EventManager, asyncio.Task | None, float | None]:  # type: ignore[override]
        ...
        * **Cross-worker path**: no local entry exists; a null ``EventManager`` and
          ...
          response's :func:`earthmind.api.build.create_flow_response.on_disconnect`
