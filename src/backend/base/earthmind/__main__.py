...
# late — see earthmind_launcher.py for the same pattern).
#
# The `earthmind` console script routes through earthmind_launcher.py which
# handles this. This guard catches the bypass paths (`python -m earthmind`,
# `uv run python -m earthmind`, etc.) so they're not silent footguns. Only
# fires for direct CLI invocation; ordinary `import earthmind.__main__` is
# unaffected.
if __name__ == "__main__":
    ...
    if _platform.system() == "Darwin" and not _os.environ.get("OBJC_DISABLE_INITIALIZE_FORK_SAFETY"):
        ...
        _os.execv(_sys.executable, [_sys.executable, "-m", "earthmind.__main__", *_sys.argv[1:]])  # noqa: S606
...
from earthmind.cli.progress import create_earthmind_progress
from earthmind.initial_setup.setup import get_or_create_default_folder
from earthmind.main import setup_app
from earthmind.services.auth.utils import get_current_user_from_access_token
from earthmind.services.database.models.api_key.crud import check_key
from earthmind.services.database.service import UnsupportedPostgreSQLVersionError, check_postgresql_version_sync
from earthmind.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from earthmind.services.utils import initialize_services
from earthmind.utils.version import fetch_latest_version, get_version_info
from earthmind.utils.version import is_pre_release as earthmind_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="EarthMind Executor commands")
...
# Platforms where `earthmind run` bypasses Gunicorn and runs uvicorn directly
...
def ensure_multi_worker_safe(num_workers: int) -> None:
    ...
    Skipped entirely when ``EARTHMIND_JOB_QUEUE_TYPE=redis`` is configured —
    ...
    msg = (
        ...
        "  * Configure a shared job queue: EARTHMIND_JOB_QUEUE_TYPE=redis. Works "
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("EARTHMIND_LOG_LEVEL") or "info").lower()
    os.environ["EARTHMIND_LOG_LEVEL"] = log_level
    ...
    progress = create_earthmind_progress(verbose=verbose)

    # Step 0: Initializing EarthMind
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("EARTHMIND_", "")
    ...
    # Step 6: Launching EarthMind
    if use_direct_uvicorn():
        # EARTHMIND_GUNICORN_PRELOAD is a Gunicorn-only knob: it triggers fork-safe
        ...
        if os.environ.get("EARTHMIND_GUNICORN_PRELOAD", "false").lower() == "true":
            logger.warning(
                "EARTHMIND_GUNICORN_PRELOAD=true is ignored on %s: this platform "
        ...
        from earthmind.helpers.windows_postgres_helper import EARTHMIND_DATABASE_URL, POSTGRESQL_PREFIXES

        db_url = os.environ.get(EARTHMIND_DATABASE_URL, "")
        loop_type = "asyncio"
        if (
        ...
        with progress.step(6):
            # Use Gunicorn with EarthMindUvicornWorker for non-Windows systems
            from earthmind.server import EarthMindApplication
            ...
            options = {
                ...
                "preload_app": os.environ.get("EARTHMIND_GUNICORN_PRELOAD", "false").lower() == "true",
            }
            server = EarthMindApplication(app_factory, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "earthmind")
        'A new version of earthmind is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=earthmind_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if earthmind_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use earthmind.utils.version to get the version info
    version_info = get_version_info()
    earthmind_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= earthmind_is_pre_release(earthmind_version)  # Update pre-release status

    notice = build_version_notice(earthmind_version, package_name)
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/earthmind-ai/earthmind\n"
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("EARTHMIND_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve EarthMind.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open EarthMind {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            "* GitHub: https://github.com/earthmind-ai/earthmind\n"
            "# Discord: https://discord.com/invite/EqksyE2EX9\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open EarthMind -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("GitHub: https://github.com/earthmind-ai/earthmind")
            logger.info("Discord: https://discord.com/invite/EqksyE2EX9")
            logger.info(f"Open EarthMind: {protocol}://{access_host}:{port}")


@app.command()
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'earthmind' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'earthmind' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="EARTHMIND_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="EARTHMIND_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set EARTHMIND_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from earthmind.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set EARTHMIND_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run earthmind api_key`")
            raise typer.Exit(1)
...
# command to copy the earthmind database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'earthmind.db' and 'earthmind-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "earthmind.db"
    pre_db_path = cache_dir / "earthmind-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from earthmind.services.database.models.api_key.crud import create_api_key, delete_api_key
            from earthmind.services.database.models.api_key.model import ApiKey, ApiKeyCreate
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"earthmind {version}")
        raise typer.Exit
