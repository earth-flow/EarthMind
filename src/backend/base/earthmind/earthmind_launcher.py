...
def main():
    """Launches earthmind with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from earthmind.__main__ import main as earthmind_main

        earthmind_main()


def _launch_with_exec():
    """Launch earthmind by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "earthmind.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec earthmind: {e}", file=sys.stderr)
        sys.exit(1)
