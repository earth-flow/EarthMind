...
from terraflow.api import health_check_router, log_router
from terraflow.api.router import router
from terraflow.api.v1.mcp_projects import init_mcp_servers
...
from terraflow.middleware import ContentSizeLimitMiddleware
from terraflow.plugin_routes import load_plugin_routes
from terraflow.services.database.models.deployment.exceptions import DeploymentGuardError
from terraflow.services.database.service import UnsupportedPostgreSQLVersionError
from terraflow.services.deps import (
    get_queue_service,
    get_service,
...
from terraflow.services.utils import initialize_services, initialize_settings_service, teardown_services
from terraflow.utils.mcp_cleanup import cleanup_mcp_sessions
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Set TERRAFLOW_CORS_ORIGINS for production. Stricter defaults in v2.0."
        )
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        from terraflow.preload import PreloadStep, get_owned_temp_dirs, is_step_complete

        configure()

        # Startup message
        if version:
            await logger.adebug(f"Starting Terraflow v{version}...")
        else:
            await logger.adebug("Starting Terraflow...")

        sync_flows_from_fs_task = None
        mcp_init_task = None
        models_dev_refresh_task = None
        terrabox_tools_refresh_task = None
        ...
        try:
            ...
            if get_settings_service().settings.sentry_dsn:
                try:
                    ...
                    await logger.awarning(
                        "TERRAFLOW_SENTRY_DSN is set but sentry-sdk is not installed; "
                    ...
                    try:
                        ...
                        await logger.awarning(f"Failed to initialize Sentry SDK (check TERRAFLOW_SENTRY_DSN): {e}")
            ...
            if is_step_complete(PreloadStep.STARTER_PROJECTS):
                ...
                if all_types_dict is None:
                    ...
                    lock_file = Path(tempfile.gettempdir()) / "terraflow_starter_projects.lock"
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_GLOBALS):
                    ...
                    from terraflow.api.utils.mcp.agentic_mcp import initialize_agentic_global_variables
            ...
            if get_settings_service().settings.agentic_experience:
                if is_step_complete(PreloadStep.AGENTIC_MCP):
                    ...
                    from terraflow.api.utils.mcp.agentic_mcp import auto_configure_agentic_mcp_server
            ...
            # TERRAFLOW_MODELS_DEV_REFRESH=false disables the live models.dev
            ...
            if os.getenv("TERRAFLOW_MODELS_DEV_REFRESH", "true").lower() not in ("false", "0", "no"):
                models_dev_refresh_task = asyncio.create_task(refresh_models_dev_periodically())
            else:
                await logger.adebug("models.dev refresh disabled via TERRAFLOW_MODELS_DEV_REFRESH")

            async def refresh_terrabox_tools_periodically() -> None:
                """Periodically re-fetch Terrabox's live tool catalog into the sidebar.

                `get_and_cache_all_types_dict` only calls
                `apply_earthflow_component_policy` once, the first time the
                component cache is built after process start -- so even
                though `load_terrabox_tool_specs` fetches Terrabox's tool
                list live over HTTP, a toolkit or tool Terrabox adds later
                would otherwise stay invisible until this process restarts.
                This loop calls `refresh_terrabox_components_cache` on an
                interval so new Terrabox toolkits/tools show up in the
                sidebar without a restart. Best-effort: a failed fetch (e.g.
                Terrabox briefly unreachable) just leaves the previous tool
                set in place and retries next interval.
                """
                from lfx.interface.earthflow_components import (
                    refresh_terrabox_components_cache,
                )

                refresh_interval_seconds = int(os.getenv("EARTHFLOW_TERRABOX_TOOLS_REFRESH_INTERVAL_SECONDS", "600"))

                while True:
                    try:
                        count = await asyncio.to_thread(refresh_terrabox_components_cache)
                        await logger.adebug(f"Terrabox tools refreshed: {count} components cached")
                    except asyncio.CancelledError:
                        raise
                    except Exception as e:  # noqa: BLE001
                        await logger.awarning(f"Terrabox tools refresh failed: {e}")

                    await asyncio.sleep(refresh_interval_seconds)

            # EARTHFLOW_TERRABOX_TOOLS_REFRESH=false disables the periodic
            # live Terrabox tool re-fetch (e.g. tests, or deployments that
            # don't run Terrabox and would otherwise log a fetch warning
            # every interval).
            if os.getenv("EARTHFLOW_TERRABOX_TOOLS_REFRESH", "true").lower() not in ("false", "0", "no"):
                terrabox_tools_refresh_task = asyncio.create_task(refresh_terrabox_tools_periodically())
            else:
                await logger.adebug("Terrabox tools refresh disabled via EARTHFLOW_TERRABOX_TOOLS_REFRESH")

            # v1 and project MCP server context managers
            from terraflow.api.v1.mcp import start_streamable_http_manager
            from terraflow.api.v1.mcp_projects import start_project_task_group
            ...
            if "terraflow migration --fix" not in str(exc):
            ...
            from terraflow.__main__ import get_number_of_workers
            from terraflow.cli.progress import create_terraflow_shutdown_progress

            log_level = os.getenv("TERRAFLOW_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_terraflow_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(1):
                    from terraflow.api.v1.mcp import stop_streamable_http_manager
                    from terraflow.api.v1.mcp_projects import stop_project_task_group
                    ...
                    if terrabox_tools_refresh_task and not terrabox_tools_refresh_task.done():
                        terrabox_tools_refresh_task.cancel()
                        tasks_to_cancel.append(terrabox_tools_refresh_task)
                    if tasks_to_cancel:
                ...
                with shutdown_progress.step(2):
                    ...
                    try:
                        from terraflow.services.authorization.utils import drain_pending_audit_writes
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Terraflow shutdown complete")
...
def create_app():
    ...
    from terraflow.utils.version import get_version_info
    ...
    async def set_locale(request: Request, call_next):
        ...
        if _supported_locales is None:
            from terraflow.utils.i18n import get_supported_locales
    ...
    if prome_port_str := os.environ.get("TERRAFLOW_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from terraflow.api.v1 import mcp_router
    ...
    # Discover and register additional routers from plugins (terraflow.plugins entry-point)
    ...
    async def rate_limit_exception_handler(request: Request, _exc: RateLimitExceeded):
        ...
        from terraflow.services.rate_limit.service import get_limiter_key
    ...
    from terraflow.services.rate_limit import get_rate_limiter
...
def get_static_files_dir():
    """Get the static files directory relative to Terraflow's main.py file."""
