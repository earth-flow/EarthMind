...
from terraflow.initial_setup.constants import (
...
from terraflow.services.database.models.folder.constants import (
    DEFAULT_FOLDER_DESCRIPTION,
    DEFAULT_FOLDER_NAME,
    LEGACY_FOLDER_NAMES,
)
from terraflow.services.database.models.folder.model import Folder, FolderCreate, FolderRead
...
async def get_or_create_assistant_folder(session, user_id: UUID):
    """Create or get the Terraflow Assistant folder for a specific user.
    ...
    Returns:
        The Terraflow Assistant folder
    """
...
async def create_or_update_agentic_flows(session: AsyncSession, user_id: UUID) -> None:
    """Create or update agentic flows in the Terraflow Assistant folder for a user.

    This function is called on user login to ensure that all agentic flows
    are present and up-to-date in the user's Terraflow Assistant folder.
    ...
    try:
        # Get or create the Terraflow Assistant folder
...
async def load_flows_from_directory() -> None:
    """On terraflow startup, this loads all flows from the directory specified in the settings.
    ...
    if existing:
        ...
        if not matched_by_id and not get_settings_service().settings.load_flows_overwrite_on_name_match:
            await logger.awarning(
                ...
                "Set TERRAFLOW_LOAD_FLOWS_OVERWRITE_ON_NAME_MATCH=true to overwrite."
            )
            return
        await logger.ainfo(
...
async def find_existing_flow(session, flow_id, flow_endpoint_name, *, user_id=None, name=None):
    ...
    ``TERRAFLOW_LOAD_FLOWS_PATH`` can upsert against a DB row that shares the
...
async def initialize_auto_login_default_superuser() -> None:
    ...
    async with session_scope() as async_session:
        ...
        from terraflow.api.utils.mcp.agentic_mcp import initialize_agentic_user_variables
