...
from terraflow.api.v2 import files_router as files_router_v2
from terraflow.api.v2 import mcp_router as mcp_router_v2
from terraflow.api.v2 import registration_router as registration_router_v2
from terraflow.api.v2 import workflow_router as workflow_router_v2
...
def include_deployment_router(target_router: APIRouter) -> None:
    ...
    if FEATURE_FLAGS.wxo_deployments:
        from terraflow.api.v1.deployments import router as deployment_router
...
# always mounted; a per-request guard in ``terraflow.api.v1.extensions`` reads
...
# coupling: ``terraflow.__main__`` imports ``setup_app`` (and hence this
# router module) before ``load_dotenv(env_file)`` runs, so any module-level
# read of ``TERRAFLOW_ENABLE_EXTENSION_RELOAD`` would miss the value supplied
...
def _include_agentic_router():
    from terraflow.agentic.api.files_router import router as agentic_files_router
    from terraflow.agentic.api.router import router as agentic_router
    from terraflow.agentic.api.sessions_router import router as agentic_sessions_router
