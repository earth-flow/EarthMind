...
from terraflow.services.auth.utils import get_current_active_user, get_current_active_user_mcp
...
from terraflow.services.store.utils import get_lf_version_from_pypi
from terraflow.utils.constants import TERRAFLOW_GLOBAL_VAR_HEADER_PREFIX

if TYPE_CHECKING:
    from terraflow.services.store.schema import StoreComponentCreate
...
async def check_terraflow_version(component: StoreComponentCreate) -> None:
    ...
    terraflow_version = await get_lf_version_from_pypi()
    if terraflow_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Terraflow")
    if terraflow_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Terraflow ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({terraflow_version}) and try again."
        )
...
def extract_global_variables_from_headers(headers, *, include_auth_headers: bool = False) -> dict[str, str]:
    ...
    By default, only headers with the ``X-TERRAFLOW-GLOBAL-VAR-*`` prefix are
    ...
    ``/workflow``, ``x-api-key`` is Terraflow's own authentication key — exposing
    ...
    Example:
        headers = {"X-TERRAFLOW-GLOBAL-VAR-API-KEY": "secret", "x-api-key": "mykey"}
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(TERRAFLOW_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(TERRAFLOW_GLOBAL_VAR_HEADER_PREFIX) :].upper()
