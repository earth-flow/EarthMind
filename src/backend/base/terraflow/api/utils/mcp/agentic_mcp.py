"""Utilities for auto-configuring the Terraflow Agentic MCP server."""
...
from terraflow.api.v2.mcp import get_server_list, update_server
from terraflow.services.database.models.user.model import User
from terraflow.services.deps import get_service, get_variable_service
from terraflow.services.schema import ServiceType
from terraflow.services.variable.constants import CREDENTIAL_TYPE, GENERIC_TYPE


async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the Terraflow Agentic MCP server for all users.

    This function adds the terraflow-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring Terraflow Agentic MCP server for all users...")
    ...
    try:
        ...
        server_name = "terraflow-agentic"
        python_executable = sys.executable
        server_config = {
            "command": python_executable,
            "args": ["-m", "terraflow.agentic.mcp"],
            "metadata": {
                "description": "Terraflow Agentic MCP server providing tools for flow/component operations, "
                "template search, and graph visualization",
                "auto_configured": True,
                "terraflow_internal": True,
            },
        }
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Terraflow Agentic MCP server from all users.

    This function removes the terraflow-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Terraflow Agentic MCP server from all users...")
