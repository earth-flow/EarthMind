...
async def _docx_to_markdown_via_mineru(*, file_name: str, raw_bytes: bytes, notes: list[str], mineru_overrides: dict[str, Any] | None = None) -> str | None:
    ...
    with tempfile.TemporaryDirectory(prefix="terraflow-kb-docx-") as temp_dir:
...
async def _pdf_to_markdown(*, file_name: str, raw_bytes: bytes, notes: list[str], mineru_overrides: dict[str, Any] | None = None) -> str | None:
    ...
    with tempfile.TemporaryDirectory(prefix="terraflow-kb-pdf-") as temp_dir:
...
def _load_mineru_config(overrides: dict[str, Any] | None = None) -> _MineruConfig:
    ...
    return _MineruConfig(
        api_token=overrides.get("mineru_api_token") or os.getenv("TERRAFLOW_MINERU_API_TOKEN") or os.getenv("MINERU_API_TOKEN"),
        api_base=str(overrides.get("mineru_api_base") or os.getenv("TERRAFLOW_MINERU_API_BASE", "https://mineru.net/api/v4")).rstrip("/"),
        lang=str(overrides.get("mineru_lang") or os.getenv("TERRAFLOW_MINERU_LANG", "ch")),
        enable_formula=bool(overrides.get("mineru_enable_formula")) if overrides.get("mineru_enable_formula") is not None else _env_flag("TERRAFLOW_MINERU_ENABLE_FORMULA", True),
        enable_table=bool(overrides.get("mineru_enable_table")) if overrides.get("mineru_enable_table") is not None else _env_flag("TERRAFLOW_MINERU_ENABLE_TABLE", True),
        is_ocr=bool(overrides.get("mineru_is_ocr")) if overrides.get("mineru_is_ocr") is not None else _env_flag("TERRAFLOW_MINERU_IS_OCR", True),
        model_version=str(overrides.get("mineru_model_version")) if overrides.get("mineru_model_version") else (os.getenv("TERRAFLOW_MINERU_MODEL_VERSION") or None),
        request_timeout_s=float(overrides.get("mineru_timeout_s") or os.getenv("TERRAFLOW_MINERU_TIMEOUT_S", "300")),
        poll_attempts=int(overrides.get("mineru_poll_attempts") or os.getenv("TERRAFLOW_MINERU_POLL_ATTEMPTS", "180")),
        poll_interval_s=float(overrides.get("mineru_poll_interval_s") or os.getenv("TERRAFLOW_MINERU_POLL_INTERVAL_S", "5")),
    )
