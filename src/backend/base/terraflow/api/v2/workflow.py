...
from terraflow.api.utils import extract_global_variables_from_headers
from terraflow.api.v1.schemas import RunResponse
from terraflow.api.v2.converters import (
    create_error_response,
    parse_flat_inputs,
    run_response_to_workflow_response,
)
from terraflow.api.v2.workflow_reconstruction import reconstruct_workflow_response_from_job_id
from terraflow.exceptions.api import (
...
from terraflow.services.auth.utils import api_key_security
...
from terraflow.services.database.models.user.model import UserRead
from terraflow.services.deps import get_job_service, get_memory_base_service, get_task_service
...
def _resolve_request_variables(
    ...
    values can be transported safely. The legacy ``X-TERRAFLOW-GLOBAL-VAR-*``
    ...
    if legacy_globals:
        logger.warning(
            "X-TERRAFLOW-GLOBAL-VAR-* headers are deprecated on /api/v2/workflows; "
            "send request-level globals in the JSON body instead. Header-based "
            "globals will be removed in a future Terraflow release."
        )
...
async def execute_workflow(
    ...
    **background** and **stream** can't be true at the same time.
    ...
    Args:
        ...
        http_request: The HTTP request, used to honor deprecated ``X-TERRAFLOW-GLOBAL-VAR-*`` headers
...
async def execute_sync_workflow(
    ...
    # Unicode strings transport safely. The legacy X-TERRAFLOW-GLOBAL-VAR-*
