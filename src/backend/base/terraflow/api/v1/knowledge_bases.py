...
from terraflow.api.utils import CurrentActiveUser, ingestion_run_service, knowledge_base_service
from terraflow.api.utils.kb_helpers import KBAnalysisHelper, KBIngestionHelper, KBStorageHelper
from terraflow.api.utils.kb_graph import (
    build_chunk_identifier,
    build_graph_cache_fingerprint,
    build_graph_cache_signature,
    build_kb_chunk_matcher,
    build_knowledge_graph_response,
    extract_file_label,
    get_graph_preview_scan_limit,
    extract_model_assisted_triples,
    load_persisted_graph_cache,
    parse_kb_metadata_filters,
    persist_graph_cache,
    select_knowledge_graph_sample_entries,
    KB_GRAPH_MODE_DEFAULT,
)
from terraflow.api.utils.kb_metadata import parse_per_file_metadata, parse_user_metadata
from terraflow.api.utils.kb_processing import (
    normalize_chunk_strategy,
    normalize_parser_strategy,
    process_knowledge_document,
)
from terraflow.api.v1.schemas import TaskResponse
from terraflow.schema.knowledge_base import (
    BulkDeleteRequest,
    ChunkInfo,
...
from terraflow.services.authorization import (
    KnowledgeBaseAction,
    ensure_knowledge_base_permission,
)
from terraflow.services.database.models.jobs.model import JobStatus, JobType
from terraflow.services.database.models.knowledge_base.model import KnowledgeBaseRecord
from terraflow.services.deps import get_job_service, get_settings_service, get_task_service
from terraflow.services.jobs import DuplicateJobError
from terraflow.services.jobs.service import JobService
from terraflow.services.memory_base.kb_path_helpers import validate_kb_path
from terraflow.services.task.service import TaskService
from terraflow.utils.kb_constants import (
...
_KB_GRAPH_BACKGROUND_REFRESHES: set[str] = set()
...
async def _guard_kb_action(
    *,
    ...
    ``TERRAFLOW_AUTHZ_ENABLED=true``, the helper falls back to scanning every
    ...
    """
    from terraflow.services.authorization.utils import (
        _auth_context,
        _coerce_action,
        _resolve_authz_domain,
    )
    from terraflow.services.database.models.user.crud import get_user_by_id
    from terraflow.services.deps import get_authorization_service, session_scope
...
def _validate_kb_path_containment(kb_user_path: Path, kb_path: Path, kb_name: str, username: str) -> None:
    ...
    :func:`terraflow.services.memory_base.kb_path_helpers.validate_kb_path`
...
async def _warm_graph_cache_in_background(
    *,
    cache_key: str,
    matched_entries: list[tuple[str, str, dict[str, Any]]],
    matched_count: int,
    total_files: int,
    truncated: bool,
    ...
    if cache_key in _KB_GRAPH_BACKGROUND_REFRESHES:
        return
    _KB_GRAPH_BACKGROUND_REFRESHES.add(cache_key)
    try:
        precomputed_triples = await extract_model_assisted_triples(
            matched_entries,
            user_id=current_user_id,
            kb_path=kb_path,
            quality_mode=quality_mode,
        )
        graph_response = build_knowledge_graph_response(
            matched_entries,
            ...
            max_nodes=max_nodes,
            max_edges=max_edges,
            precomputed_triples_by_chunk=precomputed_triples,
        )
        ...
        await logger.aerror("Background graph warm failed for %s: %s", cache_key, e)
    finally:
        _KB_GRAPH_BACKGROUND_REFRESHES.discard(cache_key)
...
def _coerce_backend_config(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        return {}
    # Parser/chunker UI settings are KB metadata, not vector-store backend config.
    # Strip them defensively so older rows written with polluted backend_config
    # do not break Chroma Cloud / other remote backend initialization.
    return {
        key: val
        for key, val in value.items()
        if key not in {"parser_strategy", "chunk_strategy"}
    }


def _kb_runtime_config_from_request(request: CreateKnowledgeBaseRequest) -> dict[str, Any]:
    return {
        "embedding_api_key": request.embedding_api_key,
        "embedding_api_base": request.embedding_api_base,
        "mineru_api_token": request.mineru_api_token,
        "mineru_api_base": request.mineru_api_base,
        "mineru_lang": request.mineru_lang,
        "mineru_enable_formula": request.mineru_enable_formula,
        "mineru_enable_table": request.mineru_enable_table,
        "mineru_is_ocr": request.mineru_is_ocr,
        "mineru_model_version": request.mineru_model_version,
        "mineru_timeout_s": request.mineru_timeout_s,
        "mineru_poll_attempts": request.mineru_poll_attempts,
        "mineru_poll_interval_s": request.mineru_poll_interval_s,
    }
...
def _extract_connection_config(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(model_selection, list):
        model_selection = model_selection[0] if model_selection else {}
    return {
        "embeddingApiKey": model_selection.get("api_key") or backend_config.get("embedding_api_key") or "",
        "embeddingApiBase": model_selection.get("api_base") or backend_config.get("embedding_api_base") or "",
        "mineruApiToken": backend_config.get("mineru_api_token") or "",
        "mineruApiBase": backend_config.get("mineru_api_base") or "",
        "mineruLang": backend_config.get("mineru_lang") or "",
        "mineruEnableFormula": backend_config.get("mineru_enable_formula"),
        "mineruEnableTable": backend_config.get("mineru_enable_table"),
        "mineruIsOcr": backend_config.get("mineru_is_ocr"),
        "mineruModelVersion": backend_config.get("mineru_model_version") or "",
        "mineruTimeoutS": backend_config.get("mineru_timeout_s"),
        "mineruPollAttempts": backend_config.get("mineru_poll_attempts"),
        "mineruPollIntervalS": backend_config.get("mineru_poll_interval_s"),
        "milvusUri": backend_config.get("uri") or "",
        "milvusToken": backend_config.get("token") or "",
        "milvusCollectionName": backend_config.get("collection_name") or "",
    }
...
def _build_kb_info(
    *,
    ...
    return KnowledgeBaseInfo(
        ...
        connection_config=_extract_connection_config(metadata),
    )
...
async def _delete_remote_backend_collection(
    *,
    ...
    delete the KB from Terraflow's UI at all. Before this, the backend
...
async def create_knowledge_base(
    ...
    try:
        ...
        backend_config_value = dict(request.backend_config or {})
        backend_config_value.update({k: v for k, v in _kb_runtime_config_from_request(request).items() if v is not None})
        parser_strategy_value = normalize_parser_strategy(request.parser_strategy)
        chunk_strategy_value = normalize_chunk_strategy(request.chunk_strategy)
        embedding_metadata = {
            "id": str(kb_id),
            ...
            "chunk_strategy": chunk_strategy_value,
        }
        metadata_path = kb_path / "embedding_metadata.json"
        metadata_path.write_text(json.dumps(embedding_metadata, indent=2))
        ...
        try:
            ...
            if isinstance(persisted_selection, dict):
                if request.embedding_api_key:
                    persisted_selection["api_key"] = request.embedding_api_key
                if request.embedding_api_base:
                    persisted_selection["api_base"] = request.embedding_api_base
...
async def preview_chunks(
    ...
    parser_strategy: Annotated[str, Form()] = "auto",
    chunk_strategy: Annotated[str, Form()] = "heading_markdown",
    ...
    try:
        ...
        for uploaded_file in files:
            try:
                ...
                processed = await process_knowledge_document(
                    file_name=file_name,
                ...
                preview_chunks = [
                    ...
                    for chunk in processed.chunks[:max_chunks]
                ]
                ...
                file_previews.append(
                    {
                        ...
                        "parser_strategy": processed.parser_strategy,
                        "chunk_strategy": processed.chunk_strategy,
                        "source_format": processed.source_format,
                        "has_markdown": bool(processed.markdown_content),
                ...
                file_previews.append(
                    {
                        ...
                        "parser_strategy": parser_strategy,
                        "chunk_strategy": chunk_strategy,
                        "source_format": Path(uploaded_file.filename or "unknown").suffix.lstrip("."),
                        "has_markdown": False,
                        "notes": [str(file_error)],
                    }
                )
    except HTTPException:
        raise
...
async def ingest_files_to_knowledge_base(
    ...
    * ``per_file_metadata`` — overrides keyed by filename; merged on top of
    ...
    try:
        ...
        parser_strategy = normalize_parser_strategy(parser_strategy or metadata.get("parser_strategy"))
        chunk_strategy = normalize_chunk_strategy(chunk_strategy or metadata.get("chunk_strategy"))
        if not metadata:
            raise HTTPException(
                status_code=400,
        ...
        # Persist the selected processing strategies so follow-up uploads and the KB drawer reflect the current behavior.
        cc_metadata_path = kb_path / "embedding_metadata.json"
        if cc_metadata_path.exists():
            existing_meta = json.loads(cc_metadata_path.read_text())
            existing_meta["parser_strategy"] = parser_strategy
            existing_meta["chunk_strategy"] = chunk_strategy
            cc_metadata_path.write_text(json.dumps(existing_meta, indent=2))
...
async def ingest_folder_to_knowledge_base(
    ...
    try:
        ...
        kb_path = _resolve_kb_path(kb_name, _kb_guard.owner_user)
        metadata = KBAnalysisHelper.get_metadata(kb_path, fast=False)
        parser_strategy = normalize_parser_strategy(payload.parser_strategy or metadata.get("parser_strategy"))
        chunk_strategy = normalize_chunk_strategy(payload.chunk_strategy or metadata.get("chunk_strategy"))
        if not metadata:
            raise HTTPException(
                status_code=400,
                detail="Knowledge base missing embedding configuration. Please create a new KB or reconfigure it.",
...
async def get_knowledge_base_chunks(
    ...
    try:
        ...
        matches_filters = build_kb_chunk_matcher(
            search_term=search,
            source_type=source_type,
            file_name=file_name,
            job_id=job_id,
            metadata_filter_dict=parse_kb_metadata_filters(request),
        )
            ...
            async for batch in backend.iter_documents():
                for entry in batch:
                    if not matches_filters(entry.metadata, entry.content):
                        continue
                    entry_id = build_chunk_identifier(entry.metadata, entry.content)
...
async def _materialize_knowledge_base_graph(
    *,
    kb_name: str,
    current_user: CurrentActiveUser,
    request: Request,
    ...
    try:
        ...
        metadata_filter_dict = parse_kb_metadata_filters(request)
        graph_signature = build_graph_cache_signature(
        ...
        graph_fingerprint = build_graph_cache_fingerprint(kb_path)

        if not force_refresh:
            cached_graph = load_persisted_graph_cache(
                kb_path,
                signature=graph_signature,
                fingerprint=graph_fingerprint,
            )
        ...
        matches_filters = build_kb_chunk_matcher(
            ...
            job_id=job_id,
            metadata_filter_dict=metadata_filter_dict,
            chunk_ids=set(chunk_id or []),
        )

        matched_entries: list[tuple[str, str, dict[str, Any]]] = []
        matched_count = 0
        reached_sample_cap = False
        matched_file_labels: set[str] = set()
        scan_limit = get_graph_preview_scan_limit(sample_limit, full_graph=full_graph)
        try:
            async for batch in backend.iter_documents():
                stop_scan = False
                for entry in batch:
                    if not matches_filters(entry.metadata, entry.content):
                        continue
                    matched_count += 1
                    matched_file_labels.add(extract_file_label(entry.metadata))
                    matched_entries.append(
                        (
                            build_chunk_identifier(entry.metadata, entry.content),
                            entry.content,
                            dict(entry.metadata),
                        )
                    )
                    if scan_limit is not None and len(matched_entries) >= scan_limit:
            ...
            await logger.aerror("iter_documents failed for graph '%s': %s", kb_name, iter_error)
            raise HTTPException(status_code=500, detail="Error building knowledge graph.") from iter_error

        graph_entries = select_knowledge_graph_sample_entries(
            matched_entries,
            sample_limit=sample_limit,
            full_graph=full_graph,
        )
        matched_file_labels.discard("")
        should_defer_llm = (
            not force_refresh
            and full_graph
            and quality_mode == "high"
            and len(graph_entries) > 80
        )
        # standard 模式跳过 LLM，只用启发式正则抽取，保证快速响应
        should_skip_llm = quality_mode != "high"
        precomputed_triples = (
            {}
            if should_defer_llm or should_skip_llm
            else await extract_model_assisted_triples(
                graph_entries,
                user_id=current_user.id,
                kb_path=kb_path,
                quality_mode=quality_mode,
        ...
        graph_response = build_knowledge_graph_response(
            graph_entries,
            matched_chunks=matched_count,
            total_files=len(matched_file_labels),
            truncated=(sample_limit is not None and matched_count > len(graph_entries)) or reached_sample_cap,
            full_graph=full_graph,
            graph_mode=graph_mode,
            max_nodes=max_nodes,
            max_edges=max_edges,
        ...
        if should_defer_llm and matched_entries:
            cache_key = f"{kb_path}:{graph_signature}:{graph_fingerprint}:{quality_mode}"
            asyncio.create_task(
                _warm_graph_cache_in_background(
                    cache_key=cache_key,
                    matched_entries=list(graph_entries),
        ...
        await logger.aerror("Error building graph for '%s': %s", kb_name, e)
        raise HTTPException(status_code=500, detail="Error building knowledge graph.") from e
    finally:
        if backend is not None:
            try:
                await backend.teardown()
...
async def get_knowledge_base_graph(
    ...
    current_user: CurrentActiveUser,
    request: Request,
    full_graph: Annotated[bool, Query(description="Return the full graph without node/edge trimming.")] = False,
    quality_mode: Annotated[str, Query(description="Graph extraction quality mode: standard or high.")] = "high",
    graph_mode: Annotated[str, Query(description="Graph presentation mode.")] = KB_GRAPH_MODE_DEFAULT,
    search: Annotated[str, Query(description="Filter graph chunks whose text contains this substring")] = "",
    source_type: Annotated[
        str | None,
        Query(description="Only include chunks ingested via the given source type."),
    ] = None,
    file_name: Annotated[
        str | None,
        Query(description="Only include chunks whose source filename exactly matches."),
    ] = None,
    job_id: Annotated[
        str | None,
        Query(description="Only include chunks written by the given ingestion job_id."),
    ] = None,
    sample_limit: Annotated[
        int | None,
        Query(ge=1, le=2000, description="Optional cap on matched chunks used to build the graph. Omit to process the full filtered KB."),
    ] = None,
    max_nodes: Annotated[int | None, Query(ge=1, le=200, description="Optional cap on returned graph nodes.")] = None,
    max_edges: Annotated[int | None, Query(ge=1, le=500, description="Optional cap on returned graph edges.")] = None,
    chunk_id: Annotated[
        list[str] | None,
        Query(description="Optional repeated chunk_id filter for page-local graph rendering."),
    ] = None,
) -> KnowledgeGraphResponse:
    return await _materialize_knowledge_base_graph(
        kb_name=kb_name,
        current_user=current_user,
        request=request,
        search=search,
...
@router.post(
    "/{kb_name}/graph/refresh",
    status_code=HTTPStatus.OK,
    dependencies=[Depends(_check_memory_base_association)],
)
async def refresh_knowledge_base_graph(
    kb_name: str,
    current_user: CurrentActiveUser,
    request: Request,
    full_graph: Annotated[bool, Query(description="Rebuild and persist the full graph without node/edge trimming.")] = False,
    quality_mode: Annotated[str, Query(description="Graph extraction quality mode: standard or high.")] = "high",
    graph_mode: Annotated[str, Query(description="Graph presentation mode.")] = KB_GRAPH_MODE_DEFAULT,
    search: Annotated[str, Query(description="Filter graph chunks whose text contains this substring")] = "",
    source_type: Annotated[
