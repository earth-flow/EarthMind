...
from terraflow.api.utils import (
    CurrentActiveUser,
    DbSession,
    cascade_delete_flow,
    custom_params,
)
from terraflow.api.v1.auth_helpers import handle_auth_settings_update
from terraflow.api.v1.mappers.deployments.sync import (
    retry_flow_operation_on_deployment_guard,
    retry_project_operation_on_deployment_guard,
)
from terraflow.api.v1.mcp_projects import register_project_with_composer
from terraflow.api.v1.projects_files import download_project_flows, upload_project_flows
from terraflow.api.v1.projects_mcp_helpers import (
    cleanup_mcp_on_delete,
    handle_mcp_server_rename,
    reconcile_mcp_server_for_auth_update,
    register_mcp_servers_for_project,
)
from terraflow.initial_setup.constants import ASSISTANT_FOLDER_NAME, STARTER_FOLDER_NAME
...
from terraflow.services.authorization.utils import _resolve_authz_domain
from terraflow.services.database.models.deployment.exceptions import (
    araise_if_deployment_guard_error_or_skip,
    remap_flow_guard_for_project_delete,
)
from terraflow.services.database.models.deployment.guards import check_project_has_deployments
from terraflow.services.database.models.deployment.orm_guards import ensure_flow_moves_allowed
...
from terraflow.services.database.models.folder.model import (
    Folder,
    FolderCreate,
    FolderRead,
    FolderReadWithFlows,
...
from terraflow.services.database.models.folder.pagination_model import FolderWithPaginatedFlows
from terraflow.services.deps import get_service, get_settings_service
...
async def read_project(
    *,
    ...
    try:
        ...
        from terraflow.services.deps import get_authorization_service
...
@router.get("/{project_id}/files", status_code=200)
async def list_project_files(
    *,
    ...
    """List files generated by every flow in this project (v1 flow-scoped storage).

    v1 file storage (``storage_service.list_files``) is per-flow only — there is
    no project-level grouping in the storage layer itself — so this aggregates
    across every flow the caller can see in the project, tagging each entry
    with the originating flow. Mirrors ``read_project``'s share-aware fetch +
    per-flow visibility filter so a project-level file view never leaks files
    from a flow the caller can't see, even under a project-level share grant.
    """
    from terraflow.services.deps import get_authorization_service, get_storage_service
    ...
    for flow in visible_flows:
        try:
            flow_files = await storage_service.list_files(flow_id=str(flow.id))
        except Exception:  # noqa: BLE001 — a broken/missing flow directory shouldn't fail the whole listing
            logger.warning(f"Failed to list files for flow {flow.id} in project {project_id}", exc_info=True)
            continue
        files.extend({"flow_id": str(flow.id), "flow_name": flow.name, "file_name": name} for name in flow_files)
...
async def delete_project(
    *,
    ...
    # Prevent deletion of the Terraflow Assistant folder
