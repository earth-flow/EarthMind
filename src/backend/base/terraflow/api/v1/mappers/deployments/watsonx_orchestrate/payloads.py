...
from terraflow.api.v1.schemas.deployments import ValidatedUrl
from terraflow.services.database.models.deployment_provider_account.utils import validate_provider_url

if TYPE_CHECKING:
    from terraflow.api.v1.schemas.deployments import DeploymentCreateRequest
...
class WatsonxApiDeploymentCreatePayload(BaseModel):
    ...
    display_name: NormalizedStr | None = Field(
        ...
        description=(
            ...
            "Required unless tracking an existing wxO agent in Terraflow (via ``existing_agent_id`` field)."
        ),
    )
    llm: NormalizedStr | None = Field(
        default=None,
    ...
    existing_agent_id: NormalizedStr | None = Field(
        default=None,
        description=(
            "Provider-owned agent id to track in Terraflow instead of creating a new wxO agent. "
    ...
    def validate_create_operation_requirements(self) -> WatsonxApiDeploymentCreatePayload:
        if "existing_agent_id" in self.model_fields_set:
            ...
            if len(self.model_fields_set) > 1:
                msg = (
                    "existing_agent_id is only for tracking existing wxO agents in Terraflow and cannot include "
    ...
    def validate_with_outer_fields(self, outer_payload: DeploymentCreateRequest) -> None:
        if self.existing_agent_id is None or "description" not in outer_payload.model_fields_set:
            return
        msg = "When existing_agent_id is provided, Terraflow uses the description already set for the agent in wxO."
        raise ValueError(msg)


class WatsonxApiCreatedTool(BaseModel):
    """API response shape for a tool created from a Terraflow flow version."""
...
class _WatsonxApiAgentExecutionResultBase(BaseModel):
    ...
    ``provider_data``.  The enclosing response only carries Terraflow-owned
    fields (``deployment_id``).  ``deployment_id`` (Terraflow DB UUID) is
