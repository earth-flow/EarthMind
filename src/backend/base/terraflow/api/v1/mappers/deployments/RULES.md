...
- Terraflow API orchestration (`src/backend/base/terraflow/api/v1`)
- Mapper translation/reconciliation layer (`src/backend/base/terraflow/api/v1/mappers/deployments`)
- Adapter execution layer (`src/backend/base/terraflow/services/adapters/deployment/*`)
...
The mapper translates API payloads to Adapters and the Terraflow DB:
...
- Reconciliation extraction needed by Terraflow persistence logic
...
- Depend on Terraflow flow-version concepts directly
- Encode Terraflow DB semantics
- Assume API route-level orchestration details

---

## 2) Terraflow vs Adapter Vocabulary Rules

### 2.1 No Terraflow-specific identity terms in adapter contracts
...
Terraflow can choose to populate `source_ref` with serialized flow-version IDs, but this is an implementation decision at the mapper/API boundary.
...
- Prefer **clean break** for changes internal to Terraflow/adapter implementation layers when public API contracts are unchanged.
...
Both create and update follow a **provider-first** strategy: the provider is called first, then the Terraflow DB is updated and committed. If the DB commit fails, the route issues a best-effort compensating call to the provider.
...
- **Rollback** compensates for a provider-side change that the Terraflow DB failed to record (write-path problem). Its goal is to undo the provider-side change so that both sides remain consistent.
- **Synchronization** detects provider-side deletions (and maybe mutations) that the Terraflow DB hasn't been notified about (read-path problem). Its goal is to remove (or update, respectively) stale DB rows that no longer correspond to (or accurately represent, respectively) provider resources.
...
### 14.1 Do not mix Terraflow-owned and provider-owned fields at the same level

API response schemas must keep a clear ownership boundary between Terraflow-managed data and provider-managed data.

- **Top-level fields** should be Terraflow-owned: identifiers that Terraflow persists and controls (e.g. `deployment_id`, `name`, `created_at`).
- **Provider-owned data** (execution identifiers, agent metadata, status, timestamps, errors) belongs inside the `provider_data` dict.

This prevents future collisions — for example, if Terraflow starts persisting its own execution records, a top-level `execution_id` would be ambiguous against the provider's opaque run identifier.

### 14.2 Classification of fields

**Terraflow-owned** — fields derived from the Terraflow database or assigned by Terraflow logic:

- `deployment_id` (DB UUID), `id`, `name`, `description`, `type`
- `created_at`, `updated_at` (DB timestamps)
- `resource_key` — provider-originated but stored and indexed by Terraflow, so treated as Terraflow-owned once persisted.

**Provider-owned** — values returned by the external provider that Terraflow passes through without persisting or interpreting:
...
1. Is Terraflow the source of truth for this value? → top level.
2. Does this value come from the provider and Terraflow just relays it? → inside `provider_data`.
3. Does the provider supply it but Terraflow persists and indexes it (like `resource_key`)? → top level is acceptable.

### 14.4 Hard placement rule for non-persisted provider data

If data is not persisted in the Terraflow DB and comes directly from the provider,
it must go into `provider_data`.

Rules:

- Top-level response fields are reserved for values that Terraflow persists and controls.
- Provider-originated data that Terraflow only relays must stay in `provider_data` without exception.
