...
from terraflow.api.v1.mappers.deployments.registry import register_mapper
...
class WatsonxOrchestrateDeploymentMapper(BaseDeploymentMapper):
    ...
    def resolve_deployment_model_for_create(
        ...
        *,
        ...
        adapter_provider_result = self.parse_adapter_slot(
            ...
            operation="creating the deployment row in Terraflow",
        )
        return Deployment(
            user_id=user_id,
    ...
    def resolve_deployment_model_from_existing_resource_for_create(
        ...
        *,
        ...
        existing_provider_data = self.parse_adapter_slot(
            ...
            operation="reading deployment metadata from provider when attempting to track the resource in Terraflow",
        )
        return Deployment(
            user_id=user_id,
            project_id=project_id,
    ...
    async def resolve_rollback_update(
        ...
        *,
        ...
        """
        from terraflow.services.database.models.deployment.crud import get_deployment
        from terraflow.services.database.models.flow_version_deployment_attachment.crud import (
        ...
        # and include it here so rollback can restore wxO `name` from Terraflow's stored metadata.
