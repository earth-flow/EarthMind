...
from terraflow.api.utils import CurrentActiveUser, DbSession, DbSessionReadOnly
from terraflow.api.v1.mappers.deployments import get_deployment_mapper
from terraflow.api.v1.mappers.deployments.helpers import (
...
from terraflow.services.adapters.deployment.context import deployment_provider_scope
from terraflow.services.authorization import DeploymentAction, ensure_deployment_permission, filter_visible_resources
from terraflow.services.authorization.fetch import deny_to_404
...
from terraflow.services.database.models.deployment_provider_account.crud import (
...
from terraflow.services.deps import get_authorization_service, get_telemetry_service
from terraflow.services.telemetry.schema import DeploymentPayload
...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="Terraflow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="Terraflow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="Terraflow DB deployment UUID (`deployment.id`)."),
]
...
DeploymentIdQuery = Annotated[
    UUID,
    Query(description="Terraflow DB deployment UUID (`deployment.id`)."),
]
IncludeProviderDeleteQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "When false, only the local Terraflow DB row is removed; "
...
async def _delete_local_deployment_row_with_commit_retry(
    *,
    ...
    stale Terraflow row that still blocks later reads or provider-account
    deletion.
    """
    try:
...
async def create_deployment(
    ...
    if existing_resource_key is not None:
        ...
        if existing_deployment is not None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"The agent '{existing_resource_key}' is already managed by Terraflow. "
...
async def list_deployments(
    ...
    *,
    load_from_provider: Annotated[
        bool,
        Query(
            description=("When true, list deployments directly from the provider (bypassing Terraflow deployment rows).")
        ),
    ] = False,
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional Terraflow flow version ids (pass as repeated query params, "
...
async def update_snapshot(
    ...
    *,
    ...
    ``provider_snapshot_id``.  Only Terraflow-tracked snapshots (those with
    a ``flow_version_deployment_attachment`` row) can be updated.
    """
    from terraflow.services.database.models.deployment.crud import get_deployment as get_deployment_row
    from terraflow.services.database.models.flow_version.crud import get_flow_version_entry
