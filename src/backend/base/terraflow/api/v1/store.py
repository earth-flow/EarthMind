...
from terraflow.api.utils import CurrentActiveUser, check_terraflow_version
from terraflow.services.auth import utils as auth_utils
from terraflow.services.deps import get_settings_service, get_store_service
from terraflow.services.store.exceptions import CustomError
from terraflow.services.store.schema import (
...
async def share_component(
    ...
    try:
        await check_terraflow_version(component)
        return await get_store_service().upload(store_api_key, component)
    except Exception as exc:  # store may be unavailable offline
