...
from terraflow.api.utils.mcp import (
...
from terraflow.api.v1.mcp import ResponseNoOp
from terraflow.api.v1.mcp_utils import (
...
from terraflow.services.auth.constants import AUTO_LOGIN_WARNING
from terraflow.services.auth.mcp_encryption import decrypt_auth_settings, encrypt_auth_settings
from terraflow.services.database.models import Flow, Folder
from terraflow.services.database.models.api_key.crud import check_key, create_api_key
from terraflow.services.database.models.api_key.model import ApiKeyCreate
from terraflow.services.database.models.user.crud import get_user_by_username
...
async def verify_project_auth(
    ...
    # OAuth projects must present a valid API key at the Terraflow transport endpoint: network-level
    ...
    # composer-to-Terraflow hop should be authenticated explicitly once mcp-composer can forward
...
async def verify_project_auth_conditional(
    ...
    async with session_scope() as session:
        ...
        from terraflow.services.auth.utils import get_current_user_mcp
    ...
    try:
        async with session_scope() as session:
            ...
            if auth_settings_updated and new_auth_type in {"apikey", "none"}:
                from terraflow.api.v1.projects_mcp_helpers import reconcile_mcp_server_for_auth_update
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Terraflow API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Terraflow API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    terraflow_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", terraflow_api_key])
        ...
        # Remove stale entries that point to the same Terraflow URLs (e.g. after the project is renamed)
...
class ProjectMCPServer:
    def __init__(self, project_id: UUID):
        ...
        self.server = Server(f"terraflow-mcp-project-{project_id}")
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Terraflow host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
                ...
                try:
                    ...
                    async with session.begin_nested():
                        ...
                        if should_reconcile_project_server:
                            from terraflow.api.v1.projects_mcp_helpers import register_mcp_servers_for_project
