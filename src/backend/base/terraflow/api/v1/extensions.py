...
``TERRAFLOW_ENABLE_EXTENSION_RELOAD`` is off.
"""
...
from terraflow.api.utils.core import CurrentActiveUser
...
def _require_extension_reload_enabled() -> None:
    ...
    The route is always mounted (see ``terraflow.api.router``) so that
    ``--env-file`` can opt in to it without forcing route registration
    to happen after the env file is loaded.  Operators on Mode B/C
    deployments leave ``TERRAFLOW_ENABLE_EXTENSION_RELOAD`` unset; the
        ...
        raise _typed_http_exception(
            ...
            error=ExtensionError(
                ...
                message=(
                    ...
                    "Set TERRAFLOW_ENABLE_EXTENSION_RELOAD=true to enable it on "
                    "a local-development install (Mode A)."
                ),
                hint=(
                    "Local development: ``lfx extension dev`` sets the flag "
                    "for you.  Self-hosted: export TERRAFLOW_ENABLE_EXTENSION_RELOAD=true "
