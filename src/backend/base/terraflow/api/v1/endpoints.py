...
from terraflow.api.utils import CurrentActiveUser, DbSession, extract_global_variables_from_headers, parse_value
from terraflow.api.v1.files import get_flow
from terraflow.api.v1.global_variable_defaults import apply_global_variable_defaults
from terraflow.api.v1.schemas import (
    ConfigResponse,
...
from terraflow.events.event_manager import create_stream_tokens_event_manager
from terraflow.exceptions.api import APIException, InvalidChatInputError
from terraflow.exceptions.serialization import SerializationError
from terraflow.helpers.flow import get_flow_by_id_or_endpoint_name
from terraflow.interface.initialize.loading import update_params_with_load_from_db_fields
from terraflow.processing.process import process_tweaks, run_graph_internal
from terraflow.schema.graph import Tweaks
from terraflow.services.auth.utils import (
    api_key_security,
    get_current_active_user,
    get_current_user_for_sse,
    get_optional_user,
)
from terraflow.services.authorization import FlowAction, ensure_flow_permission
from terraflow.services.cache.utils import save_uploaded_file
from terraflow.services.database.models.flow.model import Flow, FlowRead
from terraflow.services.database.models.flow.utils import get_all_webhook_components_in_flow
from terraflow.services.database.models.jobs.model import JobType
...
from terraflow.services.event_manager import create_webhook_event_manager, webhook_event_manager
from terraflow.services.telemetry.schema import RunPayload
from terraflow.utils.compression import compress_response
...
async def get_all(request: Request):
    ...
    """
    from terraflow.interface.components import get_and_cache_all_types_dict
    from terraflow.utils.i18n import build_component_display_names, translate_component_dict
...
@router.post("/all/refresh-terrabox-tools", dependencies=[Depends(get_current_active_user)])
async def refresh_terrabox_tools(current_user: CurrentActiveUser) -> dict:
    """On-demand refresh of Terrabox's tool catalog into the sidebar.

    `GET /all` serves components from an in-memory cache that
    `apply_earthflow_component_policy` only populates once per process --
    Terrabox toolkit discovery itself is a live HTTP call (see
    `lfx.interface.earthflow_terrabox.load_terrabox_tool_specs`) and is also
    refreshed automatically on an interval (`EARTHFLOW_TERRABOX_TOOLS_REFRESH_INTERVAL_SECONDS`,
    default 10 minutes), but this lets an operator pull a newly added
    Terrabox toolkit/tool in immediately rather than waiting for the next
    tick or restarting Terraflow. Superuser-only since it affects every
    user's sidebar, not just the caller's.
    """
    if not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Only superusers can refresh the Terrabox tool catalog.")

    from lfx.interface.earthflow_components import refresh_terrabox_components_cache
    ...
    return {"terrabox_components_cached": count}
...
async def simple_run_flow(
    ...
    *,
    ...
    try:
        ...
        # See: https://github.com/terraflow-ai/terraflow/issues/11781
...
async def _run_flow_internal(
    *,
    ...
    # Extract request-level variables from headers with prefix X-TERRAFLOW-GLOBAL-VAR-*
...
async def simplified_run_flow(
    *,
    ...
    Notes:
        ...
        - Extracts global variables from HTTP headers with prefix X-TERRAFLOW-GLOBAL-VAR-*
...
# get endpoint to return version of terraflow
@router.get("/version")
...
async def custom_component(
    ...
    locale = getattr(request.state, "locale", "en")
    if locale != "en":
        from terraflow.utils.i18n import translate_component_node
