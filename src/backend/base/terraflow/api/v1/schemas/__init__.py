...
from terraflow.schema.schema import InputType, OutputType, OutputValue
from terraflow.serialization.serialization import get_max_items_length, get_max_text_length, serialize
from terraflow.services.database.models.api_key.model import ApiKeyRead
from terraflow.services.database.models.base import orjson_dumps
from terraflow.services.database.models.flow.model import FlowCreate, FlowRead
...
class SimplifiedAPIRequest(BaseModel):
    ...
    user_id: str | None = Field(
        ...
        description=(
            ...
            "API key owner remains the effective Terraflow user."
        ),
    )
...
class BaseConfigResponse(BaseModel):
    ...
    # but ``lfx extension dev`` and ``--env-file TERRAFLOW_ENABLE_EXTENSION_RELOAD=true``
    # opt in after the build is frozen, so the UI consults this field too.
    enable_extension_reload: bool
    # Mirrors ``TERRAFLOW_AUTHZ_ENABLED``. EE/custom frontends gate the Access
