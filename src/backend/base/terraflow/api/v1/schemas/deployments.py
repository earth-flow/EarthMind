...
* **Terraflow-managed (UUID)** -- ``id``, ``provider_id``, ``project_id``,
  ``deployment_id``. These reference rows in the Terraflow database.
...
* **Provider-owned (str)** -- ``reference_id``, ``config_id``,
  ...
  ``provider_key`` is Terraflow-owned adapter vocabulary.
  Provider-specific metadata (for example URL and tenant/account identifiers)
  belongs inside ``provider_data``.

* **Provider-originated but Terraflow-owned once persisted** -- ``resource_key``.
  Terraflow stores and indexes this as part of its own deployment record.

``provider_data`` dicts are opaque pass-through containers whose contents
are defined by the provider adapter. Terraflow forwards them without
...
from terraflow.services.database.models.deployment_provider_account.schemas import (
    DeploymentProviderKey,
)
from terraflow.services.database.models.deployment_provider_account.utils import (
...
class DeploymentProviderAccountGetResponse(BaseModel):
    id: UUID = Field(description="Terraflow DB provider-account UUID (`deployment_provider_account.id`).")
    name: str = Field(description="User-chosen display name for this provider account.")
    provider_key: DeploymentProviderKey = Field(description="Official provider name used by Terraflow.")
    provider_data: dict[str, Any] | None = Field(
        default=None,
        description=(
    ...
    created_at: datetime | None = Field(default=None, description="Terraflow DB row creation timestamp.")
    updated_at: datetime | None = Field(default=None, description="Terraflow DB row update timestamp.")
...
class _DeploymentResponseCommon(BaseModel):
    ...
    id: UUID = Field(description="Terraflow DB deployment UUID.")
    provider_id: UUID = Field(description="Terraflow DB provider-account UUID (`deployment_provider_account.id`).")
...
class DeploymentGetResponse(_DeploymentResponseWithProviderData):
    ...
    resource_key: str = Field(description="Terraflow-persisted stable provider resource identifier.")
...
class DeploymentFlowVersionListItem(BaseModel):
    ...
    **Identity model:** Terraflow tracks provider tools by their immutable
    ...
    * **Tool renamed in provider** — Same ``provider_snapshot_id``, different
      ``provider_data.tool_display_name``. Terraflow picks up the new label on the next fetch.
    ...
    * **Tool deleted + new tool created with same name** — The new tool has
      a different ID.  Terraflow's attachment still points to the old
      (missing) ID.  The new tool is invisible to Terraflow until explicitly
    ...
    id: UUID = Field(description="Terraflow flow version UUID (`flow_version.id`).")
    flow_id: UUID = Field(description="Terraflow flow UUID (`flow.id`) for this version.")
...
class DeploymentCreateRequest(BaseModel):
    ...
    project_id: UUID | None = Field(
        default=None,
        description="Terraflow DB project id to persist the deployment under. Defaults to user's Starter Project.",
    )
    provider_data: dict[str, Any] | None = Field(
        default=None,
...
class _RunResponseBase(BaseModel):
    ...
    Only Terraflow-owned identifiers live at the top level.  All
    provider-owned data (including the provider's ``id``)
    is returned inside ``provider_data`` so that ownership boundaries
    stay clear and a future Terraflow-managed run id won't
    collide with provider terminology.
    """

    deployment_id: UUID = Field(description="Terraflow DB deployment UUID.")
    provider_data: dict[str, Any] | None = Field(
        default=None,
        description=(
...
class SnapshotUpdateRequest(BaseModel):
    ...
    flow_version_id: UUID = Field(
        description="Terraflow flow version whose artifact will replace the snapshot content.",
    )
