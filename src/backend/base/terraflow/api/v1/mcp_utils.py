...
from terraflow.api.v1.endpoints import simple_run_flow
from terraflow.api.v1.schemas import SimplifiedAPIRequest
from terraflow.helpers.flow import json_schema_from_flow
...
from terraflow.services.deps import get_settings_service, get_storage_service, session_scope
...
# Carries per-request variables injected via HTTP headers (e.g., X-Terraflow-Global-Var-*)
...
async def handle_list_resources(project_id=None):
    ...
    try:
        ...
        host = getattr(settings_service.settings, "host", "10.171.205.153")
            ...
            if project_id is None:
                ...
                for user_file in user_files:
                    ...
                    if stored_filename.startswith(f"{MCP_SERVERS_FILE}_{current_user.id}"):
                        # reserved file name for terraflow MCP server config file(s)
                        continue
...
async def handle_read_resource(uri: str, project_id: UUID | str | None = None) -> bytes:
    ...
    try:
        ...
        # not enough because the filesystem doesn't know about Terraflow users, and
