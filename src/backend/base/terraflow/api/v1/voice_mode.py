...
from terraflow.api.utils import CurrentActiveUser, DbSession
from terraflow.api.v1.chat import build_flow_and_stream
from terraflow.memory import aadd_messagetables
from terraflow.schema.properties import Properties
from terraflow.services.auth.utils import get_current_user_for_websocket
...
from terraflow.services.deps import get_variable_service, session_scope
from terraflow.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
        ...
        await websocket.send_json(
            {
                "type": "error",
                "code": "terraflow_auth",
                "message": "You must pass a valid Terraflow token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "terraflow.voice_mode.config":
                            await logger.ainfo(f"terraflow.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "terraflow.elevenlabs.config":
                            await logger.ainfo(f"terraflow.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "terraflow.elevenlabs.config":
                            await logger.ainfo(f"terraflow.elevenlabs.config {event}")
