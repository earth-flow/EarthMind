...
class TerraflowUvicornWorker(UvicornWorker):
...
class TerraflowApplication(BaseApplication):
    def __init__(self, app_factory: Callable[[], Any], options=None) -> None:
        self.options = options or {}

        self.options["worker_class"] = "terraflow.server.TerraflowUvicornWorker"
    ...
    def pre_fork(cls, server, _worker):
        ...
        debug_mode = os.environ.get("TERRAFLOW_DEBUG_FORK_GHOSTS", "").lower() in ("1", "true", "yes")
    ...
    def load(self):
        if self.application is None:
            ...
            if self.cfg.preload_app:
                from terraflow.preload import preload_master
