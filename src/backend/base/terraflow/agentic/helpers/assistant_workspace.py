"""Resolve the FileSystem sandbox root for the Terraflow Assistant.

The shipped TerraflowAssistant flow embeds a FileSystemTool whose `root_path`
...
that PR's contract:
    ...
    Env var:  TERRAFLOW_FS_TOOL_BASE_DIR  (same name PR #13031 uses)
    Default:  ~/.terraflow/fs_tool/fs_sandbox  (same default PR #13031 uses)
...
Resolution order:
    ...
    2. TERRAFLOW_FS_TOOL_BASE_DIR env var (after expanduser + strip), if set.
    3. ~/.terraflow/fs_tool/fs_sandbox.
...
DEFAULT_BASE_SUBPATH = Path(".terraflow") / "fs_tool" / "fs_sandbox"
