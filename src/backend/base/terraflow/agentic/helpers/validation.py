...
from terraflow.agentic.api.schemas import ValidationResult
...
class _ReturnChecker(ast.NodeVisitor):
    ...
    # Terraflow components commonly use async output methods
    ...
    try:
        ...
        if _RESERVED_OUTPUT_NAME in output_names:
            return ValidationResult(
                ...
                code=code,
                error=(
                    f"Output name {_RESERVED_OUTPUT_NAME!r} is reserved by Terraflow for the "
        ...
        if _RESERVED_OUTPUT_METHOD in output_methods:
            return ValidationResult(
                ...
                code=code,
                error=(
                    f"Output method {_RESERVED_OUTPUT_METHOD!r} is reserved by Terraflow for "
