...
from terraflow.api.utils import cascade_delete_flow
from terraflow.api.v1.projects import delete_project
...
async def test_delete_project_raises_guard_error_from_app_level_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "terraflow.api.v1.projects.get_settings_service",
        lambda: SimpleNamespace(settings=SimpleNamespace(add_projects_to_mcp_servers=False)),
    )
    monkeypatch.setattr("terraflow.api.v1.projects.cleanup_mcp_on_delete", AsyncMock())
    monkeypatch.setattr("terraflow.api.v1.mappers.deployments.sync.sync_project_deployments", AsyncMock())
...
async def test_delete_project_remaps_flow_guard_to_project_guard(monkeypatch):
    ...
    monkeypatch.setattr("terraflow.api.v1.mappers.deployments.sync.sync_project_deployments", AsyncMock())
    monkeypatch.setattr(
        "terraflow.api.v1.projects.cascade_delete_flow",
        AsyncMock(
...
async def test_delete_flow_remaps_guard_error_to_flow_delete_message(monkeypatch):
    from terraflow.api.v1.flows import delete_flow

    flow_id = uuid4()
    user_id = uuid4()

    fake_flow = SimpleNamespace(id=flow_id, user_id=user_id)
    monkeypatch.setattr("terraflow.api.v1.flows._read_flow", AsyncMock(return_value=fake_flow))
    monkeypatch.setattr(
        "terraflow.api.v1.flows.retry_flow_operation_on_deployment_guard",
        AsyncMock(
            side_effect=DeploymentGuardError(
                code="FLOW_HAS_DEPLOYED_VERSIONS",
                technical_detail=(
...
async def test_update_flow_translates_guard_error_from_flush(monkeypatch):
    ...
    from terraflow.api.v1.flows import update_flow
    from terraflow.services.database.models.flow.model import FlowUpdate
    ...
    monkeypatch.setattr("terraflow.api.v1.flows._read_flow", AsyncMock(return_value=fake_flow))
    monkeypatch.setattr(
        "terraflow.api.v1.flows._patch_flow",
        AsyncMock(
            side_effect=DeploymentGuardError(
    ...
    monkeypatch.setattr("terraflow.api.v1.mappers.deployments.sync.sync_flow_deployment_state", AsyncMock())
    monkeypatch.setattr(
        "terraflow.api.v1.flows.get_settings_service",
...
async def test_delete_multiple_flows_propagates_guard_error(monkeypatch):
    ...
    from terraflow.api.v1.flows import delete_multiple_flows
    ...
    monkeypatch.setattr(
        "terraflow.api.v1.flows.cascade_delete_flow",
        AsyncMock(
            side_effect=DeploymentGuardError(
                code="FLOW_HAS_DEPLOYED_VERSIONS",
                technical_detail=(
