...
# late — see terraflow_launcher.py for the same pattern).
#
# The `terraflow` console script routes through terraflow_launcher.py which
# handles this. This guard catches the bypass paths (`python -m terraflow`,
# `uv run python -m terraflow`, etc.) so they're not silent footguns. Only
# fires for direct CLI invocation; ordinary `import terraflow.__main__` is
# unaffected.
if __name__ == "__main__":
    ...
    if _platform.system() == "Darwin" and not _os.environ.get("OBJC_DISABLE_INITIALIZE_FORK_SAFETY"):
        ...
        _os.execv(_sys.executable, [_sys.executable, "-m", "terraflow.__main__", *_sys.argv[1:]])  # noqa: S606
...
from terraflow.cli.progress import create_terraflow_progress
from terraflow.initial_setup.setup import get_or_create_default_folder
from terraflow.main import setup_app
from terraflow.services.auth.utils import get_current_user_from_access_token
from terraflow.services.database.models.api_key.crud import check_key
from terraflow.services.database.service import UnsupportedPostgreSQLVersionError, check_postgresql_version_sync
from terraflow.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
from terraflow.services.utils import initialize_services
from terraflow.utils.version import fetch_latest_version, get_version_info
from terraflow.utils.version import is_pre_release as terraflow_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="Terraflow Executor commands")
...
# Platforms where `terraflow run` bypasses Gunicorn and runs uvicorn directly
...
def ensure_multi_worker_safe(num_workers: int) -> None:
    ...
    Skipped entirely when ``TERRAFLOW_JOB_QUEUE_TYPE=redis`` is configured —
    ...
    msg = (
        ...
        "  * Configure a shared job queue: TERRAFLOW_JOB_QUEUE_TYPE=redis. Works "
...
def run(
    *,
    ...
    log_level = (log_level or os.environ.get("TERRAFLOW_LOG_LEVEL") or "info").lower()
    os.environ["TERRAFLOW_LOG_LEVEL"] = log_level
    ...
    progress = create_terraflow_progress(verbose=verbose)

    # Step 0: Initializing Terraflow
    ...
    with progress.step(1):
        for key, value in os.environ.items():
            new_key = key.replace("TERRAFLOW_", "")
    ...
    # Step 6: Launching Terraflow
    if use_direct_uvicorn():
        # TERRAFLOW_GUNICORN_PRELOAD is a Gunicorn-only knob: it triggers fork-safe
        ...
        if os.environ.get("TERRAFLOW_GUNICORN_PRELOAD", "false").lower() == "true":
            logger.warning(
                "TERRAFLOW_GUNICORN_PRELOAD=true is ignored on %s: this platform "
        ...
        from terraflow.helpers.windows_postgres_helper import TERRAFLOW_DATABASE_URL, POSTGRESQL_PREFIXES

        db_url = os.environ.get(TERRAFLOW_DATABASE_URL, "")
        loop_type = "asyncio"
        if (
        ...
        with progress.step(6):
            # Use Gunicorn with TerraflowUvicornWorker for non-Windows systems
            from terraflow.server import TerraflowApplication
            ...
            options = {
                ...
                "preload_app": os.environ.get("TERRAFLOW_GUNICORN_PRELOAD", "false").lower() == "true",
            }
            server = TerraflowApplication(app_factory, options)
...
def is_port_in_use(port, host="10.171.205.153"):
...
def get_best_access_host(host: str, port: int) -> str:
    ...
    if host != "10.171.205.153" and can_connect("10.171.205.153", port):
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "terraflow")
        'A new version of terraflow is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=terraflow_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if terraflow_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use terraflow.utils.version to get the version info
    version_info = get_version_info()
    terraflow_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= terraflow_is_pre_release(terraflow_version)  # Update pre-release status

    notice = build_version_notice(terraflow_version, package_name)
    ...
    info_text = (
        f"{github_icon} GitHub: Star for updates {arrow} https://github.com/terraflow-ai/terraflow\n"
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("TERRAFLOW_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Terraflow.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Terraflow {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            "* GitHub: https://github.com/terraflow-ai/terraflow\n"
            "# Discord: https://discord.com/invite/EqksyE2EX9\n\n"
            f"{telemetry_text}\n\n"
            f"[OK] Open Terraflow -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("GitHub: https://github.com/terraflow-ai/terraflow")
            logger.info("Discord: https://discord.com/invite/EqksyE2EX9")
            logger.info(f"Open Terraflow: {protocol}://{access_host}:{port}")


@app.command()
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'terraflow' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'terraflow' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="TERRAFLOW_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="TERRAFLOW_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set TERRAFLOW_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    from terraflow.services.database.models.user.crud import get_all_superusers
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set TERRAFLOW_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run terraflow api_key`")
            raise typer.Exit(1)
    ...
    async with session_scope() as session:
        from terraflow.services.deps import get_auth_service
...
# command to copy the terraflow database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'terraflow.db' and 'terraflow-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("terraflow"))
    db_path = cache_dir / "terraflow.db"
    pre_db_path = cache_dir / "terraflow-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from terraflow.services.database.models.api_key.crud import create_api_key, delete_api_key
            from terraflow.services.database.models.api_key.model import ApiKey, ApiKeyCreate
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"terraflow {version}")
        raise typer.Exit
