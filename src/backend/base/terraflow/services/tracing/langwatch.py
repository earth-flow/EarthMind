...
class LangWatchTracer(BaseTracer):
    ...
    def setup_langwatch(self) -> bool:
        ...
        try:
            ...
            if self.tracer_provider is None:
                ...
                resource = Resource.create(attributes={"service.name": "terraflow"})
    ...
    def _warn_langwatch_unavailable(cls) -> None:
        ...
        the official Terraflow Docker images. Warn only once to avoid log spam, since a tracer
        ...
        if sys.version_info >= (3, 14):
            logger.warning(
                ...
                "it is excluded from Python 3.14+ environments, including the official Terraflow "
                "Docker images. To enable LangWatch tracing, run Terraflow on Python 3.10-3.13 "
                "(for example via 'pip install terraflow')."
            )
        else:
            logger.warning(
                "LANGWATCH_API_KEY is set but the 'langwatch' package is not installed, so "
                "LangWatch tracing is disabled. Install it with 'pip install langwatch' or "
                "'pip install \"terraflow-base[langwatch]\"'."
            )
