...
from terraflow.services.cache.factory import CacheServiceFactory
...
from terraflow.services.database.utils import initialize_database
...
async def setup_superuser(settings_service: SettingsService, session: AsyncSession) -> SetupSuperuserResult:
    if settings_service.auth_settings.AUTO_LOGIN:
        ...
        lock_file = Path(gettempdir()) / "terraflow_auto_login_superuser.lock"
        ...
        try:
            with lock:
                ...
                if super_user:  # Only initialize if user was created
                    ...
                    # Recover MCP server config files orphaned when DB was reset but TERRAFLOW_CONFIG_DIR was kept.
...
async def migrate_orphaned_mcp_servers_config(
    ...
    via a ``File`` row. When Terraflow starts with a fresh database but the same
    ...
    * If multiple orphans are found, skip and log — we can't safely identify the
    ...
    from terraflow.services.database.models.file.model import File as UserFile
...
def initialize_session_service() -> None:
    ...
    from terraflow.services.cache import factory as cache_factory
    from terraflow.services.session import factory as session_service_factory
...
async def clean_authz_audit_log(settings_service: SettingsService, session: AsyncSession) -> int:
    ...
    from terraflow.services.database.models.auth import AuthzAuditLog
...
def register_all_service_factories() -> None:
    ...
    from terraflow.services.auth import factory as auth_factory
    from terraflow.services.auth.service import AuthService
    from terraflow.services.authorization import factory as authorization_factory
    from terraflow.services.authorization.service import TerraflowAuthorizationService
    from terraflow.services.cache import factory as cache_factory
    from terraflow.services.chat import factory as chat_factory
    from terraflow.services.database import factory as database_factory
    from terraflow.services.job_queue import factory as job_queue_factory
    from terraflow.services.session import factory as session_factory
    from terraflow.services.shared_component_cache import factory as shared_component_cache_factory
    from terraflow.services.state import factory as state_factory
    from terraflow.services.storage import factory as storage_factory
    from terraflow.services.store import factory as store_factory
    from terraflow.services.task import factory as task_factory
    from terraflow.services.telemetry import factory as telemetry_factory
    from terraflow.services.telemetry_writer import factory as telemetry_writer_factory
    from terraflow.services.tracing import factory as tracing_factory
    from terraflow.services.transaction import factory as transaction_factory
    from terraflow.services.variable import factory as variable_factory
    ...
    # Override LFX's no-op auth service with Terraflow's full JWT implementation
    ...
    # ``override=True`` so Terraflow always has a default. A registered
    # authorization plugin replaces it by listing its class in
    # ``TERRAFLOW_CONFIG_DIR/lfx.toml`` (config files use ``override=True`` via
    ...
    service_manager.register_service_class(
        ServiceType.AUTHORIZATION_SERVICE, TerraflowAuthorizationService, override=True
    )
...
def register_builtin_adapters() -> None:
    ...
    try:
        import_module("terraflow.services.adapters.deployment.watsonx_orchestrate.register")
...
def register_builtin_deployment_mappers() -> None:
    ...
    try:
        import_module("terraflow.api.v1.mappers.deployments.watsonx_orchestrate")
    except ModuleNotFoundError as exc:
