...
from terraflow.services.database.models.deployment.orm_guards import ensure_deployment_immutable_fields
...
from terraflow.services.database.utils import parse_uuid
...
def _deployment_metadata_changed_fields(update_item: DeploymentMetadataUpdate) -> list[str]:
    terraflow_data = update_item.terraflow_db_row
    changed_fields: list[str] = []
    if update_item.display_name != terraflow_data.display_name:
        changed_fields.append("display_name")
    if update_item.description != terraflow_data.description:
...
def _deployment_metadata_updates_cte(
    ...
    return (
        ...
        .data(
            [
                (
                    item.terraflow_db_row.id,
                    item.terraflow_db_row.user_id,
                    item.display_name,
                    item.description,
                )
                for item in updates
