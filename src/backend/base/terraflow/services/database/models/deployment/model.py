...
from terraflow.schema.serialize import UUIDstr
from terraflow.services.database.utils import validate_non_empty_string, validate_non_empty_string_preserve_value
...
class Deployment(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # ``deployment_type_enum`` constraint is defined by a Terraflow alembic
    ...
    #                           by the DB until a Terraflow migration adds it
    #                           to the enum type.  Terraflow explicitly opts
    ...
    #   2. Create a Terraflow alembic migration that runs:
