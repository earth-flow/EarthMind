...
from terraflow.services.database.models.api_key.model import ApiKey, ApiKeyCreate, ApiKeyRead, UnmaskedApiKeyRead
...
async def check_key(session: AsyncSession, api_key: str) -> User | None:
    ...
    Validates API keys based on the TERRAFLOW_API_KEY_SOURCE setting:
...
async def _check_key_from_env(session: AsyncSession, api_key: str, settings_service) -> User | None:
    ...
    TERRAFLOW_API_KEY environment variable. If valid, returns the superuser for authorization.
    """
    from terraflow.services.database.models.user.crud import get_user_by_username

    env_api_key = os.getenv("TERRAFLOW_API_KEY")
    if not env_api_key:
        return None
