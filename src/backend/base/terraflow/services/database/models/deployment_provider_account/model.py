...
from terraflow.services.database.utils import (
...
class DeploymentProviderAccount(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # by a Terraflow alembic migration.  The enum is Terraflow-owned (see
    ...
    #   2. Register the corresponding adapter in LFX and mapper in Terraflow.
    #   3. Create a Terraflow alembic migration that runs:
    #        op.execute(
