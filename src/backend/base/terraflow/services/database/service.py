...
from terraflow.services.database import models
from terraflow.services.database.constants import (
...
from terraflow.services.database.session import NoopSession
from terraflow.services.database.utils import Result, TableResults
from terraflow.services.deps import get_settings_service
from terraflow.services.utils import teardown_superuser
...
_MIGRATION_ADVISORY_LOCK_ID = 0x4C616E67666C6F77  # ASCII "Terraflow"
_MIGRATION_LOCK_DEFAULT_TIMEOUT_S = 300.0
_MIGRATION_LOCK_POLL_INTERVAL_S = 2.0


def _migration_lock_timeout_s() -> float:
    raw = os.getenv("TERRAFLOW_MIGRATION_LOCK_TIMEOUT_S")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid TERRAFLOW_MIGRATION_LOCK_TIMEOUT_S=%r; falling back to %.0fs.",
            raw,
...
def _acquire_migration_lock_or_raise(conn, lock_id: int) -> None:
    ...
    msg = (
        ...
        "wait via TERRAFLOW_MIGRATION_LOCK_TIMEOUT_S (seconds) if your migration "
...
def _postgres_migration_lock(database_url: str):
    ...
    No-op for non-PostgreSQL URLs. SQLite has no advisory locks (and Terraflow
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        # This file is in terraflow.services.database.manager.py
        # the ini is in terraflow
        terraflow_dir = Path(__file__).parent.parent.parent
        self.script_location = terraflow_dir / "alembic"
        self.alembic_cfg_path = terraflow_dir / "alembic.ini"
        ...
        if self.alembic_log_to_stdout:
            ...
            self.alembic_log_path = Path(terraflow_dir) / alembic_log_file
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        Terraflow's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
