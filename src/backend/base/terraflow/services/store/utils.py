...
if TYPE_CHECKING:
    from terraflow.services.store.schema import ListComponentResponse
...
# Get the latest released version of terraflow (https://pypi.org/project/terraflow/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/terraflow/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.debug("Error getting the latest version of terraflow from PyPI", exc_info=True)
        return None
