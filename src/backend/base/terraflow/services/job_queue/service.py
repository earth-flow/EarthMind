...
_STREAM_PREFIX = "terraflow:queue:"
_OWNER_PREFIX = "terraflow:owner:"
...
_CANCEL_CHANNEL_PREFIX = "terraflow:cancel:"
...
_ACTIVITY_PREFIX = "terraflow:activity:"
...
class RedisQueueWrapper:
    ...
    * End-of-stream →  ``XADD key * event_id __sentinel__ data __sentinel__ ts <float>``
    ...
    # value can be driven from settings (TERRAFLOW_REDIS_QUEUE_STARTUP_GRACE_S).
...
class RedisJobQueueService(JobQueueService):
    ...
    Configuration
    -------------
    Set ``TERRAFLOW_JOB_QUEUE_TYPE=redis`` and, optionally:

    * ``TERRAFLOW_REDIS_QUEUE_DB`` (default ``1``, separate from cache DB ``0``)
    * ``TERRAFLOW_REDIS_QUEUE_URL`` (full URL, overrides host/port/db)
    * ``TERRAFLOW_REDIS_QUEUE_HOST`` / ``TERRAFLOW_REDIS_QUEUE_PORT``
    ...
    ``terraflow:cancel:*``. Any worker can publish a cancel signal via
    ...
    completion. See ``terraflow.api.build.create_flow_response``.
    """
    ...
    async def _check_connection(self) -> None:
        ...
        try:
            ...
            await logger.aerror(
                ...
                "Set TERRAFLOW_JOB_QUEUE_TYPE=asyncio or start Redis before running Terraflow."
            )
    ...
    _CANCEL_MARKER_PREFIX = "terraflow:cancel-marker:"
    ...
    def get_queue_data(self, job_id: str) -> tuple[asyncio.Queue, EventManager, asyncio.Task | None, float | None]:  # type: ignore[override]
        ...
        * **Cross-worker path**: no local entry exists; a null ``EventManager`` and
          ...
          response's :func:`terraflow.api.build.create_flow_response.on_disconnect`
