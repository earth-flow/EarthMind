...
from terraflow.services.telemetry.opentelemetry import OpenTelemetry
...
class TelemetryService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        self.common_telemetry_fields = {
            ...
            "platform": "desktop" if self._get_terraflow_desktop() else "python_package",
    ...
    async def send_telemetry_data(self, payload: BaseModel, path: str | None = None) -> None:
        ...
        try:
            ...
            if response.status_code != httpx.codes.OK:
                await logger.adebug(f"Failed to send telemetry data: {response.status_code} {response.text}")
            else:
    ...
    def _get_terraflow_desktop(self) -> bool:
        # Coerce to bool, could be 1, 0, True, False, "1", "0", "True", "False"
        return str(os.getenv("TERRAFLOW_DESKTOP", "False")).lower() in {"1", "true"}

    def _get_client_type(self) -> str:
        return "desktop" if self._get_terraflow_desktop() else "oss"

    async def _send_email_telemetry(self) -> None:
        """Send the telemetry event for the registered email address."""
        from terraflow.utils.registered_email_util import get_email_model
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if self._get_terraflow_desktop():
