...
TERRAFLOW_WXO_RESOURCE_NAME_PREFIX = "terraflow_"
T = TypeVar("T")
...
def build_terraflow_wxo_resource_name(display_name: str, *, resource: str) -> str:
    normalized_display_name = normalize_wxo_display_name_segment(display_name, resource=resource)
    return f"{TERRAFLOW_WXO_RESOURCE_NAME_PREFIX}{normalized_display_name}_{uuid4().hex[:8]}"
...
class WatsonxFlowArtifactProviderData(BaseModel):
    ...
    project_id: NormalizedId = Field(description="Terraflow project id carried for watsonx snapshot creation.")
    ...
    def validate_or_generate_tool_name(self) -> WatsonxFlowArtifactProviderData:
        ...
        try:
            self.tool_name = build_terraflow_wxo_resource_name(self.tool_display_name, resource="Tool")
        except InvalidContentError as exc:
            raise ValueError(exc.message) from exc
        return self
...
class WatsonxRenameToolOperation(BaseModel):
    """Update a Terraflow-managed tool's user-facing label on the provider."""
...
class WatsonxAgentExecutionResultData(BaseModel):
    ...
    agent_id: NonEmptyString | None = Field(
        default=None,
        description="WXO agent identifier (resource_key in Terraflow DB).",
    )
