...
from ibm_watsonx_orchestrate_core.types.tools.terraflow_tool import create_terraflow_tool
...
from terraflow.services.adapters.deployment.watsonx_orchestrate.utils import (
    dedupe_list,
...
if TYPE_CHECKING:
    from ibm_watsonx_orchestrate_core.types.tools.terraflow_tool import TerraflowTool
    from lfx.services.adapters.deployment.schema import BaseFlowArtifact, SnapshotItems, SnapshotListResult

    from terraflow.services.adapters.deployment.watsonx_orchestrate.types import WxOClient
...
def ensure_terraflow_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Ensure ``binding.terraflow.connections`` exists in *tool_payload* and return the mutable dict.
    ...
    terraflow = _ensure_dict(binding, "terraflow")
    return _ensure_dict(terraflow, "connections")


def verify_terraflow_owned(tool: dict[str, Any], *, tool_id: str) -> None:
    """Raise ``InvalidContentError`` if the tool lacks ``binding.terraflow``.

    Call before any mutating operation on an existing tool to ensure
    Terraflow created it.  Tools created manually in the wxO console or
    by other integrations will not have this marker.
    """
    binding = tool.get("binding")
    if not isinstance(binding, dict) or "terraflow" not in binding:
        msg = f"Cannot modify tool '{tool_id}': it does not have a Terraflow binding and may not be managed by Terraflow."
        raise InvalidContentError(message=msg)


def extract_terraflow_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Extract ``binding.terraflow.connections`` from a provider tool payload.
    ...
    if not isinstance(binding, dict):
        return {}
    terraflow = binding.get("terraflow")
    if not isinstance(terraflow, dict):
        return {}
    connections = terraflow.get("connections")
...
def extract_terraflow_artifact_from_zip(artifact_zip_bytes: bytes, *, snapshot_id: str) -> dict[str, Any]:
    """Read and parse the Terraflow flow JSON from a wxO snapshot artifact zip."""
    try:
...
def build_terraflow_artifact_bytes(
    *,
    tool: TerraflowTool,
    ...
    logger.debug("build_terraflow_artifact_bytes: filename='%s', requirements=%s", filename, requirements)
    ...
    *,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running Terraflow version for snapshot creation."
            raise InvalidContentError(message=msg)
        flow_definition["last_tested_version"] = detected_version

    tool: TerraflowTool = create_terraflow_tool(
    ...
    (tool_payload.setdefault("binding", {}).setdefault("terraflow", {})["project_id"]) = project_id
    logger.debug(
        "create_wxo_flow_tool_payload",
        tool_name=tool_payload["name"],
        tool_display_name=tool_payload["display_name"],
        project_id=project_id,
        binding=tool_payload.get("binding", {}).get("terraflow"),
    )

    artifacts: bytes = build_terraflow_artifact_bytes(
        tool=tool,
...
async def process_raw_flows_with_app_id(
    ...
    """Create terraflow tools in wxO and connect them to the given app_id."""
...
async def verify_tools_by_ids(
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_terraflow_connections_binding(tool)
