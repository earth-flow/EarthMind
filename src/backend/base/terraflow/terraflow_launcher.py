...
def main():
    """Launches terraflow with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from terraflow.__main__ import main as terraflow_main

        terraflow_main()


def _launch_with_exec():
    """Launch terraflow by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "terraflow.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec terraflow: {e}", file=sys.stderr)
        sys.exit(1)
