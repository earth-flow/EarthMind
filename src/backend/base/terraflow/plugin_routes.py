...
Plugins register via the ``terraflow.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing Terraflow routes.
"""
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing Terraflow routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``terraflow.plugins`` entry-point group.
    ...
    raw_eps = list(entry_points(group="terraflow.plugins"))
