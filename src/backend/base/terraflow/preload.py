"""Gunicorn master preload hook for Terraflow.

When ``TERRAFLOW_GUNICORN_PRELOAD=true`` (which flips gunicorn's
``preload_app`` option), this module runs the fork-safe parts of
Terraflow's startup in the master process so that worker processes
...
async def _run_master_preload() -> None:
    ...
    from terraflow.main import load_bundles_with_error_handling
    ...
    try:
        ...
        if settings_service.settings.agentic_experience:
            from terraflow.api.utils.mcp.agentic_mcp import (
...
def preload_master() -> None:
    """Run one-time Terraflow initialization in the gunicorn master before workers are forked.
