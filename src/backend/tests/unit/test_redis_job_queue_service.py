...
import pytest
from terraflow.api.build import (
    cancel_flow_build,
    create_flow_response,
    get_flow_events_response,
)
from terraflow.api.utils import EventDeliveryType
from terraflow.events.event_manager import EventManager
from terraflow.services.job_queue.service import (
...
async def test_redis_queue_wrapper_reads_events():
    ...
    stream_key = f"terraflow:queue:{job_id}"
...
async def test_redis_service_cleanup_deletes_redis_keys():
    ...
    try:
        ...
        stream_key = f"terraflow:queue:{job_id}"
        owner_key = f"terraflow:owner:{job_id}"
...
async def test_redis_service_owner_stored_in_redis():
    ...
    try:
        ...
        raw = await fake_client.get(f"terraflow:owner:{job_id}")
...
async def test_redis_service_owner_cleaned_up_after_cleanup_job():
    ...
    try:
        ...
        assert not await fake_client.exists(f"terraflow:owner:{job_id}")
    finally:
        await _stop_service(service)
...
async def test_streaming_heartbeat_runs_independent_of_event_yield(monkeypatch):
    ...
    """
    from terraflow.api import build as build_module
...
async def test_metrics_snapshot_for_in_memory_queue():
    ...
    from terraflow.services.job_queue.service import JobQueueService
...
async def test_get_flow_events_response_rejects_unknown_event_delivery():
    ...
    try:
        ...
        assert "TERRAFLOW_EVENT_DELIVERY" in detail
...
async def test_task_service_launch_does_not_trigger_polling_watchdog(monkeypatch):
    ...
    monkeypatch.setattr("terraflow.services.task.service.get_queue_service", lambda: svc)
...
async def test_generate_flow_events_calls_end_all_traces_on_cancel(monkeypatch):
    ...
    from terraflow.api.build import generate_flow_events
    from terraflow.events.event_manager import EventManager
    ...
    monkeypatch.setattr("terraflow.api.build.get_chat_service", lambda: mock_chat)
    monkeypatch.setattr("terraflow.api.build.get_telemetry_service", lambda: mock_telemetry)
    monkeypatch.setattr("terraflow.api.build.session_scope", _fake_session_scope)
    monkeypatch.setattr(
        "terraflow.api.build.build_graph_from_db",
        AsyncMock(return_value=mock_graph),
    )
    monkeypatch.setattr("terraflow.api.build.get_job_service", lambda: mock_job_svc)
    monkeypatch.setattr("terraflow.api.build.get_task_service", lambda: MagicMock())
    monkeypatch.setattr("terraflow.api.build.get_memory_base_service", lambda: MagicMock())
    monkeypatch.setattr("terraflow.api.build.get_top_level_vertices", lambda *_: [])
