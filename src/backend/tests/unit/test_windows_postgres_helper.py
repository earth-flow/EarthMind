...
import pytest
from terraflow.helpers.windows_postgres_helper import (
    TERRAFLOW_DATABASE_URL,
    POSTGRESQL_PREFIXES,
...
class TestWindowsPostgresHelper:
    ...
    def test_constants_defined(self):
        ...
        assert TERRAFLOW_DATABASE_URL == "TERRAFLOW_DATABASE_URL"
    ...
    def test_non_windows_returns_false(self, mock_platform):
        ...
        for system in ["Linux", "Darwin", "FreeBSD"]:
            ...
            os.environ[TERRAFLOW_DATABASE_URL] = "postgresql://user:pass@localhost/db"
    ...
    @patch("platform.system")
    @patch.dict(os.environ, {"TERRAFLOW_DATABASE_URL": "sqlite:///test.db"}, clear=True)
    ...
    @patch("platform.system")
    @patch.dict(os.environ, {"TERRAFLOW_DATABASE_URL": "postgresql://user:pass@localhost/db"}, clear=True)
    ...
    @patch.object(asyncio, "WindowsSelectorEventLoopPolicy", MockWindowsSelectorPolicy, create=True)
    @patch("platform.system")
    @patch.dict(os.environ, {"TERRAFLOW_DATABASE_URL": "postgres://user:pass@localhost/db"}, clear=True)
    @patch("asyncio.get_event_loop_policy")
    @patch("asyncio.set_event_loop_policy")
    ...
    @patch("asyncio.get_event_loop_policy")
    @patch("asyncio.set_event_loop_policy")
    @patch("terraflow.helpers.windows_postgres_helper.logger")
    ...
    @patch("platform.system")
    @patch.dict(os.environ, {"TERRAFLOW_DATABASE_URL": "mysql://user:pass@localhost/db"}, clear=True)
    ...
    def test_docker_environment_not_affected(self, mock_platform):
        ...
        os.environ[TERRAFLOW_DATABASE_URL] = "postgresql://user:pass@postgres:5432/terraflow"
    ...
    def test_various_postgresql_urls_handled(self, mock_set_policy, mock_get_policy, mock_platform, db_url):
        ...
        with patch.dict(os.environ, {TERRAFLOW_DATABASE_URL: db_url}, clear=True):
            result = configure_windows_postgres_event_loop()
