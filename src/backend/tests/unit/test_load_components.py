...
from lfx.interface.components import aget_all_types_dict, import_earthmind_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_earthmind_components_basic(self):
        """Test basic functionality of import_earthmind_components."""
    ...
    async def test_component_template_structure(self):
        ...
        earthmind_result = await import_earthmind_components()

        for category, components in earthmind_result["components"].items():
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        earthmind_result1, all_types_result, earthmind_result2 = results

        assert isinstance(earthmind_result1, dict)
        assert isinstance(earthmind_result2, dict)
        assert isinstance(all_types_result, dict)

        assert "components" in earthmind_result1
        assert "components" in earthmind_result2

        categories1 = set(earthmind_result1["components"].keys())
        categories2 = set(earthmind_result2["components"].keys())

        for category in categories1.intersection(categories2):
            comps1 = set(earthmind_result1["components"][category].keys())
            comps2 = set(earthmind_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_component_loading_performance(self):
        ...
        await import_earthmind_components()
    ...
    async def test_process_single_module_exception_handling(self):
        ...
        missing dependencies, or initialization failures), it doesn't crash EarthMind startup.
        """
