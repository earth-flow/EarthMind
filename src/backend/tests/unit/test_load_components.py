...
from lfx.interface.components import aget_all_types_dict, import_terraflow_components
...
class TestComponentLoading:
    ...
    @pytest.mark.asyncio
    async def test_import_terraflow_components_basic(self):
        """Test basic functionality of import_terraflow_components."""
    ...
    async def test_component_template_structure(self):
        ...
        terraflow_result = await import_terraflow_components()

        for category, components in terraflow_result["components"].items():
    ...
    async def test_concurrent_loading(self, base_components_path):
        ...
        terraflow_result1, all_types_result, terraflow_result2 = results

        assert isinstance(terraflow_result1, dict)
        assert isinstance(terraflow_result2, dict)
        assert isinstance(all_types_result, dict)

        assert "components" in terraflow_result1
        assert "components" in terraflow_result2

        categories1 = set(terraflow_result1["components"].keys())
        categories2 = set(terraflow_result2["components"].keys())

        for category in categories1.intersection(categories2):
            comps1 = set(terraflow_result1["components"][category].keys())
            comps2 = set(terraflow_result2["components"][category].keys())
            if comps1 != comps2:
    ...
    async def test_component_loading_performance(self):
        ...
        await import_terraflow_components()
    ...
    async def test_process_single_module_exception_handling(self):
        ...
        missing dependencies, or initialization failures), it doesn't crash Terraflow startup.
        """
