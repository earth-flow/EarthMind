"""Reproduction tests for the F2 Terraflow Assistant API bug report (2026-06-02).
...
failing test to flip:
  ...
  Issue 3 — POST /agentic/execute/TerraflowAssistant returns HTTP 500 for a
...
async def test_execute_named_assistant_flow_is_graceful_not_500(client, logged_in_headers):
    ...
    body = {"flow_id": str(uuid4()), "input_value": "In one short sentence, what is Terraflow?"}
    response = await client.post("api/v1/agentic/execute/TerraflowAssistant", json=body, headers=logged_in_headers)
