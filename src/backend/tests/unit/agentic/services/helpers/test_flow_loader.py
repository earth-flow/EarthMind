...
from terraflow.agentic.services.helpers.flow_loader import (
...
class TestResolveFlowPath:
    ...
    def test_should_return_json_path_for_explicit_json_extension(self, tmp_path):
        ...
        with patch("terraflow.agentic.services.helpers.flow_loader.FLOWS_BASE_PATH", tmp_path):
...
class TestLoadGraphFromPython:
    ...
    async def test_should_load_graph_from_get_graph_function(self):
        ...
        with (
            ...
            patch("terraflow.agentic.services.helpers.flow_loader._temporary_sys_path"),
            patch("terraflow.agentic.services.helpers.flow_loader.validate_flow_for_current_settings") as mock_validate,
        ):
            mock_spec = MagicMock()
    ...
    async def test_should_pass_provider_params_to_get_graph(self):
        ...
        with (
            ...
            patch("importlib.util.module_from_spec") as mock_module_from_spec,
            patch("terraflow.agentic.services.helpers.flow_loader._temporary_sys_path"),
            patch("terraflow.agentic.services.helpers.flow_loader.validate_flow_for_current_settings"),
    ...
    async def test_should_raise_validation_error_for_blocked_python_graph(self):
        ...
        with (
            ...
            patch("terraflow.agentic.services.helpers.flow_loader._temporary_sys_path"),
            patch(
                "terraflow.agentic.services.helpers.flow_loader.validate_flow_for_current_settings",
...
class TestLoadGraphForExecution:
    ...
    async def test_should_use_python_loader_for_python_type(self):
        ...
        with patch(
            "terraflow.agentic.services.helpers.flow_loader._load_graph_from_python",
    ...
    async def test_should_use_json_loader_for_json_type(self):
        ...
        with (
            patch(
                "terraflow.agentic.services.helpers.flow_loader.load_and_prepare_flow",
                return_value='{"data": {"nodes": []}}',
            ) as mock_prepare,
            patch(
                "terraflow.agentic.services.helpers.flow_loader.aload_flow_from_json",
                new_callable=AsyncMock,
                return_value=mock_graph,
            ) as mock_load_json,
        ):
...
class TestBugsAndEdgeCases:
    ...
    def test_resolve_flow_path_traversal_escape(self, tmp_path):
        ...
        with patch("terraflow.agentic.services.helpers.flow_loader.FLOWS_BASE_PATH", flows_dir):
            with pytest.raises(HTTPException) as exc_info:
