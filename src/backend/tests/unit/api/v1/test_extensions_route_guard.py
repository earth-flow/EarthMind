...
allowing ``--env-file TERRAFLOW_ENABLE_EXTENSION_RELOAD=true`` to opt
...
from terraflow.api.v1.extensions import _require_extension_reload_enabled
...
def test_guard_raises_404_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "terraflow.api.v1.extensions.get_settings_service",
    ...
    assert "TERRAFLOW_ENABLE_EXTENSION_RELOAD" in detail["message"]
    assert detail["hint"]
...
def test_route_is_mounted_unconditionally() -> None:
    ...
    Regression: an earlier implementation read ``TERRAFLOW_ENABLE_EXTENSION_RELOAD``
    at import time, which meant ``--env-file`` could not turn the route on
    because ``terraflow.__main__`` imports the router before ``load_dotenv``
...
def test_no_runtime_mutation_routes() -> None:
    ...
    Fix hint:
      ...
      TERRAFLOW_ENABLE_EXTENSION_RELOAD=true.
    """
    from terraflow.api.router import router
