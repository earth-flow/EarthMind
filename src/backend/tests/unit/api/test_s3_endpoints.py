...
class TestS3FileEndpoints:
    ...
    async def test_download_file_parses_path_correctly(self, mock_storage_service, mock_settings):
        ...
        with (
            patch("terraflow.services.deps.get_storage_service", return_value=mock_storage_service),
            patch("terraflow.services.deps.get_settings_service", return_value=mock_settings),
        ):
            mock_user = MagicMock()
            ...
            with (
                patch("terraflow.api.v2.files.fetch_file_object", return_value=mock_file),
                patch("terraflow.api.v2.files.CurrentActiveUser", return_value=mock_user),
                patch("terraflow.api.v2.files.ensure_file_permission", new=AsyncMock()),
            ):
                from terraflow.api.v2.files import download_file
    ...
    async def test_delete_file_calls_storage_with_correct_params(self, mock_storage_service, mock_settings):
        ...
        with (
            ...
            with (
                ...
                from terraflow.api.v2.files import delete_file
    ...
    async def test_upload_saves_to_storage_service(self, mock_storage_service, mock_settings):
        ...
        with (
            ...
            with patch("terraflow.api.v2.files.upload_user_file"):
                from terraflow.api.v2.files import save_file_routine
