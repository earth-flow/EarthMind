...
from terraflow.api.v2.files import (
    delete_all_files,
    delete_file,
    delete_files_batch,
    is_permanent_storage_failure,
)
from terraflow.api.v2.mcp import get_mcp_file
...
def max_file_size_upload_fixture(monkeypatch):
    monkeypatch.setenv("TERRAFLOW_MAX_FILE_SIZE_UPLOAD", "1")
    yield
    monkeypatch.undo()
...
def max_file_size_upload_10mb_fixture(monkeypatch):
    monkeypatch.setenv("TERRAFLOW_MAX_FILE_SIZE_UPLOAD", "10")
    yield
    monkeypatch.undo()
...
async def test_batch_download_files_non_ascii_content_disposition(files_client, files_created_api_key):
    ...
    assert "filename*=UTF-8''" in content_disposition
    assert re.search(r"\d{8}_\d{6}_terraflow_files\.zip", content_disposition)
...
async def s3_files_client_fixture(
    ...
    if "noclient" in request.keywords:
        ...
        def init_app():
            ...
            monkeypatch.setenv("TERRAFLOW_STORAGE_TYPE", "s3")
            monkeypatch.setenv(
                "TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME",
                os.environ.get("TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME", "terraflow-ci"),
            )
            # Use unique prefix per test run to avoid conflicts
            test_prefix = f"test-files-api-{uuid.uuid4().hex[:8]}"
            monkeypatch.setenv("TERRAFLOW_OBJECT_STORAGE_PREFIX", test_prefix)
            tags_json = json.dumps({"env": "test-api", "type": "file-upload"})
            monkeypatch.setenv("TERRAFLOW_OBJECT_STORAGE_TAGS", tags_json)

            from terraflow.services.manager import service_manager
        ...
        try:
            ...
            s3 = boto3.client("s3")
            bucket_name = os.environ.get("TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME", "terraflow-ci")
...
class TestS3FileOperations:
    ...
    async def test_s3_delete_verifies_s3_cleanup(self, s3_files_client, s3_files_created_api_key):
        ...
        test_prefix = os.environ.get("TERRAFLOW_OBJECT_STORAGE_PREFIX")
