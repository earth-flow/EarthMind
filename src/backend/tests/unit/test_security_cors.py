...
class TestCORSConfiguration:
    ...
    def test_default_cors_settings_current_behavior(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch.dict(os.environ, {"TERRAFLOW_CONFIG_DIR": temp_dir}):
            settings = Settings()
            ...
            warnings.warn(
                ...
                "to your Terraflow instance and potentially steal user credentials. "
                "This will be changed to more secure defaults in v1.7. "
                "Please configure TERRAFLOW_CORS_ORIGINS with specific domains for production use.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def test_cors_origins_string_to_list_conversion(self):
        ...
        with (
            ...
            patch.dict(
                os.environ,
                {
                    "TERRAFLOW_CONFIG_DIR": temp_dir,
                    "TERRAFLOW_CORS_ORIGINS": "https://app1.example.com,https://app2.example.com",
                },
            ),
        ):
            settings = Settings()
    ...
    def test_single_origin_converted_to_list(self):
        ...
        with (
            ...
            patch.dict(
                ...
                {
                    "TERRAFLOW_CONFIG_DIR": temp_dir,
                    "TERRAFLOW_CORS_ORIGINS": "https://app.example.com",
                },
            ),
        ):
            settings = Settings()
    ...
    def test_wildcard_with_credentials_allowed_current_behavior(self):
        ...
        with (
            ...
            settings = Settings()
            # pydantic-settings parses TERRAFLOW_CORS_ORIGINS="*" as the raw string
    ...
    @patch("terraflow.main.add_sentry_middleware")  # Mock Sentry setup
    @patch("terraflow.main.get_settings_service")
...
class TestRefreshTokenSecurity:
    ...
    async def test_refresh_token_type_validation(self):
        ...
        with patch("terraflow.services.auth.utils.jwt.decode") as mock_decode:
            ...
            with patch("terraflow.services.auth.utils.get_settings_service") as mock_settings:
    ...
    async def test_refresh_token_user_active_check(self):
        ...
        with patch("terraflow.services.auth.utils.jwt.decode") as mock_decode:
            ...
            with patch("terraflow.services.auth.utils.get_settings_service") as mock_settings:
                ...
                with patch("terraflow.services.auth.utils.get_user_by_id") as mock_get_user:
    ...
    async def test_refresh_token_valid_flow(self):
        ...
        from terraflow.services.auth.utils import create_refresh_token
        ...
        # Create a terraflow AuthService instance (not lfx) with mocked settings
        mock_settings_service = MagicMock()
        auth_service = AuthService(mock_settings_service)

        with (
            patch("terraflow.services.auth.utils._auth_service", return_value=auth_service),
            patch("terraflow.services.auth.service.jwt.decode") as mock_decode,
        ):
            mock_decode.return_value = {"sub": str(user_id), "type": "refresh"}  # Correct type

            with patch("terraflow.services.auth.utils.get_jwt_verification_key") as mock_verification_key:
                mock_verification_key.return_value = "secret"

                with patch("terraflow.services.auth.service.get_user_by_id", new_callable=AsyncMock) as mock_get_user:
                    mock_get_user.return_value = mock_user
    ...
    def test_refresh_token_samesite_setting_future_secure(self):
        ...
        # from terraflow.services.settings.auth import AuthSettings
        # with tempfile.TemporaryDirectory() as temp_dir, patch.dict(os.environ, {"TERRAFLOW_CONFIG_DIR": temp_dir}):
        #     auth_settings = AuthSettings(CONFIG_DIR=temp_dir)
...
class TestCORSIntegration:
    ...
    async def test_cors_headers_in_response_current_behavior(self, mock_add_sentry_middleware):
        ...
        with patch("terraflow.main.get_settings_service") as mock_get_settings:
            mock_settings = MagicMock()
            mock_settings.settings.cors_origins = ["https://app.example.com"]
            mock_settings.settings.cors_allow_credentials = True
            mock_settings.settings.cors_allow_methods = "*"
