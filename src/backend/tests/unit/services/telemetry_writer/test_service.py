...
we don't have to bring up the entire terraflow service manager — instead each
...
import pytest
from terraflow.services.database.models.transactions.model import TransactionBase, TransactionTable
from terraflow.services.database.models.vertex_builds.model import VertexBuildBase, VertexBuildTable
from terraflow.services.telemetry_writer.service import (
...
def _find_dead_pid(start: int = 99_999, max_checks: int = 50_000) -> int:
    from terraflow.services.telemetry_writer.service import _pid_alive
...
async def test_teardown_spills_remaining_buffer(tmp_path: Path) -> None:
    ...
    # going through start() (which needs the full terraflow service stack).
    writer._started = True
    writer._shutdown_event = asyncio.Event()
    writer._shutdown_event.set()
    ...
    fallback = Path(tempfile.gettempdir()) / "terraflow_telemetry_outbox"
