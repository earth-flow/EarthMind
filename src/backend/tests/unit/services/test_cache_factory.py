...
def test_cache_type_disk_is_rejected(monkeypatch):
    """EARTHMIND_CACHE_TYPE=disk must fail validation now that the disk backend is gone."""
    monkeypatch.setenv("EARTHMIND_CACHE_TYPE", "disk")
...
def test_supported_cache_types_validate(monkeypatch, cache_type):
    monkeypatch.setenv("EARTHMIND_CACHE_TYPE", cache_type)
    settings = Settings()
