...
from terraflow.services.auth.exceptions import (
    InactiveUserError,
    InvalidTokenError,
...
async def test_get_current_user_from_access_token_returns_active_user(auth_service: AuthService):
    ...
    with patch("terraflow.services.auth.service.get_user_by_id", new=AsyncMock(return_value=fake_user)) as mock_get_user:
...
async def test_get_current_user_from_access_token_requires_active_user(auth_service: AuthService):
    ...
    with (
        patch("terraflow.services.auth.service.get_user_by_id", new=AsyncMock(return_value=inactive_user)),
...
async def test_authenticate_with_credentials_auto_login_skip_returns_superuser(
    ...
    with (
        patch(
            "terraflow.services.auth.service.get_user_by_username",
            new=AsyncMock(return_value=superuser),
        ) as mock_lookup,
        patch("terraflow.services.auth.service.logger") as mock_logger,
    ):
...
async def test_authenticate_with_credentials_auto_login_skip_missing_superuser_raises(
    ...
    from terraflow.services.auth.exceptions import InvalidCredentialsError
...
def test_add_padding_no_extra_chars_when_divisible_by_4():
    ...
    from terraflow.services.auth.utils import add_base64_padding
...
def test_ensure_fernet_key_with_44_char_key():
    ...
    from terraflow.services.auth.utils import ensure_fernet_key
...
async def test_create_user_tokens_updates_last_login(auth_service: AuthService):
    ...
    with patch("terraflow.services.auth.service.update_user_last_login_at", new=AsyncMock()) as mock_update:
...
async def test_create_refresh_token_valid(auth_service: AuthService):
    ...
    with patch("terraflow.services.auth.service.get_user_by_id", new=AsyncMock(return_value=fake_user)):
...
async def test_create_refresh_token_user_not_found(auth_service: AuthService):
    ...
    with (
        patch("terraflow.services.auth.service.get_user_by_id", new=AsyncMock(return_value=None)),
...
async def test_authenticate_user_success(auth_service: AuthService):
    ...
    with patch("terraflow.services.auth.service.get_user_by_username", new=AsyncMock(return_value=user)):
...
async def test_authenticate_user_not_found(auth_service: AuthService):
    ...
    with patch("terraflow.services.auth.service.get_user_by_username", new=AsyncMock(return_value=None)):
...
async def test_authenticate_user_inactive_never_logged_in(auth_service: AuthService):
    ...
    with (
        patch("terraflow.services.auth.service.get_user_by_username", new=AsyncMock(return_value=user)),
        pytest.raises(HTTPException) as exc,
    ):
...
async def test_ws_api_key_security_auto_login_skip_rejects_missing_superuser(
    ...
    with (
        patch("terraflow.services.auth.service.session_scope", _mock_session_scope),
        patch(
            "terraflow.services.auth.service.get_user_by_username",
            new=AsyncMock(return_value=None),
        ),
