"""Tests for the dynamic ``terraflow.components.*`` -> ``lfx.components.*`` bridge.

Saved flows in the wild import their components via the legacy
``terraflow.components.<sub>.<leaf>`` paths (the integration test
...
``terraflow/__init__.py`` installs a meta path finder that bridges the
two namespaces dynamically so saved flows keep loading without
modification.

This suite locks the contract:

    1. Generic prefix bridge: ``terraflow.components.<rest>`` resolves to
       ``lfx.components.<rest>`` for arbitrary subpackages and submodules,
       including new bundles extracted in the future.
    2. Renamed-package override: ``terraflow.components.knowledge_bases``
       (and any submodule under it) resolves to
       ``lfx.components.files_and_knowledge`` because the package was
       renamed during the move while the terraflow-side path stays for
       saved-flow compat.
    3. Class identity is preserved: a class loaded via the terraflow path
...
import terraflow  # noqa: F401  -- triggers the meta finder install


def test_dotted_submodule_import_resolves() -> None:
    """``from terraflow.components.processing.converter import X`` works.

    Regression: the deletion of the physical ``terraflow/components/processing/converter.py``
...
def test_helpers_subpackage_imports() -> None:
    """``from terraflow.components.helpers import X`` works for any helper class."""
...
def test_knowledge_bases_override_resolves_to_files_and_knowledge() -> None:
    """``terraflow.components.knowledge_bases[.<rest>]`` -> ``lfx.components.files_and_knowledge[.<rest>]``.

    The lfx package was renamed during the move; the terraflow-side path
    stays as ``knowledge_bases`` so saved flows that imported via that
    name continue to resolve.
    """
    from terraflow.components.knowledge_bases.retrieval import KnowledgeBaseComponent
...
def test_class_identity_preserved_across_bridge() -> None:
    """Same class is returned via ``terraflow.components.X`` and ``lfx.components.X``.

    Critical for ``isinstance`` checks against types resolved through
    either path -- e.g. a saved flow imports the legacy terraflow path
    while runtime code imports the lfx path; both must compare equal.
    """
    import terraflow.components.processing.converter as terraflow_module
    import lfx.components.processing.converter as lfx_module

    assert terraflow_module is lfx_module
    assert terraflow_module.convert_to_dataframe is lfx_module.convert_to_dataframe


def test_top_level_components_is_aliased_to_lfx_components() -> None:
    """``import terraflow.components`` resolves to the lfx package.

    Catches the case where the meta finder isn't installed yet when
    ``terraflow.components`` is first imported -- happens on cold imports
    in test runs that don't go through ``terraflow.__init__`` first.
    """
    import terraflow.components
    import lfx.components

    assert terraflow.components is lfx.components


def test_arbitrary_extracted_bundle_resolves_via_dynamic_bridge() -> None:
    """A bundle that exists in lfx but had no physical ``terraflow/components`` shim resolves.
    ...
    module is reachable via the legacy terraflow path immediately.
    """
    # Pick any subpackage under lfx.components that didn't have a
    # physical shim under terraflow/components/ before the deletion.
    target = "lfx.components.helpers.calculator_core"
    importlib.import_module(target)
    bridge = importlib.import_module("terraflow.components.helpers.calculator_core")
