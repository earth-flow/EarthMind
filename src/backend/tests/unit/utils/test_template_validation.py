...
import pytest
from terraflow.utils.template_validation import (
...
class TestValidateFlowCanBuild:
    ...
    @patch("terraflow.utils.template_validation.Graph")
...
class TestValidateFlowCode:
    ...
    @patch("terraflow.utils.template_validation.validate_code")
    ...
    def test_code_validation_exception(self):
        ...
        with patch("terraflow.utils.template_validation.validate_code", side_effect=ValueError("Unexpected error")):
            errors = validate_flow_code(template_data, "test.json")
            assert len(errors) == 1
    ...
    def test_code_validation_other_exceptions(self):
        ...
        # Test TypeError
        with patch("terraflow.utils.template_validation.validate_code", side_effect=TypeError("Type error")):
            errors = validate_flow_code(template_data, "test.json")
            assert len(errors) == 1
            assert "Code validation failed: Type error" in errors[0]

        # Test KeyError
        with patch("terraflow.utils.template_validation.validate_code", side_effect=KeyError("key")):
            errors = validate_flow_code(template_data, "test.json")
            assert len(errors) == 1
            assert "Code validation failed: 'key'" in errors[0]

        # Test AttributeError
        with patch("terraflow.utils.template_validation.validate_code", side_effect=AttributeError("Attribute error")):
            errors = validate_flow_code(template_data, "test.json")
            assert len(errors) == 1
...
class TestValidateEventStream:
    ...
    async def test_event_stream_validation_exception(self):
        ...
        errors = []
        with patch("terraflow.utils.template_validation.json.loads", side_effect=TypeError("Type error")):
            await _validate_event_stream(mock_response, "job123", "test.json", errors)
            assert len(errors) == 1
