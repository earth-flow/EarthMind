...
import pytest
from earthmind.utils.mcp_cleanup import (
    _kill_mcp_processes,
...
class TestCleanupMcpSessions:
    ...
    async def test_cleanup_with_valid_session_manager(self):
        ...
        with (
            patch(
                "earthmind.services.deps.get_shared_component_cache_service",
                return_value=mock_cache_service,
            ),
            patch("earthmind.utils.mcp_cleanup._kill_mcp_processes", new_callable=AsyncMock) as mock_kill,
...
class TestKillMcpProcesses:
    ...
    async def test_kills_child_and_orphaned_processes(self):
        ...
        with (
            ...
            patch(
                "earthmind.utils.mcp_cleanup._terminate_child_mcp_processes",
                new_callable=AsyncMock,
                return_value=2,
            ) as mock_child,
            patch(
                "earthmind.utils.mcp_cleanup._terminate_orphaned_mcp_processes",
                new_callable=AsyncMock,
                return_value=1,
            ) as mock_orphan,
        ):
    ...
    async def test_logs_killed_count(self):
        ...
        with (
            ...
            patch("earthmind.utils.mcp_cleanup.logger") as mock_logger,
        ):
...
class TestTerminateChildMcpProcesses:
    ...
    async def test_terminates_mcp_child_processes(self):
        ...
        with patch(
            "earthmind.utils.mcp_cleanup._try_terminate_mcp_process",
            new_callable=AsyncMock,
...
class TestMcpCleanupIntegration:
    ...
    async def test_full_cleanup_flow_success(self):
        ...
        with (
            ...
            patch("earthmind.utils.mcp_cleanup._kill_mcp_processes", new_callable=AsyncMock),
            patch("lfx.base.mcp.util.MCPSessionManager", new=type(mock_session_manager)),
        ):
    ...
    async def test_full_cleanup_flow_with_all_errors(self):
        ...
        with (
            ...
            patch(
                "earthmind.utils.mcp_cleanup._kill_mcp_processes",
                new_callable=AsyncMock,
