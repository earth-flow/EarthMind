...
from terraflow.services.variable.constants import CREDENTIAL_TYPE
...
def migrate_module():
    ...
    repo_root = test_file.parents[5]  # Goes to terraflow repo root
...
class TestDatabaseMigrationUnit:
    ...
    def test_migrate_user_store_api_key(self, migrate_module, sqlite_db, old_key, new_key):
        ...
        user_id = str(uuid4())
        original_value = "terraflow-store-api-key"
...
class TestKeyFileManagement:
    ...
    def test_get_config_dir_default(self, migrate_module, monkeypatch):
        ...
        monkeypatch.delenv("TERRAFLOW_CONFIG_DIR", raising=False)
        result = migrate_module.get_config_dir()
        expected = Path(user_cache_dir("terraflow", "terraflow"))
        assert result == expected

    def test_get_config_dir_from_env(self, migrate_module, monkeypatch):
        """Test config directory from environment variable."""
        monkeypatch.setenv("TERRAFLOW_CONFIG_DIR", "/custom/config")
        result = migrate_module.get_config_dir()
...
class TestMigrationWithRealDatabase:
    """Integration tests using real Terraflow database fixtures."""
...
class TestMigrationCompatibility:
    """Test that migration script is compatible with Terraflow's encryption."""

    def test_script_encryption_matches_terraflow(self, migrate_module):
        """Verify migration script produces same results as Terraflow's auth utils."""
        ...
        # Encrypt with Terraflow
        terraflow_encrypted = auth_utils.encrypt_api_key(plaintext, settings_service)

        # Decrypt with migration script
        script_decrypted = migrate_module.decrypt_with_key(terraflow_encrypted, secret_key)
        ...
        # Decrypt with Terraflow
        terraflow_decrypted = auth_utils.decrypt_api_key(script_encrypted, settings_service)
        assert terraflow_decrypted == plaintext
