...
class TestComponentLoadingFix:
    ...
    @pytest.fixture
    def mock_terraflow_components(self):
        """Create mock terraflow components response."""
        return {
            "components": {
                "category1": {
    ...
    async def test_base_components_path_filtering(
        self, mock_settings_service, mock_terraflow_components, mock_custom_components
    ):
        ...
        with (
            patch("lfx.interface.components.import_terraflow_components", return_value=mock_terraflow_components),
            ...
            # Verify result contains both terraflow and custom components
    ...
    @pytest.mark.asyncio
    async def test_only_base_components_path_in_list(self, mock_settings_service, mock_terraflow_components):
        ...
        with (
            ...
            # Verify result contains only terraflow components
            assert "category1" in result
            assert "category2" in result
            assert "Component1" in result["category1"]
    ...
    @pytest.mark.asyncio
    async def test_empty_components_path(self, mock_settings_service, mock_terraflow_components):
    ...
    @pytest.mark.asyncio
    async def test_none_components_path(self, mock_settings_service, mock_terraflow_components):
    ...
    @pytest.mark.asyncio
    async def test_lazy_loading_mode_with_base_path_filtering(self, mock_settings_service, mock_terraflow_components):
    ...
    async def test_multiple_custom_paths_with_base_path(
        ...
        with (
            ...
            assert "category1" in result  # From terraflow components
            assert "custom_category" in result  # From custom components

    @pytest.mark.asyncio
    async def test_component_merging_logic(self, mock_settings_service, mock_terraflow_components):
        """Test that terraflow and custom components are properly merged."""
        # Setup
        ...
        overlapping_custom_components = {
            "category1": {  # Same category as terraflow
                "Component1": {"display_name": "CustomComponent1", "type": "category1"},  # Same name as terraflow
        ...
        with (
            ...
            # Verify that custom components override terraflow components with same name
            assert "category1" in result
            assert "category2" in result  # From terraflow
            assert "new_category" in result  # From custom

            # Custom category should completely override terraflow category
            ...
            assert "Component2" not in result["category1"]  # Terraflow component is replaced by custom category
    ...
    @pytest.mark.asyncio
    async def test_component_cache_behavior(self, mock_settings_service, mock_terraflow_components):
    ...
    @pytest.mark.asyncio
    async def test_logging_behavior(self, mock_settings_service, mock_terraflow_components, mock_custom_components):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_custom_component_loading(self, mock_settings_service, mock_terraflow_components):
    ...
    async def test_base_components_path_constant_value(self):
        ...
        # Should be an absolute path containing "terraflow" and "components"
        assert "terraflow" in BASE_COMPONENTS_PATH.lower()
        assert "components" in BASE_COMPONENTS_PATH.lower()

    @pytest.mark.asyncio
    async def test_path_filtering_edge_cases(self, mock_settings_service, mock_terraflow_components):
    ...
    @pytest.mark.asyncio
    async def test_component_count_calculation(self, mock_settings_service, mock_terraflow_components):
        ...
        with (
            ...
            # Verify result structure
            assert len(result) >= 2  # At least terraflow categories + custom categories
    ...
    async def test_integration_with_real_base_components_path(self, mock_settings_service):
        ...
        # This test should work with real terraflow components
        with patch("lfx.interface.components.aget_all_types_dict", return_value={}) as mock_aget_all_types_dict:
            ...
            # Verify we got real terraflow components
            assert isinstance(result, dict)
            assert len(result) >= 0  # Should not have terraflow components
