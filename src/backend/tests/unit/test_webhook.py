...
import anyio
import pytest
from terraflow.services.event_manager import WebhookEventManager
...
def test_webhook_auth_enable_defaults_to_true():
    ...
    directly so a stray TERRAFLOW_WEBHOOK_AUTH_ENABLE env var can't mask a regression.
    """
...
async def test_webhook_invalid_api_key(client, added_webhook_test):
    ...
    with patch("terraflow.api.v1.endpoints.get_auth_service", return_value=mock_auth_service):
        endpoint_name = added_webhook_test["endpoint_name"]
        endpoint = f"api/v1/webhook/{endpoint_name}"
        payload = {"test": "data"}
...
class TestGetVertexIdsFromFlow:
    ...
    def test_returns_empty_list_when_flow_data_is_none(self):
        ...
        from terraflow.api.v1.endpoints import _get_vertex_ids_from_flow
...
class TestSimpleRunFlowTask:
    ...
    async def test_emits_vertices_sorted_event_when_emit_events_true(self):
        ...
        from terraflow.api.v1.endpoints import simple_run_flow_task
        ...
        with (
            patch("terraflow.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("terraflow.api.v1.endpoints.webhook_event_manager") as mock_manager,
        ):
    ...
    async def test_emits_error_event_on_exception(self):
        ...
        with (
            patch("terraflow.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run,
            patch("terraflow.api.v1.endpoints.webhook_event_manager") as mock_manager,
            patch("terraflow.api.v1.endpoints.logger") as mock_logger,
        ):
    ...
    async def test_logs_telemetry_on_success(self):
        ...
        with patch("terraflow.api.v1.endpoints.simple_run_flow", new_callable=AsyncMock) as mock_run:
            mock_run.return_value = {"result": "success"}
...
class TestWebhookEventsStreamAuth:
    ...
    async def test_raises_404_when_user_does_not_own_flow(self):
        ...
        from fastapi import HTTPException
        from terraflow.api.v1.endpoints import get_flow_for_sse_user

        mock_user = Mock()
        mock_user.id = "different-user-id"

        # Mock get_flow_by_id_or_endpoint_name to raise 404 as it would if user doesn't own it
        with patch("terraflow.api.v1.endpoints.get_flow_by_id_or_endpoint_name") as mock_get_flow:
    ...
    async def test_sse_route_calls_ensure_flow_permission_for_shared_flow(self):
        ...
        from fastapi import HTTPException
        from terraflow.api.v1 import endpoints as endpoints_module
