"""Terraflow API Locust Load Testing File.

Comprehensive load testing for Terraflow API with multiple user behaviors and performance analysis.
...
Environment Variables:
    - TERRAFLOW_HOST: Base URL for the Terraflow server (default: http://localhost:7860)
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://docs.terraflow.org for me? Later, could you calculate 1390 / 192 ?",
}
...
ERROR_LOG_FILE = None
TERRAFLOW_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, TERRAFLOW_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"terraflow_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("terraflow_load_test_errors")
    ...
    # Try to capture Terraflow logs
    terraflow_log_paths = ["terraflow.log", "logs/terraflow.log", "../../../terraflow.log", "../../../../terraflow.log"]

    for log_path in terraflow_log_paths:
        if Path(log_path).exists():
            TERRAFLOW_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if TERRAFLOW_LOG_FILE:
        print(f"   • Terraflow logs: {TERRAFLOW_LOG_FILE}")
    else:
        print("   • Terraflow logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"terraflow_load_test_error_summary_{timestamp}.json"
...
def capture_terraflow_logs():
    """Capture recent Terraflow logs if available."""
    if not TERRAFLOW_LOG_FILE or not Path(TERRAFLOW_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Terraflow log
        with open(TERRAFLOW_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"terraflow_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Terraflow server logs captured during load test\n")
            f.write(f"# Original log file: {TERRAFLOW_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Terraflow logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Terraflow logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"TERRAFLOW API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
    ...
    # Capture Terraflow logs
    capture_terraflow_logs()
...
class BaseTerraflowUser(FastHttpUser):
    """Base class for all Terraflow API load testing user types."""

    abstract = True
    REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "30.0"))

    # Use the host provided by environment variable or default
    host = os.getenv("TERRAFLOW_HOST", "http://localhost:7860")
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_terraflow_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_terraflow_test.py first.")
    ...
    def make_request(self, message_type="simple", tag_suffix=""):
        ...
        # Terraflow API payload structure
        payload = {
            "input_value": message,
            "output_type": "chat",
            "input_type": "chat",
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Terraflow API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseTerraflowUser):
...
class AggressiveUser(BaseTerraflowUser):
...
class SustainedLoadUser(BaseTerraflowUser):
...
class TailLatencyHunter(BaseTerraflowUser):
...
class ScalabilityTestUser(BaseTerraflowUser):
...
class BurstUser(BaseTerraflowUser):
