...
import pytest
from terraflow.services.storage.s3 import S3StorageService
...
def mock_settings_service():
    ...
    - TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME: S3 bucket name (default: terraflow-ci)
    - TERRAFLOW_OBJECT_STORAGE_PREFIX: S3 prefix (default: test-files-1)
    - TERRAFLOW_OBJECT_STORAGE_TAGS: S3 tags as JSON string (default: {"env": "test-1"})

    Note: All settings use TERRAFLOW_OBJECT_STORAGE_* names to test that
    the S3StorageService properly respects these settings.
    """
    settings_service = Mock()
    settings_service.settings.config_dir = str(Path(tempfile.gettempdir()) / "terraflow_test")

    # Bucket name from env or default
    settings_service.settings.object_storage_bucket_name = os.environ.get(
        "TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME", "terraflow-ci"
    )

    # Prefix from env - using standard TERRAFLOW env var name
    settings_service.settings.object_storage_prefix = os.environ.get("TERRAFLOW_OBJECT_STORAGE_PREFIX", "test-files-1")

    # Tags from env - using standard TERRAFLOW env var name
    default_tags = {"env": "test-1"}
    tags_str = os.environ.get("TERRAFLOW_OBJECT_STORAGE_TAGS")
    if tags_str:
        try:
...
class TestS3StorageServiceInitialization:
    ...
    async def test_initialization(self, s3_storage_service):
        ...
        expected_bucket = os.environ.get("TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME", "terraflow-ci")
        assert s3_storage_service.bucket_name == expected_bucket

        # Verify prefix matches env or default (with trailing slash)
        # This tests that S3StorageService respects TERRAFLOW_OBJECT_STORAGE_PREFIX
        expected_prefix = os.environ.get("TERRAFLOW_OBJECT_STORAGE_PREFIX", "test-files-1")
        assert s3_storage_service.prefix == f"{expected_prefix}/"

        # Verify tags match env or default
        # This tests that S3StorageService respects TERRAFLOW_OBJECT_STORAGE_TAGS
        default_tags = {"env": "test-1"}
        tags_str = os.environ.get("TERRAFLOW_OBJECT_STORAGE_TAGS")
