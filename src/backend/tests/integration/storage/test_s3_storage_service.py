...
import pytest
from earthmind.services.storage.s3 import S3StorageService
...
def mock_settings_service():
    ...
    - EARTHMIND_OBJECT_STORAGE_BUCKET_NAME: S3 bucket name (default: earthmind-ci)
    - EARTHMIND_OBJECT_STORAGE_PREFIX: S3 prefix (default: test-files-1)
    - EARTHMIND_OBJECT_STORAGE_TAGS: S3 tags as JSON string (default: {"env": "test-1"})

    Note: All settings use EARTHMIND_OBJECT_STORAGE_* names to test that
    the S3StorageService properly respects these settings.
    """
    settings_service = Mock()
    settings_service.settings.config_dir = str(Path(tempfile.gettempdir()) / "earthmind_test")

    # Bucket name from env or default
    settings_service.settings.object_storage_bucket_name = os.environ.get(
        "EARTHMIND_OBJECT_STORAGE_BUCKET_NAME", "earthmind-ci"
    )

    # Prefix from env - using standard EARTHMIND env var name
    settings_service.settings.object_storage_prefix = os.environ.get("EARTHMIND_OBJECT_STORAGE_PREFIX", "test-files-1")

    # Tags from env - using standard EARTHMIND env var name
    default_tags = {"env": "test-1"}
    tags_str = os.environ.get("EARTHMIND_OBJECT_STORAGE_TAGS")
    if tags_str:
        try:
...
class TestS3StorageServiceInitialization:
    ...
    async def test_initialization(self, s3_storage_service):
        ...
        expected_bucket = os.environ.get("EARTHMIND_OBJECT_STORAGE_BUCKET_NAME", "earthmind-ci")
        assert s3_storage_service.bucket_name == expected_bucket

        # Verify prefix matches env or default (with trailing slash)
        # This tests that S3StorageService respects EARTHMIND_OBJECT_STORAGE_PREFIX
        expected_prefix = os.environ.get("EARTHMIND_OBJECT_STORAGE_PREFIX", "test-files-1")
        assert s3_storage_service.prefix == f"{expected_prefix}/"

        # Verify tags match env or default
        # This tests that S3StorageService respects EARTHMIND_OBJECT_STORAGE_TAGS
        default_tags = {"env": "test-1"}
        tags_str = os.environ.get("EARTHMIND_OBJECT_STORAGE_TAGS")
