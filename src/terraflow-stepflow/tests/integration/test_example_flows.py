...
Tests the full end-to-end path used by the ``stepflow-terraflow run`` CLI:
TerraflowConverter -> StepflowClient -> orchestrator -> terraflow worker.
...
def _find_poc_flow() -> Path:
    ...
    # here.parents[4] = terraflow repo root (e.g. /Users/.../terraflow)
    terraflow_repo = here.parents[4]
    candidate = terraflow_repo.parent / "stepflow" / "docs" / "static" / "files" / "2025-09-terraflow-poc-flow.json"
    if candidate.exists():
        return candidate
...
def test_poc_flow_translation(converter, poc_flow_data):
    ...
    for step in flow.steps:
        assert step.component.startswith(("/terraflow/", "/builtin/")), f"unexpected component prefix: {step.component}"
...
async def test_poc_flow_execution(runner, poc_flow_data):
    ...
    as ``stepflow-terraflow run --local``.
    """
