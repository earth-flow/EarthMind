...
class TestCoreExecutorWithRealComponent:
    """Integration tests with real Terraflow components.

    Note: Some Terraflow components have complex dependencies (database, etc.)
    ...
    async def test_import_prompt_component(self, executor, mock_context):
        ...
        try:
            module = importlib.import_module("terraflow.components.prompts.prompt")
            ...
            # terraflow.components.prompts may not be available in all environments
            # Skip test if not available
            pytest.skip("terraflow.components.prompts not available")
