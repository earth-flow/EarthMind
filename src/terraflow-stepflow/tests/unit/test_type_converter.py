...
from terraflow_stepflow.worker.base_executor import BaseExecutor
from terraflow_stepflow.worker.handlers import (
    BaseModelInputHandler,
    BaseModelOutputHandler,
)
from terraflow_stepflow.worker.handlers.base_model import (
    _is_secret_str_type,
)
...
class TestOutputTreeWalker:
    ...
    @pytest.mark.asyncio
    async def test_terraflow_types_still_work(self):
        """Test that existing Terraflow type serialization still works."""
        try:
            ...
            handlers = [TerraflowTypeOutputHandler(), BaseModelOutputHandler()]
            serialized = await self.executor._apply_output_handlers(message, handlers)

            # Should have Terraflow type metadata, not BaseModel metadata
            assert "__terraflow_type__" in serialized
            assert serialized["__terraflow_type__"] == "Message"

        except ImportError:
            # Skip if Terraflow not available
            pytest.skip("Terraflow not available for testing")
