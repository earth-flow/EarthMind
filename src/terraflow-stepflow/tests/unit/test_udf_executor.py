"""Unit test for custom code execution in isolation (with real Terraflow code)."""
...
from terraflow_stepflow.exceptions import ExecutionError
from terraflow_stepflow.worker.custom_code_executor import (
    CustomCodeExecutor,
)


class TestCustomCodeExecutor:
    """Test CustomCodeExecutor functionality with real Terraflow component code."""
...
''',
    ...
    def chat_input_component_blob(self):
        """Real ChatInput component blob data from Terraflow."""
        return {
            "code": '''
from terraflow.base.io.chat import ChatComponent
from terraflow.inputs.inputs import BoolInput
from terraflow.io import DropdownInput, MessageTextInput, MultilineInput, Output
from terraflow.schema.message import Message
from terraflow.utils.constants import (
...
''',
    ...
    async def test_execute_simple_component(
        ...
        # Terraflow Message: result data is nested in ["result"] field
    ...
    async def test_execute_chat_input_component(
        ...
        # Should have Terraflow Message structure
        # Handle nested result structure from Terraflow Messages
        message_data = result_data["result"] if "result" in result_data else result_data
        assert "text" in message_data
        assert message_data["text"] == "What is Python?"
        assert message_data["sender"] == "User"
        assert message_data["sender_name"] == "Test User"
        assert message_data.get("__terraflow_type__") == "Message"
...
''',
    ...
    async def test_plain_pandas_dataframe_serializes(
        ...
        assert output["__terraflow_type__"] == "DataFrame"
        assert "json_data" in output


class TestCustomCodeExecutorIntegration:
    """Integration tests with mock Terraflow imports."""

    @pytest.fixture
    def executor_with_mocks(self):
        """Create CustomCodeExecutor with mocked Terraflow imports."""
        # We'll test this without real Terraflow imports to avoid dependency issues
        return CustomCodeExecutor()
...
class TestCustomCodeExecutorWithRealTerraflowComponents:
    """Test CustomCodeExecutor with real converted Terraflow workflow components."""
...
"""
        ...
        stepflow_workflow = converter.convert(terraflow_data)

        # Find steps that use /terraflow/custom_code
        custom_code_steps = [step for step in stepflow_workflow.steps if step.component == "/terraflow/custom_code"]
...
""",
    ...
    def basic_prompting_flow(self):
        ...
        flow_path = Path(__file__).parent.parent / "fixtures" / "terraflow" / "basic_prompting.json"
    ...
    async def test_prompt_component(self, executor: CustomCodeExecutor, mock_context, prompt_component_data):
        ...
        has_message_marker = "__terraflow_type__" in output or "__class_name__" in output
    ...
    async def test_language_model_message_to_string_conversion(
        ...
        This test verifies the fix for Terraflow 1.6.4+ lfx components that
    ...
    async def test_chat_input_message_to_string_conversion(
        ...
        This is a specific test for the Terraflow 1.6.4+ fix where Message objects
