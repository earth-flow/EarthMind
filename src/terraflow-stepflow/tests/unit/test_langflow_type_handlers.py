"""Unit tests for TerraflowType input/output handlers and DataFrame output handler.

Tests serialization (output) and deserialization (input) of Terraflow
...
from terraflow_stepflow.worker.handlers import (
    DataFrameOutputHandler,
    TerraflowTypeInputHandler,
    TerraflowTypeOutputHandler,
)

# ---------------------------------------------------------------------------
# TerraflowTypeOutputHandler — matches()
# ---------------------------------------------------------------------------


class TestTerraflowTypeOutputHandlerMatches:
    def setup_method(self):
        self.handler = TerraflowTypeOutputHandler()
...
# ---------------------------------------------------------------------------
# TerraflowTypeOutputHandler — process()
# ---------------------------------------------------------------------------


class TestTerraflowTypeOutputHandlerProcess:
    def setup_method(self):
        self.handler = TerraflowTypeOutputHandler()
    ...
    async def test_serialize_message(self):
        ...
        assert result["__terraflow_type__"] == "Message"
    ...
    async def test_serialize_data(self):
        ...
        assert result["__terraflow_type__"] == "Data"
...
class TestDataFrameOutputHandlerProcess:
    ...
    async def test_serialize_dataframe(self):
        ...
        assert result["__terraflow_type__"] == "DataFrame"
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — matches()
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerMatches:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()

    def test_matches_dict_with_marker(self):
        value = {"__terraflow_type__": "Message", "text": "hi"}
        assert self.handler.matches(template_field={}, value=value) is True

    def test_matches_list_with_marked_items(self):
        value = [
            {"__terraflow_type__": "Data", "text": "row1"},
            {"__terraflow_type__": "Data", "text": "row2"},
        ]
        assert self.handler.matches(template_field={}, value=value) is True
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — prepare() (Message)
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerMessage:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_message(self):
        serialized = {"__terraflow_type__": "Message", "text": "hello"}
    ...
    async def test_deserialize_message_empty_text(self):
        serialized = {"__terraflow_type__": "Message", "text": ""}
        fields = {"msg": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — prepare() (Data)
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerData:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_data(self):
        serialized = {"__terraflow_type__": "Data", "text": "some data"}
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — prepare() (DataFrame)
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerDataFrame:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — prepare() (list handling)
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerLists:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()
    ...
    async def test_deserialize_list_of_messages(self):
        value = [
            {"__terraflow_type__": "Message", "text": "msg1"},
            {"__terraflow_type__": "Message", "text": "msg2"},
        ]
        fields = {"msgs": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_list_of_data(self):
        value = [
            {"__terraflow_type__": "Data", "text": "d1"},
            {"__terraflow_type__": "Data", "text": "d2"},
        ]
        fields = {"items": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_multiple_fields(self):
        ...
        fields = {
            "msg": ({"__terraflow_type__": "Message", "text": "hello"}, {}),
            "data": ({"__terraflow_type__": "Data", "text": "world"}, {}),
        }
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# TerraflowTypeInputHandler — unknown / edge cases
# ---------------------------------------------------------------------------


class TestTerraflowTypeInputHandlerEdgeCases:
    def setup_method(self):
        self.handler = TerraflowTypeInputHandler()

    @pytest.mark.asyncio
    async def test_unknown_terraflow_type_returns_dict(self):
        """Unknown __terraflow_type__ values should return the raw dict."""
        serialized = {"__terraflow_type__": "UnknownType", "data": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)

        assert isinstance(result["field"], dict)
        assert result["field"]["__terraflow_type__"] == "UnknownType"

    @pytest.mark.asyncio
    async def test_empty_terraflow_type_returns_dict(self):
        """Empty __terraflow_type__ value should return the raw dict."""
        serialized = {"__terraflow_type__": "", "text": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
class TestTerraflowTypeRoundTrip:
    def setup_method(self):
        self.output_handler = TerraflowTypeOutputHandler()
        self.input_handler = TerraflowTypeInputHandler()
...
class TestDataFrameRoundTrip:
    ...
    deserialize with TerraflowTypeInputHandler."""
    ...
    async def test_dataframe_round_trip(self):
        ...
        serialized = await self.output_handler.process(original)
        assert serialized["__terraflow_type__"] == "DataFrame"
...
class TestPandasDataFrameRoundTrip:
    ...
    DataFrames instead of lfx DataFrames, depending on the Terraflow
