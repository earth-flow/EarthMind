"""Stepflow-level tweaks for Terraflow component configuration.

This module provides utilities for applying tweaks to Stepflow workflows
that were translated from Terraflow. Tweaks are applied by modifying the
input fields of Terraflow UDF executor steps before execution.

Key features:
- Apply tweaks to translated Stepflow workflows at execution time
- Target UDF executor steps using original Terraflow node IDs
- Modify step input fields directly (not blob data)
- Compatible with Terraflow API tweak format
...
def apply_stepflow_tweaks_to_dict(
    ...
    Args:
        ...
        tweaks: Dict mapping terraflow_node_id -> {field_name: new_value}
    ...
    for step_dict in modified_dict.get("steps", []):
        ...
        # Check if this is a Terraflow executor step (custom_code or core)
        is_terraflow_step = (
            step_id.startswith("terraflow_")
            and not step_id.endswith("_blob")
            and (component == "/terraflow/custom_code" or component.startswith("/terraflow/core/"))
        )

        if is_terraflow_step:
            # Extract Terraflow node ID
            terraflow_node_id = step_id[len("terraflow_") :]

            if terraflow_node_id in tweaks:
                ...
                # Apply tweaks
                for field_name, new_value in tweaks[terraflow_node_id].items():
...
def convert_tweaks_to_overrides(
    ...
    """Convert Terraflow tweaks format to Stepflow overrides format.
    ...
    Examples:
        ...
        >>> print(overrides)
        {
            "terraflow_LanguageModelComponent-kBOja": {
                "$type": "merge_patch",
                "value": {
                    "input": {
                        "api_key": "new_test_key",  # pragma: allowlist secret
    ...
    for terraflow_node_id, field_tweaks in tweaks.items():
        # Convert Terraflow node ID to Stepflow step ID format
        step_id = f"terraflow_{terraflow_node_id}"
