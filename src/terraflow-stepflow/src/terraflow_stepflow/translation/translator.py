"""Main Terraflow to Stepflow converter implementation."""
...
class WorkflowAnalysis:
    """Typed analysis results from analyzing a Terraflow workflow."""

    node_count: int  # Total number of nodes in the Terraflow workflow
...
class TerraflowConverter:
    """Convert Terraflow JSON workflows to Stepflow YAML workflows."""
    ...
    def convert_file(self, input_path: str | Path) -> str:
        """Convert a Terraflow JSON file to Stepflow YAML.

        Args:
            input_path: Path to the Terraflow JSON file
        ...
        workflow = self.convert(terraflow_data)
        return self.to_yaml(workflow)

    def convert(self, terraflow_data: dict[str, Any]) -> Flow:
        """Convert Terraflow data structure to Stepflow workflow.

        Args:
            terraflow_data: Parsed Terraflow JSON data
        ...
        try:
            ...
            if "data" not in terraflow_data:
                raise ConversionError("Invalid Terraflow JSON: missing 'data' key")

            data = terraflow_data["data"]
            nodes = data.get("nodes", [])
            edges = data.get("edges", [])

            if not nodes:
                raise ConversionError("No nodes found in Terraflow workflow")
            ...
            # Create FlowBuilder
            builder = FlowBuilder(name=self._generate_workflow_name(terraflow_data))
    ...
    def analyze(self, terraflow_data: dict[str, Any]) -> WorkflowAnalysis:
        """Analyze Terraflow workflow structure without conversion.
    ...
    def _generate_workflow_name(self, terraflow_data: dict[str, Any]) -> str:
        """Generate a workflow name from Terraflow data."""
        # Try to get name from various sources
        if "name" in terraflow_data:
            name = terraflow_data["name"]
            return str(name) if name is not None else "Converted Terraflow Workflow"
        ...
        return "Converted Terraflow Workflow"
    ...
    def _build_flow_output(
        ...
        Args:
            ...
            nodes: Original Terraflow nodes
