"""Hash-to-module mappings for known Terraflow core components.

This module contains mappings of code hashes to component module paths for
known Terraflow core components. When a component's code_hash matches an entry
here and the module matches, we can use the core executor instead of compiling
custom code from a blob.

The hashes are derived from the Terraflow workflow metadata and can be found in
...
# These hashes are version-specific and may need updating when Terraflow versions change
KNOWN_COMPONENTS: dict[str, KnownComponent] = {
    ...
    # Note: PromptComponent (terraflow.components.prompts.prompt) is NOT included
