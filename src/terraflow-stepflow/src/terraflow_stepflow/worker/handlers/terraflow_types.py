"""Handlers for Terraflow core types (Message, Data, DataFrame).

Input: deserialize dicts with ``__terraflow_type__`` markers back to lfx objects.
Output: serialize Message/Data objects to dicts with ``__terraflow_type__`` markers.
"""
...
# back to the canonical Terraflow type names used in serialized payloads.
_CLASS_TO_TERRAFLOW_TYPE: dict[str, str] = {
    "JSON": "Data",
    "Table": "DataFrame",
}


def _terraflow_type_name(value: Any) -> str:
    """Return the canonical Terraflow type name for a value."""
    cls_name = type(value).__name__
    return _CLASS_TO_TERRAFLOW_TYPE.get(cls_name, cls_name)


def _is_terraflow_type_dict(value: Any) -> bool:
    """Check if a single value is a dict with a ``__terraflow_type__`` marker."""
    return isinstance(value, dict) and "__terraflow_type__" in value


def _has_terraflow_type_marker(value: Any) -> bool:
    """Check if a value (or list of values) contains __terraflow_type__ markers."""
    if _is_terraflow_type_dict(value):
        return True
    if isinstance(value, list):
        return any(_is_terraflow_type_dict(item) for item in value)
    return False


def _deserialize_single(obj: dict[str, Any]) -> Any:
    """Deserialize a single dict with __terraflow_type__ marker."""
    try:
    ...
    terraflow_type = obj.get("__terraflow_type__")
    if not terraflow_type:
        return obj

    obj_data = {k: v for k, v in obj.items() if k != "__terraflow_type__"}

    if terraflow_type == "Message":
        return Message(**obj_data)
    elif terraflow_type == "Data":
        return Data(**obj_data)
    elif terraflow_type == "DataFrame":
...
class TerraflowTypeInputHandler(InputHandler):
    """Deserialize dicts with ``__terraflow_type__`` markers to lfx objects.
    ...
    def matches(self, *, template_field: dict[str, Any], value: Any) -> bool:
        return _has_terraflow_type_marker(value)
    ...
    async def prepare(self, fields: dict[str, tuple[Any, dict[str, Any]]], context: Any) -> dict[str, Any]:
        ...
        for key, (value, _template_field) in fields.items():
            if _is_terraflow_type_dict(value):
                ...
                result[key] = [_deserialize_single(item) if _is_terraflow_type_dict(item) else item for item in value]

        return result


class TerraflowTypeOutputHandler(OutputHandler):
    """Serialize Message and Data objects with ``__terraflow_type__`` markers.

    Matches ``isinstance(value, (Message, Data))`` and produces dicts with
    ``model_dump(mode="json")`` plus a ``__terraflow_type__`` key.
    """
    ...
    async def process(self, value: Any) -> Any:
        ...
        serialized["__terraflow_type__"] = _terraflow_type_name(value)
        return serialized
