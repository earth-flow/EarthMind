"""Input handler that coerces Terraflow Message objects to plain strings."""
...
class StringCoercionInputHandler(InputHandler):
    """Coerce Terraflow Message objects to their ``.text`` for string fields.

    Matches template fields with ``type == "str"`` whose runtime value is a
    Terraflow ``Message`` instance. Extracts the ``text`` attribute so the
