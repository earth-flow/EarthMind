"""Component for creating tool wrappers from Terraflow components."""
...
async def component_tool_executor(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Create a tool wrapper from Terraflow component code and inputs.

    This component takes a Terraflow component's code/JSON and static inputs,
    ...
    try:
        ...
        for field_name, field_data in template.items():
            if isinstance(field_data, dict) and field_data.get("tool_mode", False):
                ...
                prop: dict[str, Any] = {
                    "type": _map_terraflow_type_to_json_schema(field_type),
...
def _map_terraflow_type_to_json_schema(terraflow_type: str) -> str:
    """Map Terraflow field types to JSON Schema types.

    Args:
        terraflow_type: Terraflow field type (e.g., "str", "int", "bool")
    ...
    return type_mapping.get(terraflow_type, "string")
