"""Standalone Stepflow component server for Terraflow integration.

Run with: python -m terraflow_stepflow.worker
"""
...
from terraflow_stepflow.worker.component_tool import component_tool_executor
from terraflow_stepflow.worker.core_executor import CoreExecutor
from terraflow_stepflow.worker.custom_code_executor import CustomCodeExecutor
...
async def custom_code_component(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Execute a Terraflow custom code component."""
...
async def core_component(
    ...
    """Execute a known core Terraflow component by module path."""
...
async def component_tool_component(input_data: dict[str, Any], context: StepflowContext) -> dict[str, Any]:
    """Create tool wrappers from Terraflow components."""
    return await component_tool_executor(input_data, context)


def main():
    """Main entry point for the Terraflow component server.
    ...
    - STEPFLOW_QUEUE_NAME: Queue name for gRPC transport (default: terraflow)
    ...
    if os.environ.get("TERRAFLOW_DATABASE_URL"):
    ...
    parser = argparse.ArgumentParser(description="Terraflow Stepflow Component Server")
    parser.add_argument(
        "--tasks-url",
        type=str,
    ...
    parser.add_argument(
        "--queue-name",
        type=str,
        default=os.environ.get("STEPFLOW_QUEUE_NAME", "terraflow"),
