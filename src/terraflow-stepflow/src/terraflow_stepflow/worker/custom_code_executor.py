"""Custom code executor for Terraflow components.

Executes Terraflow components by compiling their code from blob storage.
...
class CustomCodeExecutor(BaseExecutor):
    """Executes Terraflow custom code components by compiling code from blobs.
    ...
    def _get_input_handlers(self) -> list[InputHandler]:
        ...
        conversion needed for Terraflow type transformations.
        """
        return [
            TerraflowTypeInputHandler(),
            BaseModelInputHandler(),
            ...
            if blob_id not in self.compiled_components:
                try:
                    ...
                    raise ExecutionError(
                        f"No component code found for blob {blob_id}. All components must have real Terraflow code."
                    ) from e
