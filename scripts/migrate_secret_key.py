...
- user.store_api_key: Terraflow Store API keys
...
# Must match terraflow.services.variable.constants.CREDENTIAL_TYPE
CREDENTIAL_TYPE = "Credential"


def get_default_config_dir() -> Path:
    """Get the default Terraflow config directory using platformdirs."""
    return Path(user_cache_dir("terraflow", "terraflow"))


def get_config_dir() -> Path:
    """Get the Terraflow config directory from environment or default."""
    config_dir = os.environ.get("TERRAFLOW_CONFIG_DIR")
    if config_dir:
...
def ensure_valid_key(s: str) -> bytes:
    ...
    NOTE: This function is duplicated from terraflow.services.auth.utils.ensure_valid_key
    to keep the migration script self-contained (can run without full Terraflow installation).
...
def get_default_database_url(config_dir: Path) -> str | None:
    ...
    default_db = config_dir / "terraflow.db"
...
def migrate(
    ...
    *,
    ...
    Args:
        config_dir: Path to Terraflow config directory containing secret_key file.
    ...
    if dry_run:
        ...
        print("\nNext steps:")
        print("1. Start Terraflow and verify everything works")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate Terraflow encrypted data to a new secret key",
...
Examples:
  ...
  %(prog)s --database-url postgresql://user:pass@host/db --config-dir /etc/terraflow  # pragma: allowlist secret
  ...
  %(prog)s --old-key "current-key" --new-key "replacement-key"
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=f"Terraflow config directory (default: {default_config})",
    )
    parser.add_argument(
        "--database-url",
        type=str,
    ...
    if not database_url:
        ...
        print(f"  No database found at {args.config_dir}/terraflow.db")
