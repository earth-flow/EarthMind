...
The nightly publishes ``lfx`` and ``terraflow-sdk`` under their CANONICAL names as ``.devN``
pre-releases -- it does NOT rename them to ``lfx-nightly`` / ``terraflow-sdk-nightly``, and it does
...
lfx's ``terraflow-sdk`` dependency to the exact canonical dev version.
"""
...
def update_sdk_dependency_in_lfx(pyproject_path: str, sdk_version: str) -> None:
    """Pin lfx's ``terraflow-sdk`` dependency to the exact canonical dev version.

    An exact ``==<dev>`` pin keeps the SDK in lockstep with the lfx built in the same run and,
    because it names a pre-release explicitly, enables pre-release resolution for ``terraflow-sdk``
    down the dependency tree without requiring ``--pre``.
    """
    filepath = BASE_DIR / pyproject_path
    content = filepath.read_text(encoding="utf-8")

    pattern = re.compile(r'"terraflow-sdk(?:-nightly)?(?:==|~=|>=)[\d.]+(?:\.(?:post|dev|a|b|rc)\d+)*"')
    replacement = f'"terraflow-sdk=={sdk_version}"'
