...
def main():
    ...
    The packages keep their CANONICAL names (``terraflow``, ``terraflow-base``) -- they are NOT
    ...
    # Lockstep invariant: terraflow-base's published dev version (set here from `base_tag`) and
    # terraflow's exact `==` pin on it (set below from the same `base_tag`) MUST come from the same
    # value, so the latest nightly `terraflow` always pins a base version published in the same run.
    ...
    # Update LFX dependency in terraflow-base (exact canonical dev pin).
    ...
    # Update terraflow-base dependency version (strip 'v' prefix if present).
