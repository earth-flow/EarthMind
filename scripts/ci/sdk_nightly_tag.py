"""Generate the nightly tag for the canonical ``terraflow-sdk`` package.

Mirrors ``lfx_nightly_tag.py`` but for the SDK -- the nightly publishes ``terraflow-sdk==<base>.devN``
to the canonical ``terraflow-sdk`` PyPI project, so the dev counter is computed against that
...
PYPI_SDK_URL = "https://pypi.org/pypi/terraflow-sdk/json"
...
def create_sdk_tag() -> str:
    """Return the next ``terraflow-sdk`` nightly tag (with a leading ``v``)."""
