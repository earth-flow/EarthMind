...
HOST="${TERRAFLOW_HOST:-127.0.0.1}"
PORT="${TERRAFLOW_PORT:-7860}"
SUITES="${SUITES:-curl,python,javascript}"
EXECUTE_MODE="${EXECUTE_MODE:-true}"

export TERRAFLOW_AUTO_LOGIN="${TERRAFLOW_AUTO_LOGIN:-true}"
# /api/v2/workflows (docs Python workflow examples) requires this. Always enable for this
# harness so a user-wide TERRAFLOW_DEVELOPER_API_ENABLED=false does not break the suite.
export TERRAFLOW_DEVELOPER_API_ENABLED=true

cleanup() {
  if [[ -f /tmp/terraflow-server.pid ]]; then
    SERVER_PID="$(< /tmp/terraflow-server.pid)"
    ...
    rm -f /tmp/terraflow-server.pid
  fi
}
trap cleanup EXIT

# If PORT is already taken, Terraflow may bind to PORT+1 while this script still uses PORT for
...
if port_is_in_use "$HOST" "$PORT"; then
  PORT="$(pick_free_port)"
  echo "Port $REQUESTED_PORT was in use; using $PORT for this run (set TERRAFLOW_PORT to pin a port)."
fi

echo "Starting Terraflow on http://$HOST:$PORT (developer API enabled for /api/v2/workflows)"
# Set on the command line so the server process always sees it (macOS launcher/exec paths).
TERRAFLOW_DEVELOPER_API_ENABLED=true uv run terraflow run --backend-only --host "$HOST" --port "$PORT" >/tmp/terraflow-server.log 2>&1 &
echo $! >/tmp/terraflow-server.pid

echo "Waiting for Terraflow readiness (DB + services via /health_check)..."
# /health can respond before the Terraflow app is fully up (uvicorn default).
for _ in {1..60}; do
...
if ! curl -sf "http://$HOST:$PORT/health_check" >/dev/null; then
  echo "Terraflow did not become ready in time."
  echo "See /tmp/terraflow-server.log for details."
  exit 1
fi
...
user = os.environ.get("TERRAFLOW_SUPERUSER", "terraflow")
password = os.environ.get("TERRAFLOW_SUPERUSER_PASSWORD", "terraflow")
...
if [[ -z "$TERRAFLOW_API_KEY" ]]; then
  echo "Failed to create API key."
  exit 1
fi

export TERRAFLOW_URL="http://$HOST:$PORT"
export TERRAFLOW_SERVER_URL="http://$HOST:$PORT"
export TERRAFLOW_API_KEY
...
base_url = os.environ["TERRAFLOW_URL"]
api_key = os.environ["TERRAFLOW_API_KEY"]
...
try:
    ...
    project_zip_path = Path("/tmp/terraflow-project-import.zip")
    try:
...
echo "Done. Server log: /tmp/terraflow-server.log"
