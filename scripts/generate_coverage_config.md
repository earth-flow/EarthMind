...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/earthmind/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/earthmind --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/earthmind/components/**/*.py` (legacy components)
