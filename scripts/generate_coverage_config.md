...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/terraflow/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/terraflow --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/terraflow/components/**/*.py` (legacy components)
