# Deploy EarthMind on Google Cloud Platform
...
In this tutorial, you will learn how to deploy EarthMind on [Google Cloud Platform](https://cloud.google.com/) (GCP) using Google Cloud Shell.
...
- A project on GCP where you want to deploy EarthMind
...
In the next step, you'll configure the GCP environment and deploy EarthMind.

## Configure the GCP environment and deploy EarthMind
Run the deploy_earthmind_gcp_spot.sh script to configure the GCP environment and deploy EarthMind:

```sh
gcloud config set project <walkthrough-project-id/>
bash ./deploy_earthmind_gcp_spot.sh
```

The script will:

1. Check if the required resources (VPC, subnet, firewall rules, and Cloud Router) exist and create them if needed
2. Create a startup script to install Python, EarthMind, and Nginx
3. Create a Compute Engine VM instance with the specified configuration and startup script
4. Run EarthMind to serve content on TCP port 7860
...
In the next step, you'll learn how to connect to the EarthMind VM.

## Connect to the EarthMind VM
To connect to your new EarthMind VM, follow these steps:
...
<br>**or**
3. Run the following command to display the URL for your EarthMind environment:
```bash
export EARTHMIND_IP=$(gcloud compute instances list --filter="NAME=earthmind-dev" --format="value(EXTERNAL_IP)")

echo http://$EARTHMIND_IP:7860
```

4. Click on the EarthMind URL in cloudshell to be greeted by the EarthMind Dev environment

Congratulations! You have successfully deployed EarthMind on Google Cloud Platform.
...
```sql
gcloud compute instances delete earthmind-dev --zone us-central1-a --quiet
```
