...
if [ -z "$EARTHMIND_MAX_FILE_SIZE_UPLOAD" ]; then
  EARTHMIND_MAX_FILE_SIZE_UPLOAD="1"
fi
if [ -z "$BACKEND_URL" ]; then
...
export BACKEND_URL FRONTEND_PORT EARTHMIND_MAX_FILE_SIZE_UPLOAD

# Use envsubst to substitute environment variables in the template
envsubst '${BACKEND_URL} ${FRONTEND_PORT} ${EARTHMIND_MAX_FILE_SIZE_UPLOAD}' < /etc/nginx/conf.d/default.conf.template > $CONFIG_DIR/default.conf
