...
Each Terraflow server has a file management system where you can store files that you want to use in your flows.

Files uploaded to Terraflow file management are stored in Terraflow's [storage backend (local or AWS S3)](/concepts-file-management#configure-file-storage), and they are available to all of your flows.

Uploading files to Terraflow file management keeps your files in a central location, and allows you to reuse files across flows without repeated manual uploads.

## Use the file management UI

You can use the file management UI to upload files from your local machine to your own Terraflow server.
You can also manage all files that have been uploaded to your Terraflow server.

1. Navigate to Terraflow file management:

    * **Terraflow Desktop**: In Terraflow, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * **Terraflow OSS**: From a browser, navigate to your Terraflow server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your Terraflow server.
    * **Backend-only**: For programmatic file management, use the [Terraflow API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
## Upload and manage files with the Terraflow API

With the Terraflow API, you can upload and manage files in Terraflow file management, and you can send files to flows programmatically at runtime.
...
To modify this value, change the `TERRAFLOW_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

## Use files in a flow

To use files in your Terraflow file management system in a flow, add a component that accepts file input to your flow, such as the **Read File** component.
...
3. Expand the **File** section, find the **Files** row, and then enable **Expose Input** to allow the parameter to be set at runtime through the Terraflow API.
...
5. When you run this flow programmatically, your script must upload a file to Terraflow file management, and then pass the returned `file_path` to the `path` tweak in the `/run` request:
...
Terraflow supports base64 images in the following formats:
...
You can upload images to the **Playground** chat interface and as runtime input with the Terraflow API.
...
* When you trigger a flow with the `/api/v1/run/$FLOW_ID` endpoint, you can use the `files` parameter to attach image data as a base64-encoded string:
   ...
   ```bash
   curl -X POST "http://$TERRAFLOW_SERVER_ADDRESS/api/v1/run/$FLOW_ID" \
...
Terraflow supports two storage backends for file management.

* **Local storage**: Terraflow's default storage backend. Files are stored locally in your [Terraflow configuration directory](/memory). Set `TERRAFLOW_STORAGE_TYPE=local` or leave it unset to use local storage.

* **S3 storage**: Files are stored in an AWS S3 bucket.
Terraflow uses the [boto3](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html) library to interact with S3.

To use S3 as your file storage backend, add the following configuration to your `.env` file:

```text
# S3 Storage Configuration
TERRAFLOW_STORAGE_TYPE=s3
TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME=S3_BUCKET_NAME
TERRAFLOW_OBJECT_STORAGE_PREFIX=S3_BUCKET_DIRECTORY
...
The following environment variables configure file storage backends for Terraflow's file management system:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `TERRAFLOW_STORAGE_TYPE` | String | `local` | Set the file storage backend. Supported values: `local` (files stored in the Terraflow configuration directory) or `s3` (files stored in AWS S3). For S3 storage, you must also configure AWS credentials and bucket settings. |
| `TERRAFLOW_OBJECT_STORAGE_BUCKET_NAME` | String | Not set | The name of the S3 bucket to use for file storage. Required when `TERRAFLOW_STORAGE_TYPE=s3`. |
| `TERRAFLOW_OBJECT_STORAGE_PREFIX` | String | Not set | Optional prefix/folder path within the S3 bucket where files will be stored. If not set, files are stored at the bucket root. |
| `TERRAFLOW_OBJECT_STORAGE_TAGS` | JSON object | Not set | Optional S3 object tags applied to stored files when `TERRAFLOW_STORAGE_TYPE=s3`. Ignored for local storage. Provided as a JSON map of string keys to string values, such as `{"env": "prod", "owner": "data-team"}`. |
