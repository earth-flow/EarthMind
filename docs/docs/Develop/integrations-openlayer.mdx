...
You can configure Terraflow to collect tracing data about your flow executions and automatically send the data to Openlayer for analysis, monitoring, and evaluation.
...
:::tip
If you need a flow to test the Openlayer integration, see the [Terraflow quickstart](/get-started-quickstart).
:::
...
3. Set your Openlayer credentials as environment variables in the same environment where you run Terraflow.
...
## Start Terraflow and view traces in Openlayer

1. Start Terraflow in the same environment where you set the Openlayer environment variables:

    ```bash
    uv run terraflow run
    ```

2. Run a flow in Terraflow.

    Terraflow automatically collects and sends tracing data about the flow execution to Openlayer, including:
...
```bash
export OPENLAYER_TERRAFLOW_MAPPING='{"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}'
```
...
```cmd
set OPENLAYER_TERRAFLOW_MAPPING={"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}
```
...
To disable the Openlayer integration, remove the `OPENLAYER_API_KEY` environment variable, and then restart Terraflow.
