...
You can configure EarthMind to collect tracing data about your flow executions and automatically send the data to Openlayer for analysis, monitoring, and evaluation.
...
:::tip
If you need a flow to test the Openlayer integration, see the [EarthMind quickstart](/get-started-quickstart).
:::
...
3. Set your Openlayer credentials as environment variables in the same environment where you run EarthMind.
...
## Start EarthMind and view traces in Openlayer

1. Start EarthMind in the same environment where you set the Openlayer environment variables:

    ```bash
    uv run earthmind run
    ```

2. Run a flow in EarthMind.

    EarthMind automatically collects and sends tracing data about the flow execution to Openlayer, including:
...
```bash
export OPENLAYER_EARTHMIND_MAPPING='{"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}'
```
...
```cmd
set OPENLAYER_EARTHMIND_MAPPING={"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}
```
...
To disable the Openlayer integration, remove the `OPENLAYER_API_KEY` environment variable, and then restart EarthMind.
