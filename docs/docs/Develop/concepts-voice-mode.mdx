...
:::warning
Voice mode is deprecated as of Terraflow 1.10.
...
Speech-to-text transcribes speech into the Playground's chat input field, but does not provide voice interaction with the Terraflow UI or text-to-speech responses.

The `api/v1/voice` WebSocket endpoints described below are still available.
:::

:::info
Voice mode is not available in Terraflow Desktop.
To use voice mode, [Install the Terraflow OSS Python package](/get-started-installation#install-and-run-the-terraflow-oss-python-package).
:::

You can use Terraflow's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because Terraflow uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. Terraflow saves the key as a [global variable](/configuration-global-variables).
...
Terraflow saves this key as a global variable.
...
Terraflow exposes an OpenAI Realtime API-compatible websocket endpoint for your flows.
...
The Terraflow API's websocket endpoint requires an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and supports an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the Terraflow **Playground**.
...
If omitted, Terraflow uses the flow ID as the [session ID](/session-id).
