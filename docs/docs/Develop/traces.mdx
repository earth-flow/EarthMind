...
Terraflow’s **Traces** feature records detailed execution traces for your flows and components so that you can debug issues, measure latency, and track token usage without relying on external observability services.

Trace data is stored in the Terraflow database in the `trace` and `span` tables.
...
To disable Terraflow tracing and use a different tracing provider, set `TERRAFLOW_NATIVE_TRACING` to `false`.
...
To view traces in the Terraflow UI, do the following:
