---
title: EarthMind database guide for enterprise DBAs
...
The EarthMind database stores data that is essential for more EarthMind operations, including startup, flow execution, user interactions, and administrative tasks.
The database supports both frontend (visual editor) and backend (API) operations, making its availability critical to EarthMind's stability and functionality.
For details about the database schema, see [Memory management options](/memory).

This guide is designed for enterprise database administrators (DBAs) and operators responsible for deploying and managing EarthMind in production environments.
It explains how to configure EarthMind to use PostgreSQL, including high availability (HA) and active-active configurations, as well as best practices for monitoring, maintenance, and security.

## Configure EarthMind with PostgreSQL

EarthMind's default database is SQLite.
However, PostgreSQL is recommended for production deployments due to its scalability, performance, and robustness.

The following steps explain how to configure EarthMind to use PostgreSQL for a standalone or containerized deployment.
...
1. Set up PostgreSQL:
   ...
   2. Create a database for EarthMind.
   3. Create a PostgreSQL user with appropriate, minimal permissions to manage and write to the database, such as CREATE, SELECT, INSERT, UPDATE, DELETE on your EarthMind tables.

2. Obtain the connection string in the format `postgresql://user:password@host:port/dbname`, such as`postgresql://earthmind:securepassword@postgres:5432/earthmind`.
...
3. Configure EarthMind with the `.env` or `docker-compose.yml` files.

    <Tabs groupId="environment">
    <TabItem value=".env" label=".env" default>

    1. Create a `.env` file in the `earthmind` directory:
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        EARTHMIND_DATABASE_URL="postgresql://earthmind:securepassword@postgres:5432/earthmind"
        ```

    For more environment variables, see the `.env.example` file in the EarthMind repository.

    </TabItem>
    <TabItem value="docker-compose.yml" label="docker-compose.yml">

    Use the sample `docker-compose.yml` from the EarthMind Repository.
...
4. Start EarthMind with your PostgreSQL connection:
...
5. Optional: Run migrations.
    ...
    EarthMind uses migrations to manage its database schema.
    When you first connect to PostgreSQL, EarthMind automatically runs migrations to create the necessary tables.

    Direct schema modification can cause conflicts with EarthMind's built-in schema management.
    If you need to update the schema, you can manually run migrations with the EarthMind CLI:

    1. Run `earthmind migration` to preview the changes.

    2. Review the changes to ensure that it's safe to proceed with the migration.

    3. Run `earthmind migration --fix` to run the migration and permanently apply the changes.

        This is a destructive operation that can delete data.
        For more information, see [`earthmind migration`](/configuration-cli#earthmind-migration).

6. To verify the configuration, create any flow using the EarthMind visual editor or API, and then query your database to confirm the tables and activity are recorded there. The content of the flow doesn't matter; you only need to confirm that the flow is stored in your PostgreSQL database.
You can query the database in two ways:

    * Query the database container:

        ```
        docker exec -it <postgres-container> psql -U earthmind -d earthmind
        ```

    * Use SQL:

        ```
        SELECT * FROM pg_stat_activity WHERE datname = 'earthmind';
        ```
...
This is recommended for production deployments to minimize downtime and ensure continuous operations if your database server fails, especially when multiple EarthMind instances rely on the same database.
...
If you have a multi-instance deployment, make sure all of your EarthMind instances connect to the same HA PostgreSQL database.

    The connection string you use depends on your HA configuration and services.

    * Use a virtual IP or DNS name that resolves to the current primary database, such as `postgresql://earthmind:securepassword@db-proxy:5432/earthmind?sslmode=require`.
    * Use the provided endpoint for a managed service, such as `earthmind.cluster-xyz.us-east-1.rds.amazonaws.com`.
...
4. Optional: Implement load balancing for read-heavy workloads:
    ...
    2. Configure EarthMind to use a single connection string pointing to the primary PostgreSQL database or proxy.

</TabItem>
<TabItem value="AA" label="Active-Active HA">

To implement Active-Active HA, you must deploy multiple EarthMind instances, use load balancing to distribute traffic across the instances, and ensure all instances connect to the same HA PostgreSQL database:

1. Deploy multiple EarthMind instances using Kubernetes or Docker Swarm.

    You must configure your instances to use a shared PostgreSQL database.
    For more information, see [Best practices for EarthMind on Kubernetes](/deployment-prod-best-practices).
...
Make sure all of your EarthMind instances connect to the same HA PostgreSQL database.
...
The following example configuration is for a production deployment that has three `earthmind-runtime` replicas, uses the Kubernetes load balancer service to distribute traffic to healthy pods, and uses the HA PostgreSQL database connection string.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: earthmind-runtime
spec:
  replicas: 3
  selector:
    matchLabels:
      app: earthmind-runtime
  template:
    metadata:
      labels:
        app: earthmind-runtime
    spec:
      ...
      - name: earthmind
        ...
        - containerPort: 7860
        env:
        - name: EARTHMIND_DATABASE_URL
          value: "postgresql://earthmind:securepassword@db-proxy:5432/earthmind?sslmode=require"
---
apiVersion: v1
kind: Service
metadata:
...
EarthMind, through [SQLAlchemy](https://docs.sqlalchemy.org/en/20/), supports reconnection attempts if `EARTHMIND_DATABASE_CONNECTION_RETRY=True`, ensures recovery after failover, and reduces disruption once the database is back online.
...
If the PostgreSQL database becomes unavailable, the following EarthMind functions will fail:
...
* [EarthMind architecture on Kubernetes](/deployment-architecture)
