...
In the EarthMind **Playground**, current sessions are listed on the left side of the pane.

EarthMind uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
Replace `EARTHMIND_SERVER_ADDRESS`, `FLOW_ID`, and `EARTHMIND_API_KEY` with the values from your EarthMind deployment.
```
curl --request POST \
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `earthmind.db` with their respective `session_id` values.

## Retrieval of messages from memory by session ID

To retrieve messages from local EarthMind memory, add a [**Message History** component](/message-history) to your flow.
...
Messages can be retrieved by `session_id` from the EarthMind API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.earthmind.org/api-monitor).

For an example of session ID in action, see [Use Session IDs in EarthMind](https://www.youtube.com/watch?v=nJiF_eF21MY).