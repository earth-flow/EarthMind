...
Terraflow provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your Terraflow instance.
The default storage location depends on your operating system and installation method:

- **Terraflow Desktop**:
    - **macOS**: `/Users/<username>/.terraflow/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.TerraflowDesktop\knowledge_bases`
- **Terraflow OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/terraflow/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/terraflow/knowledge_bases`

If you set the `TERRAFLOW_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `TERRAFLOW_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export TERRAFLOW_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
...
2. To connect Chroma Cloud to Terraflow, click **Settings**, and then click **DB Providers**.
...
5. Click **Save and Use Chroma Cloud**.
    ...
    Optionally, click **Test Connection** to verify that Terraflow can reach your Chroma Cloud instance before saving.

    The Chroma Cloud database is now connected to Terraflow as a knowledge base provider.
    To create a knowledge base using this provider, see [Create a knowledge base](#create-a-knowledge-base).

#### OpenSearch

To use OpenSearch as a database provider, you need a running OpenSearch cluster that is accessible to your Terraflow instance.
...
3. To connect the OpenSearch database to Terraflow as a knowledge base, click **Settings**, and then click **DB Providers**.
4. Select **OpenSearch**.
5. Enter the following values for the local OpenSearch container:
    ...
    - **Default Index name**: Enter `terraflow_knowledge`. The OpenSearch index to write and read from. This index is created in the later ingestion step, so it isn't immediately available.
...
6. Click **Save and Use OpenSearch**.
    ...
    Optionally, click **Test Connection** to verify that Terraflow can reach your OpenSearch cluster before saving.

    The OpenSearch database is now connected to Terraflow as a knowledge base provider.
