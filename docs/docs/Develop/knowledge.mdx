...
EarthMind provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your EarthMind instance.
The default storage location depends on your operating system and installation method:

- **EarthMind Desktop**:
    - **macOS**: `/Users/<username>/.earthmind/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.EarthMindDesktop\knowledge_bases`
- **EarthMind OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/earthmind/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/earthmind/knowledge_bases`

If you set the `EARTHMIND_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `EARTHMIND_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export EARTHMIND_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
...
2. To connect Chroma Cloud to EarthMind, click **Settings**, and then click **DB Providers**.
...
5. Click **Save and Use Chroma Cloud**.
    ...
    Optionally, click **Test Connection** to verify that EarthMind can reach your Chroma Cloud instance before saving.

    The Chroma Cloud database is now connected to EarthMind as a knowledge base provider.
    To create a knowledge base using this provider, see [Create a knowledge base](#create-a-knowledge-base).

#### OpenSearch

To use OpenSearch as a database provider, you need a running OpenSearch cluster that is accessible to your EarthMind instance.
...
3. To connect the OpenSearch database to EarthMind as a knowledge base, click **Settings**, and then click **DB Providers**.
4. Select **OpenSearch**.
5. Enter the following values for the local OpenSearch container:
    ...
    - **Default Index name**: Enter `earthmind_knowledge`. The OpenSearch index to write and read from. This index is created in the later ingestion step, so it isn't immediately available.
...
6. Click **Save and Use OpenSearch**.
    ...
    Optionally, click **Test Connection** to verify that EarthMind can reach your OpenSearch cluster before saving.

    The OpenSearch database is now connected to EarthMind as a knowledge base provider.
