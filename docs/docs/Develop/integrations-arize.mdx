...
To enable Arize tracing, set the required Arize environment variables in your Terraflow deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating Terraflow and Arize are also available in the Arize documentation:

* [Terraflow tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/terraflow/terraflow-tracing)
* [Terraflow tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/terraflow/terraflow-tracing)
:::
...
2. In the root of your Terraflow application, edit your existing Terraflow `.env` file or create a new one.
...
4. Start your Terraflow application with your `.env` file:
...
1. In Terraflow, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each Terraflow execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the Terraflow components.
