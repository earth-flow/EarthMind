...
Terraflow components are designed to accept and produce specific types of inputs and outputs.
...
:::tip
In Terraflow version 1.9.0, the `Data` type and port were renamed to `JSON`.
...
The schema is defined in [`data.py`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/schema/data.py).
...
:::tip
In Terraflow version 1.9.0, the `DataFrame` type and port were renamed to `Table`.
...
The schema is defined in [`dataframe.py`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/schema/dataframe.py).

The following attributes are available:

- **Full pandas compatibility**: All pandas DataFrame methods and functionality are supported

- **Terraflow integration**: Accepts lists of [`JSON` objects](#json), dictionaries, or existing DataFrames.
...
Because Terraflow is built on LangChain, `LanguageModel` is actually an instance of a [LangChain chat model](https://docs.langchain.com/oss/python/integrations/chat) that uses the configuration parameters set in the originating component.
...
The `Message` schema is defined in [`message.py`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/schema/message.py).
Some `Message` attributes have their own schema definitions, such as [`content_block.py`](https://github.com/terraflow-ai/terraflow/blob/main/src/backend/base/terraflow/schema/content_block.py).
...
When using the Terraflow API, the response includes the `Message` object along with other response data from the flow run.
Terraflow API responses can be extremely verbose, so your applications must include code to extract relevant data from the response to return to the user.
For an example, see the [Terraflow quickstart](/get-started-quickstart).
...
For more information, see [Configure tools for agents](/agents-tools) and [Use Terraflow as an MCP client](/mcp-client).
...
In Terraflow, you can use <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to view the output of individual components.
