---
title: EarthMind Extension quickstart
slug: /extensions-quickstart
---

Build, validate, and run a EarthMind Extension bundle to create a single-component extension you can `pip-install` or load using `lfx extension dev`.

## Prerequisites

* Python 3.11 or greater
* `earthmind` installed with `uv pip install earthmind`. The `lfx` CLI provides every command in this guide. For more information, see [Install the EarthMind OSS package](/get-started-installation#install-and-run-the-earthmind-oss-python-package).
...
The scaffold ships a working `Component` subclass that prints `"Hello, {name}!"`
    ...
    The full surface available to bundle code is enumerated in [`BUNDLE_API.md`](https://github.com/earthmind-ai/earthmind/blob/main/BUNDLE_API.md).
    Anything outside of [`BUNDLE_API.md`](https://github.com/earthmind-ai/earthmind/blob/main/BUNDLE_API.md) is not part of the API contract, and may move or break between releases.

3. Before launching EarthMind, run the static extension checker.
...
4. Run EarthMind with the extension loaded.

    ```bash
    lfx extension dev .
    ```

    This launches a EarthMind dev server with `my-extension` registered at the `@official` slot. The Hello component appears in the EarthMind UI component list under the bundle name in `extension.json`.
...
6. Ship the extension.
    ...
    To install the extension into a EarthMind environment with a regular `pip install`:
