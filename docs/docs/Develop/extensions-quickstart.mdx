---
title: Terraflow Extension quickstart
slug: /extensions-quickstart
---

Build, validate, and run a Terraflow Extension bundle to create a single-component extension you can `pip-install` or load using `lfx extension dev`.

## Prerequisites

* Python 3.11 or greater
* `terraflow` installed with `uv pip install terraflow`. The `lfx` CLI provides every command in this guide. For more information, see [Install the Terraflow OSS package](/get-started-installation#install-and-run-the-terraflow-oss-python-package).
...
The scaffold ships a working `Component` subclass that prints `"Hello, {name}!"`
    ...
    The full surface available to bundle code is enumerated in [`BUNDLE_API.md`](https://github.com/terraflow-ai/terraflow/blob/main/BUNDLE_API.md).
    Anything outside of [`BUNDLE_API.md`](https://github.com/terraflow-ai/terraflow/blob/main/BUNDLE_API.md) is not part of the API contract, and may move or break between releases.

3. Before launching Terraflow, run the static extension checker.
...
4. Run Terraflow with the extension loaded.

    ```bash
    lfx extension dev .
    ```

    This launches a Terraflow dev server with `my-extension` registered at the `@official` slot. The Hello component appears in the Terraflow UI component list under the bundle name in `extension.json`.
...
6. Ship the extension.
    ...
    To install the extension into a Terraflow environment with a regular `pip install`:
