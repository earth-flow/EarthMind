...
This page is the reference for the `extension.json` manifest the Terraflow Extension loader consumes.

The canonical JSON Schema lives at [`https://schemas.terraflow.org/extension/v1.json`](https://schemas.terraflow.org/extension/v1.json) and is generated from the Pydantic model at [`lfx.extension.manifest.ExtensionManifest`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/extension/manifest.py).
...
| `name` | string | yes | Human-readable display name shown in the Terraflow palette. 1-200 chars. |
...
The schema strips these names from the published `properties` map but reserves them via [`x-deferred-fields`](https://schemas.terraflow.org/extension/v1.json), so a manifest that sets one gets a specific error code instead of the generic "additional property" message.
...
If you'd rather not ship a separate `extension.json` next to `pyproject.toml`, declare the same fields under `[tool.terraflow.extension]`:
...
The loader and validator both emit typed errors keyed by the manifest field that triggered them. The full code list is at [`lfx.extension.errors.ERROR_CODES`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/extension/errors.py); the codes most relevant when authoring a manifest are:

| Code | Cause |
| --- | --- |
| `manifest-invalid` | Schema validation failed; the message names the field. |
| `manifest-not-found` | No `extension.json` and no `[tool.terraflow.extension]` section at the extension root. |
| `version-constraint-unsatisfied` | `lfx.compat` does not include this Terraflow's `BUNDLE_API_VERSION`. |
