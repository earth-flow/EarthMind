...
description: Instrument Terraflow with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your Terraflow application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your Terraflow application, edit your existing Terraflow `.env` file or create a new one.
...
## Start Terraflow with Traceloop environment variables

Launch your Terraflow application with your `.env` file:
...
1. Run a flow in Terraflow to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `Terraflow`.
4. Click **Terraflow** to view and analyze the associated calls.
