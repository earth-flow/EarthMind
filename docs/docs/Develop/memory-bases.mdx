...
A memory base stores the long-term chat history for a Terraflow agent in a vectorized format, enabling semantic retrieval of past conversations across sessions.
...
Unlike a [knowledge base](/knowledge), which you populate manually with files, a memory base fills itself from a flow's chat sessions, which are stored in Terraflow's [messages](/memory#store-chat-memory) table.
...
This section includes preprocessing prompts you can copy and paste directly into Terraflow.
...
- **Terraflow OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/terraflow/knowledge_bases`
...
To change the default directory, set `TERRAFLOW_KNOWLEDGE_BASES_DIR`:
