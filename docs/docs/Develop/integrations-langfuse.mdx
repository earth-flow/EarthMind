...
- A [running Terraflow server](/get-started-installation) with a [flow](/concepts-flows) that you want to trace

:::tip
If you need a flow to test the Langfuse integration, see the [Terraflow quickstart](/get-started-quickstart).
:::
...
2. Copy the following API key information:
    ...
    :::tip
    Terraflow previously used `LANGFUSE_HOST` as the variable for the Langfuse base URL.
...
4. Start Terraflow with the configuration in the `.env` file:
...
5. Run a flow.
    ...
    Terraflow automatically collects and sends tracing data about the flow execution to Langfuse.
...
To disable the Langfuse integration, remove the [Langfuse environment variables](#langfuse-credentials), and then restart Terraflow.

## Run Langfuse and Terraflow with Docker Compose
...
2. Copy the following API key information:
    ...
    If both values are set, then `TERRAFLOW_BASE_URL` is used.
    :::

3. Add your Terraflow credentials to your Terraflow `docker-compose.yml` file in the `environment` section.

    The following example is based on the [example `docker-compose.yml`](https://github.com/terraflow-ai/terraflow/blob/main/docker_example/docker-compose.yml).

    ```yml
    services:
      terraflow:
        image: terraflowai/terraflow:latest # or another version tag on https://hub.docker.com/r/terraflowai/terraflow
...
5. To confirm Langfuse is connected to your Terraflow container, run the following command:

    ```sh
    docker compose exec terraflow python -c "import requests, os; addr = os.environ.get('LANGFUSE_BASE_URL'); print(addr); res = requests.get(addr, timeout=5); print(res.status_code)"
    ```
