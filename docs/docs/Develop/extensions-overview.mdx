---
title: Terraflow Extensions overview
slug: /extensions-overview
---

import Icon from "@site/src/components/icon";

Terraflow ships component bundles that are maintained and released separately from the core Terraflow server as Extensions.
Instead of every integration being built into `terraflow`, each bundle is its own pip-installable package, such as `lfx-duckduckgo` or `lfx-arxiv`.

When you run `uv pip install terraflow`, you install:

* All Terraflow core components
* All bundles that ship as dependencies of the `terraflow` metapackage

Some bundles that were previously included in `terraflow` are moved into standalone packages.
The `terraflow` metapackage adds them as regular pip dependencies, so `pip install terraflow` continues to include them.

## Current Extension bundles

The following Extension bundles are included with `uv pip install terraflow`:
...
Terraflow rewrites these references automatically when you open a saved flow, so in most cases the migration is invisible.
...
If a bundle you need is not included in your Terraflow environment, install it with pip:
...
Terraflow discovers the package at server startup and loads the components into the palette automatically.
...
If you have custom components that you want to distribute and version independently of your Terraflow install, you can package them as an extension.

For more information, see [Build your first Terraflow Extension](./extensions-quickstart.mdx).