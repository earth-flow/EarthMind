...
Terraflow is an open-source, Python-based, customizable framework for building AI applications.
...
:::tip Try it
Build and run your first flow in minutes: [Install Terraflow](/get-started-installation), and then try the [Quickstart](/get-started-quickstart).
:::
...
<Cards cols={3}>
  ...
  <Card icon={<Icon name="Cloud" size={20} />} title="Deploy" href="/deployment-overview">
    Deploy Terraflow to production environments.
  </Card>
</Cards>

## Application development and prototyping

Terraflow can help you develop a wide variety of AI applications, such as chatbots, document analysis systems, content generators, and agentic applications.

Terraflow includes several pre-built templates that are ready to use or customize to your needs.

### Create flows in minutes

The primary purpose of Terraflow is to create and serve flows, which are functional representations of application workflows.

To [build a flow](/concepts-flows), you connect and configure component nodes. Each component is a single step in the workflow.

With Terraflow's [visual editor](/concepts-overview), you can drag and drop components to quickly build and test a functional AI application workflow.
...
![Basic Prompting flow in Terraflow](/img/workspace-basic-prompting.png)
...
You can use your flows as prototypes for more formal application development, or you can use the Terraflow API to embed your flows into your application code.

For more extensive development, you can build Terraflow as a dependency or deploy a Terraflow server to serve flows over the public internet.

<Cards cols={2}>
  <Card icon={<Icon name="Zap" size={20} />} title="Trigger flows with the API" href="/concepts-publish">
    Use the Terraflow API to run flows from your application code.
  </Card>
  <Card icon={<Icon name="Container" size={20} />} title="Containerize your app" href="/develop-application">
    Build and deploy Terraflow as a containerized application.
  </Card>
</Cards>

## Endless modifications and integrations

Terraflow provides [components](/concepts-components) that support many services, tools, and functionality that are required for AI applications.
...
<Cards cols={2}>
  <Card icon={<Icon name="Bot" size={20} />} title="Terraflow Agents" href="/agents">
    Build and configure AI agents with Terraflow.
  </Card>
  ...
  <Card icon={<Icon name="Plug" size={20} />} title="MCP server" href="/mcp-server">
    Expose Terraflow as an MCP server.
  </Card>
  <Card icon={<Icon name="Cable" size={20} />} title="MCP client" href="/mcp-client">
    Connect Terraflow to external MCP servers.
  </Card>
</Cards>

### Extensibility

In addition to the core components, Terraflow supports custom components.

You can use custom components developed by others, and you can develop your own custom components for personal use or to share with other Terraflow users.

<Cards cols={3}>
  <Card icon={<Icon name="GitPullRequest" size={20} />} title="Contribute" href="/contributing-how-to-contribute">
    Help build Terraflow.
  </Card>
...
* [Install Terraflow](/get-started-installation)
