...
Get started with Terraflow by loading a template flow, running it, and then serving it at the `/run` API endpoint.
...
- Create a [Terraflow API key](/api-keys-and-authentication)
    ...
    <details>
    <summary>Create a Terraflow API key</summary>

    A Terraflow API key is a user-specific token you can use with Terraflow.

    To create a Terraflow API key, do the following:

    1. In Terraflow, click your user icon, and then select **Settings**.
    2. Click **Terraflow API Keys**, and then click <Icon name="Plus" aria-hidden="true"/> **Add New**.
    3. Name your key, and then click **Create API Key**.
    4. Copy the API key and store it securely.
    5. To use your Terraflow API key in a request, set a `TERRAFLOW_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
    For example:
        ...
        # Send request
        curl --request POST \
        --url "http://TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID" \
...
3. In the **Agent** component, select your configured model from the **Language Model** dropdown.
    ...
    * Edit Terraflow's global <Icon name="BrainCog" aria-hidden="true" /> **Models** configuration. These providers and models are part of Terraflow's core functionality. Use the **Ollama** provider to connect to any model hosted on a local or remote Ollama instance.
...
Terraflow is an IDE, but it's also a runtime you can call through the [Terraflow API](/api-reference-api-examples) with Python, JavaScript, or HTTP.

When you start Terraflow locally, you can send requests to the local Terraflow server.
For production applications, you need to [deploy a stable Terraflow instance](/deployment-overview) to handle API calls.

For example, you can use the `/run` endpoint to run a flow and get the result.

Terraflow provides code snippets to help you get started with the Terraflow API.

1. When editing a flow, click **Share**, and then click **API access**.

    The default code in the API access pane constructs a request with the Terraflow server `url`, `headers`, and a `payload` of request data.
    The code snippets automatically include the `TERRAFLOW_SERVER_ADDRESS` and `FLOW_ID` values for the flow, and a script to include your `TERRAFLOW_API_KEY` if you've set it as an environment variable in your terminal session.
    ...
    url = "http://TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    fetch('http://TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID', options)
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from a Terraflow API response to use in your application.
...
This script runs a question-and-answer chat in your terminal and stores the agent's previous answer so you can compare them.
    ...
    const url = 'http://TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID';
...
To assist with formatting, you can define tweaks in Terraflow's **Input Schema** pane before copying the code snippet.
...
Terraflow updates the `tweaks` object in the code snippets based on your input parameters, and includes default values to guide you.
...
* [Use Terraflow as a Model Context Protocol (MCP) server](/mcp-server)
