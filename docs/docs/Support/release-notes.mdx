...
This page summarizes significant changes to Terraflow in each release.
For all changes, see the [Changelog](https://github.com/terraflow-ai/terraflow/releases/latest).

Due to strict SemVer requirements, Terraflow Desktop can have different patch versions than the core Terraflow OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::warning
Whenever possible, the Terraflow team recommends installing new Terraflow versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the Terraflow team recommends the following upgrade process:

1. Recommended: [Export your projects](/api-projects#export-a-project) to create backups of your flows:

    ```bash
    curl -X GET \
    "$TERRAFLOW_SERVER_URL/api/v1/projects/download/$PROJECT_ID" \
...
2. Install the new version:
   ...
   * **Terraflow OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the Terraflow OSS Python package](/get-started-installation#install-and-run-the-terraflow-oss-python-package).
   * **Terraflow Docker image**: Run the new image in a separate container, or upgrade your existing image. For more information, see [Upgrade the Terraflow Docker image](/deployment-docker#upgrade-the-terraflow-docker-image).
   * **Terraflow Desktop**: To upgrade in place, open Terraflow Desktop, and then click **Upgrade Available** in the Terraflow header. If you want to isolate the new version, you must install Terraflow Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
For all changes, see the [Changelog](https://github.com/terraflow-ai/terraflow/releases).
...
- Upcoming breaking change: bundle separation
    ...
    Terraflow 1.10.x introduces the Extension bundles model.
    ...
    For more information, see [Terraflow Extensions overview](../Develop/extensions-overview.mdx).

### New features and enhancements

- **Terraflow Assistant**: build complete flows

    **Terraflow Assistant** can now build entire flows, and not only individual components.

    For more information, see [Build flows and components with Terraflow Assistant](/terraflow-assistant).

- Redis-backed job queue for multi-worker deployments

    Terraflow now supports a Redis-backed job queue that allows flow build events to be shared across multiple Gunicorn/Uvicorn workers and across multiple replicas behind a load balancer.

    Single-process deployments are unaffected. The default `asyncio` in-memory queue is unchanged.

    For setup instructions and configuration details, see [Deploy Terraflow with multiple workers](./deployment-multi-worker).
...
- Python 3.14 support
    ...
    Terraflow now supports Python 3.10 through 3.14 on macOS, Linux, and Windows.
    The Terraflow Docker images now use Python 3.14.
...
- Extension bundles
    ...
    Terraflow 1.10 introduces Extension bundles: component providers that are packaged and versioned as standalone pip packages, independent of the core Terraflow server.
    All bundles are still included in `uv pip install terraflow`, so nothing changes for existing users.

    The following Extension bundles ship with Terraflow 1.10:
    ...
    To install a missing bundle, install its standalone package and restart Terraflow:
    ...
    Terraflow migrates saved flow references to the new format automatically when you open a flow.
...
- MCP server management lock
    ...
    Set `TERRAFLOW_MCP_SERVERS_LOCKED=true` to prevent non-superusers from adding, editing, or removing MCP server connections.
...
- Embedded mode UI flags
    ...
    Set `TERRAFLOW_EMBEDDED_MODE=true` to hide standalone UI elements when embedding the Terraflow visual editor in another application.
...
- Custom component admin-only restriction
    ...
    Set `TERRAFLOW_CUSTOM_COMPONENT_ADMIN_ONLY=true` to restrict custom component creation and code editing to superusers.
...
- Internationalization
    ...
    The Terraflow interface is now available in multiple languages.
...
- Login endpoint rate limiting
    ...
    Terraflow now applies IP-based rate limiting to the `/login` endpoint to protect against brute-force attacks.
...
For 1.9.x release notes, see the [1.9.x documentation](https://docs.terraflow.org/1.9.0/release-notes).

## 1.8.x

For 1.8.x release notes, see the [1.8.x documentation](https://docs.terraflow.org/1.8.0/release-notes).

## Earlier releases

See the [Changelog](https://github.com/terraflow-ai/terraflow/releases).
