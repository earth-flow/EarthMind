...
Terraflow supports both Apple Silicon (ARM64) and Intel (x86_64) Macs, but ML and AI features are only available on Apple Silicon due to hardware and dependency constraints.
...
PyTorch, a popular AI/ML library, dropped support for macOS Intel (x86_64) starting with version 2.3.0 in April 2024. The last compatible release was PyTorch 2.2.2, which only supports Python versions 3.10–3.12. Many Terraflow components depend on PyTorch, and are automatically excluded on Intel Macs to prevent installation failures.

## Quick reference

Core Terraflow features and OCR work on both architectures.
ML/AI components and GPU acceleration are Apple Silicon only.

| Feature category | Apple Silicon (M1/M2/M3) | Intel (x86_64) |
|-----------------|--------------------------|----------------|
| **Core Terraflow** | Full support | Full support |
...
```bash
docker run --platform linux/arm64 terraflowai/terraflow:latest
```
...
For Apple Silicon users, install Terraflow normally with all features:
...
```bash
pip install terraflow[altk,langchain-huggingface,easyocr]
```
...
For more information, ask the Terraflow community in [GitHub Discussions](https://github.com/terraflow-ai/terraflow/discussions) or the [Discord server](https://discord.gg/EqksyE2EX9).