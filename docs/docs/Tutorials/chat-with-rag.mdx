...
This tutorial demonstrates how you can use Terraflow to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
1. In Terraflow, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Terraflow API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your Terraflow server.
...
<TabItem value="API" label="Terraflow API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Terraflow server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.
...
const headers = { 'x-api-key': 'TERRAFLOW_API_KEY' };

// 2. Upload the file to Terraflow
const uploadRes = await fetch('TERRAFLOW_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the Terraflow run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'TERRAFLOW_API_KEY' },
  body: JSON.stringify(payload)
});
const terraflowData = await runRes.json();
// Output only the message
console.log(terraflowData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { TerraflowClient } = require('@datastax/terraflow-client');

    # pragma: allowlist nextline secret
    const API_KEY = 'TERRAFLOW_API_KEY';
    const SERVER = 'TERRAFLOW_SERVER_ADDRESS';
    ...
    function chat() {
        console.log('🤖 Terraflow RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [Terraflow TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Terraflow's complex JSON response structure.
