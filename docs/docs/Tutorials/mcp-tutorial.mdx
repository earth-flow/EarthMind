...
This tutorial shows you how to connect MCP servers to your applications using Terraflow's [**MCP Tools** component](/mcp-client).
...
You can run Terraflow as an MCP client and an MCP server:

* [Use Terraflow as an MCP client](/mcp-client): When run as an MCP client, an **Agent** component in a Terraflow flow can use connected components as tools to handle requests.
...
* [Use Terraflow as an MCP server](/mcp-server): When run as an MCP server, your flows become tools that can be used by an MCP client, which could be an external client or another Terraflow flow.

In this tutorial, you will use the Terraflow **MCP Tools** component to connect multiple MCP servers to your flow, and then you'll use a Python application to run your flow and chat with the agent programmatically.

## Prerequisites

* [Install and start Terraflow](/get-started-installation)
...
1. In Terraflow, click **New Flow**, and then select the **Simple Agent** template.
...
In Terraflow, you use the **MCP Tools** component to connect a specific MCP server to a flow.
...
1. For this tutorial, install a [weather MCP server](https://github.com/isdaniel/mcp_weather_server) on your local machine with uv and Python:
    ...
    Make sure you install the server in the same Python environment where Terraflow is running:

    * Terraflow in a virtual environment: Activate the environment before installing the server.
    * Terraflow Docker image: Install the server inside the Docker container.
    * Terraflow Desktop or system-wide Terraflow OSS: Install the server globally or in the same user environment where you run Terraflow.
...
4. In the **Add MCP Server** pane, provide the server startup command and arguments to connect the weather MCP server to your flow. For this tutorial, use either the **JSON** or **STDIO** option.
    ...
    Terraflow runs the command to launch the server when the agent determines that it needs to use a tool provided by that server.
...
Note that this tool returns the IP geolocation of your Terraflow server, so if your server is deployed remotely, consider alternative approaches for getting user-specific location data, such as browser geolocation APIs.
...
## Create a Python application that connects to Terraflow
...
In the last part of this tutorial, you'll learn how to use the Terraflow API to run a flow in a script.
...
When you use the Terraflow API to run a flow, you can change some aspects of the flow without changing the code.
For example, you can add more MCP servers to your flow in Terraflow, and then use the same script to run the flow.
...
1. For this tutorial's Python script, gather the following information:
    ...
    * `TERRAFLOW_SERVER_ADDRESS`: Your Terraflow server's domain. The default value is `127.0.0.1:7860`. You can get this value from the code snippets on your flow's [**API access** pane](/concepts-publish#api-access).
    ...
    * `TERRAFLOW_API_KEY`: A valid [Terraflow API key](/api-keys-and-authentication).
...
2. Copy the following script into a Python file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    url = "TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    Additionally, this script includes parsing code to extract the LLM's reply from the entire Terraflow API response.
    You will want to use similar extraction in your own applications because the Terraflow API response includes metadata and other information that isn't relevant to the reply passed to the user.
...
3.  Save and run the script to send the request and test the flow.
    ...
    Remember, the `ip_geolocation` tool used in this tutorial uses your Terraflow server's location, which can be different from your actual location.
...
* [Use Terraflow Agents](/agents)
* [Use Terraflow as an MCP server](/mcp-server)
* [Terraflow deployment overview](/deployment-overview)