...
The main focus of this tutorial is to show you how to provide files as input to a Terraflow flow, so your chatbot can use the content of those files in its responses.
...
1. In Terraflow, click **New Flow**, and then select the **Basic Prompting** template.
...
To do this, your application must send a `POST /run` request to your Terraflow server with the file you want to upload and a text prompt.
The result includes the outcome of the flow run and the LLM's response.

This example uses a local Terraflow instance, and it asks the LLM to evaluate a sample resume.
...
1. To construct the request, gather the following information:
    ...
    * `FILE_COMPONENT_ID`: The UUID of the **Read File** component in your flow, such as `File-KZP68`. To find the component ID, open your flow in Terraflow, click the **Read File** component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
...
2. Copy the following script into a Python file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    url = "http://TERRAFLOW_SERVER_ADDRESS/api/v2/files/"
    ...
    # 3. Upload the file to Terraflow
    ...
    # 5. Call the Terraflow run endpoint with the uploaded file path
    run_url = "http://TERRAFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID"
    run_payload = {
    ...
    terraflow_data = run_response.json()
    # Output only the message
    message = None
    try:
        message = terraflow_data['outputs'][0]['outputs'][0]['results']['message']['data']['text']
    ...
    The first request uploads a file, such as `fake-resume.txt`, to your Terraflow server at the `/v2/files` endpoint. This request returns a file path that can be referenced in subsequent Terraflow requests, such as `02791d46-812f-4988-ab1c-7c430214f8d5/fake-resume.txt`

    The second request sends a chat message to the Terraflow flow at the `/v1/run/` endpoint.
...
3. Save and run the script to send the requests and test the flow.
    ...
    The initial output contains the JSON response object from the file upload endpoint, including the internal path where Terraflow stores the file.
...
def chat_with_flow(input_message, file_paths):
    ...
    run_url = f"{TERRAFLOW_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}"
...
To upload files from another machine that isn't your local environment, your Terraflow server must first be accessible over the internet. Then, authenticated users can upload files to your public Terraflow server's `/v2/files/` endpoint, as shown in the tutorial. For more information, see [Terraflow deployment overview](/deployment-overview).
...
In the visual editor, you can preload files to the **Read File** component by selecting them from your local machine or [Terraflow file management](/concepts-file-management).
