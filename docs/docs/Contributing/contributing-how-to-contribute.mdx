...
This guide is intended to help you start contributing to Terraflow.
As an open-source codebase in a rapidly developing field, Terraflow welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to Terraflow, follow the [pull request guide](#open-a-pull-request).

## Terraflow services overview

This overview will help you understand how to set up your development environment.

Terraflow consists of two main services:
...
When you install Terraflow as an end user, the backend serves the frontend internally, making both services available on a single port (`7860` by default).

## Install Terraflow from source

Install Terraflow from source by forking the repository and setting up your development environment.

### Prerequisites

* [Python](https://www.python.org/downloads/) 3.10 to 3.14
...
### Clone the Terraflow repository
...
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/terraflow.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run Terraflow from source

You can run Terraflow from source after cloning the repository, even if you aren't contributing to the codebase.
This builds the frontend and serves it through the backend on port `7860`.

The instructions below are for running Terraflow from source. For development with hot reload, see [Set up development environment](#set-up-your-terraflow-development-environment).

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the Terraflow directory, and then run `make run_cli`.
...
The Terraflow frontend is served at `http://localhost:7860`.
</details>

<details>
<summary>Run from source with Windows CMD</summary>

To run Terraflow from source on Windows, you can use the Terraflow repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run Terraflow with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run Terraflow from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/terraflow/frontend`.
...
* To install and run Terraflow with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run Terraflow from a PowerShell terminal:
...
### Set up your Terraflow development environment
...
If you just want to run Terraflow locally without making code changes, see [Run Terraflow from source](#run-terraflow-from-source).

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the Terraflow development environment:
...
Since Windows doesn't include `make`, building and running Terraflow from source uses `npm` and `uv`.

To set up the Terraflow development environment, run the frontend and backend in separate terminals:
...
2. Fork the [Terraflow GitHub repository](https://github.com/terraflow-ai/terraflow).
...
4. From the root of your local Terraflow fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `terraflow/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `terraflow/docs/sidebars.js`.
...
* Pull request titles appear in Terraflow's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A Terraflow maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
