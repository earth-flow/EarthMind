...
It can store and retrieve chat messages from either [Terraflow storage](/memory) _or_ a dedicated chat memory database like Mem0 or Redis.

:::tip
The **Agent** component has built-in chat memory that is enabled by default and uses Terraflow storage.
...
* You want to store memories in a specific database that is separate from Terraflow storage.
...
* **Retrieve mode**: The component retrieves chat messages from your Terraflow database or external memory.
* **Store mode**: The component stores chat messages in your Terraflow database or external memory.
...
<Tabs>
<TabItem value="terraflow" label="Use Terraflow storage" default>

The following steps explain how to create a chat-based flow that uses **Message History** components to store and retrieve chat memory from your Terraflow installation's database:
...
4. Add a **Prompt Template** component, add a `{memory}` variable to the **Template** field, and then connect the **Message History** output to the **memory** input.
    ...
    Variables (`{variable}`) in the template dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from elsewhere, such as other components, Terraflow global variables, or runtime input.
...
| **External Memory** (`memory`) | External Memory | Input parameter. Retrieve messages from an external memory. If empty, Terraflow storage is used. |
...
| **External Memory** (`memory`) | External Memory | Input parameter. Store messages in external memory. If empty, Terraflow storage is used. |
