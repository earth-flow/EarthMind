...
<Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) contain custom components that support specific third-party integrations with Terraflow.
...
The following steps explain how to create a service account for Terraflow, create a service account JSON key file, and then use the JSON key file in Terraflow:
...
2. Create a [service account](https://cloud.google.com/iam/docs/service-account-overview) in your Google Cloud project.
    ...
    It is recommended that you [create a custom service account for Vertex AI](https://cloud.google.com/vertex-ai/docs/general/custom-service-account) because Terraflow uses this account to access the Vertex AI APIs.

3. Assign the **Vertex AI Service Agent** role to the service account.

    This role allows Terraflow to access Vertex AI resources.
    For more information, see [Vertex AI access control with IAM](https://cloud.google.com/vertex-ai/docs/general/access-control).

4. [Create a service account key](https://cloud.google.com/iam/docs/keys-create-delete#console) in JSON format for your Terraflow Vertex AI service account.

    When you click **Create**, the JSON key file is downloaded.

5. In Terraflow, create or open a flow with the **Vertex AI** or **Vertex AI Embeddings** component.
...
6. In the component's **Credentials** field, do one of the following:
    ...
    * Leave the **Credentials** field empty if you want to pull the key file from an environment variable.
    Then, you must provide the path to your JSON file in a `GOOGLE_APPLICATION_CREDENTIALS` environment variable that is set somewhere relative to your Terraflow instance.
    For example, you can set the environment variable in your terminal, your Terraflow `.env` file, or otherwise in the environment where your Terraflow server or application runs.

    ![Select a local JSON key file for the Vertex AI component](/img/configure-vertex-ai-credentials-in-terraflow.gif)

7. Run your flow to verify that Terraflow can use the service account credentials successfully.
