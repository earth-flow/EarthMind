...
Terraflow's **Agent** component is critical for building agent flows.
...
* [Terraflow quickstart](/get-started-quickstart): Start with the **Simple Agent** template, modify its tools, and then learn how to use an agent flow in an application.

    The **Simple Agent** template creates a basic agent flow with an **Agent** component that can use two other Terraflow components as tools.
...
* [Use Terraflow as an MCP client](/mcp-client) and [Use Terraflow as an MCP server](/mcp-server): Use the **Agent** and [**MCP Tools** component](/mcp-tools) to implement the Model Context Protocol (MCP) in your flows.
...
You can connect any Terraflow component as a tool, including other **Agent** components and MCP servers through the [**MCP Tools** component](/mcp-tools).

For more information about using this component, see [Use Terraflow agents](/agents).
