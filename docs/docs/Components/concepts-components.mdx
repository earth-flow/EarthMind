...
import RecursiveCharacterSource from "!!raw-loader!@terraflow/src/lfx/src/lfx/components/langchain_utilities/recursive_character.py";
...
:::tip
Terraflow provides keyboard shortcuts for the workspace.

In the Terraflow header, click your profile icon, select **Settings**, and then click **Shortcuts** to view the available shortcuts.
:::
...
* <Icon name="Component" aria-hidden="true" /> **Core components**: Terraflow's base components are grouped by purpose, such as **Inputs and Outputs** or **Data**.
...
For information about the programmatic representation of each data type, see [Terraflow data types](/data-types).
...
For example, the [**Recursive Character Text Splitter** component](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/components/langchain_utilities/recursive_character.py) is a child of the [`LCTextSplitterComponent`](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/src/lfx/base/textsplitters/model.py) class.
...
Component versions and states are stored in an internal Terraflow database. When you add a component to a flow, you create a detached copy of the component based on the information in the Terraflow database.
These copies are detached from the primary Terraflow database, and they don't synchronize with any updates that can occur when you upgrade your Terraflow version.
...
When editing a flow in the workspace, Terraflow notifies you if a component's workspace version is behind the database version so you can update the component's workspace version:
...
Components are updated to the latest available version, based on the version of Terraflow you are running.
