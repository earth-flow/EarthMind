...
Bundles contain custom components that support specific third-party integrations with EarthMind.
You add them to your flows and configure them in the same way as EarthMind's core components.
...
Many bundled components are developed by third-party contributors to the EarthMind codebase.
...
To find documentation for a specific bundled component, browse the EarthMind docs and your provider's documentation.
...
The EarthMind documentation focuses on using bundles within flows.
For that reason, it focuses on the EarthMind-specific configuration steps for bundled components.
...
:::tip
The EarthMind documentation doesn't list all bundles or components in bundles.
For the most accurate and up-to-date list of bundles and components for your version of EarthMind, check <Icon name="Blocks" aria-hidden="true" /> **Bundles** in the visual editor.

If you can't find a component that you used in an earlier version of EarthMind, it may have been removed or marked as a [legacy component](#legacy-bundles).
:::

EarthMind offers generic <Icon name="Component" aria-hidden="true" /> **Core components** in addition to third-party, provider-specific bundles.
...
The **Local DB** component reads and writes to a persistent, in-memory Chroma DB instance intended for use with EarthMind.
It has separate modes for reads and writes, automatic collection management, and default persistence in your EarthMind cache directory.
...
<!-- Not documented but in EarthMind as of 1.5.11 -->
<!--
* AgentQL
* Confluence
* Git
