...
1. Install the Agentics package in Terraflow's virtual environment:

   ```bash
   uv pip install agentics-py
   ```

2. Restart Terraflow so the Agentics components are available:
...
- **Errors:** If Agentics is not found, run `uv pip install agentics-py==0.3.1` and restart Terraflow. For API/key errors, set the provider’s API key as a global variable or env var. For DataFrame errors, ensure input is a list of dicts or use a DataFrame component output.
