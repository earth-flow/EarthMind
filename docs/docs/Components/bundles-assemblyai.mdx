...
* An [AssemblyAI account](https://www.assemblyai.com/dashboard/signup) and an AssemblyAI API key.
    ...
    Enter the key in the *AssemblyAI API Key* field in all Terraflow components that require the AssemblyAI key.
...
1. Build the flow manually or import a pre-build JSON file:
    ...
    * Recommended: [Download the AssemblyAI Transcription and Speech AI flow JSON](/files/AssemblyAI_Flow.json), and then [import the flow](/concepts-flows-import) into Terraflow.
