...
In EarthMind version 1.7.0, this component was renamed from **Save File** to **Write File**.

The **Write File** component creates a file containing data produced by another component.
Several file formats are supported, and you can store files in [EarthMind storage](/memory), AWS S3, Google Drive, or the local file system.
...
3. In **File Name**, enter a file name and an optional path.
    ...
    * **Default location**: If you only provide a file name, then the file is stored in the EarthMind data directory. For example,`~/Library/Caches/earthmind/data` on macOS.

    * **Subdirectory**: To store files in subdirectories, add the path to the **File Name** parameter.
    If a given subdirectory doesn't already exist, EarthMind automatically creates it.
...
4. Select the desired file format, and then click **Close**.
    ...
    * `Message` can be saved to TXT, JSON (default), or Markdown.
    ...
    EarthMind doesn't block overwrites if a matching file already exists.
...
5. To test the **Write File** component, click <Icon name="Play" aria-hidden="true" /> **Run component**, and then click <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to get the filepath where the file was saved.
    ...
    ```text
    DataFrame saved successfully as 'my_file.csv' at /Users/user.name/Library/Caches/earthmind/data/my_file.csv
    ```
