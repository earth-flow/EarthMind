...
Variables in a **Prompt Template** component dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from other components, Terraflow global variables, or fixed input.
...
5. Provide input for the variable fields:
   ...
   * Use Terraflow global variables.
...
Terraflow supports variable replacement with double brackets, but does not support the full Mustache engine.
