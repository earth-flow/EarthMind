...
Language model components in Terraflow generate text using a specified Large Language Model (LLM).
These components accept inputs like chat messages, files, and instructions in order to generate a text response.

Terraflow includes a **Language Model** core component that has built-in support for many LLMs.
...
1. <PartialGlobalModelProviders />
    ...
    If you want to use a provider or model that isn't built-in to Terraflow's global <Icon name="BrainCog" aria-hidden="true" /> **Models**, you can replace the **Language Model** component with any [additional language model component](#additional-language-models).
    ...
    Alternatively, you can use Ollama to host your preferred model, and then configure your Ollama service in Terraflow's global <Icon name="BrainCog" aria-hidden="true" /> **Models**.
...
10. Optional: Try a different model or provider to see how the response changes.
    ...
    If you enabled multiple models in Terraflow's global **Model Providers** pane, select a different model in the **Language Model** field. To open the **Model Providers** pane, click your profile icon, select **Settings**, and then click <Icon name="Brain" aria-hidden="true"/> **Model Providers**.
