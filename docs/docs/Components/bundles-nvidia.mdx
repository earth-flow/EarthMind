...
You can use the **NVIDIA** component to connect Terraflow with NVIDIA NIM on an RTX Windows system with [Windows Subsystem for Linux 2 (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install) installed.
The following example connects an **NVIDIA** language model component in Terraflow to a deployed `mistral-nemo-12b-instruct` NIM on an **RTX Windows system** with **WSL2**.
...
1. Prepare your system:
    ...
    * The **NVIDIA Retriever Extraction** component requires the installation of additional dependencies to your Terraflow environment. To install the dependencies in a virtual environment, run the following commands.

        * If you have the Terraflow repository cloned and installed from source:
        ```bash
        ...
        * If you are installing Terraflow from the Python Package Index:
        ```bash
        source **YOUR_TERRAFLOW_VENV**/bin/activate
        uv pip install --prerelease=allow 'terraflow[nv-ingest]'
        uv run terraflow run
        ```
...
1. Prepare your system:
    ...
    * It uses the `gassist.rise` package, which is installed with all Terraflow versions that include this component.
