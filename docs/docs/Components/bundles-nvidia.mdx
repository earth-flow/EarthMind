...
You can use the **NVIDIA** component to connect EarthMind with NVIDIA NIM on an RTX Windows system with [Windows Subsystem for Linux 2 (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install) installed.
The following example connects an **NVIDIA** language model component in EarthMind to a deployed `mistral-nemo-12b-instruct` NIM on an **RTX Windows system** with **WSL2**.
...
1. Prepare your system:
    ...
    * The **NVIDIA Retriever Extraction** component requires the installation of additional dependencies to your EarthMind environment. To install the dependencies in a virtual environment, run the following commands.

        * If you have the EarthMind repository cloned and installed from source:
        ```bash
        ...
        * If you are installing EarthMind from the Python Package Index:
        ```bash
        source **YOUR_EARTHMIND_VENV**/bin/activate
        uv pip install --prerelease=allow 'earthmind[nv-ingest]'
        uv run earthmind run
        ```
...
1. Prepare your system:
    ...
    * It uses the `gassist.rise` package, which is installed with all EarthMind versions that include this component.
