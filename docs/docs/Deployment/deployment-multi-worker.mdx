---
title: Deploy Terraflow with multiple workers
...
By default, Terraflow runs with a single worker process and stores build job state in memory.
...
A multi-worker deployment runs more than one worker process on the same host. Concurrency can be increased by increasing the number of `TERRAFLOW_WORKERS`, but each process keeps its own in-memory build queue unless you add a shared store.
...
To configure a multi-worker Terraflow process, follow the steps to enable the Redis job queue.

After the Redis job queue is configured, you can optionally follow [recommended Gunicorn settings](#recommended-gunicorn-settings) to reduce memory use and keep workers healthy. This tuning applies to Linux production hosts only. On Windows and macOS, `terraflow run` uses a single Uvicorn process.

## Prerequisites

- **Redis 6 or later**, reachable from all Terraflow worker processes.
- All workers configured with the same `TERRAFLOW_JOB_QUEUE_TYPE`. Mixed-mode deployments (some workers using `asyncio`, others using `redis`) are not supported.
...
```text
TERRAFLOW_WORKERS=3  # any value > 1
TERRAFLOW_JOB_QUEUE_TYPE=redis
TERRAFLOW_REDIS_QUEUE_URL=redis://your-redis-host:6379/1
```

Redis authentication and TLS are only supported through `TERRAFLOW_REDIS_QUEUE_URL`.
The individual host/port settings `TERRAFLOW_REDIS_QUEUE_HOST` and `TERRAFLOW_REDIS_QUEUE_PORT` create a plain, unauthenticated connection.
If you use a managed Redis service with auth or TLS, you must use `TERRAFLOW_REDIS_QUEUE_URL`.

## Example: multi-worker Docker Compose

This example runs three Terraflow workers sharing a Redis job queue and a PostgreSQL database.
...
1. Paste the following example into a Docker Compose file named `docker-compose.yml`:
   ...
   services:
     terraflow:
       image: terraflowai/terraflow:1.10.0
       pull_policy: always
       ports:
         - "7860:7860"
       depends_on:
       ...
       environment:
         - TERRAFLOW_DATABASE_URL=postgresql://terraflow:terraflow@postgres:5432/terraflow
         - TERRAFLOW_CONFIG_DIR=/app/terraflow
         - TERRAFLOW_WORKERS=3  # any value > 1
         - TERRAFLOW_GUNICORN_PRELOAD=true
         - TERRAFLOW_JOB_QUEUE_TYPE=redis
         - TERRAFLOW_REDIS_QUEUE_URL=redis://redis:6379/1
         - TERRAFLOW_SUPERUSER=admin
         - TERRAFLOW_SUPERUSER_PASSWORD=changeme
     ...
     postgres:
       ...
       environment:
         POSTGRES_USER: terraflow
         POSTGRES_PASSWORD: terraflow
...
3. Watch the logs until Terraflow is ready:

   ```bash
   docker compose logs -f terraflow
   ```
...
7. Open the Terraflow UI and build a flow by sending a message to the flow in the **Playground**.
...
|---|---|---|
| `TERRAFLOW_JOB_QUEUE_TYPE` | `asyncio` | Job queue backend. Set to `redis` to enable the cross-worker queue. |
| `TERRAFLOW_REDIS_QUEUE_URL` | Not set | Full Redis connection URL. Takes priority over `HOST`/`PORT`/`DB` when set. Use this for any Redis instance that requires authentication or TLS. |
| `TERRAFLOW_REDIS_QUEUE_HOST` | `TERRAFLOW_REDIS_HOST` | Redis host for the job queue. Falls back to the general Redis host setting. Does not support auth or TLS (use `TERRAFLOW_REDIS_QUEUE_URL` instead for secured instances). |
| `TERRAFLOW_REDIS_QUEUE_PORT` | `TERRAFLOW_REDIS_PORT` | Redis port for the job queue. Falls back to the general Redis port setting. |
| `TERRAFLOW_REDIS_QUEUE_DB` | `1` | Redis database index for the job queue. Must differ from the cache database index (default `0`) to avoid key collisions. |
| `TERRAFLOW_REDIS_QUEUE_TTL` | `3600` | TTL in seconds for job stream and ownership keys in Redis. |
| `TERRAFLOW_REDIS_QUEUE_STARTUP_GRACE_S` | `30.0` | Seconds a consumer waits for the producer's first write before treating a missing stream as end-of-stream. Increase this if your workers have slow cold-starts. Setting to `0` removes the grace period so a not-yet-created stream is treated as EOF immediately. |
| `TERRAFLOW_REDIS_QUEUE_CANCEL_CHANNEL_ENABLED` | `True` | When `true`, each worker runs a Redis pub/sub dispatcher so that `POST /build/{id}/cancel` cancels a build on any worker, not just the one that received the request. Closing a browser tab also signals cancel cross-worker. |
| `TERRAFLOW_REDIS_QUEUE_CANCEL_MARKER_TTL` | `60` | TTL in seconds for the cancel-marker key. The marker closes a race where a cancel signal is published before the target worker's dispatcher has subscribed. A non-positive value is rejected at startup. |
| `TERRAFLOW_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S` | `90.0` | Seconds without client activity before the watchdog cancels an abandoned polling build. Set to `0` to disable the watchdog entirely. |
| `TERRAFLOW_REDIS_QUEUE_POLLING_WATCHDOG_INTERVAL_S` | `15.0` | How often in seconds the watchdog scans for stale jobs. Lower values reclaim resources faster at the cost of more Redis reads. |
| `TERRAFLOW_GUNICORN_PRELOAD` | `False` | **Experimental.** Loads the app in the Gunicorn master process before workers fork, reducing per-worker startup overhead. Pairs well with `TERRAFLOW_WORKERS`. Non-Windows only. |
...
```bash
curl -H "Authorization: Bearer $TERRAFLOW_SUPERUSER_TOKEN" \
...
| `cancel_stats.polling_watchdog_kills` | Number of abandoned polling builds reclaimed by the watchdog. Non-zero is normal under load. A very high count may indicate frequent client disconnects (consider increasing `TERRAFLOW_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S`). |
...
On Windows and macOS, `terraflow run` uses one Uvicorn process, so `TERRAFLOW_GUNICORN_PRELOAD` and `GUNICORN_CMD_ARGS` have no effect.
:::

These recommendations are starting points from Terraflow engineering benchmarks on Linux multi-worker deployments.
After you apply these values, monitor performance, and then make adjustments using `htop` or `btop` while you run flows.

Add the starter values to your server's `.env` file after the [Redis job queue](#enable-the-redis-job-queue) settings, then restart Terraflow. Copy the block that best matches your server:

<Tabs>
<TabItem value="small" label="Small / dev (4–8 GB RAM)" default>

Start with fewer workers so Terraflow and local databases do not hit OOM.

```text
TERRAFLOW_WORKERS=5
TERRAFLOW_WORKER_TIMEOUT=120
TERRAFLOW_GUNICORN_PRELOAD=true
...
A balanced default for a dedicated Terraflow host.

```text
TERRAFLOW_WORKERS=15
...
```text
TERRAFLOW_WORKERS=30
TERRAFLOW_WORKER_TIMEOUT=600
TERRAFLOW_GUNICORN_PRELOAD=true
...
- **`TERRAFLOW_WORKERS`** — How many Gunicorn worker processes run for concurrency.
- **`TERRAFLOW_WORKER_TIMEOUT`** — How long a worker may handle a single request before Gunicorn kills it. Raise it if you expect long agent runs.
- **`TERRAFLOW_GUNICORN_PRELOAD`** — Loads the app once in the Gunicorn master before workers fork so Linux can share memory across workers through Copy-on-Write. Recommended to leave enabled in multi-worker deployments for memory savings. Safe to leave off; behavior matches older releases when `false`.
...
For more information, see the [Scaling Terraflow blog post](https://www.terraflow.org/blog/scaling-terraflow).