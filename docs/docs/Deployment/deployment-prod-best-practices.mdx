---
title: Best practices for Terraflow on Kubernetes
slug: /deployment-prod-best-practices
---

This guide provides best practices for deploying Terraflow in production environments on Kubernetes.

## Resources and scaling

Terraflow's minimum resource requirements vary by deployment type:

* **IDE (development)**: Deploy both the Terraflow visual editor (frontend) and API (backend). Typically, this is used for development environments where developers use the visual editor to create and manage flows before packaging and serving them through a production runtime deployment.
...
* **Runtime (production)**: Deploy the Terraflow runtime for production flows, which is headless (backend only) service focused on serving the Terraflow API. This is used for production environments where flows are executed programmatically without the need for the visual editor.

    Minimum requirements include 2Gi RAM and 1000m (1 CPU) per instance with 3 replicas.

For more information about Terraflow deployment types, see [Terraflow architecture on Kubernetes](/deployment-architecture).
...
* Storage requirements for cache, file management, and the Terraflow database.
...
* Shared storage, such as NFS or cloud storage, for multi-instance deployments to access large files stored on disk, such as in `/opt/terraflow/data/`.

For more information, see [Configure an external PostgreSQL database](/configuration-custom-database) and [Terraflow database guide for enterprise DBAs](/enterprise-database-guide).
...
The following example shows a Terraflow HPA configuration with CPU-based scaling:

```yaml
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: terraflow-runtime-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
...
Terraflow's reliability in production depends on mitigating key failure points, particularly around the database, file system, and instance availability:

* **Database failures**: See [Terraflow database guide for enterprise DBAs](/enterprise-database-guide).
...
Effective monitoring ensures Terraflow operates reliably and performs well under varying loads:

* **Database monitoring**: See [Terraflow database guide for enterprise DBAs](/enterprise-database-guide).
* **Application logs**: Collect and analyze logs for errors, warnings, and flow execution issues. Centralize logs using tools like ELK Stack or Fluentd. You can also inspect [Terraflow logs](/logging).
* **Resource usage**: Track CPU, memory, and disk usage of Terraflow instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.

    To expose your Terraflow server's Prometheus metrics, set `TERRAFLOW_PROMETHEUS_ENABLED=True` (the default is `false`).
    The default port for the Prometheus metrics is 9090.
    To change the port, set `TERRAFLOW_PROMETHEUS_PORT`.
...
Running Terraflow in production requires robust security measures to protect the application, data, and users.
Follow industry best practices and use secure Terraflow configurations, such as the following:
...
* **Authentication, authorization, and access control**: Start your Terraflow server with authentication enabled, as explained in [API keys and authentication](/api-keys-and-authentication). Restrict network and resource access with firewalls, network policies, network security groups, or VPCs. For example, restrict PostgreSQL database access to Terraflow instances.
