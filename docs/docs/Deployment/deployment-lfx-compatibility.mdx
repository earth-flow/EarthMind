---
title: LFX and Terraflow version compatibility
slug: /lfx-compatibility
---

Terraflow and LFX are versioned together on the same `major.minor` line, so you can always tell which LFX release runs which flows.
...
The patches `.N` for LFX and `.M` for Terraflow are released independently. An LFX patch fix does not require a Terraflow patch release, and vice versa.

When LFX and a flow share the same `major` version but differ on `minor`, such as a flow exported from Terraflow 1.9.x run with LFX 1.10.x,  individual components may have been updated since the flow was saved.
...
| Terraflow version | Compatible LFX version | Notes |
|---|---|---|
...
| Any 1.x.x | 0.5.x | LFX 0.5.x was a standalone release before version alignment. It is no longer compatible with Terraflow 1.10+ flows. |
...
Pin `lfx~=1.10.0` in `requirements.txt` to track all patch releases for a given Terraflow minor without crossing into the next minor automatically.
...
LFX was realigned from its standalone `0.5.x` line onto Terraflow's `major.minor` line, so the version number jumps from `0.5.0` to `1.10.0` in one release. This is a version-numbering change, not 95 minor releases of new features.
