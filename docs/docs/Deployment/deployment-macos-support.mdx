...
EarthMind supports both Apple Silicon (ARM64) and Intel (x86_64) Macs, but with different feature availability due to hardware capabilities and upstream dependency support.
...
Install EarthMind normally with all features:
...
- Deploy EarthMind on a cloud instance with GPU support
- Use the EarthMind API to connect from your Intel Mac
...
EarthMind will continue to support core functionality on Intel Macs as long as Python and essential dependencies remain available, but ML features will remain Apple Silicon exclusive.
...
2. **Solution** - Use core EarthMind features or API-based alternatives
...
If EarthMind feels slow on Intel Mac:
...
- [GitHub Discussions](https://github.com/earthmind-ai/earthmind/discussions)
