...
Terraflow supports both Apple Silicon (ARM64) and Intel (x86_64) Macs, but with different feature availability due to hardware capabilities and upstream dependency support.
...
Install Terraflow normally with all features:
...
- Deploy Terraflow on a cloud instance with GPU support
- Use the Terraflow API to connect from your Intel Mac
...
Terraflow will continue to support core functionality on Intel Macs as long as Python and essential dependencies remain available, but ML features will remain Apple Silicon exclusive.
...
2. **Solution** - Use core Terraflow features or API-based alternatives
...
If Terraflow feels slow on Intel Mac:
...
- [GitHub Discussions](https://github.com/terraflow-ai/terraflow/discussions)
