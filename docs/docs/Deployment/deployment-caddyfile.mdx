---
title: Deploy Terraflow on a remote server
slug: /deployment-caddyfile
---

Learn how to deploy Terraflow on your own remote server with secure web access.
This guide walks you through setting up Terraflow on a remote server using [Docker](https://docs.docker.com/) and configuring secure web access with [Caddy](https://caddyserver.com/docs/).
...
## Deploy Terraflow on your server
...
3. Add the following values to `docker-compose.yml`, and then save the file.
    ...
    The following example defines the Terraflow service from the `terraflow:latest` image and a Caddy service to expose Terraflow through a reverse proxy.

    :::tip
    The [host-terraflow](https://github.com/datastax/host-terraflow) repository offers pre-built copies of this `docker-compose.yml` and `Caddyfile`, if you prefer to fork the repository to your server.
    :::
...
5. Add the following values to `Caddyfile`, and then save the file.
    ...
    This Caddyfile configures Caddy to listen on port 80, and forward all incoming requests to port 80 to the Terraflow service at port 7860.

    ```
    :80 {
        reverse_proxy terraflow:7860
    }
    ```

6. To deploy your server, run `docker-compose up`.

    When the `Welcome to Terraflow` message appears, Terraflow is running and accessible internally at `http://0.0.0.0:7860` inside the Docker network.

7. To access your Terraflow server over the public internet, navigate to your server's public IP address, such as `http://5.161.250.132`.
...
8. Recommended: Enable HTTPS:
    ...
    3. Modify your Caddyfile to include port `443` so Caddy can forward both HTTP (port 80) and HTTPS (port 443) requests to the Terraflow service:
...
To package your local flows into a custom Docker image, see [Containerize a Terraflow application](/develop-application).

For a step-by-step guide to deploying Terraflow, including deployments to [fly.io](https://fly.io/) and [Flightcontrol.dev](https://www.flightcontrol.dev/), see [How to Host Terraflow Anywhere](https://www.youtube.com/watch?v=q4qt5hSnte4).