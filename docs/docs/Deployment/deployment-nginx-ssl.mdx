---
title: Deploy Terraflow with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy Terraflow on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your Terraflow server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install Terraflow on your server

To install Terraflow on your server, do the following:
...
3. Install uv to manage Python packages.
    Terraflow recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for Terraflow.
    ```bash
    uv venv terraflow-venv
    source terraflow-venv/bin/activate
    ```

5. Install Terraflow using uv.
    ```bash
    uv pip install terraflow
    ```

6. Optionally, start Terraflow.
    ```bash
    uv run terraflow run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since Terraflow is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your Terraflow server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
Replace **DOMAIN_NAME** with your actual domain name, such as `terraflow.example.com`.
    ```bash
...
4. Paste the following content to the configuration file you created at `/etc/nginx/sites-available/DOMAIN_NAME`.
    ...
    server {
        ...
        location / {
            ...
            # WebSocket support for Terraflow
...
3. Start Terraflow from the virtual environment.
    ...
    b. Start Terraflow in the background.
    ```bash
    uv run terraflow run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy Terraflow on a remote server with Caddy](/deployment-caddyfile)
