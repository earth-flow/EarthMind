...
Terraflow's **Playground** is a dynamic interface you can use to test your LLM-based flows in real-time.
...
![The Terraflow visual builder with the Playground active](/img/playground.png)
...
For flows that require another type of input, such as a webhook event, file upload, or text input, you can [use the Terraflow API to trigger the flow](/api-flows-run), and then open the **Playground** to review the LLM activity for the flow run, if applicable.
:::
...
**Editing message logs edits Terraflow's internal `messages` table, which is the default chat memory storage.**
For more information about managing sessions and chat memory in Terraflow, see [Use custom session IDs](#session-ids) and [Memory management options](/memory).
...
When you trigger a flow with the Terraflow API, include the `session_id` parameter in the request payload.
For example:
...
The **Playground** isn't meant for embedding flows in applications. For information about running flows in applications or websites, see [Trigger flows with the Terraflow API](/concepts-publish).

The **Shareable Playground** isn't available in Terraflow Desktop.
:::

The **Shareable Playground** option exposes the **Playground** for a single flow at the `/public_flow/$FLOW_ID` endpoint.

After you [deploy a public Terraflow server](/deployment-overview), you can share this public URL with another user to allow them to access the specified flow's **Playground** only.
The user can interact with the flow's chat input and output and view the results without installing Terraflow or generating a Terraflow API key.

To share a flow's **Playground** with another user, do the following:

1. In Terraflow, open the flow you want share.
