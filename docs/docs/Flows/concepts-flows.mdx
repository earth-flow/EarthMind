...
Terraflow flows are fully serializable and can be saved and loaded from the file system where Terraflow is installed.

:::tip
To try building and running a flow in a few minutes, see the [Terraflow quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the Terraflow codebase.
...
You can also create a flow with the [Terraflow API](/api-flows), but the Terraflow team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use Terraflow for application development, learn how to [trigger flows with the Terraflow API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your Terraflow application](/develop-application).

When you're ready to go to production or deploy a Terraflow MCP server for access over the public internet, see [Terraflow deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, Terraflow builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch Terraflow.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

Terraflow projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or Terraflow icon in the Terraflow header.
:::
...
Saved flow versions are stored in the database configured for your Terraflow deployment.

When restoring a saved version, if **Save current draft before restoring** is enabled, then Terraflow creates a backup of your current work before the selected version replaces it.
...
When previewing a saved version, Terraflow temporarily pauses auto-save so the preview doesn't overwrite your current draft.
...
By default, flows and flow execution data are stored in the Terraflow database, and flow logs are stored with other Terraflow logs in the Terraflow config directory.
