...
1. In Terraflow, open the flow where you want to use the **Webhook** component.
...
7. Send a POST request with `data` to the flow's `webhook` endpoint to trigger the flow.
    ...
    A successful response indicates that Terraflow started the flow.
...
To learn about triggering flows with payloads from external applications, see the video tutorial [How to Use Webhooks in Terraflow](https://www.youtube.com/watch?v=IC1CAtzFRE0).

## Require authentication for webhooks {#require-authentication-for-webhooks}

By default, webhooks require API key authentication (`TERRAFLOW_WEBHOOK_AUTH_ENABLE=True`).

To allow webhooks to run without authentication (not recommended; use only in trusted environments), in your Terraflow `.env` file, set `TERRAFLOW_WEBHOOK_AUTH_ENABLE=False`. When disabled, requests to the webhook endpoint are treated as being sent by the flow owner.

When webhook authentication is enabled, you must provide a Terraflow API key with each webhook request.

To include the API key with a request as an HTTP header:
```bash
curl -X POST "http://TERRAFLOW_SERVER_ADDRESS/api/v1/webhook/FLOW_ID" \
    -H "Content-Type: application/json" \
    -H "x-api-key: TERRAFLOW_API_KEY" \
    -d '{"id": "12345", "name": "alex", "email": "alex@email.com"}'
```
...
```bash
curl -X POST "http://TERRAFLOW_SERVER_ADDRESS/api/v1/webhook/FLOW_ID?x-api-key=TERRAFLOW_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"id": "12345", "name": "alex", "email": "alex@email.com"}'
```

Replace `TERRAFLOW_SERVER_ADDRESS`, `FLOW_ID`, and `TERRAFLOW_API_KEY` with values from your deployment.
...
- [Get started with the Terraflow API](/api-reference-api-examples)
