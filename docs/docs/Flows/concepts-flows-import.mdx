...
You can export flows to transfer them between Terraflow instances, share them with others, or create backups.
...
* **Export with the Terraflow API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [Terraflow JSON file contents](#terraflow-json-file-contents).
...
If your key is stored in a Terraflow global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another Terraflow instance, that instance must have Terraflow global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import Terraflow JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the Terraflow JSON file to import.
* **Import anywhere**: Drag and drop Terraflow JSON files from your file explorer into your Terraflow window to import a flow from any Terraflow page.
* **Import with the Terraflow API**: To import one Terraflow JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of Terraflow JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your Terraflow instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## Terraflow JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

Terraflow JSON files contain the following:
...
For example Terraflow JSON files, you can examine any of the https://github.com/terraflow-ai/terraflow/tree/main/src/backend/base/terraflow/initial_setup/starter_projects[templates in the Terraflow repository], or you can create a flow from a template in Terraflow, export it, and then open the exported JSON file in a text editor.
