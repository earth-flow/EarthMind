A EarthMind knowledge base is a vector database that stores embeddings for use in your flows.
...
* **Local storage by default**: EarthMind knowledge bases use Chroma local storage by default.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: EarthMind knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying EarthMind knowledge bases, only standard similarity search is supported.
...
* **Structured data**: EarthMind knowledge bases only support structured data.
