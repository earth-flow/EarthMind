...
Terraflow's [**Agent** component](/components-agents) is critical for building agent flows.
...
The following steps explain how to create an agent flow in Terraflow from a blank flow.
For a prebuilt example, use the **Simple Agent** template or the [Terraflow quickstart](/get-started-quickstart).
...
To continue building on this tutorial, try connecting other tool components or [use Terraflow as an MCP client](/mcp-client) to support more complex and specialized tasks.
...
An **Agent** component can use any Terraflow component as a tool, including other agents and MCP servers.
...
Terraflow agents have built-in chat memory that is enabled by default.
...
By default, the **Agent** component uses your Terraflow installation's storage, and it retrieves a limited number of chat messages, which you can configure with the **Number of Chat History Messages** parameter.
...
Terraflow sends events to the Playground during each agent run: the input message, each tool call with its input and result, streaming tokens as they arrive, and the final answer.
Terraflow writes the completed message to chat history when the run finishes.
When using the **Structured Response** output, Terraflow does not send events to the Playground and does not write to chat history.
