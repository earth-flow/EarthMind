---
title: Use EarthMind as an MCP client
slug: /mcp-client
---
...
This page describes how to use EarthMind as an MCP client with the [**MCP Tools** component](#use-the-mcp-tools-component) and connected [MCP servers](#manage-connected-mcp-servers).

For information about using EarthMind as an MCP server, see [Use EarthMind as an MCP server](/mcp-server).

## Use the MCP tools component {#use-the-mcp-tools-component}

The **MCP Tools** component connects to an MCP server so that a [EarthMind agent](/agents) can use the server's tools when responding to user queries.

Register MCP servers, and then add **MCP Tools** from the <McpIcon /> **MCP** sidebar as described in [Connect to a non-EarthMind MCP server](#mcp-stdio-mode).

This component has two modes, depending on the type of server you want to access:

* [Connect to a non-EarthMind MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or HTTP/SSE URL to access tools provided by external, non-EarthMind MCP servers.
* [Connect to a EarthMind MCP server](#mcp-http-mode) to use flows from your [EarthMind projects](/concepts-flows#projects) as MCP tools.

### Connect to a non-EarthMind MCP server {#mcp-stdio-mode}
...
4. In the **MCP Server** field on the **MCP Tools** component, select the server you registered.
    ...
    * **HTTP/SSE**: Enter your MCP server's **Name**, **URL**, and any **Headers** and **Environment Variables** the server uses, and then click **Add Server**.
    The default **URL** for EarthMind MCP servers is `http://localhost:7860/api/v1/mcp/project/PROJECT_ID/streamable` or `http://localhost:7860/api/v1/mcp/streamable`. For more information, see [Connect to a EarthMind MCP server](#mcp-http-mode).
...
5. To configure headers for your MCP server, enter each header in the **Headers** fields as key-value pairs.
    ...
    If this server is nested inside a EarthMind flow that runs as an MCP server, you can propagate the incoming request's `x-api-key` or `Authorization` header directly to this server at runtime.
...
### Connect a EarthMind MCP server {#mcp-http-mode}

Every EarthMind project runs a separate MCP server that exposes the project's flows as MCP tools.
For more information about your projects' MCP servers, including exposing flows as MCP tools, see [Use EarthMind as an MCP server](/mcp-server).

EarthMind MCP servers support both the **streamable HTTP** transport and **Server-Sent Events (SSE)** as a fallback.

To leverage flows-as-tools, register your EarthMind MCP endpoint as a server first, and then add the **MCP Tools** component from the <McpIcon /> **MCP** sidebar.

1. Register the EarthMind MCP server.
...
2. In the **MCP URL** field, enter your EarthMind server's MCP endpoint.
   ...
   - Default for EarthMind Desktop: `http://localhost:7868/`

   All flows available from the targeted server are treated as tools. For authentication and project URLs, see [Use EarthMind as an MCP server](/mcp-server).
...
To add a new MCP server, click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**, register the server, and then select the server on the **MCP Tools** component as described in [Connect to a non-EarthMind MCP server](#mcp-stdio-mode).

Click <Icon name="Ellipsis" aria-hidden="true"/> **More** to edit or delete an MCP server connection.

If your EarthMind server administrator has locked MCP server management, you will receive a locked message when you try to add or modify servers. For more information, see [Lock MCP server management](/mcp-server#restrict-mcp-server-management).

## Modify MCP server environment variables with the API {#mcp-api-tweaks}

You can modify MCP server environment variables at runtime when running flows through the [EarthMind API](/api-reference-api-examples) by tweaking the **MCP Tools** component.

You can include tweaks with any EarthMind API request that supports the `tweaks` parameter, such as POST requests to the `/run` or `/webhook` endpoints.
...
3. Send a POST request to the EarthMind server's `/run` endpoint, and include tweaks to the **MCP Tools** component.
    ...
    EARTHMIND_SERVER_ADDRESS = "http://localhost:7860"
    FLOW_ID = "your-flow-id"
    EARTHMIND_API_KEY = os.getenv("EARTHMIND_API_KEY")
    MCP_TOOLS_COMPONENT_ID = "MCPTools-Bzahc"

    url = f"{EARTHMIND_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}?stream=false"
    headers = {
    ...
    ```typescript
    const EARTHMIND_SERVER_ADDRESS = "http://localhost:7860";
    ...
    const url = `${EARTHMIND_SERVER_ADDRESS}/api/v1/run/${FLOW_ID}?stream=false`;
    ...
    ```bash
    curl --request POST \
      --url "http://EARTHMIND_SERVER_ADDRESS/api/v1/run/FLOW_ID?stream=false" \
    ...
    Replace `MCP_TOOLS_COMPONENT_ID`, `EARTHMIND_API_KEY`, `EARTHMIND_SERVER_ADDRESS`, and `FLOW_ID` with the actual values from your EarthMind deployment.

    EarthMind doesn't automatically discover or expose which environment variables your MCP server accepts from the **MCP Tools** component.
...
Enter a global variable name as the header value, and EarthMind resolves the global variable name to its actual value before making the MCP server request. EarthMind only passes the token value to your server; it doesn't validate tokens on behalf of your MCP server.
...
5. To manage MCP server connections for your EarthMind client, click <McpIcon /> **MCP servers**, and then click <Icon name="ArrowUpRight" aria-hidden="true"/> **Manage Servers**, or click your profile icon, select **Settings**, and then click **MCP Servers**.
...
8. Click **Create Server**.
   ...
   If the connection succeeds, EarthMind shows the number of tools exposed by the server.
...
12. Test your flow to make sure the agent uses your server to respond to queries. Open the **Playground**, and then enter a prompt that uses a tool that you connected through the **MCP Tools** component.
    ...
    EarthMind automatically resolves `TEST_BEARER_TOKEN` to its actual value before sending the request to the MCP server. When your MCP server receives the request, the `Authorization` header contains the resolved token value.

## Propagate `x-api-key` to nested MCP servers {#propagate-x-api-key}

When EarthMind runs as an MCP server and a flow contains an [**MCP Tools** component](/mcp-tools) that calls an external server, you can forward the outer client's `x-api-key` or `Authorization` header at runtime.

In the nested server's **Headers** config, set the key and value to the same header name, such as `x-api-key` and `x-api-key`.
EarthMind reads the matching header from the incoming request and substitutes it before calling the nested server.

If the incoming request does not include the configured header, the literal string is passed through unchanged.

## See also

- [EarthMind MCP Client](./earthmind-mcp-client.mdx)
- [Use EarthMind as an MCP server](/mcp-server)
