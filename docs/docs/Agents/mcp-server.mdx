---
title: Use EarthMind as an MCP server
slug: /mcp-server
---
...
EarthMind integrates with the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) as both an MCP server and an MCP client.

This page describes how to use EarthMind as an MCP server that exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use when generating responses.

EarthMind MCP servers support both the **streamable HTTP** transport and **Server-Sent Events (SSE)** as a fallback.
The default project MCP server configuration uses streamable HTTP transport at the URL path `/streamable`.

For information about using EarthMind as an MCP client and managing MCP server connections within flows, see [Use EarthMind as an MCP client](/mcp-client).

## Prerequisites

* A [EarthMind project](/concepts-flows#projects) with at least one flow that has a [**Chat Output** component](/chat-input-and-output).
...
* [ngrok installed](https://ngrok.com/docs/getting-started/#1-install-ngrok) and an [ngrok authtoken](https://dashboard.ngrok.com/get-started/your-authtoken) if you want to [deploy a public EarthMind server](/deployment-public-server).

## Serve flows as MCP tools {#select-flows-to-serve}

When you create a [EarthMind project](/concepts-flows#projects), EarthMind automatically adds the project to your MCP server's configuration and makes the project's flows available as MCP tools.

If your EarthMind server has authentication enabled (`AUTO_LOGIN=false`), the project's MCP server is automatically configured with API key authentication, and a new API key is generated specifically for accessing the new project's flows.
For more information, see [MCP server authentication](#authentication).


### Prevent automatic MCP server configuration for EarthMind projects

To disable automatic MCP server configuration for new projects, set the `EARTHMIND_ADD_PROJECTS_TO_MCP_SERVERS` environment variable to `false`.
For more information, see [MCP server environment variables](#mcp-server-environment-variables).

### Selectively enable and disable MCP servers for EarthMind projects
...
To edit the names and descriptions of flow tools on a EarthMind MCP server, do the following:
...
Because MCP clients treat your EarthMind project as a single MCP server with all of your enabled flows listed as tools, unclear names and descriptions can cause the agent to select tools incorrectly or inconsistently.
...
To provide more context about your flows, make sure to name and describe your flows clearly when configuring your EarthMind project's MCP server.
...
After connecting your EarthMind MCP server to Cursor, you can ask Cursor about the resume, such as `What job experience does Emily have?`.
...
There are three ways to set tool execution timeouts: the MCP Tools component timeout setting, a global environment variable, and EarthMind's default fallback. How you configure these determines their effect in resolving the actual timeout limit for each tool call.

EarthMind times are evaluated in the following order:
...
2. Global timeout is set with the `EARTHMIND_MCP_TOOL_EXECUTION_TIMEOUT` environment variable. The default is 180 seconds.

3. If neither `EARTHMIND_MCP_TOOL_EXECUTION_TIMEOUT` or `EARTHMIND_MCP_SERVER_TIMEOUT` is configured, EarthMind defaults to 180 seconds.

To support tool calls that take longer than 180 seconds, set `EARTHMIND_MCP_TOOL_EXECUTION_TIMEOUT` to a value greater than `180` to raise the global limit, or set the **Tool Execution Timeout (seconds)** field on a specific MCP Tools component to a value greater than `180` to raise the limit for that component only.

If `EARTHMIND_MCP_SERVER_TIMEOUT` is set to a value greater than `EARTHMIND_MCP_TOOL_EXECUTION_TIMEOUT`, the server timeout takes precedence and becomes the actual limit for tool calls.

{/* The anchor on this section (connect-clients-to-use-the-servers-actions) is currently a link target in the EarthMind UI. Do not change. */}
## Connect clients to your EarthMind MCP server {#connect-clients-to-use-the-servers-actions}

EarthMind provides automatic installation and code snippets to help you deploy your EarthMind MCP servers to your local MCP clients.

<Tabs>
<TabItem value="JSON" label="JSON" default>

The JSON option allows you to connect a EarthMind MCP server to any local or remote MCP client.
...
4. In EarthMind, on the **Projects** page, click the **MCP Server** tab.
...
For example:
    ...
    The **MCP Server** tab automatically populates the `EARTHMIND_SERVER_ADDRESS` and `PROJECT_ID` values.

    The default EarthMind server address is `http://localhost:7860`.
    If you are using a [public EarthMind server](/deployment-public-server), the server address is automatically included.

    If your EarthMind server requires authentication, you must include your EarthMind API key or OAuth settings in the configuration.
    For more information, see [MCP server authentication](#authentication).
...
8. Confirm that your EarthMind MCP server is on the client's list of MCP servers.
...
Auto install requires the client to be installed locally so EarthMind can write to the client's configuration file.
...
1. Install [Cursor](https://docs.cursor.com/get-started/installation), [Claude](https://claude.ai/download), or [Windsurf](https://windsurf.com/download/editor) on the same computer where your EarthMind server is running.

2. Recommended: Configure [authentication](#authentication) for your MCP server.

3. In EarthMind, on the **Projects** page, click the **MCP Server** tab.

4. On the **Auto install** tab, find your MCP client provider, and then click <Icon name="Plus" aria-hidden="true"/> **Add**.

    Your EarthMind project's MCP server is automatically added to the configuration file for your local Cursor, Claude, or Windsurf client.
    For example, with Cursor, the server configuration is added to the `mcp.json` configuration file.

    EarthMind attempts to add this configuration even if the selected client isn't installed.
    To verify the installation, check the available MCP servers in your client.

</TabItem>
</Tabs>

Once your MCP client is connected to your EarthMind project's MCP server, your flows are registered as tools.
...
Each [EarthMind project](/concepts-flows#projects) has its own MCP server with its own MCP server authentication settings.

When you create a new project, EarthMind automatically configures authentication for the project's MCP server based on your EarthMind server's authentication settings. If authentication is enabled (`AUTO_LOGIN=false`), the project is automatically configured with API key authentication, and a new API key is generated for accessing the project's flows.

To configure authentication for a EarthMind MCP server, go to the **Projects** page in EarthMind, click the **MCP Server** tab, click <Icon name="Fingerprint" aria-hidden="true"/> **Edit Auth**, and then select your preferred authentication method:

<Tabs groupId="auth-type">
<TabItem value="API key" label="API key">

When authenticating your MCP server with a EarthMind API key, your project's MCP server **JSON** code snippets and **Auto install** configuration automatically include the `--headers` and `x-api-key` arguments in the **args** array (for streamable transport).

Click <Icon name="key" aria-hidden="true"/> **Generate API key** to automatically insert a new EarthMind API key into the code template.
Alternatively, you can replace `YOUR_API_KEY` with an existing EarthMind API key.
...
When OAuth is enabled, EarthMind automatically starts an [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure client-side proxy between MCP clients and the `mcp-proxy` on your server.

OAuth integration allows your EarthMind MCP server to authenticate users and applications through any OAuth 2.0 compliant service. When users or applications connect to your MCP server, they are redirected to your chosen OAuth provider to authenticate. Upon successful authentication, they are granted access to your flows as MCP tools.

Before configuring OAuth in EarthMind, you must first set up an OAuth application with an external OAuth 2.0 service provider.
You must register your EarthMind server as an OAuth client, and then obtain the required values to complete the configuration in EarthMind.
...
| **MCP Scope** | Scope for MCP operations | You define this. As of EarthMind 1.6, `user` is the only available value. | `user` |
| **Provider Scope** | OAuth provider scope | You define this. As of EarthMind 1.6, `openid` is the only available value. | `openid` |
...
2. Configure the OAuth settings with the values from your OAuth deployment.
    ...
    The OAuth credentials are encrypted and stored securely in your EarthMind database.
...
4. If you already installed your EarthMind MCP server in your MCP client, you must update your MCP client configuration to use the new OAuth settings after enabling OAuth on your MCP server.
...
Only use this option when EarthMind is running in a trusted environment.
...
{/* The anchor on this section (deploy-your-server-externally) is currently a link target in the EarthMind UI. Do not change. */}
## Deploy your EarthMind MCP server externally {#deploy-your-server-externally}

To deploy your EarthMind MCP server externally, see [Deploy a public EarthMind server](/deployment-public-server).
...
3. In the MCP Inspector UI, enter the connection details for your EarthMind project's MCP server.
    ...
    <Tabs groupId="auth-type">
      <TabItem value="API key" label="API key" default>
        ...
        - **Command**: `uvx`
        - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `EARTHMIND_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your EarthMind MCP server. For example:
            ```bash
            mcp-proxy --headers x-api-key YOUR_API_KEY http://EARTHMIND_SERVER_ADDRESS/api/v1/mcp/project/PROJECT_ID/streamable
            ```
      ...
      <TabItem value="None" label="None">
        ...
        - **URL**: Enter the EarthMind MCP server's endpoint. For example:
            ```bash
...
To prevent non-superusers from editing MCP server connections, set `EARTHMIND_MCP_SERVERS_LOCKED=true`.
...
Locking the MCP server configuration for users does not disable EarthMind's built-in MCP server for serving flows as MCP tools.

## MCP server environment variables

The following environment variables set behaviors related to your EarthMind projects' MCP servers:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `EARTHMIND_MCP_SERVER_ENABLED` | Boolean | `True` | Whether to initialize an MCP server for each of your EarthMind projects. If `false`, EarthMind doesn't initialize MCP servers. |
| `EARTHMIND_MCP_SERVER_ENABLE_PROGRESS_NOTIFICATIONS` | Boolean | `False` | If `true`, EarthMind MCP servers send progress notifications. |
| `EARTHMIND_MCP_SERVER_TIMEOUT` | Integer | `20` | Timeout in seconds for MCP connection setup and tool execution. See [Configure tool execution timeouts](#configure-tool-execution-timeouts). |
| `EARTHMIND_MCP_TOOL_EXECUTION_TIMEOUT` | Integer | `180` | Global timeout in seconds for MCP tool calls. See [Configure tool execution timeouts](#configure-tool-execution-timeouts). |
| `EARTHMIND_MCP_MAX_SESSIONS_PER_SERVER` | Integer | `10` | Maximum number of MCP sessions to keep per unique server. |
| `EARTHMIND_ADD_PROJECTS_TO_MCP_SERVERS` | Boolean | `True` | Whether to automatically add newly created projects to the user's MCP servers configuration. If `false`, projects must be manually added to MCP servers. |
| `EARTHMIND_MCP_SERVERS_LOCKED` | Boolean | `False` | If `true`, non-superusers cannot add, edit, or remove MCP server connections through the UI or API. Superusers retain full access. For more information, see [Restrict MCP server management to superusers](#restrict-mcp-server-management). |

## Troubleshoot EarthMind MCP servers {#troubleshooting-mcp-server}

For troubleshooting advice for MCP servers and clients, see [Troubleshoot EarthMind: MCP issues](/troubleshoot#mcp).

## See also

* [Use EarthMind as an MCP client](/mcp-client)
