...
To edit Terraflow's global model provider configuration, do the following:
...
3. In the **API Key** field, add your provider's API key. Some providers require additional configuration fields. For more information, see the model provider's documentation.
    ...
    You can only add one key for each provider. Make sure the key has access to _all_ models that you want to use in Terraflow.
4. Click **Save**.
5. Enable the specific models that you want to use in Terraflow.
...
After you enable a model in Terraflow's global model configuration, you can use that model in any model-driven component in your flows.
