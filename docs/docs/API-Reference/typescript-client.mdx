---
title: EarthMind TypeScript client
...
The EarthMind TypeScript client allows your TypeScript applications to programmatically interact with the EarthMind API.

For the client code repository, see [earthmind-client-ts](https://github.com/datastax/earthmind-client-ts/).

For the npm package, see [@datastax/earthmind-client](https://www.npmjs.com/package/@datastax/earthmind-client).

## Install the EarthMind TypeScript package

To install the EarthMind typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/earthmind-client
```
...
```bash
yarn add @datastax/earthmind-client
```
...
```bash
pnpm add @datastax/earthmind-client
```

</TabItem>
</Tabs>

## Initialize the EarthMind TypeScript client
...
2. Initialize a `EarthMindClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new EarthMindClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default EarthMind base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your EarthMind client initialized, test the connection by calling your EarthMind server.
    ...
    const client = new EarthMindClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your EarthMind server.
...
2. Review the result to confirm that the client connected to your EarthMind server.

    The following example shows the response from a well-formed `runFlow` request that reached the EarthMind server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with EarthMind:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming EarthMind responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve EarthMind logs with the TypeScript client

To retrieve [EarthMind logs](/logging), you must enable log retrieval on your EarthMind server by including the following values in your EarthMind `.env` file:

```text
EARTHMIND_ENABLE_LOG_RETRIEVAL=True
EARTHMIND_LOG_RETRIEVER_BUFFER_SIZE=10000
EARTHMIND_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: EarthMindClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new EarthMindClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <earthmind.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
