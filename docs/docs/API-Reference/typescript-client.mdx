---
title: Terraflow TypeScript client
...
The Terraflow TypeScript client allows your TypeScript applications to programmatically interact with the Terraflow API.

For the client code repository, see [terraflow-client-ts](https://github.com/datastax/terraflow-client-ts/).

For the npm package, see [@datastax/terraflow-client](https://www.npmjs.com/package/@datastax/terraflow-client).

## Install the Terraflow TypeScript package

To install the Terraflow typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/terraflow-client
```
...
```bash
yarn add @datastax/terraflow-client
```
...
```bash
pnpm add @datastax/terraflow-client
```

</TabItem>
</Tabs>

## Initialize the Terraflow TypeScript client
...
2. Initialize a `TerraflowClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new TerraflowClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default Terraflow base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your Terraflow client initialized, test the connection by calling your Terraflow server.
    ...
    const client = new TerraflowClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your Terraflow server.
...
2. Review the result to confirm that the client connected to your Terraflow server.

    The following example shows the response from a well-formed `runFlow` request that reached the Terraflow server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with Terraflow:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Terraflow responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve Terraflow logs with the TypeScript client

To retrieve [Terraflow logs](/logging), you must enable log retrieval on your Terraflow server by including the following values in your Terraflow `.env` file:

```text
TERRAFLOW_ENABLE_LOG_RETRIEVAL=True
TERRAFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000
TERRAFLOW_LOG_LEVEL=DEBUG
```
...
async function runFlow(client: TerraflowClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new TerraflowClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <terraflow.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
