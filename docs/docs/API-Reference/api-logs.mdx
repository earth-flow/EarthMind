...
Retrieve logs for your Terraflow flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Terraflow instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your Terraflow `.env` file, and then start Terraflow with `uv run terraflow run --env-file .env`:

```text
TERRAFLOW_ENABLE_LOG_RETRIEVAL=True
TERRAFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
TERRAFLOW_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
