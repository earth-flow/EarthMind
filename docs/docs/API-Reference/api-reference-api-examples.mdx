---
title: Get started with the Terraflow API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormTerraflowApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-terraflow-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormTerraflowApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-terraflow-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormTerraflowApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-terraflow-api-requests.js';
...
You can use the Terraflow API for programmatic interactions with Terraflow, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build Terraflow as a dependency of a larger application, codebase, or service.
* Contribute to the overall Terraflow codebase.

To view and test all available endpoints, you can access the Terraflow API's OpenAPI specification at your Terraflow deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Terraflow API in a script, see the [Terraflow quickstart](/get-started-quickstart).
...
## Form Terraflow API requests

While individual options vary by endpoint, all Terraflow API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Terraflow API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormTerraflowApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormTerraflowApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormTerraflowApiRequests} language="bash" />
...
By default, local deployments serve the Terraflow API at `http://localhost:7860/api`.

Remotely hosted Terraflow deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:TERRAFLOW_PORT/api`.

You can configure the Terraflow port number in the `TERRAFLOW_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:TERRAFLOW_PORT/api`

### Authentication

In Terraflow versions 1.5 and later, most API endpoints require authentication with a Terraflow API key in either an `x-api-key` header or query parameter.
...
Terraflow API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Terraflow API serves `/v1` and `/v2` endpoints.
...
Commonly used values in Terraflow API requests include your [Terraflow server URL](#base-url), [Terraflow API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Terraflow API requests

Once you have your Terraflow server URL, try calling these endpoints that return Terraflow metadata.

### Health check

Returns the health status of the Terraflow database and chat services:
...
Terraflow provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Terraflow is fully initialized, so it's not reliable for checking Terraflow service health.

### Get version

Returns the current Terraflow API version:
...
Returns configuration details for your Terraflow deployment.
Requires a [Terraflow API key](/api-keys-and-authentication).
...
Returns a dictionary of all Terraflow components.
Requires a [Terraflow API key](/api-keys-and-authentication).
...
Because you can run Terraflow as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Terraflow database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Terraflow codebase, you won't directly call most of the Terraflow endpoints.
...
The following endpoints are useful for developing applications with Terraflow and administering Terraflow deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return Terraflow version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom Terraflow components for your own use or to share with the Terraflow community:
  * GET `/v1/all`: Return all available Terraflow component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Terraflow codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing Terraflow MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the Terraflow frontend and when running flows that call MCP servers.
Terraflow MCP servers support both streamable HTTP and SSE transport.
...
* Use the Terraflow API to [run a flow](/api-flows-run).
* Use the Terraflow API to [upload files](/api-files).
* Use the Terraflow API to [get flow logs](/api-logs).
* Explore all endpoints in the [Terraflow API specification](/api).
