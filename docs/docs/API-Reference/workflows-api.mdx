...
The Workflow API provides programmatic access to execute Terraflow workflows synchronously or asynchronously.
...
The Workflow API is part of the Terraflow Developer v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.
...
| `globals` | `object` | No | `{}` | Request-level global variables available to workflow components. Use this body field for arbitrary strings, including Unicode values. Keys are limited to 256 characters and values to 64 KB. See [Migrating from `X-TERRAFLOW-GLOBAL-VAR-*` headers](#migrating-from-x-terraflow-global-var--headers). |
...
## Migrating from `X-TERRAFLOW-GLOBAL-VAR-*` headers

Earlier preview releases of the Workflow API accepted request-level global variables via `X-TERRAFLOW-GLOBAL-VAR-*` HTTP headers. HTTP headers cannot reliably carry arbitrary Unicode values, so the v2 Workflow API now accepts globals as a typed `globals` field in the JSON request body.
...
- A deprecation warning is logged on every request that carries `X-TERRAFLOW-GLOBAL-VAR-*` headers.
- Header-based globals will be **removed in a future Terraflow release**. Migrate to the `globals` body field at your earliest convenience.

**Before (deprecated):**

```bash
curl -X POST "$TERRAFLOW_URL/api/v2/workflows" \
  -H "x-api-key: $TERRAFLOW_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-TERRAFLOW-GLOBAL-VAR-FILENAME: relatorio-final.pdf" \
  -H "X-TERRAFLOW-GLOBAL-VAR-OWNER_NAME: Jose" \
  --data '{"flow_id": "..."}'
```
...
Keys may use the same characters supported by the global variables panel in the Terraflow UI; they are bounded to 256 characters, and values are bounded to 64 KB.
...
To find the component ID in the Terraflow UI, open your flow in Terraflow, click the component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
