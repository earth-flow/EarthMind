...
Terraflow includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with Terraflow's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
Terraflow uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Terraflow with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `TERRAFLOW_SERVER_URL`, `TERRAFLOW_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your Terraflow API key for authentication | `"sk-..."` |
| `Content-Type` | Yes | Specifies the JSON format | `"application/json"` |
| `X-TERRAFLOW-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-TERRAFLOW-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that Terraflow sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by Terraflow:
...
Variables passed with `X-TERRAFLOW-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your Terraflow database, the flow fails by default.
