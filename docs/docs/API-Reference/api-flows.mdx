...
If you want to use the Terraflow API to run a flow, see [Flow trigger endpoints](/api-flows-run).
...
This endpoint downloads a ZIP file containing [Terraflow JSON files](/concepts-flows-import#terraflow-json-file-contents) for each flow ID listed in the request body.
...
Imports flows by uploading a [Terraflow-compatible JSON file](/concepts-flows-import#terraflow-json-file-contents).
