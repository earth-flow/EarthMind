...
Instead of manually exporting, sharing, and importing flow JSON files from the Terraflow UI, the Flow DevOps toolkit offers terminal-based workflows for versioning, environment variables, testing, and deployment.

## Prerequisites

- [Install and start Terraflow](/get-started-installation)
- Create a [Terraflow API key](/api-keys-and-authentication)
- Install the Terraflow `lfx` package
   ...
   2. Activate the virtual environment.
      ...
      ```
   3. Install the Terraflow LFX package in the virtual environment:
      ```bash
      uv pip install lfx
      ```
   4. Run Flow DevOps Toolkit commands in the virtual environment that has LFX installed.

      Alternatively, you can run `uvx lfx` commands, or run LFX from the `src/lfx` directory in a cloned Terraflow repo.
      For more information, see the [Terraflow LFX README](https://github.com/terraflow-ai/terraflow/blob/main/src/lfx/README.md).
...
3. To initialize a project, run:
    ...
    ├── .github/
    │   └── workflows/
    │       ├── terraflow-push.yml       # CI workflow
    │       ├── terraflow-test.yml       # CI workflow
    │       └── terraflow-validate.yml   # CI workflow
    ...
    ✔ Project scaffolded. Next steps:
      ...
      2. export TERRAFLOW_LOCAL_API_KEY=<key>   (Settings -> API Keys)
...
4. Add your Terraflow API key to your `.env` file, or export it within the terminal session.
    ...
    For example, to add a Terraflow API key to a local Terraflow server, set:

    ```text
    export TERRAFLOW_LOCAL_API_KEY=TERRAFLOW_API_KEY
    ```
...
7. In the Terraflow UI, open the Simple Agent flow and change the flow.
...
9. To pull the reported changes from the Terraflow server to your local project folder, run `lfx pull`.
10. To _push_ flow changes from flows stored locally in `demo-project/flows` to the Terraflow server, run `lfx push`.

## Validate flows

The Flow DevOps SDK can validate that local flows are correctly formed before pushing to the Terraflow with `lfx validate`.
...
2. Optionally, you can now share and serve the flow by keeping the flow JSON and `requirements.txt` in the same environment.
   To serve the flow without the Terraflow UI, do the following:
   ...
   4. To set a Terraflow API key, run:
   ```bash
   export TERRAFLOW_API_KEY=TERRAFLOW_API_KEY
   ```

   5. To serve the flow without the Terraflow UI, pass the flow JSON path to the `lfx serve` command:
   ```bash
...
production:
  ...
  api_key_env: TERRAFLOW_PRODUCTION_API_KEY
```

Each entry contains a `url` for the Terraflow base URL, and an `api_key_env` field.
...
`environments.yaml` is distinct from the Terraflow or LFX `.env` file.
`environments.yaml` controls which remote Terraflow instance you're deploying flows to, flow versioning, and environment variable _names_ for API keys.
The `.env` contains runtime values for the Terraflow server, and might also contain _actual secret values_, so the `.env` should not be committed to version control.

Commands that call a Terraflow server over HTTP, such as `lfx pull` or `lfx push`, use `--env ENVIRONMENT_NAME` to determine which Terraflow instance to send the request to.
...
This command will send the request to the Terraflow base URL at `http://127.0.0.1:7860` using a Terraflow API key named `TERRAFLOW_LOCAL_API_KEY`.
