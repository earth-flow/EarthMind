...
:::tip
Terraflow automatically generates Python, JavaScript, and curl code snippets for the `/v1/run/$FLOW_ID` endpoint for all flows.
...
For an example of a script that extracts data from a Terraflow API response, see the [Quickstart](/get-started-quickstart).
...
| x-api-key | Required. Your Terraflow API key for authentication. Can be passed as a header or query parameter. | "sk-..." |
| `X-TERRAFLOW-GLOBAL-VAR-*` | Optional. Pass global variables to the flow. Variable names are automatically converted to uppercase. These variables take precedence over OS environment variables and are only available during this specific request execution. | `"X-TERRAFLOW-GLOBAL-VAR-API_KEY: sk-..."` |
...
You can pass global variables to your flow using HTTP headers with the format `X-TERRAFLOW-GLOBAL-VAR-{VARIABLE_NAME}`.
...
Variable names are automatically converted to uppercase. For example, `X-TERRAFLOW-GLOBAL-VAR-api-key` becomes `API_KEY` in your flow.

You don't need to create these variables in Terraflow's Global Variables section first. Pass any variable name using this header format.
...
If your flow components reference variables that aren't provided in headers or your Terraflow database, the flow fails by default. To avoid this, you can set `TERRAFLOW_FALLBACK_TO_ENV_VAR=True` in your `.env` file, which allows the flow to use values from OS environment variables if they aren't otherwise specified.
