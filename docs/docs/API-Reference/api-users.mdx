...
Use the `/users` endpoint to manage user accounts in Terraflow.

## Add user

Create a new user account with a given username and password.

Requires authentication as a superuser if the Terraflow server has authentication enabled.
...
The user's UUID is stored in `user_id` in the Terraflow database, and returned as `id` in the `/users` API response.
This `user_id` key is specifically for Terraflow user management.
