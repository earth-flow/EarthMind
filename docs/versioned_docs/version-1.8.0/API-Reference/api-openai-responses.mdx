...
EarthMind includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with EarthMind's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
EarthMind uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to EarthMind with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `EARTHMIND_SERVER_URL`, `EARTHMIND_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="EARTHMIND_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "EARTHMIND_API_KEY"},
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your EarthMind API key for authentication | `"sk-..."` |
| `Content-Type` | Yes | Specifies the JSON format | `"application/json"` |
| `X-EARTHMIND-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-EARTHMIND-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that EarthMind sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by EarthMind:
...
```bash
curl -X POST \
  "http://$EARTHMIND_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $EARTHMIND_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-EARTHMIND-GLOBAL-VAR-{VARIABLE_NAME}`.
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable EarthMind automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in EarthMind's Global Variables section - you can pass any variable name in the `X-EARTHMIND-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
Variables passed with `X-EARTHMIND-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your EarthMind database, the flow fails by default.
