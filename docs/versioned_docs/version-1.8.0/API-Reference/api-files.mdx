...
Use the `/files` endpoints to move files between your local machine and EarthMind.

All `/files` endpoints (both `/v1/files` and `/v2/files`) require authentication with a EarthMind API key.
...
- `/v2` files are organized by `user_id` instead of `flow_id`.
  ...
  You can upload a file to EarthMind one time, and use it with multiple flows.
- `/v2` files are tracked in the EarthMind database.
...
Send image files to EarthMind to use them in flows.

The default file limit is 1024 MB.
To change this limit, set the `EARTHMIND_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:
    ...
    A successful request returns the `file_path` for the image in the EarthMind file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
    For example:
...
To create a EarthMind API key and export it as an environment variable, see [Get started with the EarthMind API](/api-reference-api-examples).
...
This endpoint uploads files to your EarthMind server's file management system.
To use an uploaded file in a flow, send the file path to a flow with a [**Read File** component](/read-file).

The default file limit is 1024 MB. To configure this value, change the `EARTHMIND_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
This component loads files into flows from your local machine or EarthMind file management.
