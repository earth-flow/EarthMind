...
:::tip
EarthMind automatically generates Python, JavaScript, and curl code snippets for the `/v1/run/$FLOW_ID` endpoint for all flows.
...
url = "http://EARTHMIND_SERVER_URL/api/v1/run/FLOW_ID"
...
fetch('http://EARTHMIND_SERVER_URL/api/v1/run/FLOW_ID', options)
...
For an example of a script that extracts data from a EarthMind API response, see the [Quickstart](/get-started-quickstart).
...
url = "http://EARTHMIND_SERVER_URL/api/v1/run/FLOW_ID?stream=true"
...
fetch('http://EARTHMIND_SERVER_URL/api/v1/run/FLOW_ID?stream=true', options)
  .then(async response => {
...
| x-api-key | Required. Your EarthMind API key for authentication. Can be passed as a header or query parameter. | "sk-..." |
| `X-EARTHMIND-GLOBAL-VAR-*` | Optional. Pass global variables to the flow. Variable names are automatically converted to uppercase. These variables take precedence over OS environment variables and are only available during this specific request execution. | `"X-EARTHMIND-GLOBAL-VAR-API_KEY: sk-..."` |
...
You can pass global variables to your flow using HTTP headers with the format `X-EARTHMIND-GLOBAL-VAR-{VARIABLE_NAME}`.
...
Variable names are automatically converted to uppercase. For example, `X-EARTHMIND-GLOBAL-VAR-api-key` becomes `API_KEY` in your flow.

You don't need to create these variables in EarthMind's Global Variables section first. Pass any variable name using this header format.
...
If your flow components reference variables that aren't provided in headers or your EarthMind database, the flow fails by default. To avoid this, you can set `EARTHMIND_FALLBACK_TO_ENV_VAR=True` in your `.env` file, which allows the flow to use values from OS environment variables if they aren't otherwise specified.
