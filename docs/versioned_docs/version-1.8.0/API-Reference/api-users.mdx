...
Use the `/users` endpoint to manage user accounts in EarthMind.

## Add user

Create a new user account with a given username and password.

Requires authentication as a superuser if the EarthMind server has authentication enabled.
...
The user's UUID is stored in `user_id` in the EarthMind database, and returned as `id` in the `/users` API response.
This `user_id` key is specifically for EarthMind user management.
