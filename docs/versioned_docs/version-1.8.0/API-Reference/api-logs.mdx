...
Retrieve logs for your EarthMind flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your EarthMind instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your EarthMind `.env` file, and then start EarthMind with `uv run earthmind run --env-file .env`:

```text
EARTHMIND_ENABLE_LOG_RETRIEVAL=True
EARTHMIND_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
EARTHMIND_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/earthmind/profile_pictures/Space.\n"
}
```
