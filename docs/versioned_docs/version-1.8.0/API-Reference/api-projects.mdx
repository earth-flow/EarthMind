...
Use the `/projects` endpoint to create, read, update, and delete [EarthMind projects](/concepts-flows#projects).

## Read projects

Get a list of EarthMind projects, including project IDs, names, and descriptions.
...
Import a project and its flows by uploading a EarthMind project zip file:
...
curl -X POST \
  ...
  -H "x-api-key: $EARTHMIND_API_KEY" \
  -F "file=@20241230_135006_earthmind_flows.zip;type=application/zip"
```