Because EarthMind is based on LangChain, vector store components use an instance of [LangChain vector store](https://docs.langchain.com/oss/python/integrations/vectorstores) to drive the underlying read and write functions.
...
Depending on the provider, these options might use default values or allow modification through environment variables, if they are supported in EarthMind.
