...
This tutorial demonstrates how you can use EarthMind to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
1. In EarthMind, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the EarthMind API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your EarthMind server.
...
<TabItem value="API" label="EarthMind API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your EarthMind server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.
...
const headers = { 'x-api-key': 'EARTHMIND_API_KEY' };

// 2. Upload the file to EarthMind
const uploadRes = await fetch('EARTHMIND_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the EarthMind run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('EARTHMIND_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'EARTHMIND_API_KEY' },
  body: JSON.stringify(payload)
});
const earthmindData = await runRes.json();
// Output only the message
console.log(earthmindData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { EarthMindClient } = require('@datastax/earthmind-client');

    # pragma: allowlist nextline secret
    const API_KEY = 'EARTHMIND_API_KEY';
    const SERVER = 'EARTHMIND_SERVER_ADDRESS';
    ...
    function chat() {
        console.log('🤖 EarthMind RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [EarthMind TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with EarthMind's complex JSON response structure.
