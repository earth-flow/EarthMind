...
-   `earthmind-base` dependency structure refactored

    The `earthmind-base` package now uses granular optional dependency groups. As a result, many dependencies that were previously included in the `earthmind-base` installation were moved to optional extras.

    If you installed EarthMind with `uv pip install earthmind`, this isn't a breaking change. Installing `earthmind` automatically installs `earthmind-base[complete]`, which includes all optional dependencies and maintains the same functionality as before.

    However, if you installed EarthMind with `uv pip install earthmind-base` without specifying extra dependencies, this _is_ a breaking change.
    Some dependencies that were previously included by default are now available only through optional extras.
    Therefore, installing `earthmind-base` directly only installs the [core base dependencies](https://github.com/earthmind-ai/earthmind/blob/main/src/backend/base/pyproject.toml).

    If you installed `earthmind-base`, there are two ways to resolve dependency errors that result from this breaking change:

    * If you need the full set of dependencies, you must install `earthmind-base` with the `complete` extra:

        ```bash
        uv pip install "earthmind-base[complete]"
        ```

    * If you need specific dependencies, you must install `earthmind-base` with those optional dependency groups. For example:

       ```bash
       uv pip install "earthmind-base[postgresql,openai,chroma]"
       ```

    For more information about available optional dependency groups, see [Install optional dependency groups for `earthmind-base`](/install-custom-dependencies#install-optional-dependency-groups-for-earthmind-base).

-   Docker image does not include `uv` or `uvx`

    :::tip
    Starting with EarthMind 1.8.1, the official Docker images include `uv` and `uvx` again.
    If you're using EarthMind 1.8.0, follow the steps in this release note to add `uv` and `uvx` in a derived image.
    :::

    In EarthMind 1.8.0, the EarthMind Docker image does not include `uv` or `uvx` in the runtime image.
    This means that MCP server configurations, including the default EarthMind MCP server, that rely on commands like `uvx mcp-proxy` will fail inside the container with a `command not found` error.

    If you use MCP from within a EarthMind 1.8.0 Docker image, you must install `uv` in an image derived from the official `earthmindai/earthmind` image.

    To install `uv` and `uvx` in a derived image based on the official `earthmindai/earthmind` image, do the following:

    1. Create a `Dockerfile` in your project.
    2. Use the official EarthMind image as the base and install `uv` into the existing virtual environment:
        ```dockerfile
        FROM earthmindai/earthmind:1.8.0
    ...
    For more information, see [Upgrade the EarthMind Docker image](/deployment-docker#upgrade-the-earthmind-docker-image).
...
- Developer API: `/workflow` synchronous endpoints (Beta)
    ...
    The Developer API is part of a larger effort to improve EarthMind's APIs with enhanced capabilities and better developer experience.
...
- Traces and trace view
    ...
    EarthMind now records execution traces for flows and components.
...
- More configuration options for JWT-based session authentication
    ...
    EarthMind 1.8 offers additional configuration options for JWT algorithms, including support for RS256/RS512 algorithms, configurable keys, and token lifetimes. For more information, see [JWT authentication](/jwt-authentication).
...
- Docker AMD vs ARM image sizes
    ...
    EarthMind 1.8.0 addresses the AMD vs ARM Docker image size gap.
...
- New [**Openlayer** observability integration](/integrations-openlayer)
    ...
    Configures EarthMind to send tracing data to Openlayer for analysis, monitoring, and evaluation of your flow executions.
...
All EarthMind versions upgrading to 1.7.0 are affected.

Don't upgrade to EarthMind version 1.7.0.
...
1. Revert EarthMind to version 1.6.9:

   ```bash
   uv pip install earthmind==1.6.9
   ```
...
3. Upgrade directly to version 1.7.1, which includes the fix for this issue:
   ...
   ```bash
   uv pip install earthmind==1.7.1
   ```
...
- Support for streamable HTTP transport for MCP clients and servers
    ...
    EarthMind now supports [streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) for both MCP clients and servers.
    When using EarthMind as an [MCP client](/mcp-client), you can connect to MCP servers with streamable HTTP transport.
    When using EarthMind as an [MCP server](/mcp-server), clients can connect using streamable HTTP transport.
    SSE transport is still supported as a fallback for backwards compatibility.

- Webhook authentication

    Added the `EARTHMIND_WEBHOOK_AUTH_ENABLE` environment variable for authenticating requests to the [`/webhook` endpoint](/api-flows-run#webhook-run-flow). When `EARTHMIND_WEBHOOK_AUTH_ENABLE=TRUE`, webhook endpoints require API key authentication and validate that the authenticated user owns the flow being executed. When `FALSE`, no EarthMind API key is required and all requests to the webhook endpoint are treated as being sent by the flow owner. For more information, see [Trigger flows with webhooks](/webhook).

- Configurable API key validation

    Added the `EARTHMIND_API_KEY_SOURCE` environment variable to control how EarthMind validates API keys. When set to `db`, EarthMind validates API keys against keys stored in the database. When set to `env`, EarthMind validates API keys against the `EARTHMIND_API_KEY` environment variable. For more information, see [API keys and authentication](/api-keys-and-authentication#earthmind-api-key-source).

- SSRF protection

    Added SSRF (Server-Side Request Forgery) protection to the [**API Request** component](/api-request). HTTP redirects are disabled by default to prevent SSRF bypass attacks. To enable SSRF protection, set `EARTHMIND_SSRF_PROTECTION_ENABLED=TRUE`. Configure allowed hosts with `EARTHMIND_SSRF_ALLOWED_HOSTS`. Flows that relied on automatic redirects will need to enable it manually.

- Email registration in EarthMind Desktop

    EarthMind Desktop now includes an email registration screen at startup. The registered email address helps track user registrations and understand the user base. For more information, see [Telemetry](/contributing-telemetry#email).
...
EarthMind versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes EarthMind to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any EarthMind version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of EarthMind Desktop should **not** use the in-app update feature to upgrade to EarthMind version 1.6.0.
Only Windows Desktop users upgrading to EarthMind version 1.6.0 are affected.
:::

The **Update** button in EarthMind Desktop will not work for Windows users, and may result in data loss.

Instead, download a fresh installation from the [EarthMind website](https://earthmind.org) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the EarthMind database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.earthmind\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.earthmind\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of EarthMind.
3. Close EarthMind, and ensure no EarthMind process is running in Task Manager.
4. The EarthMind 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your EarthMind database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.earthmind\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [EarthMind website](https://earthmind.org).
...
The EarthMind 1.6.0 installer automatically performs a database backup.
These steps install EarthMind from scratch, and result in two EarthMind installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of EarthMind, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start EarthMind, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of EarthMind, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for EarthMind API requests by default

    In EarthMind version 1.6, `EARTHMIND_AUTO_LOGIN=True` and `EARTHMIND_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for EarthMind API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`EARTHMIND_AUTO_LOGIN`](/api-keys-and-authentication#earthmind-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  EarthMind now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **Read File** component's new advanced parsing feature, the Docling dependency is now included with EarthMind for all operating systems except macOS Intel (x86_64).
...
- EarthMind API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, EarthMind API endpoints now require authentication with a EarthMind API key, even when `EARTHMIND_AUTO_LOGIN=True`.
    This change will be enforced in a future release.
    For temporary backwards compatibility, this release adds the `EARTHMIND_SKIP_AUTH_AUTO_LOGIN` environment variable.
    The default value is `true`, which disables API authentication enforcement.
    To enforce API authentication, set `EARTHMIND_SKIP_AUTH_AUTO_LOGIN=False`.
    For more information, see the documentation for [`EARTHMIND_AUTO_LOGIN`](/api-keys-and-authentication#earthmind-auto-login).
...
- MCP server one-click installation
    ...
    On your EarthMind project's **MCP server** page, click **Auto install** to install your EarthMind MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use EarthMind as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your EarthMind **Settings** page.
    For more information, see [Use EarthMind as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/earthmind-ai/earthmind/issues).
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating EarthMind.
...
- Introduced MCP server functionality to serve EarthMind tools to MCP-compatible clients.
