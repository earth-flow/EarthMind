...
You can configure EarthMind to collect [tracing](https://www.comet.com/docs/opik/tracing/log_traces) data about your flow executions and automatically send the data to Opik.
...
:::tip
If you need a flow to test the Opik integration, see the [EarthMind quickstart](/get-started-quickstart).
:::

## Integrate Opik with EarthMind
...
2. Call the `opik configure` CLI to save your Opik configuration in the same environment where you run EarthMind:
...
3. Start EarthMind in the same terminal or environment where you set the environment variables:

    ```bash
    uv run earthmind run
    ```

4. In EarthMind, run a flow to produce activity for Opik to trace.

5. Navigate to your Opik project dashboard and view the collected tracing data.

## Disable the Opik integration

To disable the Opik integration, remove the environment variables you set with `opik configure`, and then restart EarthMind.