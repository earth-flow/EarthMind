...
The `build_df` method processes data and returns a `DataFrame`. The `-> DataFrame` type annotation helps EarthMind validate connections and provides color-coding in the visual editor. You can also set `self.status` to show progress messages in the UI.
