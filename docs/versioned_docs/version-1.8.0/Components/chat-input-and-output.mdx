...
**Chat Input and Output** components are designed to handle conversational interactions in EarthMind.
...
For information about these data types, see [Use EarthMind data types](/data-types).
...
When using the EarthMind API, the API response includes the **Chat Output** `Message` object along with other response data from the flow run.
...
* [EarthMind quickstart](/get-started-quickstart): Create and run a basic agent flow.
* **Basic Prompting** template: Create an LLM-based chat flow that accepts chat input as well as a prompt with additional instructions for the LLM. Many other EarthMind templates also use **Chat Input and Output** components.
...
### Send chat input with the EarthMind API

You can use the EarthMind API to run a flow by sending input to a **Chat Input** component:
...
When triggering flows with the EarthMind API, the payload must contain the values for the **Chat Input** component's input parameters, such as `input_value`.
...
For more information, see [Trigger flows with the EarthMind API](/concepts-publish).