...
The **MCP Tools** component connects to a Model Context Protocol (MCP) server and exposes the MCP server's functions as tools for EarthMind agents to use to respond to input.

In addition to publicly available MCP servers and your own custom-built MCP servers, you can connect EarthMind MCP servers, which allow your agent to use your EarthMind flows as tools.
To do this, use the **MCP Tools** component's [HTTP/SSE mode](/mcp-client#mcp-http-mode) to connect to your EarthMind project's MCP server.

For more information, see [Use EarthMind as an MCP client](/mcp-client) and [Use EarthMind as an MCP server](/mcp-server).
...
* In EarthMind version 1.5, the **MCP Connection** component was renamed to the **MCP Tools** component.
* In EarthMind version 1.3, the **MCP Tools (stdio)** and **MCP Tools (SSE)** components were removed and replaced by the unified **MCP Connection** component, which was later renamed to **MCP Tools**.
