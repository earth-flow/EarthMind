...
You can create new databases and collections through this component, but this is only possible in the EarthMind visual editor (not at runtime), and you must wait while the database or collection initializes before proceeding with flow configuration.
...
| cache_vector_store | Cache Vector Store | Input parameter. Whether to cache the vector store in EarthMind memory for faster reads. Default: Enabled (`true`). |
...
However, EarthMind's **Agent** component includes built-in chat memory that is enabled by default.
...
These components are legacy and should be replaced with EarthMind's native agent components.
...
These components are legacy. Use EarthMind's built-in environment variable support or global variables instead.
...
This component was deprecated in EarthMind version 1.1.2.
