...
Use archive and compressed formats to bundle multiple files together, or use the [**Directory** component](/legacy-core-components) to load all files in a directory.
...
| file_path | Server File Path | Input parameter. A `Data` object with a `file_path` property pointing to a file in [EarthMind file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
