...
The Docling dependency is required to use the Docling components in EarthMind.
    ...
    * **EarthMind version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: EarthMind versions earlier than 1.6 don't include the Docling dependency.
    For EarthMind OSS, install the Docling extra with `uv pip install 'earthmind[docling]'`.
    For EarthMind Desktop, add the Docling dependency to EarthMind Desktop's `requirements.txt`.
