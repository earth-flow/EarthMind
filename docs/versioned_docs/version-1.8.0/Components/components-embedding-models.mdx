...
Embedding model components in EarthMind generate text embeddings using a specified Large Language Model (LLM).

EarthMind includes an **Embedding Model** core component that has built-in support for some LLMs.
...
2. <PartialGlobalModelProviders />
    ...
    If your preferred embedding model provider or model isn't available in EarthMind's global <Icon name="BrainCog" aria-hidden="true" /> **Models**, you can use any [additional embedding models](#additional-embedding-models) in place of the core component.
...
If your provider or model isn't available in EarthMind's global <Icon name="BrainCog" aria-hidden="true" /> **Models**, you can replace the **Embedding Model** core component with any other component that generates embeddings.
