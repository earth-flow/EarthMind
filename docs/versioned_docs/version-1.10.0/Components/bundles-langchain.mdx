...
:::warning Security
Setting `allow_dangerous_code` to `true` allows the agent to execute arbitrary, LLM-generated Python code at runtime on the EarthMind server.
...
To replace these components, consider other components in the **LangChain** bundle or general EarthMind components, such as the [**Agent** component](/components-agents) or the [**SQL Database** component](/sql-database).