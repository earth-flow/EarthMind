...
The Docling bundle is included in the `lfx-docling` Extension bundle, which is installed automatically as part of `uv pip install earthmind`.
...
Alternatively, if you installed EarthMind as a package:

```bash
uv pip install "earthmind[docling]"
```
