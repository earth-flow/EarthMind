...
Create your own custom components to add any functionality you need to EarthMind, from API integrations to data processing.

In EarthMind's node-based environment, each node is a "component" that performs discrete functions.
Custom components in EarthMind are built upon:
...
Use the [Custom component quickstart](#quickstart) to add an example component to EarthMind, and then use the reference guide that follows for more advanced component customization.

## Custom component quickstart {#quickstart}

Create a custom `DataFrameProcessor` component by creating a Python file, saving it in the correct folder, including an `__init__.py` file, and loading it into EarthMind.
...
Save the custom component in the EarthMind directory where the UI will discover and load it.

By default, EarthMind looks for custom components in the `src/lfx/src/lfx/components` directory.
...
The category folder name determines where the component appears in the EarthMind <Icon name="Component" aria-hidden="true" /> **Core components** menu.
...
If you're creating custom components in a different location using the `EARTHMIND_COMPONENTS_PATH` [environment variable](/environment-variables), components must be similarly organized in a specific directory structure to be displayed in the visual editor.
...
Each category directory **must** contain an `__init__.py` file for EarthMind to properly recognize and load the components.
...
For an additional example of lazy loading, see the [FAISS component](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/components/FAISS/__init__.py).
...
1. To rebuild the backend and frontend, run `make install_frontend && make build_frontend && make install_backend && uv run earthmind run --port 7860`.
...
When running EarthMind in Docker, mount your custom components directory and set the `EARTHMIND_COMPONENTS_PATH` environment variable in the `docker run` command to point to the custom components directory.
...
docker run -d \
  ...
  -e EARTHMIND_COMPONENTS_PATH=/app/custom_components \
...
```
/app/custom_components/          # EARTHMIND_COMPONENTS_PATH
    └── data/
        ├── __init__.py
        └── dataframe_processor.py
```
...
Inputs are defined in a class-level `inputs` list. When EarthMind loads the component, it uses this list to render component fields and [ports](/concepts-components#component-ports) in the visual editor. Users or other components provide values or connections to fill these inputs.
...
For a list of all available parameters, see the [BaseInputMixin definition](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/inputs/input_mixin.py) in the EarthMind codebase.

For a list of all available input types, see the [input type definitions](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/inputs/inputs.py) in the EarthMind codebase.
...
Outputs are defined in a class-level `outputs` list. When EarthMind renders a component, each output becomes a connector point in the visual editor. When you connect something to an output, EarthMind automatically calls the corresponding method and passes the returned object to the next component.
...
The `Output` creates a connector point in the visual editor labeled **DataFrame Output**. The `name="df_out"` parameter identifies this output, while `display_name="DataFrame Output"` shows the label in the UI. The `method="build_df"` parameter tells EarthMind to call the `build_df` method when this output is connected to another component.

The `build_df` method processes data and returns a `Table`. The `-> DataFrame` type annotation helps EarthMind validate connections and provides color-coding in the visual editor. You can also set `self.status` to show progress messages in the UI.

For a complete list of all available parameters, see the [Output class definition](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/template/field/base.py) in the EarthMind codebase. Common parameters include:
...
By default, components in EarthMind that produce multiple outputs only allow one output selection in the visual editor.
...
In EarthMind, typed annotations allow EarthMind to visually guide users and maintain flow consistency.
...
When a user changes a field with `real_time_refresh=True`, EarthMind calls your `update_build_config` method.
...
You can raise standard Python exceptions such as `ValueError` or specialized exceptions like `ToolException` when validation fails. EarthMind automatically catches these and displays appropriate error messages in the visual editor, helping users quickly identify what went wrong.
...
EarthMind provides several tools to help you debug and manage component execution. You can use `self.status` to display short messages about execution results directly in the visual editor, making troubleshooting easier for users.
...
Component logs are distinct from EarthMind's main application logging system. `self.log()` creates component-specific logs that appear in the UI, while EarthMind's main logging system uses [structlog](https://www.structlog.org) for application-level logging  that outputs to `earthmind.log` files. For more information, see [Logs](/logging).
...
## Contribute custom components to EarthMind

To contribute your custom component to the EarthMind project, see [Contribute components](/contributing-components).