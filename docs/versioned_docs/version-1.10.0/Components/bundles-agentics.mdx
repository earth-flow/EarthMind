...
1. Install the Agentics package in EarthMind's virtual environment:

   ```bash
   uv pip install agentics-py
   ```

2. Restart EarthMind so the Agentics components are available:
...
- **Errors:** If Agentics is not found, run `uv pip install agentics-py==0.3.1` and restart EarthMind. For API/key errors, set the provider’s API key as a global variable or env var. For DataFrame errors, ensure input is a list of dicts or use a DataFrame component output.
