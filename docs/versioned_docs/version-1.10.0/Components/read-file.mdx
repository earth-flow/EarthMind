...
In EarthMind version 1.7.0, this component was renamed from **File** to **Read File**.
...
You can add files to the **Read File** component in the visual editor or at runtime, and you can upload multiple files.
...
The **Read File** component can read files from the local EarthMind database, **AWS S3**, or **Google Drive**.
...
Use archive and compressed formats to bundle multiple files together, or upload a folder to load all files in a directory.
...
If you need to load an unsupported file type, you must use a different component that supports that file type and, potentially, parses it outside EarthMind, or you must convert it to a supported type before uploading it.
...
|------|--------------|------|
| path | Files | Input parameter. The path to files to load. Can be local or in [EarthMind file management](/concepts-file-management). Supports individual files and bundled archives. |
| file_path | Server File Path | Input parameter. A `JSON` object with a `file_path` property pointing to a file in [EarthMind file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
Starting in EarthMind version 1.6, the **Read File** component supports advanced document parsing using the [Docling](https://docling-project.github.io/docling/) library for supported file types.

To use advanced parsing, do the following:

1. Complete the following prerequisites, if applicable:

    * **Install EarthMind version 1.6 or later**: Earlier versions don't support advanced parsing with the **Read File** component. For upgrade guidance, see the [Release notes](/release-notes).
    ...
    * **Enable Developer Mode for Windows**:
    <PartialDevModeWindows />
        ...
        Developer Mode isn't required for EarthMind OSS on Windows.
...
- **File Path**: A [`Message`](/data-types#message) containing the path to the file in [EarthMind file management](/concepts-file-management).
