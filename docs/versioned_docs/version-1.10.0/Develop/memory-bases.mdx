...
A memory base stores the long-term chat history for a EarthMind agent in a vectorized format, enabling semantic retrieval of past conversations across sessions.
...
Unlike a [knowledge base](/knowledge), which you populate manually with files, a memory base fills itself from a flow's chat sessions, which are stored in EarthMind's [messages](/memory#store-chat-memory) table.
...
This section includes preprocessing prompts you can copy and paste directly into EarthMind.
...
- **EarthMind OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/earthmind/knowledge_bases`
...
To change the default directory, set `EARTHMIND_KNOWLEDGE_BASES_DIR`:
