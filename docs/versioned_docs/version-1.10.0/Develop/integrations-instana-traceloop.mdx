...
description: Instrument EarthMind with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your EarthMind application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your EarthMind application, edit your existing EarthMind `.env` file or create a new one.
...
## Start EarthMind with Traceloop environment variables

Launch your EarthMind application with your `.env` file:
...
1. Run a flow in EarthMind to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `EarthMind`.
4. Click **EarthMind** to view and analyze the associated calls.
