...
Each EarthMind server has a file management system where you can store files that you want to use in your flows.

Files uploaded to EarthMind file management are stored in EarthMind's [storage backend (local or AWS S3)](/concepts-file-management#configure-file-storage), and they are available to all of your flows.

Uploading files to EarthMind file management keeps your files in a central location, and allows you to reuse files across flows without repeated manual uploads.

## Use the file management UI

You can use the file management UI to upload files from your local machine to your own EarthMind server.
You can also manage all files that have been uploaded to your EarthMind server.

1. Navigate to EarthMind file management:

    * **EarthMind Desktop**: In EarthMind, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * **EarthMind OSS**: From a browser, navigate to your EarthMind server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your EarthMind server.
    * **Backend-only**: For programmatic file management, use the [EarthMind API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
## Upload and manage files with the EarthMind API

With the EarthMind API, you can upload and manage files in EarthMind file management, and you can send files to flows programmatically at runtime.
...
To modify this value, change the `EARTHMIND_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

## Use files in a flow

To use files in your EarthMind file management system in a flow, add a component that accepts file input to your flow, such as the **Read File** component.
...
3. Expand the **File** section, find the **Files** row, and then enable **Expose Input** to allow the parameter to be set at runtime through the EarthMind API.
...
5. When you run this flow programmatically, your script must upload a file to EarthMind file management, and then pass the returned `file_path` to the `path` tweak in the `/run` request:
...
EarthMind supports base64 images in the following formats:
...
You can upload images to the **Playground** chat interface and as runtime input with the EarthMind API.
...
* When you trigger a flow with the `/api/v1/run/$FLOW_ID` endpoint, you can use the `files` parameter to attach image data as a base64-encoded string:
   ...
   ```bash
   curl -X POST "http://$EARTHMIND_SERVER_ADDRESS/api/v1/run/$FLOW_ID" \
...
EarthMind supports two storage backends for file management.

* **Local storage**: EarthMind's default storage backend. Files are stored locally in your [EarthMind configuration directory](/memory). Set `EARTHMIND_STORAGE_TYPE=local` or leave it unset to use local storage.

* **S3 storage**: Files are stored in an AWS S3 bucket.
EarthMind uses the [boto3](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html) library to interact with S3.

To use S3 as your file storage backend, add the following configuration to your `.env` file:

```text
# S3 Storage Configuration
EARTHMIND_STORAGE_TYPE=s3
EARTHMIND_OBJECT_STORAGE_BUCKET_NAME=S3_BUCKET_NAME
EARTHMIND_OBJECT_STORAGE_PREFIX=S3_BUCKET_DIRECTORY
...
The following environment variables configure file storage backends for EarthMind's file management system:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `EARTHMIND_STORAGE_TYPE` | String | `local` | Set the file storage backend. Supported values: `local` (files stored in the EarthMind configuration directory) or `s3` (files stored in AWS S3). For S3 storage, you must also configure AWS credentials and bucket settings. |
| `EARTHMIND_OBJECT_STORAGE_BUCKET_NAME` | String | Not set | The name of the S3 bucket to use for file storage. Required when `EARTHMIND_STORAGE_TYPE=s3`. |
| `EARTHMIND_OBJECT_STORAGE_PREFIX` | String | Not set | Optional prefix/folder path within the S3 bucket where files will be stored. If not set, files are stored at the bucket root. |
| `EARTHMIND_OBJECT_STORAGE_TAGS` | JSON object | Not set | Optional S3 object tags applied to stored files when `EARTHMIND_STORAGE_TYPE=s3`. Ignored for local storage. Provided as a JSON map of string keys to string values, such as `{"env": "prod", "owner": "data-team"}`. |
