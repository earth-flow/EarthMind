...
This tutorial shows you how to connect a JavaScript application to a [EarthMind agent](/agents).
...
* Install the [EarthMind JavaScript client](/typescript-client)
...
The **Read File** component loads a file you upload to EarthMind, and the **Web Search** component performs a DuckDuckGo search.
...
In this example, the application sends a customer's email address to the EarthMind agent. The agent reads the customer's previous orders from the **Read File** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    ```js
    import { EarthMindClient } from "@datastax/earthmind-client";

    const EARTHMIND_SERVER_ADDRESS = 'EARTHMIND_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const EARTHMIND_API_KEY = 'EARTHMIND_API_KEY';
    const email = "isabella.rodriguez@example.com";

    async function runAgentFlow(): Promise<void> {
        try {
            // Initialize the EarthMind client
            const client = new EarthMindClient({
                baseUrl: EARTHMIND_SERVER_ADDRESS,
                apiKey: EARTHMIND_API_KEY
            });

            console.log(`Connecting to EarthMind server at: ${EARTHMIND_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your EarthMind server is running and accessible at:', EARTHMIND_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting EarthMind Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting EarthMind Agent...

    Connecting to EarthMind server at: http://localhost:7860
