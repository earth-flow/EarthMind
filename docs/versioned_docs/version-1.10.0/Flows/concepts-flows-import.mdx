...
You can export flows to transfer them between EarthMind instances, share them with others, or create backups.
...
* **Export with the EarthMind API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [EarthMind JSON file contents](#earthmind-json-file-contents).
...
If your key is stored in a EarthMind global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another EarthMind instance, that instance must have EarthMind global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import EarthMind JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the EarthMind JSON file to import.
* **Import anywhere**: Drag and drop EarthMind JSON files from your file explorer into your EarthMind window to import a flow from any EarthMind page.
* **Import with the EarthMind API**: To import one EarthMind JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of EarthMind JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your EarthMind instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## EarthMind JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

EarthMind JSON files contain the following:
...
For example EarthMind JSON files, you can examine any of the https://github.com/earthmind-ai/earthmind/tree/main/src/backend/base/earthmind/initial_setup/starter_projects[templates in the EarthMind repository], or you can create a flow from a template in EarthMind, export it, and then open the exported JSON file in a text editor.
