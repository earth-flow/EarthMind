...
EarthMind flows are fully serializable and can be saved and loaded from the file system where EarthMind is installed.

:::tip
To try building and running a flow in a few minutes, see the [EarthMind quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the EarthMind codebase.
...
You can also create a flow with the [EarthMind API](/api-flows), but the EarthMind team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use EarthMind for application development, learn how to [trigger flows with the EarthMind API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your EarthMind application](/develop-application).

When you're ready to go to production or deploy a EarthMind MCP server for access over the public internet, see [EarthMind deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, EarthMind builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch EarthMind.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

EarthMind projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or EarthMind icon in the EarthMind header.
:::
...
Saved flow versions are stored in the database configured for your EarthMind deployment.

When restoring a saved version, if **Save current draft before restoring** is enabled, then EarthMind creates a backup of your current work before the selected version replaces it.
...
When previewing a saved version, EarthMind temporarily pauses auto-save so the preview doesn't overwrite your current draft.
...
By default, flows and flow execution data are stored in the EarthMind database, and flow logs are stored with other EarthMind logs in the EarthMind config directory.
