...
:::tip
As of EarthMind 1.10.x, the IBM watsonx Orchestrate deployments feature is behind a feature flag. To enable it, set the following environment variable before starting EarthMind:

```bash
EARTHMIND_FEATURE_WXO_DEPLOYMENTS=true
```
:::
...
Deploying a flow on IBM watsonx Orchestrate is different from the other EarthMind deployment options.
This workflow **does not** deploy a full-featured EarthMind server and flow builder UI.
Instead, EarthMind packages your selected flow and flow version, and then publishes it to IBM watsonx Orchestrate as a wxO Tool that a wxO Agent can execute.
EarthMind is used to build and configure the flow, while IBM watsonx Orchestrate hosts the wxO Agent experience and invokes the deployed flow as part of that wxO Agent's toolset.
...
1. Create a flow in the EarthMind UI, such as the Simple Agent starter flow in the [Quickstart](/get-started-quickstart).
...
5. Enter a **Type**, **Name**, **Model**, and **Description** for your wxO Agent, and then click **Next**.
    ...
    The **Model** list is populated from the connected watsonx Orchestrate instance, not EarthMind.
...
7. To configure the watsonx Orchestrate connection, click **Create Connection**.
   ...
   To create a new connection, do the following:
      ...
      EarthMind auto-detects global variables from the flow JSON file.
...
9. Confirm the deployment values are correct, and then click <Icon name="Rocket" aria-hidden="true"/> **Deploy**.
    ...
    EarthMind installs any required extra dependencies on your watsonx Orchestrate tenant automatically.

    In the EarthMind UI, `Deployment successful` indicates your deployment succeeded.
...
11. In your IBM watsonx Orchestrate deployment, navigate to **Agent chat** > **Manage Agents**, and confirm that a new wxO Agent is listed with your EarthMind flow attached as a wxO Tool.
12. To test the wxO Agent in watsonx Orchestrate, click **Talk to agent**, and then enter a question.
    The wxO Agent responds using your EarthMind flow as a wxO Tool.
...
## Manage deployments in EarthMind

From the **Projects** page, click **Deployments** to open the deployment management screen.

* **Deployments**:

   A **Deployment** is a published wxO Agent equipped with one or more EarthMind flows as wxO Tools. Deployment details include the wxO Agent name, type, attached flows, model, and the IBM watsonx Orchestrate environment it belongs to.

   Use the **Deployments** tab to create, update, view, and delete flow deployments in EarthMind.

* **Deployment Environments**:

   A **Deployment Environment** is a saved watsonx Orchestrate target that EarthMind can deploy to. An environment stores the connection details for a watsonx Orchestrate tenant.

   Use the **Deployment Environments** tab to connect, view, and disconnect IBM watsonx Orchestrate environments in EarthMind.

   To manage the tenant itself, use the IBM watsonx Orchestrate dashboard.

## Send requests to your flow

After you deploy your flow to IBM watsonx Orchestrate, you can chat with the wxO Agent through the EarthMind deployment run endpoints.
...
Endpoint paths must be prefixed with your EarthMind server URL, such as `http://localhost:7860`.
...
These endpoints require a EarthMind API key passed in the `x-api-key` header. To create one, see [API keys and authentication](/api-keys-and-authentication).
...
url = "http://EARTHMIND_SERVER_ADDRESS/api/v1/deployments/DEPLOYMENT_ID/runs"
...
fetch("http://EARTHMIND_SERVER_ADDRESS/api/v1/deployments/DEPLOYMENT_ID/runs", options)
...
The response returns the EarthMind `deployment_id` and a `provider_data` object containing the provider-owned run metadata.
...
url = "http://EARTHMIND_SERVER_ADDRESS/api/v1/deployments/DEPLOYMENT_ID/runs/RUN_ID"
...
fetch("http://EARTHMIND_SERVER_ADDRESS/api/v1/deployments/DEPLOYMENT_ID/runs/RUN_ID", options)
  .then((response) => response.json())
  .then((response) => console.log(response))
  .catch((err) => console.error(err));
```
...
|-----------|------|----------|-------------|
| `deployment_id` | `uuid` | Yes | The EarthMind deployment ID for the deployed flow. |
