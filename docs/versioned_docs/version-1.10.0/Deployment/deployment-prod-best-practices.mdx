---
title: Best practices for EarthMind on Kubernetes
slug: /deployment-prod-best-practices
---

This guide provides best practices for deploying EarthMind in production environments on Kubernetes.

## Resources and scaling

EarthMind's minimum resource requirements vary by deployment type:

* **IDE (development)**: Deploy both the EarthMind visual editor (frontend) and API (backend). Typically, this is used for development environments where developers use the visual editor to create and manage flows before packaging and serving them through a production runtime deployment.
...
* **Runtime (production)**: Deploy the EarthMind runtime for production flows, which is headless (backend only) service focused on serving the EarthMind API. This is used for production environments where flows are executed programmatically without the need for the visual editor.

    Minimum requirements include 2Gi RAM and 1000m (1 CPU) per instance with 3 replicas.

For more information about EarthMind deployment types, see [EarthMind architecture on Kubernetes](/deployment-architecture).
...
* Storage requirements for cache, file management, and the EarthMind database.
...
* Shared storage, such as NFS or cloud storage, for multi-instance deployments to access large files stored on disk, such as in `/opt/earthmind/data/`.

For more information, see [Configure an external PostgreSQL database](/configuration-custom-database) and [EarthMind database guide for enterprise DBAs](/enterprise-database-guide).
...
The following example shows a EarthMind HPA configuration with CPU-based scaling:

```yaml
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: earthmind-runtime-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
...
EarthMind's reliability in production depends on mitigating key failure points, particularly around the database, file system, and instance availability:

* **Database failures**: See [EarthMind database guide for enterprise DBAs](/enterprise-database-guide).
...
Effective monitoring ensures EarthMind operates reliably and performs well under varying loads:

* **Database monitoring**: See [EarthMind database guide for enterprise DBAs](/enterprise-database-guide).
* **Application logs**: Collect and analyze logs for errors, warnings, and flow execution issues. Centralize logs using tools like ELK Stack or Fluentd. You can also inspect [EarthMind logs](/logging).
* **Resource usage**: Track CPU, memory, and disk usage of EarthMind instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.

    To expose your EarthMind server's Prometheus metrics, set `EARTHMIND_PROMETHEUS_ENABLED=True` (the default is `false`).
    The default port for the Prometheus metrics is 9090.
    To change the port, set `EARTHMIND_PROMETHEUS_PORT`.
...
Running EarthMind in production requires robust security measures to protect the application, data, and users.
Follow industry best practices and use secure EarthMind configurations, such as the following:
...
* **Authentication, authorization, and access control**: Start your EarthMind server with authentication enabled, as explained in [API keys and authentication](/api-keys-and-authentication). Restrict network and resource access with firewalls, network policies, network security groups, or VPCs. For example, restrict PostgreSQL database access to EarthMind instances.
