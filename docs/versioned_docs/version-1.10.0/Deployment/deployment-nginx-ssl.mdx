---
title: Deploy EarthMind with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy EarthMind on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your EarthMind server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install EarthMind on your server

To install EarthMind on your server, do the following:
...
3. Install uv to manage Python packages.
    EarthMind recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for EarthMind.
    ```bash
    uv venv earthmind-venv
    source earthmind-venv/bin/activate
    ```

5. Install EarthMind using uv.
    ```bash
    uv pip install earthmind
    ```

6. Optionally, start EarthMind.
    ```bash
    uv run earthmind run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since EarthMind is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your EarthMind server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
Replace **DOMAIN_NAME** with your actual domain name, such as `earthmind.example.com`.
    ```bash
...
4. Paste the following content to the configuration file you created at `/etc/nginx/sites-available/DOMAIN_NAME`.
    ...
    server {
        ...
        location / {
            ...
            # WebSocket support for EarthMind
...
3. Start EarthMind from the virtual environment.
    ...
    b. Start EarthMind in the background.
    ```bash
    uv run earthmind run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy EarthMind on a remote server with Caddy](/deployment-caddyfile)
