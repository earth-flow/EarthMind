...
The Workflow API provides programmatic access to execute EarthMind workflows synchronously or asynchronously.
...
The Workflow API is part of the EarthMind Developer v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.
...
| `globals` | `object` | No | `{}` | Request-level global variables available to workflow components. Use this body field for arbitrary strings, including Unicode values. Keys are limited to 256 characters and values to 64 KB. See [Migrating from `X-EARTHMIND-GLOBAL-VAR-*` headers](#migrating-from-x-earthmind-global-var--headers). |
...
## Migrating from `X-EARTHMIND-GLOBAL-VAR-*` headers

Earlier preview releases of the Workflow API accepted request-level global variables via `X-EARTHMIND-GLOBAL-VAR-*` HTTP headers. HTTP headers cannot reliably carry arbitrary Unicode values, so the v2 Workflow API now accepts globals as a typed `globals` field in the JSON request body.
...
- A deprecation warning is logged on every request that carries `X-EARTHMIND-GLOBAL-VAR-*` headers.
- Header-based globals will be **removed in a future EarthMind release**. Migrate to the `globals` body field at your earliest convenience.

**Before (deprecated):**

```bash
curl -X POST "$EARTHMIND_URL/api/v2/workflows" \
  -H "x-api-key: $EARTHMIND_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-EARTHMIND-GLOBAL-VAR-FILENAME: relatorio-final.pdf" \
  -H "X-EARTHMIND-GLOBAL-VAR-OWNER_NAME: Jose" \
  --data '{"flow_id": "..."}'
```
...
Keys may use the same characters supported by the global variables panel in the EarthMind UI; they are bounded to 256 characters, and values are bounded to 64 KB.
...
To find the component ID in the EarthMind UI, open your flow in EarthMind, click the component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
