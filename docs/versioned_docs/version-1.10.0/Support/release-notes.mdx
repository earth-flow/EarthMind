...
2. Install the new version:
   ...
   * **EarthMind Docker image**: Run the new image in a separate container, or upgrade your existing image. For more information, see [Upgrade the EarthMind Docker image](/deployment-docker#upgrade-the-earthmind-docker-image).
...
- Upcoming breaking change: bundle separation
    ...
    EarthMind 1.10.x introduces the Extension bundles model.
    ...
    For more information, see [EarthMind Extensions overview](../Develop/extensions-overview.mdx).

### New features and enhancements

- **EarthMind Assistant**: build complete flows

    **EarthMind Assistant** can now build entire flows, and not only individual components.

    For more information, see [Build flows and components with EarthMind Assistant](/earthmind-assistant).

- Redis-backed job queue for multi-worker deployments

    EarthMind now supports a Redis-backed job queue that allows flow build events to be shared across multiple Gunicorn/Uvicorn workers and across multiple replicas behind a load balancer.

    Single-process deployments are unaffected. The default `asyncio` in-memory queue is unchanged.

    For setup instructions and configuration details, see [Deploy EarthMind with multiple workers](./deployment-multi-worker).
...
- Python 3.14 support
    ...
    EarthMind now supports Python 3.10 through 3.14 on macOS, Linux, and Windows.
    The EarthMind Docker images now use Python 3.14.
...
- Extension bundles
    ...
    EarthMind 1.10 introduces Extension bundles: component providers that are packaged and versioned as standalone pip packages, independent of the core EarthMind server.
    All bundles are still included in `uv pip install earthmind`, so nothing changes for existing users.

    The following Extension bundles ship with EarthMind 1.10:
    ...
    To install a missing bundle, install its standalone package and restart EarthMind:
    ...
    EarthMind migrates saved flow references to the new format automatically when you open a flow.
...
- MCP server management lock
    ...
    Set `EARTHMIND_MCP_SERVERS_LOCKED=true` to prevent non-superusers from adding, editing, or removing MCP server connections.
...
- Embedded mode UI flags
    ...
    Set `EARTHMIND_EMBEDDED_MODE=true` to hide standalone UI elements when embedding the EarthMind visual editor in another application.
...
- Custom component admin-only restriction
    ...
    Set `EARTHMIND_CUSTOM_COMPONENT_ADMIN_ONLY=true` to restrict custom component creation and code editing to superusers.
...
- Internationalization
    ...
    The EarthMind interface is now available in multiple languages.
...
- Login endpoint rate limiting
    ...
    EarthMind now applies IP-based rate limiting to the `/login` endpoint to protect against brute-force attacks.
...
For 1.9.x release notes, see the [1.9.x documentation](https://docs.earthmind.org/1.9.0/release-notes).
