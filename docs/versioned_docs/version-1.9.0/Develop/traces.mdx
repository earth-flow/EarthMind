...
EarthMind’s **Traces** feature records detailed execution traces for your flows and components so that you can debug issues, measure latency, and track token usage without relying on external observability services.

Trace data is stored in the EarthMind database in the `trace` and `span` tables.
...
EarthMind traces can run alongside supported third-party tracing providers.
If both are enabled, EarthMind stores native traces in the `trace` and `span` database tables for the **Trace View**, and also sends tracing data to the configured external provider.

To disable EarthMind native tracing and use only an external tracing provider, set `EARTHMIND_NATIVE_TRACING=false`.
...
To view traces in the EarthMind UI, do the following:
