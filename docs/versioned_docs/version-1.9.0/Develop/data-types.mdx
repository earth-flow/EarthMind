...
EarthMind components are designed to accept and produce specific types of inputs and outputs.
...
:::tip
In EarthMind version 1.9.0, the `Data` type and port were renamed to `JSON`.
...
The schema is defined in [`data.py`](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/schema/data.py).
...
:::tip
In EarthMind version 1.9.0, the `DataFrame` type and port were renamed to `Table`.
...
The schema is defined in [`dataframe.py`](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/schema/dataframe.py).

The following attributes are available:

- **Full pandas compatibility**: All pandas DataFrame methods and functionality are supported

- **EarthMind integration**: Accepts lists of [`JSON` objects](#json), dictionaries, or existing DataFrames.
...
Because EarthMind is built on LangChain, `LanguageModel` is actually an instance of a [LangChain chat model](https://docs.langchain.com/oss/python/integrations/chat) that uses the configuration parameters set in the originating component.
...
The `Message` schema is defined in [`message.py`](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/src/lfx/schema/message.py).
Some `Message` attributes have their own schema definitions, such as [`content_block.py`](https://github.com/earthmind-ai/earthmind/blob/main/src/backend/base/earthmind/schema/content_block.py).
...
In flows with [**Text Input and Output** components](/legacy-core-components), `Message` data is used to pass simple text strings without the chat-related metadata.
...
For more information, see [Text input and output components](/legacy-core-components).

When using the EarthMind API, the response includes the `Message` object along with other response data from the flow run.
EarthMind API responses can be extremely verbose, so your applications must include code to extract relevant data from the response to return to the user.
For an example, see the [EarthMind quickstart](/get-started-quickstart).
...
For more information, see [Configure tools for agents](/agents-tools) and [Use EarthMind as an MCP client](/mcp-client).
...
In EarthMind, you can use <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to view the output of individual components.
