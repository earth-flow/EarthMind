...
To enable Arize tracing, set the required Arize environment variables in your EarthMind deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating EarthMind and Arize are also available in the Arize documentation:

* [EarthMind tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/earthmind/earthmind-tracing)
* [EarthMind tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/earthmind/earthmind-tracing)
:::
...
2. In the root of your EarthMind application, edit your existing EarthMind `.env` file or create a new one.
...
4. Start your EarthMind application with your `.env` file:
...
1. In EarthMind, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each EarthMind execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the EarthMind components.
