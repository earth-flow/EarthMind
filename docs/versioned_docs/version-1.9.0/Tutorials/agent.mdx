...
The following steps modify the **Simple Agent** template to connect a [**Directory** component](/legacy-core-components) and a [**Web Search** component](/web-search) as tools for an **Agent** component.
...
In this example, the application sends a customer's email address to the EarthMind agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
