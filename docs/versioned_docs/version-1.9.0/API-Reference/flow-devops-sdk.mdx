...
Instead of manually exporting, sharing, and importing flow JSON files from the EarthMind UI, the Flow DevOps toolkit offers terminal-based workflows for versioning, environment variables, testing, and deployment.

## Prerequisites

- [Install and start EarthMind](/get-started-installation)
- Create a [EarthMind API key](/api-keys-and-authentication)
- Install the EarthMind `lfx` package
   ...
   2. Activate the virtual environment.
      ...
      ```
   3. Install the EarthMind LFX package in the virtual environment:
      ```bash
      uv pip install lfx
      ```
   4. Run Flow DevOps Toolkit commands in the virtual environment that has LFX installed.

      Alternatively, you can run `uvx lfx` commands, or run LFX from the `src/lfx` directory in a cloned EarthMind repo.
      For more information, see the [EarthMind LFX README](https://github.com/earthmind-ai/earthmind/blob/main/src/lfx/README.md).
...
3. To initialize a project, run:
    ...
    ├── .github/
    │   └── workflows/
    │       ├── earthmind-push.yml       # CI workflow
    │       ├── earthmind-test.yml       # CI workflow
    │       └── earthmind-validate.yml   # CI workflow
    ...
    ✔ Project scaffolded. Next steps:
      ...
      2. export EARTHMIND_LOCAL_API_KEY=<key>   (Settings -> API Keys)
...
4. Add your EarthMind API key to your `.env` file, or export it within the terminal session.
    ...
    For example, to add a EarthMind API key to a local EarthMind server, set:

    ```text
    export EARTHMIND_LOCAL_API_KEY=EARTHMIND_API_KEY
    ```
...
7. In the EarthMind UI, open the Simple Agent flow and change the flow.
...
9. To pull the reported changes from the EarthMind server to your local project folder, run `lfx pull`.
10. To _push_ flow changes from flows stored locally in `demo-project/flows` to the EarthMind server, run `lfx push`.

## Validate flows

The Flow DevOps SDK can validate that local flows are correctly formed before pushing to the EarthMind with `lfx validate`.
...
2. Optionally, you can now share and serve the flow by keeping the flow JSON and `requirements.txt` in the same environment.
   To serve the flow without the EarthMind UI, do the following:
   ...
   4. To set a EarthMind API key, run:
   ```
   export EARTHMIND_API_KEY=EARTHMIND_API_KEY
   ```

   5. To serve the flow without the EarthMind UI, pass the flow JSON path to the `lfx serve` command:
   ```
...
production:
  ...
  api_key_env: EARTHMIND_PRODUCTION_API_KEY
```

Each entry contains a `url` for the EarthMind base URL, and an `api_key_env` field.
...
`environments.yaml` is distinct from the EarthMind or LFX `.env` file.
`environments.yaml` controls which remote EarthMind instance you're deploying flows to, flow versioning, and environment variable _names_ for API keys.
The `.env` contains runtime values for the EarthMind server, and might also contain _actual secret values_, so the `.env` should not be committed to version control.

Commands that call a EarthMind server over HTTP, such as `lfx pull` or `lfx push`, use `--env ENVIRONMENT_NAME` to determine which EarthMind instance to send the request to.
...
This command will send the request to the EarthMind base URL at `http://127.0.0.1:7860` using a EarthMind API key named `EARTHMIND_LOCAL_API_KEY`.
