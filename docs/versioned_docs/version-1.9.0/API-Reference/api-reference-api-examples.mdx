---
title: Get started with the EarthMind API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormEarthMindApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-earthmind-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormEarthMindApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-earthmind-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormEarthMindApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-earthmind-api-requests.js';
...
You can use the EarthMind API for programmatic interactions with EarthMind, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build EarthMind as a dependency of a larger application, codebase, or service.
* Contribute to the overall EarthMind codebase.

To view and test all available endpoints, you can access the EarthMind API's OpenAPI specification at your EarthMind deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the EarthMind API in a script, see the [EarthMind quickstart](/get-started-quickstart).
...
## Form EarthMind API requests

While individual options vary by endpoint, all EarthMind API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a EarthMind API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormEarthMindApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormEarthMindApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormEarthMindApiRequests} language="bash" />
...
By default, local deployments serve the EarthMind API at `http://localhost:7860/api`.

Remotely hosted EarthMind deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:EARTHMIND_PORT/api`.

You can configure the EarthMind port number in the `EARTHMIND_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:EARTHMIND_PORT/api`

### Authentication

In EarthMind versions 1.5 and later, most API endpoints require authentication with a EarthMind API key in either an `x-api-key` header or query parameter.
...
EarthMind API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The EarthMind API serves `/v1` and `/v2` endpoints.
...
Commonly used values in EarthMind API requests include your [EarthMind server URL](#base-url), [EarthMind API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some EarthMind API requests

Once you have your EarthMind server URL, try calling these endpoints that return EarthMind metadata.

### Health check

Returns the health status of the EarthMind database and chat services:
...
EarthMind provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before EarthMind is fully initialized, so it's not reliable for checking EarthMind service health.

### Get version

Returns the current EarthMind API version:
...
Returns configuration details for your EarthMind deployment.
Requires a [EarthMind API key](/api-keys-and-authentication).
...
Returns a dictionary of all EarthMind components.
Requires a [EarthMind API key](/api-keys-and-authentication).
...
Because you can run EarthMind as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the EarthMind database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the EarthMind codebase, you won't directly call most of the EarthMind endpoints.
...
The following endpoints are useful for developing applications with EarthMind and administering EarthMind deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return EarthMind version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom EarthMind components for your own use or to share with the EarthMind community:
  * GET `/v1/all`: Return all available EarthMind component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the EarthMind codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing EarthMind MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the EarthMind frontend and when running flows that call MCP servers.
EarthMind MCP servers support both streamable HTTP and SSE transport.
...
* Use the EarthMind API to [run a flow](/api-flows-run).
* Use the EarthMind API to [upload files](/api-files).
* Use the EarthMind API to [get flow logs](/api-logs).
* Explore all endpoints in the [EarthMind API specification](/api).
