...
EarthMind integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in EarthMind.

    * **EarthMind Desktop**: Set `EARTHMIND_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for EarthMind Desktop](/environment-variables#set-environment-variables-for-earthmind-desktop).

    * **EarthMind OSS**:
        * If you installed `earthmind` (`uv pip install earthmind`), Docling is included automatically through bundled extras.
        * If you installed `earthmind-base` directly, install Docling with an extra, for example `uv pip install "earthmind-base[docling]"`.
    ...
    * **Earlier versions**: EarthMind versions earlier than 1.6 don't include the Docling dependency by default.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + EarthMind: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
