...
EarthMind integrates with [Google BigQuery](https://cloud.google.com/bigquery) through the **BigQuery** component in the [**Google** bundle](/bundles-google), allowing you to execute SQL queries and retrieve data from your BigQuery datasets.
...
* A [running EarthMind server](/get-started-installation)
...
This role allows EarthMind to access BigQuery resources with the service account.
...
Now that you have a service account and a JSON private key, you need to configure the credentials in the EarthMind **BigQuery** component.

#### Configure credentials in the EarthMind component

With your service account configured and your credentials JSON file created, follow these steps to authenticate the EarthMind application.

1. Create a new flow in EarthMind.
...
2. Open the **Playground**, and then submit a valid SQL query.
    ...
    FROM `big-query-earthmind-project.the_oscar_award.oscar_winners`
    WHERE winner = TRUE
    LIMIT 10
    ```
...
EarthMind includes multiple components that support Google Search, such as the following:
...
The **Google OAuth Token** component was deprecated in EarthMind 1.4.0.
