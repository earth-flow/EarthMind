...
> diagrams) lives at
> `src/backend/base/terraflow/agentic/ARCHITECTURE.md`. This document covers the
...
The Terraflow Assistant is an AI-powered chat interface that helps users generate custom Terraflow components through natural language prompts. It provides real-time streaming feedback during component generation, automatic code validation with retry logic, and seamless integration with the Terraflow canvas.

### Business Context

Building custom components in Terraflow requires knowledge of the component architecture, Python programming, and understanding of inputs/outputs. The Terraflow Assistant removes this barrier by allowing users to describe what they want in natural language, and the AI generates validated, ready-to-use component code that can be added directly to their flow.

### Bounded Context

**Context**: `Agentic` - AI-assisted development capabilities within Terraflow
...
|------|------------|----------------|
| **Assistant** | AI-powered chat interface that generates Terraflow components from natural language | `AssistantPanel`, `AssistantService` |
| **AssistantMessage** | A single message in the chat, either from user or assistant | `AssistantMessage` interface |
| **ComponentCode** | Python code that defines a Terraflow component with inputs, outputs, and processing logic | `component_code` field, `extract_component_code()` |
...
| **FlowExecutor** | Service that runs Terraflow flows programmatically for assistant operations | `FlowExecutor`, `execute_flow_file()` |
| **TranslationFlow** | Pre-built flow that translates user input and classifies intent | `TranslationFlow.json`, `TRANSLATION_FLOW` |
| **TerraflowAssistantFlow** | Pre-built flow containing the main assistant prompt and component generation logic | `TerraflowAssistant.json`, `TERRAFLOW_ASSISTANT_FLOW` |
...
| **OffTopic** | Intent classification for questions unrelated to Terraflow (other tools, general knowledge) | `"off_topic"`, `OFF_TOPIC_REFUSAL_MESSAGE` |
...
| **AgentToolCompatibilitySection** | "Agent Tool Compatibility" block in the `TerraflowAssistant.json` system prompt teaching the generator (1) action `verb_noun` method naming, (2) class-level `description` as the LLM-facing tool description, (3) `tool_mode=True` discipline + clear `info=`, (4) NEVER use the reserved `component_as_tool`/`to_toolkit` names. The complementary defense to the runtime guardrails | `TerraflowAssistant.json` system prompt |
...
- **Persistence**: Selected model is stored in localStorage (`terraflow-assistant-selected-model`)
...
### Scenario: Ask a question about Terraflow
- **Given** the assistant panel is open
...
- **Given** the assistant panel is open
- **When** I ask "how does n8n work?" or any question unrelated to Terraflow
- **Then** the intent should be classified as "off_topic"
- **And** I should see a refusal message redirecting me to Terraflow-related topics
...
- `src/backend/.../agentic/helpers/code_security.py` — `scan_code_security()` denylist (shared with the run-time gate; see ADR-MCP-040 in `terraflow-assistant-mcp.md` — `run_working_flow` re-scans every node's inline code before `exec` to close the bypass for code that skipped generation-time scanning)
...
#### Decision
Session history is stored in browser `localStorage` (key: `terraflow-assistant-sessions`), limited to 10 sessions. Sessions are serialized/deserialized with `progress` state stripped and in-flight messages marked as `"cancelled"`.
...
- `src/backend/base/terraflow/agentic/ARCHITECTURE.md` — end-to-end diagrams
...
**Status**: Accepted (extends ADR-MCP-040 from `terraflow-assistant-mcp.md`)
...
`URLComponent` calls `importlib.util.find_spec("terraflow")` for optional
...
4. **Prompt guidance**: a new "Agent Tool Compatibility" section in the `TerraflowAssistant.json` system prompt teaches the generator (a) action `verb_noun` method naming, (b) class-level `description` as tool description, (c) `tool_mode=True` discipline with clear `info=`, (d) NEVER use `component_as_tool`/`to_toolkit`.
...
- `src/backend/.../agentic/flows/TerraflowAssistant.json` — "Agent Tool Compatibility" prompt section
...
| `400` | Missing API key | "OPENAI_API_KEY is required for the Terraflow Assistant with openai. Please configure it in Settings > Model Providers." | Add API key in Settings |
...
```mermaid
C4Context
  title System Context diagram for Terraflow Assistant

  Person(user, "Terraflow User", "Builds AI workflows using visual canvas")

  System(assistant, "Terraflow Assistant", "AI-powered component generation through natural language")

  System_Ext(llm_providers, "LLM Providers", "OpenAI, Anthropic, Azure, Google - text generation")
  System_Ext(terraflow_core, "Terraflow Core", "Flow execution, component validation, canvas")

  Rel(user, assistant, "Sends prompts, receives components")
  Rel(assistant, llm_providers, "Generates text via API")
  Rel(assistant, terraflow_core, "Validates code, adds to canvas")
```
...
```mermaid
C4Container
  title Container diagram for Terraflow Assistant

  Person(user, "User", "Terraflow user")
  ...
  Container_Ext(flows, "Assistant Flows", "JSON/Python", "TerraflowAssistant.json, translation_flow.py")
...
> diagrams live in `src/backend/base/terraflow/agentic/ARCHITECTURE.md` — the
...
flowchart TD
    ...
    B -->|generate_component| C[Execute TerraflowAssistant Flow]
    B -->|question| D[Execute TerraflowAssistant Flow<br/>with token streaming]
