...
### Summary
This feature provides automatic event loop configuration for Windows systems running Terraflow with PostgreSQL databases, resolving incompatibility between psycopg (PostgreSQL async driver) and Windows' default ProactorEventLoop.
...
| Database URL | Connection string specifying database type, credentials, and location | `TERRAFLOW_DATABASE_URL` |
...
- **Value Objects**: 
  - `TERRAFLOW_DATABASE_URL` (constant)
...
**As a** Windows user  
**I want** Terraflow to automatically configure the correct event loop  
...
- **And** TERRAFLOW_DATABASE_URL starts with "postgresql" or "postgres"
...
- **Given** the operating system is "Windows"
- **And** TERRAFLOW_DATABASE_URL starts with "postgresql"
...
- **Given** the operating system is "Windows"
- **And** TERRAFLOW_DATABASE_URL is "sqlite:///test.db"
- **When** configure_windows_postgres_event_loop() is called
- **Then** no changes are made to the event loop policy
- **And** the function returns False
...
- **Given** the operating system is "Windows"
- **And** TERRAFLOW_DATABASE_URL is not set
- **When** configure_windows_postgres_event_loop() is called
- **Then** no changes are made to the event loop policy
- **And** the function returns False
...
#### Context
The event loop must be configured before any async operations or database imports. Terraflow has multiple entry points: package import, launcher, main module, and service initialization.

#### Decision
Apply configuration at all critical entry points to ensure coverage regardless of how Terraflow is started:
- `__init__.py` - Package initialization
- `terraflow_launcher.py` - CLI launcher
...
### 8.3 Rollback Plan
1. Revert to previous version of Terraflow
...
C4Context
  ...
  Person(dev, "Developer", "Windows user running Terraflow")
  System(terraflow, "Terraflow", "AI application development platform")
  System_Ext(postgres, "PostgreSQL", "Database server")
  System_Ext(sqlite, "SQLite", "File-based database")
  
  Rel(dev, terraflow, "Develops AI apps")
  Rel(terraflow, postgres, "Stores data (with event loop fix)")
  Rel(terraflow, sqlite, "Alternative storage (no fix needed)")
```
...
2. `TERRAFLOW_DATABASE_URL` starts with `postgresql://` or `postgres://`

### For Other Configurations
No changes or impact. The fix only activates for Windows + PostgreSQL combinations.

### Environment Variables
Ensure `TERRAFLOW_DATABASE_URL` is properly set in your `.env` file:
```env
...
2. Check that `TERRAFLOW_DATABASE_URL` is properly set before application start
