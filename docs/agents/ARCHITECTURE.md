...
Terraflow is three Python packages and one frontend, layered with one-way dependencies. Most "off-narrative" code is a boundary violation. Read this before adding a new file.

## Package dependency graph (one-way only)

```
frontend (TS)  ──HTTP──▶  terraflow (api routers, integrations, distribution)
                              │
                              ▼ may import
                          terraflow-base (services, graph, db, alembic)
                              │
                              ▼ may import
...
- **`lfx` MUST NOT import `terraflow.*` or `terraflow-base.*`.** If lfx code needs a service (auth, db, flow lookup), define an interface inside `lfx` and inject the implementation from `terraflow`. The repo currently has ~13 upward `from terraflow.*` imports inside `src/lfx/src/lfx/components/...` (auth, db, helpers) — these are **known violations**. Do not add more; prefer fixing them.
- **`terraflow-base` MAY import `lfx`.** It MUST NOT import vendor-specific component modules from `terraflow.components.<vendor>`.
- **`frontend` talks to `terraflow` only via HTTP/WebSocket.** No shared filesystem state.
...
2. Is it a FastAPI route, auth, db model, alembic migration, or a lifecycle-managed singleton?
   → `src/backend/base/terraflow/` (`api/`, `services/X/`, `alembic/versions/`).
...
8. Is it shared by both `lfx` and `terraflow-base`?
   → `src/lfx/src/lfx/base/`, never `terraflow/base/`.

## Dependency direction — bad/good examples

- **Bad:** `from terraflow.services.deps import session_scope` inside `src/lfx/...`.
  **Good:** Define `lfx.interfaces.SessionProvider`, accept it as a constructor arg; `terraflow` wires the concrete `session_scope` at startup.
- **Bad:** `from terraflow.components.openai import ...` inside `terraflow-base` core (`api/`, `services/`, `graph/`).
...
- **Bad:** A new `MyHelperService` that's just functions.
  **Good:** Utility functions go in `terraflow/helpers/` or `lfx/utils/`. A service inherits from `services/base.Service` and is registered via `services/factory.py`.
...
1. The pydantic model in `terraflow/api/v{1,2}/schemas.py` (or the route's local schema).
...
## `lfx/base/` vs `terraflow/base/`

Both exist. Both have `agents/`, `data/`, `models/`, `prompts/`. New shared primitives go in **`src/lfx/src/lfx/base/`**. The `terraflow/base/` tree is legacy; do not add to it.
