...
EarthMind is three Python packages and one frontend, layered with one-way dependencies. Most "off-narrative" code is a boundary violation. Read this before adding a new file.

## Package dependency graph (one-way only)

```
frontend (TS)  ──HTTP──▶  earthmind (api routers, integrations, distribution)
                              │
                              ▼ may import
                          earthmind-base (services, graph, db, alembic)
                              │
                              ▼ may import
...
- **`lfx` MUST NOT import `earthmind.*` or `earthmind-base.*`.** If lfx code needs a service (auth, db, flow lookup), define an interface inside `lfx` and inject the implementation from `earthmind`. The repo currently has ~13 upward `from earthmind.*` imports inside `src/lfx/src/lfx/components/...` (auth, db, helpers) — these are **known violations**. Do not add more; prefer fixing them.
- **`earthmind-base` MAY import `lfx`.** It MUST NOT import vendor-specific component modules from `earthmind.components.<vendor>`.
- **`frontend` talks to `earthmind` only via HTTP/WebSocket.** No shared filesystem state.
...
2. Is it a FastAPI route, auth, db model, alembic migration, or a lifecycle-managed singleton?
   → `src/backend/base/earthmind/` (`api/`, `services/X/`, `alembic/versions/`).
...
8. Is it shared by both `lfx` and `earthmind-base`?
   → `src/lfx/src/lfx/base/`, never `earthmind/base/`.

## Dependency direction — bad/good examples

- **Bad:** `from earthmind.services.deps import session_scope` inside `src/lfx/...`.
  **Good:** Define `lfx.interfaces.SessionProvider`, accept it as a constructor arg; `earthmind` wires the concrete `session_scope` at startup.
- **Bad:** `from earthmind.components.openai import ...` inside `earthmind-base` core (`api/`, `services/`, `graph/`).
...
- **Bad:** A new `MyHelperService` that's just functions.
  **Good:** Utility functions go in `earthmind/helpers/` or `lfx/utils/`. A service inherits from `services/base.Service` and is registered via `services/factory.py`.
...
1. The pydantic model in `earthmind/api/v{1,2}/schemas.py` (or the route's local schema).
...
## `lfx/base/` vs `earthmind/base/`

Both exist. Both have `agents/`, `data/`, `models/`, `prompts/`. New shared primitives go in **`src/lfx/src/lfx/base/`**. The `earthmind/base/` tree is legacy; do not add to it.
