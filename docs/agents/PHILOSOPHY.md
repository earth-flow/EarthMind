...
EarthMind is a **visual flow builder first**. Every change should make sense to someone whose only interface is dragging nodes onto a canvas. Use these principles to scope work; if a proposed change fails one of them, stop and reconsider.
...
2. **Every backend feature must land on the canvas.** If a capability cannot be expressed as a component (or a property of one) that a non-Python user can wire up, it is an SDK feature and belongs in `lfx`, not `earthmind-base`. Backend changes that have no visual surface are almost always off-narrative.
...
4. **`lfx` is the runtime, `earthmind-base` is the platform, `earthmind` is the distribution.** Components and the graph engine live in `lfx` (`src/lfx/src/lfx/components/`). API, auth, persistence, and multi-user concerns live in `earthmind-base`. The `earthmind` package is the integration that ships everything together. New code goes in the lowest layer that can host it. Boundaries are enforced one-way: `earthmind` → `earthmind-base` → `lfx`. See [ARCHITECTURE.md](./ARCHITECTURE.md).
...
10. **Two audiences, one product: visual builders and Python devs.** When their needs conflict, the visual builder wins for `earthmind-base` features and the Python dev wins for `lfx` SDK features. Don't compromise the canvas to make the SDK prettier, and don't compromise the SDK to make the canvas easier.
