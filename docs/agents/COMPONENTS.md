...
Components are the unit of work in Terraflow. They render as nodes on the canvas, persist into flow JSON, and become tools for Agents. Get the contract right and the rest of the system follows; get it wrong and you orphan every saved flow that referenced your component.

Components live in **`src/lfx/src/lfx/components/<category>/`**. The mirror tree under `src/backend/base/terraflow/components/` is legacy stubs — do not add files there.
...
4. **Is this an lfx-runtime concern** (graph execution, schema, IO primitives) or a **Terraflow-app concern** (auth, DB, tracing)? Components are lfx; never import `terraflow.services.*` from a component.
...
- **`documentation = "https://docs.terraflow.org/..."`**: deep-link from the node UI. Example: `input_output/chat.py`.
