...
Components are the unit of work in EarthMind. They render as nodes on the canvas, persist into flow JSON, and become tools for Agents. Get the contract right and the rest of the system follows; get it wrong and you orphan every saved flow that referenced your component.

Components live in **`src/lfx/src/lfx/components/<category>/`**. The mirror tree under `src/backend/base/earthmind/components/` is legacy stubs — do not add files there.
...
4. **Is this an lfx-runtime concern** (graph execution, schema, IO primitives) or a **EarthMind-app concern** (auth, DB, tracing)? Components are lfx; never import `earthmind.services.*` from a component.
...
- **`documentation = "https://docs.earthmind.org/..."`**: deep-link from the node UI. Example: `input_output/chat.py`.
