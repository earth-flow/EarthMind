# Investigation: Deprecated macOS Support — Impact on Terraflow
...
macOS Intel (x86_64) is in its final phase of deprecation across the entire ecosystem — Apple hardware, operating system, GitHub Actions runners, PyTorch, and the broader ML/AI stack. Terraflow already has platform exclusion markers for the most critical torch-dependent extras (PR #12469), but additional deprecations in GitHub Actions runners, macOS versions, and upstream dependencies will require further action over the next 12–18 months.

**Key decision required:** Set a timeline to officially drop macOS x86_64 from Terraflow's support matrix (recommended: Terraflow 2.0 or Q4 2026, whichever comes first).
...
## 2. Current State of Terraflow macOS Support
...
|------------|----------|---------|
| `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` | `__main__.py`, `terraflow_launcher.py`, CI workflows | Prevents Objective-C fork safety crashes with gunicorn multiprocessing |
| `os.execv()` re-exec pattern | `terraflow_launcher.py` | Sets env vars before Objective-C runtime initializes |
...
Terraflow **core functionality** (flow builder, API, database, all non-ML components) works on macOS Intel with Python 3.10–3.13. The following features are **unavailable**:
...
| Timeline | Event | Impact on Terraflow |
|----------|-------|--------------------|
...
### 4.2 Short-Term (Terraflow 1.10 / Q2 2026)
...
### 4.3 Medium-Term (Terraflow 2.0 / Q4 2026)
...
The `OBJC_DISABLE_INITIALIZE_FORK_SAFETY` workaround in `__main__.py` and `terraflow_launcher.py` applies to all macOS, not just Intel. However, once Intel is dropped:
...
Add a deprecation notice in the Terraflow 2.0 release notes:

> **macOS Intel (x86_64) support is deprecated.** Terraflow 2.0 is the last version tested on Intel Macs. Future releases will only be tested on Apple Silicon (ARM64). Core functionality may continue to work, but ML features require Apple Silicon.

### 4.4 Long-Term (Terraflow 2.x+ / 2027)
...
| Python 3.14 drops macOS x86_64 support | Low (2027+) | Core Terraflow broken on Intel | R6: Deprecation notice well in advance |
